...
public class TgDefaultRoute extends TgRoute {
	...
	public TgMsg choiceAdminTgMsg(String key) {
		switch (key) {
			...
			default: {
				...
				TgApp.AppMode appMode = EN.valueOf(key, TgApp.AppMode.class, null);
				if (appMode != null) {
				...
				if (key.matches("a .+")) {
					...
					for (String arg : args) {
						if (arg.startsWith("u")) {
							...
							qps.add(QP.pLE(CN.DT, QDate.now().addMinutes(-1 * ERR.isInt0(arg.substring(1))).toSqlDate()));
						}
					}
					if (limit == null) {
						limit = 10;
	...
	private TgMsg runSql(String key) {
		String clazzName = USToken.first(key, ' ');
		Class clazz = RFL.clazz(clazzName);
		ERR.isClassOf(clazz, AModel.class);
		TypeDb db = TypeDbEE.getDbEE(clazz);
		String sql = ERR.notEmpty(key.substring(clazzName.length() + 1).trim());
