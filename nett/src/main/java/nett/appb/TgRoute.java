...
public abstract class TgRoute extends BotRoute {
	...
	protected NetUsrId createDefaultUsrId() {
		...
		defUsr.setUserName(chat.getFirstName());
		return defUsr;
	}
	...
	public String getAppCleanMessage(Throwable ex) {
		return getRootRoute().getTgApp().isProm() ? "Error" : UErr.getMessageWithType(ex, STR.NL, true);
	}
	...
	public Message sendMsg(String message) {
		return sendMsg(message, ParseMode.HTML, false, true, null);
	}


	public Message sendMsg(List<String> messages, boolean... single) {
		...
		return sendMsg(JOIN.allByNL(messages).toString());
	}
	...
	public Serializable sendMsg(IBotMsg message) {
		return sendMsg((TgMsg) message);
	}

	public Serializable sendMsg(TgMsg tgMsg) {
		IT.notNull(tgMsg);
	...
	public Serializable sendMsg(EditMessageText msg) {
	...
	public Message sendError(Throwable error) {
		ICleanMessage clnMsg = UErr.getCleanMessageType(error);
		if (clnMsg != null) {
			return sendError(null, error, clnMsg);
		}
		String cleanMsg = getAppCleanMessage(error);
		return sendError(cleanMsg, error, null);
	}
