...
public class UJack {
	...
	public static <T extends JsonNode> T getNode(JsonNode je, String key, Class<T> type, T... defRq) {
		...
		throw new RequiredRuntimeException("JE except '%s', but it '%s'", type.getSimpleName(), Type.of(jeByKey));
	}

	public static boolean hasSimpleValue(ArrayNode arr, String item) {
		for (JsonNode jsonNode : arr) {
			switch (UJack.Type.of(jsonNode)) {
				case PRIMITIVE:
	...
	public enum Type {
		...
		public static Type of(JsonNode el, Type... defRq) {
			if (el.isValueNode()) {
				return PRIMITIVE;
			} else if (el.isObject()) {
				return OBJECT;
