...
public class UCRest {
	...
	public static String checkGetBodyPayload(HttpServletRequest request) {
		...
		try {
			...
			if (X.empty(body)) {
				throw new ERR.CheckException("Set payload");
			}
		} catch (IOException e) {
			throw new ERR.CheckException("Error get payload (%s)", e.getMessage());
		}
		return body;
	}
	...
	public static String checkGetQuery(HttpServletRequest request) {
		...
		if (X.empty(query)) {
			throw new ERR.CheckException("Set query part in url");
		}
		return query;
	}
}
