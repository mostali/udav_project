...
//			return URest.getResponse_OK_JSON_CUSTOM(UMap.mapOf("error", ));
		...
		String nameFileOrUrl = UStream.findFirstNotEmpty(file == null ? null : file.getOriginalFilename(), url);
		...
		try {
			String text = TeseractParserPhotoViaDocker.parsePhotoRus(photoFile);
			if (L.isInfoEnabled()) {
