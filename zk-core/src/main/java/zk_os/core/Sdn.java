...
public class Sdn extends Pare<String, String> {
    ...
    public static Sdn ofPlane(String sd3) {
        return of(sd3, NodeID.PLANE_INDEX_ALIAS);
    }
    ...
    public static Sdn toSdnLocalOrFull(String tbPage) {
        return isLocalSdn(tbPage) ? Sdn.ofPlaneCurrent(tbPage) : Sdn.of(NodeID.of(tbPage).sdn());
    }

    private static boolean isLocalSdn(String tbPage) {
        return tbPage.indexOf('/') == -1;
    }

    private static Sdn ofPlaneCurrent(String tbPage) {
        return new Sdn(Sdn.PLANE(), tbPage);
    }
    ...
    public String toStringPath(String... withItem) {
        return nodeId() + (ARG.isDef(withItem) ? ARG.toDef(withItem) : "");
    }
...
//	}
    ...
    public Sdn toSdnPlaneIndex() {
        return Sdn.ofPlane(plane());
    }

    public Sdn toSdnWithPage(String page) {
