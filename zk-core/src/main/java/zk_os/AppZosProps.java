...
public class AppZosProps<T> {
	...
	public static final AppPropDef<Boolean> UI_EFFECTS_ENABLE = new AppPropDef("app.ui.effects.enable", true);

	public static List<AppPropDef> initAppProps() {
		return IAppProps.getAllProps(//
				AppZosProps.class, AppCoreNotes.TREE_PROPS,//
				AppNotesProps.class, AppCoreNotes.TREE_PROPS,//
	...
	public static final AppPropDef<String> LOG_DATE_FORMAT = new AppPropDef("log.date.format", LogGetterDate.FORMAT_DEFAULT);

	public static final AppPropDef<String> LOG_LINE_MAPPING = new AppPropDef("log.line.mapping", "0,1,2,3,4");
	public static final AppPropDef<Integer> DEF_TREE_LIMIT = new AppPropDef("tree.page.size", AppZosConfig.DEF_TREE_LIMIT);

	public static final AppPropDef<Boolean> IS_DEBUG_ENABLE = (AppPropDef<Boolean>) new AppPropDef(APP.APK_IS_DEBUG, false).autoInitHolder(APP.class);
	public static final AppPropDef<Boolean> IS_PROM_ENABLE = (AppPropDef<Boolean>) new AppPropDef(APP.APK_PROM_ENABLE, true).autoInitHolder(APP.class);
	public static final AppPropDef<Boolean> IS_DEV_ENABLE = (AppPropDef<Boolean>) new AppPropDef(APP.APK_DEV_ENABLE, true).autoInitHolder(APP.class);

	public static final AppPropDef<APP.BBL> BBLMODE = (AppPropDef<APP.BBL>) new AppPropDef(APP.APK_BBL_MODE, APP.BBL.LEARN).autoInitHolder(APP.class).desc("Black List - Security Mode");

	public static final AppPropDef<Boolean> IS_TRACE_HTTP_CALL = (AppPropDef<Boolean>) new AppPropDef("http.msg.trace", false).desc("Store http request body in ./tmp dir");

	public static final AppPropDef<Boolean> HTTP_ONOFF_SSL = new OnOffSslAppPropDef("http.ssl.enable", CON.onOffSsl);

	public static final PageStateAppPropDef<List<String>> ZOS_BGIMG = (PageStateAppPropDef<List<String>>) new PageStateAppPropDef("app.bg.img", ARR.as(BgImg.BGIMG_DEFAULT)).setDefValueSupplier(BgImg::getAllDefaultBgImages);//.viewType(AppPropDef.ViewType.DD);
...
//				return (T) as;
			...
			return X.notEmpty(pageStateValue) ? (T) ARR.as(pageStateValue) : ARG.throwMsg(() -> X.f("Not found value '%s' (not check store)", key), defRq);
		}
