...
//		Supplier<String> threadNameOrig = () -> thread;
		...
		if (thread1 instanceof LogTailReaderThread0) {
			...
			menu.addMI("Show current log", e -> {
				ZKME.openEditorTextReadonly("StackTrace", JOIN.objsByNL(logThread.getLogTailReader().getCollector()));
			});
		} else {
			...
			if (foundTask != null) {
				menu.addMI("Show persist data", e -> {
					ZKME.openEditorTextReadonly("StackTrace", JOIN.objsByNL(foundTask.getKey(), "--", foundTask.getKey(), "--", foundTask.getExt(), "--", foundTask.getTimeAsQDate()));
				});
		...
		if (threadActiveGet.get()) {
			...
			menu.addMI("Get StackTrace..", e -> {
				...
				ZKME.openEditorTextReadonly("StackTrace", JOIN.objsByNL(data));
			});
