...
public class TaskPanel extends Div0 implements IReRender {
	...
	public static UTree dbt() {
		return (UTree) UTree.tree(APP.GND_TREE()).setAutoCleanCfg(AUTOCLEAN_CTR_EVERY_MIN_MAX_PACKET_FIRSTEND);
	}

	public static UTree dbd() {
		return (UTree) UTree.tree(APP.GNDD_TREE()).setAutoCleanCfg(AUTOCLEAN_CTR_EVERY_MIN_MAX_PACKET_FIRSTEND);
	}
	...
	public static void addTask(Thread thread, TrackMap.TrackId track) {
		dbt().put(Thread0.getNameId(thread), track.nodeData.nodeDir.nodeId(), RUNNED);
	}

	public static void stopTask(Thread thread) {
		dbd().remove(Thread0.getNameId(thread));
	}
	...
	public static void addTaskD(NodeDir node, Thread thread) {
		...
		String nameId = Thread0.getNameId(thread);
	...
	public static void stopTaskLoggedD(Thread0 thread) {
		...
		String key = Thread0.getNameId(thread);

		CtxtDb.CtxTimeModel model = dbd().getCtxTimeModelByKey(key);
		if (model == null) {
...
//			rowTasksDaemons.reset();
	...
	static class RowTasks extends Div0 {
		...
		private FunctionV1<Div0> filler;
		...
		private void showDBD_RUNNED(Div0 parent) {
			...
			List<CtxtDb.CtxTimeModel> models = dbd().getModels(QP.pEQ("ext", RUNNED));

			for (CtxtDb.CtxTimeModel key : models) {
		...
		private void showDBD_NOALIVE(Div0 parent) {
			...
			List<CtxtDb.CtxTimeModel> models = dbd().getModels(QP.pEQ("ext", NOALIVE));
	...
	static class NoaliveTask extends GndTaskItem {

		public NoaliveTask(CtxtDb.CtxTimeModel model) {
	...
	static class DaemonTask extends GndTaskItem {
		public DaemonTask(CtxtDb.CtxTimeModel model) {
			super(model, model.getKey());
		}
	...
	static class CallMsgTask extends GndTaskItem {
		...
		public CallMsgTask(CtxtDb.CtxTimeModel model) {
			super(model, model.getKey());
		}
