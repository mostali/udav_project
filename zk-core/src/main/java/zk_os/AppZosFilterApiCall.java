...
public class AppZosFilterApiCall {
	...
	static boolean extractApiPostDeleteRequest(ServletRequest request, HttpServletResponse response, String servletPath, HttpServletRequest httpRequest) throws IOException {
		...
		NodeApiCallType.ApiCase apiCase = NodeApiCallType.ApiCase.ofServletPath(servletPath);
...
//					}
		...
		if (nodePath.getNameCount() == 2) {
			NodeID nodeID = NodeID.of(nodePath.toString() + "/");
			...
			case POST: {
				...
				switch (apiCase) {
					case _api:
						...
						switch (ctrlOperSym) {
							case NodeApiChars.UP: {
								if (nodeDir.fdExist()) {
									rspData = nodeDir.state().readFcData();
								} else {
							...
							case NodeApiChars.DOWN: {
								...
								rspData = X.f(NoteApi.MSG_200_ITEM_ADDED, itemPath.nodeName());
								break;
							}
							default:
					...
					case _ati:
						...
						new NodeApiCallType.TreeRest(nodeDir.nodeID(), oper).apply();
