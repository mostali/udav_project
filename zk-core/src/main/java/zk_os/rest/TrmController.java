...
//	}
	...
	public ResponseEntity exe(String q) throws Throwable {
		return new TrmRestCall(q).rq(UWeb.getRequest(), false).doCall();
	}
	...
	private class TrmRestCall extends RestCall {
		...
		public ResponseEntity callImpl() throws Exception {
			String cmd0 = UWeb.getQueryStringTyped(UWeb.getRequest()).getFirst("q", WhatIs.NB);
			String cmd = IT.NE(q);
			IT.isEq(cmd0, cmd);
			...
			switch (key) {
				case "logs":
					...
					String message = Log2HtmlConverter.fromFile(Paths.get(file), -1000, 2, LogGetterDate.buildByDefault());
