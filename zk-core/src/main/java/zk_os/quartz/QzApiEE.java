...
//							.repeatForever())
    ...
    public static class GSet {
        final Set<JobKey> set;

        public static GSet of(String group) {
            return new GSet(findAllJobKeys(group));
        }

        public boolean contain(String keyName) {
            return set.stream().anyMatch(k -> k.getName().equals(keyName));
        }
    }
