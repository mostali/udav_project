...
public class QzApiEE extends EException {
	...
	public static final String SRV_MARK = SYMJ.TIME_DONE + "QZJ";
...
//			runScheduleJob("j1", "g1", "t1", "*/1 * * ? * *", HelloJob.class);
	...
	public static Set<JobKey> getAllJobKeys() throws QzApiEE, SchedulerException {
...
//			scheduler.shutdown();
	...
	@SneakyThrows
	public static void deleteAll(String jobGroup, boolean interrupt) {
		Scheduler scheduler = QzApiEE.getScheduler();
		Collection<JobKey> all = scheduler.getJobKeys(GroupMatcher.groupEquals(jobGroup));
		deleteAll(all, interrupt);
	}

	@SneakyThrows
	public static void deleteAll(Collection<JobKey> all, boolean interrupt) {
