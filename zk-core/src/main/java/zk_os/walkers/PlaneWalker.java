...
public abstract class PlaneWalker {
	...
	protected abstract Boolean walkPlane(String plane, Path dir, ObjState planeState);

	public static <R> List<R> doWalkFunc(Function3<String, String, ObjState, R> applier) {
		List results = new ArrayList<>();
		new PlaneWalker() {
			@Override
			protected Boolean walkPlane(String plane, Path dir, ObjState planeState) {
	...
	public static List<Pare3<String, Path, ObjState>> doWalkToList(boolean withIndex, boolean withSystemPlanes, boolean withUserDomain) {
		List<Pare3<String, Path, ObjState>> list = new LinkedList<>();
		new PlaneWalker() {
			@Override
			protected Boolean walkPlane(String plane, Path dir, ObjState planeState) {
...
//		}
		...
		for (Path dirSd3 : allSd3Dir) {
			...
			if (!walkPlane(sd3, dirSd3, formState)) {
				break;
			}
		}
		return this;
