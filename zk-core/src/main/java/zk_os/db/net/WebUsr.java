...
public class WebUsr implements Serializable, InaUser, UserDetails {
	...
	public static WebUsr loadBySid(@NonNull Long sid, WebUsr... defRq) {
		...
		return webUsrSid != null ? webUsrSid : ARG.throwMsg(() -> X.f("Except user by SID[%s] and NET[%s] ", sid, netName), defRq);
	}

	public static String login(String... defRq) {
		WebUsr webUsr = get(null);
		return webUsr != null ? webUsr.getLogin() : ARG.throwErr(() -> new RequiredRuntimeException("Except known user"), defRq);
	}

	public static boolean isAnonim() {
		return WebUsr.get().isMainRole_ANONIM();
//		return SecMan.isAnonim();
	}
...
//	}
	...
	public String getNetLogin(String... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Except NID for user [%s]", toStringSidNamed()), defRq);
	}

	public Long getNidByNet(NT nt, Long... defRq) {
		return nt == getNetType(null) ? getNid() : ARG.throwMsg(() -> X.f("Not found net '%s' for user '%s'", nt, getId()), defRq);
	}
