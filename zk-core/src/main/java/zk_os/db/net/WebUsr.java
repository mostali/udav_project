...
//	}
	...
	public String getNetNidNamed(String... defRq) {
		NT nt = NT.ofNET(net, null);
		if (nt != null) {
			return nt.shortPfx() + IT.NN(nid); //TODO rm check?
		}
		return ARG.toDefThrowMsg(() -> X.f("Illegal user [%s] found. With undefined net [%s]", getId(), net), defRq);
	}
	...
	public boolean isAnonimStrict() {
		return this instanceof AnonimZkosWebUsr;
	}
}
