...
//	WebUsr findAllActiveUsers();
	...
	@Query(value = "SELECT * FROM usr u WHERE u.login =?1", nativeQuery = true)
	WebUsr findWebUserByName(String name);

	@Query(value = "SELECT * FROM usr u WHERE u.net =?1 AND u.nid =?2 AND alias NOT NULL", nativeQuery = true)
	List<WebUsr> loadUserByNidAndAllAlias(String net, Long nid);

	@Query(value = "SELECT * FROM usr u WHERE u.net =?1 AND u.nid =?2", nativeQuery = true)
	WebUsr loadUserByNidAndAnyAlias(String net, Long nid);

	@Query(value = "SELECT * FROM usr u WHERE u.net =?1 AND u.nid =?2 AND alias =?3", nativeQuery = true)
	WebUsr loadUserByNetNidAndAlias(String net, Long nid, String alias);

	@Query(value = "SELECT * FROM usr u WHERE alias =?1", nativeQuery = true)
	List<WebUsr> loadUserByAlias(String alias);

	@Query(value = "SELECT * FROM usr u WHERE id =?1 limit 1", nativeQuery = true)
	WebUsr loadUserById(Long id);

	@Query(value = "SELECT * FROM usr u WHERE u.net =?1 AND alias =?2", nativeQuery = true)
	List<WebUsr> loadUserByNetAlias(String net, String alias);

	@Query(value = "SELECT * FROM usr u WHERE u.login =?1 and u.phc =?2", nativeQuery = true)
	WebUsr findAllActiveUsers(String name, String phc);

	@Query(value = "SELECT * FROM usr u WHERE u.sid = ( SELECT MAX(sid) FROM usr )", nativeQuery = true)
	WebUsr getLastUser();
