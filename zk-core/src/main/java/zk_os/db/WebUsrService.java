...
public class WebUsrService implements UserDetailsService {
	...
	public static WebUsr findWebUsrRoot() {
		return ARRi.first(WebUsrService.get().webUsrRepo.findAllById(ARR.as(RootZkosWebUsr.ROOT_ID)));
	}
	...
	public String initDB() {
		if (webUsrRepo.existsById(1L)) {
			...
			return "FirstInit:false:" + webUsrRepo.getLastUser() + ":" + ARRi.first(allById).last_name;
		}
		String[] phc = Sec.Phc.generateRandomPass();
		WebUsr webUsr = new RootZkosWebUsr(phc[1]);
		save(webUsr);
		RW.write(Paths.get("logs/init.log"), webUsr.sid + "" + phc[1]);
		return "FirstInit:true:" + webUsr.login + ":" + phc[0] + ":" + phc[1];
	}
