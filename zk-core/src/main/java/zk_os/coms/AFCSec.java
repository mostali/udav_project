...
public class AFCSec extends SecCache {
	...
	public static Set<Path> getItemPaths(WebUsr usr, SpaceType spaceType, Sdn sdn, UO[] policy, Predicate<Path>... nodeFilter) {
		Set<Path> paths = getPathsLsClean(spaceType, sdn, nodeFilter);
			...
			case NODES:
				switch (usrRole) {
					...
					case EDITOR:
						...
						Map<String, SecBool> secTupleStateMap = getCacheBool(spaceType).get(sdnu);
...
//						}
						pathFilter = p -> {
							SecBool secBool = secTupleStateMap.get(UF.fn(p));
							if (secBool == null) {
								return false;
							}
							for (UO oper : policy) {
...
//	}
	...
	public static boolean isAllowed(WebUsr usr, SpaceType spaceType, ISpaceID itemId, UO oper) {
		LoadingCache<Sdnu, Map<String, SecBool>> cacheBool = getCacheBool(spaceType);
		Map<String, SecBool> boolMap;
		String eName;
		switch (spaceType) {
			case SPACES:
		...
		SecBool secBool = boolMap.get(eName);
		if (secBool == null) {
			//TODO???
