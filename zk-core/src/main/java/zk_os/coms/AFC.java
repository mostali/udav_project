		...
		public static AfcEntity valueOfDirName(String afcEntityName, AfcEntity... defRq) {
			switch (afcEntityName) {
				...
				case AFCC.DIR_EVENTS:
					return EVENTS;
				default:
					return ARG.throwMsg(() -> X.f("illegal entity name '%s'", afcEntityName), defRq);
			}
		}
...
//
//		public static Collection<String> DIR_USERS_NAMES_CLEAN(boolean withUsers) {
//			return DIR_USERS_LS_CLEAN(withUsers).stream().map(UF::fn).collect(Collectors.toList());
//		}
//
//		@SneakyThrows
//		public static TreeSet<Path> DIR_USERS_LS_CLEAN(boolean withUsers) {
//			return UFS.lsFilter(DIR_USERS(), p -> {
//				if (!UFS.isDir(p)) {
//					return false;
//				}
//				if (withUsers) {
...
//		}
//
//		public static Path DIR_USERS() {
//			return AppZosCore.getRpa().resolve(AFCC.DIR_USERS);
//		}
//
//		public static Path getUserDir(String sd3) {
//			return DIR_USERS().resolve(IT.NE(sd3));
//		}
//
//		public static Path getStatePath_PROPS(String sd3) {
//			return getUserDir(sd3).resolve(AFCC.FILE_USER);
//		}
//
//		public static Path DIR_USER(String sd3) {
//			return DIR_USERS().resolve(NodeID.wrapPlane(sd3));
//		}
//
//	}
	//
	...
	public static class PLANES {
		...
		public static Path DIR_PLANES() {
			return AppCoreZos.getRpa().resolve(AFCC.DIR_PLANES);
		}
...
//		}
	...
	public static class HEADS {
		...
		public static Path RPA_HEADS() {
			return AppCoreZos.getRpa().resolve(AFCC.DIR_HEADS);
		}
	...
	public static class FORMS {
		...
		public static TreeSet<Path> DIR_FORMS_LS_CLEAN(Pare<String, String> sdn, Predicate<Path>... defFilter) {
			return DIR_FORMS_LS_CLEAN(sdn.key(), sdn.val(), defFilter);
		}

		@SneakyThrows
		public static TreeSet<Path> DIR_FORMS_LS_CLEAN(String sd3, String pagename, Predicate<Path>... defFilter) {
			Predicate<Path> defFilter0 = UFS::isDir;
			Predicate<Path> defFilterFinal = ARG.isDef(defFilter) ? defFilter0.and(ARG.toDef(defFilter)) : defFilter0;
			return UFS.lsFilter(DIR_FORMS(sd3, pagename), defFilterFinal, ARR.EMPTY_TSET);
		}
		...
		public static Map<String, Path> getMapForms(Pare<String, String> sdn) {
			return UUFS.toMapByFn(AFC.FORMS.DIR_FORMS_LS_CLEAN(sdn));
		}
	}
	...
	public static class ARCHIVES {
		...
		public static Path DIR_ARCHIVES() {
			return AppCoreZos.getRpa().resolve(AFCC.DIR_ARCHIVES);
		}
	}
