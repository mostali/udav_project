...
public class AFC {
	...
	public static Collection<Path> getSpaceTypesList(SpaceType spaceType) {
		switch (spaceType) {
			case SPACES:
				return PLANES.DIR_PLANES_LS_CLEAN(false);
			case PAGES:
				return PAGES.DIR_PAGES_LS_CLEAN(Sdn.planeCurrent());
			case NODES:
				return FORMS.DIR_FORMS_LS_CLEAN(Sdn.get());
			default:
...
//		X.exit( DIR_PLANES().toAbsolutePath());
	...
	public static class PLANES {
		...
		public static Path getStatePath(String sd3) {
			return getPlaneDir(sd3).resolve(AFCC.FILE_PLANE);
		}

		public static Path DIR_PLANE(String sd3) {
			return DIR_PLANES().resolve(NodeID.wrapSd3(sd3));
		}
	...
	public static class PAGES {
		...
		public static Path getDirAsZipFile(Pare<String, String> sdn) {
			...
			Path pageDirZip = ARCHIVES.DIR_ARCHIVES().resolve(NodeID.wrapSd3(sdn.key())).resolve(pageDir.getFileName() + ".zip");
			return pageDirZip;
		}

		public static Path getDir(String sd3, String pagename) {
			return PLANES.DIR_PLANES().resolve(NodeID.wrapSd3(sd3)).resolve(NodeID.wrapSd3(pagename));
		}
		...
		public static Path getStatePathCurent() {
			return getStatePath(Sdn.get());
		}

		public static Path getStatePath(Pare sdn) {
			return getStatePath(sdn.keyStr(), sdn.valStr());
		}

		public static Path getStatePath(String sd3, String pagename) {
			return PLANES.DIR_PLANES().resolve(NodeID.wrapSd3(sd3)).resolve(normEntName(NodeID.wrapSd3(pagename))).resolve(AFCC.FILE_PAGE);
		}
	}
	...
	public static class HEADS {
		...
		public static List<Path> getSd3Dir_LS(String sd3) {
			return EFT.FILE.ls(getSd3Dir(sd3), ARR.EMPTY_LIST);
		}

		public static Path getSd3Dir(String sd3) {
	...
	public static class FORMS {
		...
		public static Path DIR_FORMS(String sd3, String pagename) {
			return PLANES.DIR_PLANE(NodeID.wrapSd3(sd3)).resolve(NodeID.wrapSd3(pagename)).resolve(AFCC.DIR_FORMS);
		}
	...
	public static class COMS {
		...
		public static Path DIR_COMS(String sd3, String pagename) {
			return PLANES.DIR_PLANE(sd3).resolve(NodeID.wrapSd3(pagename)).resolve(AFCC.DIR_COMS);
		}
	...
	public static class EVENTS {
		...
		public static Path toPathNodeEventTreePath(Pare<String, String> sdn, String nodeName, boolean... create) {
			...
			if (ARG.isDefEqTrue(create)) {
				UFS_BASE.MKDIR.createDirs(rpaEventsStatePath);
			}
	...
	public static class PAGECOMS {
		...
		public static Path getStatePath(String sd3, String pagename, String comname, EXT ext) {
			return DIR_PAGECOMS(sd3, NodeID.wrapSd3(pagename)).resolve(normEntName(comname)).resolve(toFileName_Default(ext));
		}
	}
