...
//		UTree authTree = AppNotesCore.UTreeAuth.getTreeDb();
		...
		AppCoreNotes.UTreeAuth.store(nid + "", tk, json);
	...
	public static Pare3<Boolean, String, String> isAuth_OkStatusJson(String token) {
		Ctx3Db.CtxModelCtr rowByToken = AppCoreNotes.UTreeAuth.getRowByToken(token);
		if (rowByToken == null) {
			if (L.isWarnEnabled()) {
		...
		if (diffabs_sec > TIMEOUT_MIN * 60) {
			...
			AppCoreNotes.UTreeAuth.store(rowByToken.getKey(), rowByToken.getValue(), extJsonString);
		}
