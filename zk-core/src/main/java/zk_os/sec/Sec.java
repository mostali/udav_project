...
//		AnonymousAuthenticationToken tk = null;
...
//	}
	...
	public static void checkIsAllowedOr403(PageRoute targetRoute, MatrixAccess targetRsrc) {
		if (targetRoute != null && targetRoute.role() != ROLE.USER) {
			if (!ROLE.isAllowed(targetRoute.role())) {
				if (L.isInfoEnabled()) {
					L.info("Access denied for user '{}' (Allowed '{}') ", Sec.getAuth(), targetRoute.role());
				}
		...
		if (targetRsrc != null) {
			NI.stop("Who call?" + targetRsrc);
		}
//		throw StatusException.C403();
//		if (!MatrixAccess.hasAccessForUser(getMA())) {
//			throw StatusException.C403();
//		}
	}
	...
	public static boolean isOwner() {
		WebUsr user = Sec.getUser();
		return user.getRolesListGA().contains(ROLE.GA_ROLE_OWNER);
	}
	...
	public static boolean isAdminOrOwner() {
		WebUsr user = Sec.getUser();
		return user.getRolesListGA().contains(ROLE.GA_ROLE_ADMIN) || user.getRolesListGA().contains(ROLE.GA_ROLE_OWNER);
	}

	public static boolean isEditorAdminOwner() {
		WebUsr user = Sec.getUser();
		return user.getRolesListGA().contains(ROLE.GA_ROLE_EDITOR) || user.getRolesListGA().contains(ROLE.GA_ROLE_ADMIN) || user.getRolesListGA().contains(ROLE.GA_ROLE_OWNER) || Sec.isPlaneOwner();
	}

	public static boolean isEditor() {
		WebUsr user = Sec.getUser();
		return user.getRolesListGA().contains(ROLE.GA_ROLE_EDITOR) || Sec.isPlaneOwner();
	}
		...
		switch (nt) {
			...
			case VK: {
				...
				if (webUsrExisted == null) {
					...
					webUsr.login = nt.shortPfx() + nid;//native
					webUsr.alias = webUsr.login;
					webUsr.net = nt.name();
					webUsr.roles = ROLE.ROLE_EDITOR.toString();
					webUsr.first_name = null;
...
//				return;
	...
	private static class WuUsernamePasswordAuthenticationToken extends UsernamePasswordAuthenticationToken {
		...
		private WuUsernamePasswordAuthenticationToken(WebUsr webUsr, Collection<? extends GrantedAuthority> authorities) {
			super(webUsr.login, webUsr.phc, authorities);
			this.usr = webUsr;
		}
