...
public class AuthSrv {
	...
	public static String produceTokenAndBuildFreshMsgWithLinkToken(NT nt, String additionalyMsgData, Long fromId) {
		...
		String msg = auth.key() + STR.NL + APP.HOST.getUsedHttpProtocol() + nt.shortPfx() + fromId + "." + APP.HOST.getAppHost0(null) + "?t=" + auth.key() + STR.NL + "Токен действителен до " + auth.val().f(FDate.YYYY_DB_ISO_STANDART);
		return msg;
	}
}
