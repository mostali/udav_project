...
public class AuthSrv {
	...
	public static final Logger L = LoggerFactory.getLogger(AuthSrv.class);
	...
	public static Pare3<Boolean, String, String> isAuth_OkStatusJson(String token) {
		CtxtDb.CtxTimeModel rowByToken = AppNotesCore.UTreeAuth.getRowByToken(token);
		if (rowByToken == null) {
			if (L.isWarnEnabled()) {
	...
	public static String produceTokenAndBuildFreshMsgWithLinkToken(NT nt, String additionalyMsgData, Long fromId) {
		...
		String msg = auth.key() + STR.NL + "http://" + nt.shortPfx() + fromId + "." + APP.getAppDomain(null) + "?t=" + auth.key() + STR.NL + "Токен действителен до " + auth.val().f(FDate.YYYY_DB_ISO_STANDART);
		return msg;
	}
}
