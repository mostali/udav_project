...
public class ActTabpanel0 extends Tabpanel0 {
	...
	private ActTabpanel0 buildMetrics() {
		...
		for (String metricName : metricsNames) {
			...
			child.onCLICK((eventLn -> {
				try {
					ZKM_Editor.openEditorText(metricName, actRequest.getMetricsItemGm(metricName).toStringPrettyJson().toString(), true);
				} catch (Exception ex) {
					ZKI.alert(ex);
				}
			}));
	...
	private ActTabpanel0 buildBeans() {
		...
		for (String beanName : beanNames) {
			...
			child.onCLICK((eventLn -> {
				try {
					ZKM_Editor.openEditorText(beanName, actRequest.getBeanItemGm(beanName).toStringPrettyJson().toString(), true);
				} catch (Exception ex) {
					ZKI.alert(ex);
				}
			}));
