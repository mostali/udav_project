...
//				window.appendChild(h);
				...
				Pare<FormState, Window> demoNoteOpened = getDemoNodeOpened(null, nodeType, sdn, true);
				...
				ZKS.PADDING_WIN(demoNoteOpened.val(), 0, 0);
				ZKS.PADDING_TOP_WIN(demoNoteOpened.val(), 0, 100);
...
//		NodeEvalType nodeEvalType = parentState.nodeDir().evalType(false, NodeEvalType.NODE);
	...
	public static Pare<FormState, Window> getDemoNodeOpened(String newAboutType, NodeEvalType value, Sdn sdn, boolean open, Component... parent) {
		...
		FormState formState = AppStateFactory.ofFormName_orCreate(sdn, noteName);
		if (!open) {
			return Pare.of(formState, null);
		}
		Window win;
		if (formState.existPropsFile()) {
			win = NFOpen.openNoteWin_required_open(noteName, sdn, parent);
		} else {
