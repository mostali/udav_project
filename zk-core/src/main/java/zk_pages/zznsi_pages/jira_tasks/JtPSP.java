...
import zk_pages.zznsi_pages.jira_tasks.core.JqlPageZView;
import zk_pages.zznsi_pages.jira_tasks.core.RecoveryState;

import java.util.List;

@PageRoute(pagename = JtPSP.KEY, role = ROLE.ANONIM, eqt = StringConditionType.REGEX)
public class JtPSP extends PageSP implements IPerPage, IZState, WithLogo, WithSearch {
...
//	public static final String KEY = "@@tasks@[a-zA-Z@\\d.]+";
	...
	public JtPSP(Window window, SpVM spVM) {
		super(window, spVM);
	}
	...
	public void buildPageImpl() {
		...
		JqlPageZView jqlPageView = new JqlPageZView(zPage);
		...
		{
			//reg store listener
			List<NodeDir> pageNodes = NoteWalker.toList(sdn());
			pageNodes.forEach(n -> {
				StateProxyRW.CACHE_STORE_LISNNER.put(n.toObjId(), new SerializeJqlStateListener());
			});
		}
	...
	private static class SerializeJqlStateListener implements StateProxyRW.UpdListener {
		@Override
		public void up(NodeDir nodeDir) {

			FormState state = nodeDir.state();

			String val = state.readFcData(1, null);

			GsonMap gsonMap = state.readFcDataAsGsonMap(null);
			String jsonExt = gsonMap == null ? null : gsonMap.toStringPrettyJson();
			RecoveryState.RecModel newRecModel = RecoveryState.RecModel.createNew(nodeDir.nodeName(), jsonExt, val);
