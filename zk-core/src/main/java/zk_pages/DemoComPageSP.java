...
import zk_form.events.cfrm.CfrmTbxm2_SEL;
...
import zklogapp.DirDiffViewPageSP;
...
//			FileUploaderComposer.loadComponent("asd", Paths.get("./tmp"), window);
		...
		Function2<String, String, Object> func = new Function2<String, String, Object>() {
			...
			public Object apply(String vl1, String vl2) {
				List<String> allGroup = IRegex.findGroups(vl2, vl1);
				ZKI.infoEditorDark(allGroup);
				return null;
			}
		};

		hello.getOrCreateMenupopup(window).addMI_Href_in_Self(CfrmTbxm2_SEL.toMI("22", "", new String[2], new String[2], func));
...
//				"</pre>"));
	...
	private void sectionJiraMod() {
		appendChild(new Bt("run jira").onCLICK(e -> {
//			String[] packages = {};
//			Jira

			ZJar zJar = ZJar.of(Paths.get("/opt/appVol/.bin/jira-mod.jar"));
			Object taskCreate = zJar.invokeWithArgs1("invokeLines", new String[]{"-task", "create", "-task.type", "3", "-task.project", "SUP", "-task.summary", "summarryss", "-task.desc", "descc", "-task.assignee", "daaav"});
...
//				X.exit();
	...
	private void sectionDirDiff() {
		addHeaderCom("Сравнение директорий");

		Div0 container = Div0.of();
		container.setWidth("100%");
		appendChild(container);

		// Модель данных для формы
		Map<String, Object> formModel = MAP.of("path1", "", "path2", "");

		// Контейнер для результата (будет очищаться при каждом нажатии)
		Div0 resultPanel = Div0.of();
		resultPanel.setWidth("100%");
		resultPanel.setHeight("70vh");
		resultPanel.setVflex("min");

		// Форма с обработкой отправки
		container.appendChild(new Xform(formModel) {
			@Override
			public void onSubmitXform(Map<String, Object> mapModel) {
				String dir1 = ((String) mapModel.get("path1")) == null ? "" : ((String) mapModel.get("path1")).trim();
				String dir2 = ((String) mapModel.get("path2")) == null ? "" : ((String) mapModel.get("path2")).trim();

				// Очищаем предыдущий вывод
				resultPanel.getChildren().forEach(Component::detach);

				if (dir1.isEmpty() || dir2.isEmpty()) {
					resultPanel.appendChild(new Lb("⚠️ Пожалуйста, заполните оба поля путей"));
					return;
				}
				...
				try {
					...
					// Базовая валидация существования директорий
					if (!Files.exists(p1) || !Files.isDirectory(p1)) {
						resultPanel.appendChild(new Lb("❌ Директория 1 не существует: " + p1));
						return;
					}
					if (!Files.exists(p2) || !Files.isDirectory(p2)) {
						resultPanel.appendChild(new Lb("❌ Директория 2 не существует: " + p2));
						return;
					}

//					// Используем ваши готовые компоненты для построения дерева
//					NodeDir node1 = NodeDir.ofDir(Sdn.get(), p1);
//					NodeDir node2 = NodeDir.ofDir(Sdn.get(), p2);

					List<Path>[] lists = DirDiff.compareDirectories(p1, p2);

					window.appendChild(new DirDiffViewPageSP.DiffPanel(lists));
					// DNode отрисовывает два дерева рядом (в двух колонках)
//					resultPanel.appendChild(DNode.of(node1, node2));

				} catch (Exception e) {
					resultPanel.appendChild(new Lb("❌ Ошибка при обработке: " + e.getMessage()));
				}
			}
		});
	...
	private void sectionVkPost() {
		addHeaderCom("ZRadio");

//		window.appendChild(new Top());
//		window.appendChild(new ZRadioView());

//		Xform.ofMap(form, onOk)._showInWindowEmbed();
	}

	private void sectionZRadio() {
		addHeaderCom("ZRadio");
...
//		Xform.ofMap(form, onOk)._showInWindowEmbed();
	...
	private void sectionXform() {
		addHeaderCom("Xform");
		Map<String, Object> form = MAP.of("start", "", "end", "", "filname", "");
		FunctionV1<Map<String, Object>> onOk = mapModel -> {
			X.p(mapModel);
		};

		window.appendChild(new Xform(form) {
			@Override
			public void onSubmitXform(Map<String, Object> mapModel) {
				X.p(mapModel);
			}
...
//		window.appendChild(DirView0.openWithSimpleMenu(path));
	...
	private void sectionTree() {
		try {
			...
			addErrCom(e, "Tree ZJar");
		}
	}

	private void addErrCom(Exception e, String head) {
		window.appendChild(Lb.ERR(ERR.getMessagesAsStringWithHead(e, head)));
	}
...
//		Bandbox0 bandbox1 = new FsBandbox(target);
	...
	private void sectionListboxXls() {
		addHeaderCom("Work with Xls");
		Listbox0 def = Listbox0.fromXlsx(Paths.get("/home/dav/pjm/utl-poi/src/main/java/app_poi/db.xlsx"));
		appendChild(def);
	}

	private void sectionListboxDb() {
...
//		UTree tree = UTree.tree("def");
		ICtxDb tree = AppCore.__VERSIONED();
//		UTree tree = (UTree) iCtxDb;
		if (tree.isEmptyDb()) {
			tree.put("key", "val");
		}

//		Path def1 = UTree.tree("def").getDbFilePath();

		Listbox0 def = Listbox0.fromCtxDb(tree.getDbFilePath());
