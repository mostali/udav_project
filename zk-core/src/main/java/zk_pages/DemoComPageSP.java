...
public class DemoComPageSP extends PageSP implements IPerPage {
	...
	public void buildPageImpl() {
		...
		appendChild(new Bt("run jira").onCLICK(e->{
				...
				Sys.open_Browser("https://ias-tst-job-jira.otr.ru/browse/" + gm.get("key"));
//				X.exit();
		}));
...
//			FileUploaderComposer.loadComponent("asd", Paths.get("./tmp"), window);
		...
		hello.getOrCreateMenupopup(window).addMI_Href(Tbx2_CfrmSerializableEventListener.toMI("22", "", new String[2], new String[2], func));
...
//		testDnd();
	...
	private void sectionFs() {
		...
		Path path = NodeDir.ofNodeName("dev", "page", "item").toPath();
//		window.appendChild(new DirView0(path, true));
		window.appendChild(DirView0.openWithSimpleMenuAsForm(path));
...
//		appendChild(Chart0.newChart());
	...
	private void sectionDDnode() {
		...
		window.appendChild(DNode.of(NodeDir.ofDir(Sdn.getRq(), n1), NodeDir.ofDir(Sdn.getRq(), n2)));
	}
	...
	private void sectionBandbox() {
		...
		UFS_BASE.MKDIR.createDirs(dirExample1);
		UFS_BASE.MKDIR.createDirs(dirExample2);
		UFS_BASE.MKFILE.createEmptyFileIfNotExist(fileExample1);
