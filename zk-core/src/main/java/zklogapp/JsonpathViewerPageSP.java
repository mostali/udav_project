...
@PageRoute(pagename = "jsonview", role = ROLE.ANONIM)
public class JsonpathViewerPageSP extends PageSP implements IPerPage, WithLogo, WithSearch {//, WithLogo

	public JsonpathViewerPageSP(Window window, SpVM spVM) {
		super(window, spVM);
	}
...
//		window.appendChild(new LogPageHeader());
		...
		Bt showDocs = (Bt) new Bt(SYMJ.LINK + " Show Git Documentation", "gray").onCLICK(e -> {
			ZKJS.openWindow800_1200("https://github.com/json-path/JsonPath");
...
//			ZKM.showModal("Docs", Xml.ofFrame("https://github.com/json-path/JsonPath"));
			ZKJS.openWindow800_1200("http://jsonpath.com/");
		}).padding(3);
		;
