...
public class LogDirView extends Div0 implements IReRender {
	...
	public static LogDirView removeMeFirst(LogDirView... defRq) {
		return IZCom.removeMeFirst(LogDirView.class, true, defRq);
	}
	...
	protected void init() {
		...
		Predicate<Path> pathArcPredicate = (Path p) -> GEXT.ARC.notHasIn(p);
		Predicate<Path> pathCsvPredicate = (Path p) -> GEXT.CSV.notHasIn(p);
		...
		FunctionV2<Menupopup0, Path> apllierDirMenu = (menupopup, dir) -> ALM.applyLogDir(menupopup, dir);
		FunctionV2<Menupopup0, Path> apllierFileMenu = (menupopup, file) -> ALM.applyLogFile(menupopup, file);
	...
	public static class MergeableDirView extends DirView implements IZComFadeIO {
		...
		public MergeableDirView(Path dir, boolean opened) {
			...
			ZKS.BGCOLOR(this, UColorTheme.WHITE[1]);
		}
		...
		protected void onClickFdView(Event event, FdView fdView) {
			...
			ZKI_Log.log("Choiced files to merge" + ":" + UF.fn(mergeFiles));
