package zklogapp;

import lombok.RequiredArgsConstructor;
import zk_com.base_ctr.Div0;
import zk_com.core.IZCom;
import zk_old_core.AppZosCore_Old;
import zk_os.AppZosProps;
import zk_page.ZKS;
import zk_page.node_state.PropFormItem;
import mp.utl_odb.tree.AppPropDef;

import java.util.List;

@RequiredArgsConstructor
public class AppLogSettingsPanel extends Div0 {

	public static AppLogSettingsPanel findFirst(AppLogSettingsPanel... defRq) {
		return IZCom.findFirstInPage(AppLogSettingsPanel.class, true, defRq);
	}

//	final Object[] treeConfigs;

	@Override
	protected void init() {
		super.init();

		ZKS.toggleDnone(this);
		ZKS.ABSOLUTE(this);
		ZKS.WIDTH(this, 100.0);

		List<AppPropDef> allProps = AppPropDef.getAllProps(AppZosProps.class, AppZosCore_Old.TREE_PROPS, AppLogProps.class, AppLogCore.TREE_PROPS);
		allProps.forEach(i -> appendChild(new PropFormItem(i)));

	}

}
