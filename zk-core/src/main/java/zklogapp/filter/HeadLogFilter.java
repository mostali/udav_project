...
public abstract class HeadLogFilter extends Div0 implements ILogFilterProcessor {
	...
	public HeadLogFilter(Bt btHeadOk, String path) {
		...
		this.edvPath = new EditableValue(path) {
			...
			protected void init() {
				...
				ZKS.COLOR(edvPath, UColorTheme.YELLOW[0]);
			...
			protected void onChangeStateToEditableOrLabel(boolean editOrLabel, boolean... isCancel) {
				if (editOrLabel) {
					super.onChangeStateToEditableOrLabel(editOrLabel, isCancel);
					return;
				}
				String newFd = getEditCom().getValue();
				EFT orgFileType = getOrgFileType();
				if (!orgFileType.existSave(newFd)) {
					ZKI_Log.alert("Except existed %s '%s'", orgFileType, newFd);
					return;
				}
				super.onChangeStateToEditableOrLabel(editOrLabel, isCancel);

				LogPageHeader.addLogView(newFd);
	...
	protected void init() {
		...
		cbCollapseMultiLine = (Cb) new Cb().moldToggle().checked(!(btHeadOk instanceof BtMerge)).onDefaultAction(e -> onDefaultActionEvent(e));
...
//		});
		...
		ZKS.BGCOLOR(this, UColorTheme.BLUE[0]);
	...
	private static Path toFileStopList(String path) {
		if (UFS.isDir(path)) {
			return null;
		}
		String newFileName = UF.pr(path) + EXT.fn(path, true) + "+." + EXT.ext(path, true);
		return UF.pF(newFileName, null);
	}
	...
	public ArrayList<String> processLines(LogGetterDate logGetterDate, List<String> linesIn, boolean explodeMultiline) {
		...
		ZKI_Log.log("Will be used filter:" + filters.stream().map(f -> f.toFilter()).filter(X::NN).map(StringCondition::toStringFnPart).collect(Collectors.joining("; ")));
		...
		ZKI_Log.log("Apply filter before/after %s/%s", X.sizeOf(linesIn), X.sizeOf(linesOut));
