...
public class LogLevelFilter extends Span0 implements ILogFilter {
	...
	protected void init() {
		...
		appendChild(cbs[0] = (CbLogLevel) new CbLogLevel(Lev.TRACE, state, cbOnOff).onDefaultAction(Events.ON_CLICK, event -> LogLevelFilter.this.onDefaultActionEvent(event)));
		appendChild(cbs[1] = (CbLogLevel) new CbLogLevel(Lev.DEBUG, state, cbOnOff).onDefaultAction(Events.ON_CLICK, event -> LogLevelFilter.this.onDefaultActionEvent(event)));
		appendChild(cbs[2] = (CbLogLevel) new CbLogLevel(Lev.INFO, state, cbOnOff).onDefaultAction(Events.ON_CLICK, event -> {
			LogLevelFilter.this.onDefaultActionEvent(event);
		}));
		appendChild(cbs[3] = (CbLogLevel) new CbLogLevel(Lev.WARN, state, cbOnOff).onDefaultAction(Events.ON_CLICK, event -> LogLevelFilter.this.onDefaultActionEvent(event)));
		appendChild(cbs[4] = (CbLogLevel) new CbLogLevel(Lev.ERROR, state, cbOnOff).onDefaultAction(Events.ON_CLICK, event -> LogLevelFilter.this.onDefaultActionEvent(event)));

		ZKS.FLOAT(this, false);
		ZKS.BGCOLOR(this, UColorTheme.GREEN[0]);
