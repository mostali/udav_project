...
//	private Dtx dtxFrom, dtxTo;
	...
	protected void init() {
		...
		dtxFrom = (Tbx) new Tbx().autoDateFormat().placeholder("From").onDefaultAction(Events.ON_CHANGE, event -> BwDatesFilter.this.onDefaultActionEvent(event));
		dtxTo = (Tbx) new Tbx().autoDateFormat().placeholder("To").onDefaultAction(Events.ON_CHANGE, event -> BwDatesFilter.this.onDefaultActionEvent(event));
		...
		ZKS.BGCOLOR(this, UColorTheme.GREEN[1]);
//		ZKS.DISPLAY(this, 1);
