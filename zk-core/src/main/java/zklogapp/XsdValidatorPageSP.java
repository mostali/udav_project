...
@PageRoute(pagename = "xsdv", role = ROLE.USER)
public class XsdValidatorPageSP extends PageSP implements IPerPage, WithLogo {//, WithLogo
	...
	public void buildPageImpl() {
		...
		window.appendChild(getLogoOrCreate());
...
//		xmlPan.setWidgetListener("onBind", X.f_(ondBind, sel, sel,sel)); //initialize client side paste listener
	...
	private void update(boolean... force) {
		if (ARG.isDefEqTrue(force) || start.diff() > 5000) {
			try {
