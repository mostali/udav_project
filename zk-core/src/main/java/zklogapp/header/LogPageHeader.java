...
public class LogPageHeader extends Div0 implements IReRender {

	public static LogPageHeader rerender(LogPageHeader... defRq) {
		LogPageHeader first = removeMeFirst(defRq);
		if (first != null) {
			first.rerender(true);
		}
		return first;
	...
	public static LogPageHeader removeMeFirst(LogPageHeader... defRq) {
		return IZCom.removeMeFirst(LogPageHeader.class, true, defRq);
	}
...
//	}
	...
	public static void addLogView(String file, String name) {
		AppLogCore.TREE_QUICK_HISTORY.put(file, name);
		rerender(null);
	}
...
//			center.appendLn(defAction, ALI.QUICKVIEW).td_none();
		...
		center.appendLn((DefAction) e -> NodeFactory.openFormIdentitySinglyAsWin0(NodeDir.ofNodeName("Notes", ppi().sdn()), false, false), "Config").decoration_none().padding("0 5px");
//		center.appendLn((DefAction) e -> ZKS.toggleDnoneFirst(NotesWin.class, false, null), "Notes").td_none().padding("0 5px");
//		center.appendLn((DefAction) e -> {});
	...
	private void appendLeft_LastFilesPanel() {
		...
		appendLn((DefAction) e -> {
			AppLogCore.TREE_QUICK_HISTORY.clear();
			rerender(null);
		...
		List<Pare3<String, String, String>> pares = AppLogCore.TREE_QUICK_HISTORY.getPares();
		for (Pare3<String, String, String> memLink : pares) {

			String keyFile = memLink.getKey();
			String valName = memLink.getVal();
			String valNameShort = STR.toStringSE(valName, 3, valName);
			EFT eft = EFT.of(keyFile, null);
			if (eft == null) {
				AppLogCore.TREE_QUICK_HISTORY.remove(keyFile);
				continue;
			}
			Ln lnToDirOrFile;
			switch (eft) {
	...
	private static void addMI_RemoveFromPanel(Menupopup0 menu, String keyFile) {
		menu.addMenuitem("Remove from panel", e -> AppLogCore.TREE_QUICK_HISTORY.remove(keyFile));
	}
