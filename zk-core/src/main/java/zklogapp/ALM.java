...
public class ALM {
	...
	public static void applyLogDirWithUtils(Menupopup0 menupopup, Path path) {
		DirView0.applyMenuDir(menupopup, path);
	...
	private static void applyDirMenuUtils(Menupopup0 menupopup, Path path) {
		...
		fileUtilsMenu.addMI(ALI.SEARCH_FILE + "Find file twins..", e -> {
			...
			if (X.empty(twins)) {
				...
				SerializableEventListener btRmEvent = (btE) -> ZKI_Messagebox.showMessageBoxBlueYN("Removing twins..", "Remove twins files?", (Boolean yn) -> {
					if (!yn) {
						return;
					}
		...
		fileUtilsMenu.addMI(ALI.SEARCH_FILE + "Find empty files..", e -> {
			...
			if (X.empty(emptyFiles)) {
				...
				AtomicReference<Window> ref = new AtomicReference();
				SerializableEventListener btRmEvent = (btE) -> ZKI_Messagebox.showMessageBoxBlueYN("Removing empty files..", "Remove this files?", (Boolean yn) -> {
					if (!yn) {
						return;
					}
...
//		fileUtilsMenu.addMenuitem(fillFromJiraComment);
	...
	public static void applyLogFileWithUtils(Menupopup0 menupopup, Path path) {
		DirView0.applyMenuFile(menupopup, path);
		menupopup.add_______();
	...
	public static void applyLogUtils(Menupopup0 menupopup, Path path) {
		...
		menupopup.add_______();
		menupopup.addMI(ALI.STATS + "Log Analyze GroupByThread", e -> ZKME.openEditorTextReadonly("Group By Thread", Rt.buildReport(LogStatAnalyze_GroupBy.collectStatsByThread(pathStr, AppZos.getLogGetterDate()), "LogStats:" + pathStr).toString(), true).toString());
		menupopup.addMI(ALI.STATS + "Log Analyze GroupByClass", e -> ZKME.openEditorTextReadonly("Group By Class", Rt.buildReport(LogStatAnalyze_GroupBy.collectStatsByGroup(pathStr, AppZos.getLogGetterDate()), "LogStats:" + pathStr).toString(), true).toString());
		menupopup.add_______();
		menupopup.addMI(ALI.STATS + "Log Analyze Thread History", e -> ZKME.openEditorTextReadonly("Group By Thread History", LogStatAnalyze_ThreadHistory.collectHistoryByThread_REPORT(pathStr, AppZos.getLogGetterDate(), AppZos.getLogLineMapping()).toString(), true).toString());
