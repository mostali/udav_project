...
//		fileUtilsMenu.addMenuitem(fillFromJiraComment);
	...
	public static void applyLogUtils(Menupopup0 menupopup, Path path) {
		...
		menupopup.add_______();
		menupopup.addMI(ALI.STATS + "Log Analyze GroupByThread", e -> ZKME.openEditorText("Group By Thread", Rt.buildReport(LogStatAnalyze_GroupBy.collectStatsByThread(pathStr, AppZos.getLogGetterDate()), "LogStats:" + pathStr).toString(), true).toString());
		menupopup.addMI(ALI.STATS + "Log Analyze GroupByClass", e -> ZKME.openEditorText("Group By Class", Rt.buildReport(LogStatAnalyze_GroupBy.collectStatsByGroup(pathStr, AppZos.getLogGetterDate()), "LogStats:" + pathStr).toString(), true).toString());
		menupopup.add_______();
		menupopup.addMI(ALI.STATS + "Log Analyze Thread History", e -> ZKME.openEditorText("Group By Thread History", LogStatAnalyze_ThreadHistory.collectHistoryByThread_REPORT(pathStr, AppZos.getLogGetterDate(), AppZos.getLogLineMapping()).toString(), true).toString());
