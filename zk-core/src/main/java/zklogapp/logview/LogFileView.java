...
public class LogFileView extends Div0 implements IReRender {
	...
	public static void openSingly(String pathFile, String qDelay) {
		IT.isFileExist(pathFile);
		LogFileView.removeMeFirst(null);
		ZKC.getFirstWindow().appendChild(new LogFileView(pathFile).withQdelay(qDelay));
	}

	Long filterMs = null;

	private LogFileView withQdelay(String qDelay) {
		filterMs = QDelayType.toLong(qDelay, TimeUnit.MILLISECONDS, null);
		return this;
	}
	...
	private ArrayList<String> applyFilterPreocessed() {
		...
		if (filterMs != null) {
			...
			QDate[] startEnd = QDateBetween.rangeBetween_NowAndDiffMs(filterMs);
