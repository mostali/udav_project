...
public class Grep_CfrmSerializableEventListener extends Tbx_CfrmSerializableEventListener {
	...
	public static Mi toMenuItemComponent(Path path, String initValue, Function<List<String>, Path>... successGrepCallback) {
		String title_cap_com = AZI.SEARCH_TEXT + "Grep " + (UFS.isDir(path) ? "dir" : "file") + " '" + path + "'";
	...
	public static Mi toMenuItemComponent(Object title_cap_com, Path path, String initValue, Function<List<String>, Path>... successGrepCallback) {
		...
		String label = AZI.SEARCH_TEXT + "Grep " + (isDir ? "dir.." : "file..");
		Mi menuItemComponent = Grep_CfrmSerializableEventListener.toMenuItemComponent(title_cap_com, label, initValue, "Search phrase", (String s) -> {
			try {
				...
				Set<String> logslines = lines.stream().map(l -> USToken.lastGreedy(l, ':', "").trim()).filter(X::NE).collect(Collectors.toSet());
				Date[] minMax = LogGetterDate.findMinMaxDate(logslines, logGetterDate);
				Set<String> uniqFiles = lines.stream().map(l -> isDir ? USToken.first(l, ':', "").trim() : l).filter(X::NE).collect(Collectors.toSet());
				List<String> mergedLines = ARR.mergeToList(uniqFiles, ARR.as("---------------first & last date------------"), ARR.as(logGetterDate.toString(minMax[0], null), logGetterDate.toString(minMax[1], null)), ARR.as("---------------with content------------"), lines);
				ZKI.infoEditorBw(mergedLines);
