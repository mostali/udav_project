...
public class FileView extends FdView {
	...
	protected void init() {
		...
		boolean bigFile = fileSizeMb > AppZosConfig.FILE_MAX_SIZE_MB;
		...
		if (!bigFile) {
			...
			fileRoot.onDBLCLICK(event -> ZKI.alert(SYMJ.WARN + "File has size %sMb more that %sMb", fileSizeMb, AppZosConfig.FILE_MAX_SIZE_MB));
		}
