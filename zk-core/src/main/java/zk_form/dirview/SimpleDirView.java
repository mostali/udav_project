...
//		menupopup.addContextMenuSeparator();
		menupopup.addMenuItem(ALI.OS_OPEN + "Open Dir", getEventOpenSimpleMenu_OS(path));
		menupopup.addMenuItem(SYMJ.SEARCH_LUPA_RIGHT + " Search Files (Glob)", SLib.of(path.getParent()).toEventShowInModal());
	...
	public static SerializableEventListener getEventOpenDirViewWithSimpleMenu(Path path) {
		return e -> SimpleDirView.openWithSimpleMenu(path)._modal();
	}
