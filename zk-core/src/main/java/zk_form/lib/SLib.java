...
public class SLib {
	...
	private static String name(int code) {
		Map<String, Integer> codes = RFL.fieldValuesMap(SLib.class, null, int.class, true, false);
		...
		switch (code) {
			...
			case SQLITE:
				if (isTreeDbType()) {
					...
					Ddl ddl = new Ddl(Db.getAllTableNames(path)) {
						@Override
...
//							ZKM.showModal( Listbox0.fromSqliteDb(path))
	...
	public HtmlBasedComponent toCom() {
		switch (code) {
			...
			case SQLITE:
				if (isTreeDbType()) {
					return Listbox0.fromTreeDb(path);
				}
