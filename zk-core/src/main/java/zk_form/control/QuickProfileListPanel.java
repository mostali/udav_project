...
public class QuickProfileListPanel extends Div0 {
	...
	private void appendRight_StopPhrasesPanel(Div0 topDiv) {
		...
		for (Pare3<String, String, String> profiles : pares) {
			...
			DefAction defAction = e -> ZKME.openEditorWithBtSave(() -> profileData, newSaveCallback(tbxKey), Span0.of(tbxKey, btRmList), false);
	...
	private SerializableEventListener newProfileEventAction(String... withContent) {
		...
		SerializableEventListener defAction = e -> ZKME.openEditorWithBtSave(() -> ARG.toDefOr(PFX_QUICK_DATA, withContent), newSaveCallback(tbxKey, needCheckExist), tbxKey, false);
		return defAction;
	}
