...
public class QuickProfileListPanel extends Div0 {
	...
	public static QuickProfileListPanel findFirst(QuickProfileListPanel... defRq) {
		return ZKCFinder.findFirstIn_Win(QuickProfileListPanel.class, true, defRq);
	}
	...
	private void appendRight_StopPhrasesPanel(Div0 topDiv) {
		...
		for (Pare3<String, String, String> profiles : pares) {
			...
			DefAction defAction = e -> ZKM_Editor.openEditorWithBtSave(() -> profileData, newSaveCallback(tbxKey), Span0.of(tbxKey, btRmList), false);
	...
	private SerializableEventListener newProfileEventAction(String... withContent) {
		...
		SerializableEventListener defAction = e -> ZKM_Editor.openEditorWithBtSave(() -> ARG.toDefOr(PFX_QUICK_DATA, withContent), newSaveCallback(tbxKey, needCheckExist), tbxKey, false);
		return defAction;
	}

	private SerializableEventListener rmProfileEventAction(String profileName) {
		return e -> ZKI_Messagebox.showMessageBoxBlueYN(X.f("Removing profile '%s'", profileName), X.f("Remove profile '%s'?", profileName), (yn) -> {
			if (yn) {
