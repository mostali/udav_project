...
public class Cnn {
	...
	public Cnn ref(Component ref) {
		this.ref = ref;
		return this;
	}

	public static Cnn ERR(String message, Pos... pos) {
		return Cnn.builder().level(NotifyMessageRtException.LEVEL.RED).duration(DEF_DURATION).pos(ARG.toDefOr(DEF_POS, pos)).msg(message).build();
	}

	public static Cnn INFO(String message, Pos... pos) {
		return Cnn.builder().level(NotifyMessageRtException.LEVEL.GREEN).duration(DEF_DURATION).pos(ARG.toDefOr(DEF_POS, pos)).msg(message).build();
	}
