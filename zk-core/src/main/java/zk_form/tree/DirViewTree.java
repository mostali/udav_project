...
public class DirViewTree extends Div0 {
	...
	public static class RootTreeNode extends DefaultTreeNode {
		public RootTreeNode(File dir) {
			super(dir.getName(), PathTreeNode.toList(dir));
		}
	}
	...
	public static class PathTreeNode extends DefaultTreeNode {
		...
		public static PathTreeNode of(File file) {
			if (file.isDirectory()) {
				return new PathTreeNode(file.getName(), toList(file));
			}
			return new PathTreeNode(file.getName());
		}

		public static PathTreeNode[] toList(File file) {
...
//		}
//		private void loadChildrenIfNeed() {
//			if (!loadChildren) {
//				navigationService.reloadWithChildren(this);
//				loadChildren = true;
//			}
//		}
	}
	...
	protected void init() {
		...
		tree.addEventListener(Events.ON_SELECT, (SelectEvent event) -> {
			...
			X.p(sb);
			X.p("----------");
			Pare3<TreeNode, Treeitem, Boolean> selected = Tree0.findSelectedNode(event);
			X.p(selected);
			Tree0.swapAllChilds(tree, event, true);

		});
		if (true) {
			DefaultTreeModel stm = new DefaultTreeModel(new RootTreeNode(new File("/home/dav/pjnsi_tasks/21")));
//			DefaultTreeModel stm = new DefaultTreeModel(new DefaultTreeNode("ROOT",
//					Arrays.asList(new DefaultTreeNode("David", new ArrayList()),
//							new DefaultTreeNode("Thomas", new ArrayList()),
//							new DefaultTreeNode("Steven", new ArrayList()))));
			stm.setMultiple(true);
			tree.setModel(stm);
