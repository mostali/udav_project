...
import zk_com.base_ctr.Panel0;
import zk_com.listbox.CellDbEditableValue;
...
public class XTableView extends Div0 {
    public final String pathDb;
    private final boolean isModal;
    private final AtomicReference<String> activeTable = new AtomicReference<>();

    public static XTableView openAsModalWindow(Path pathDb) {
        XTableView ctxTreeView = new XTableView(pathDb.toString(), true);
        ctxTreeView.init();
        return ctxTreeView;
    }

    public static XTableView createView(Path pathDb) {
        XTableView ctxTreeView = new XTableView(pathDb.toString(), false);
        ctxTreeView.init();
        return ctxTreeView;
    }
    ...
    protected void init() {
        ...
        AtomicReference<ICtxDb> treeDbRef = new AtomicReference<>();
        if (activeTable.get() == null) {
            activeTable.set(ICtxDb.getTablename(path));
        }
        treeDbRef.set(ICtxDb.of(path, activeTable.get()));

        if (!treeDbRef.get().isExistDb()) {
        ...
        ICtxDb iCtxDb = treeDbRef.get();
        Lb calLb = Lb.of("Db View");
        ...
        Div0 cap = Div0.of(closeDbView, calLb, Xml.NBSP(3), putNew, Xml.NBSP(2), putRemove, Xml.NBSP(2), choiceTable);
        ...
        FunctionV rebuilder = () -> {
            ...
            if (parent == null) {
//				Window win = ZKM.showModal(cap, listbox, ZKC.getFirstWindow(), new String[]{"100%", "6334px"});
                Panel0 win = Panel0.of(cap, listbox);
                win.onConfigure();
                ZKC.getFirstWindow().appendChild(win);
                holderCom.set(Pare.of(win, listbox));
            } else {
...
//			ZKS.HEIGHT(listbox, 6333);
        ...
        //
        putNew.onCLICK(pe -> {
            treeDbRef.get().put(null);
            rebuilder.apply();
        });

        putRemove.onCLICK(re -> {
            Listbox0 listbox0 = (Listbox0) holderCom.get().val();
            List<Component> children = listbox0.getChildren();
            for (Component child : children) {
                if (child instanceof Listitem) {
                    boolean selected = ((Listitem) child).isSelected();
                    if (selected) {
                        CellDbEditableValue cellDbEditableValue = (CellDbEditableValue) child.getChildren().get(0).getFirstChild();
                        String name = cellDbEditableValue.getVal().name();
                        if (CN.ID.equals(name)) {
                            Long id = (Long) cellDbEditableValue.getVal().getValue();
                            String msg = "Remove row #" + id + " ?";
                            ZKI_Quest.showMessageBoxBlueYN(msg, msg, y -> {
                                if (y) {
                                    treeDbRef.get().removeById(id);
...
//            switch (tablename) {
//                case ICtxDb.D10:
//                    treeDbRef.set(Ctx10Db.of(path));
//                    break;
//                case ICtxDb.D5:
//                    treeDbRef.set(Ctx5Db.of(path));
//                    break;
//                case Ctx3Db.CtxModelCtr.TN_DATAWT:
//                    treeDbRef.set(Ctx3Db.of(path));
//                    break;
//                default:
//                    Listbox0 modalCom = Listbox0.fromSqliteDb(path, AppZosProps.DEF_TREE_LIMIT.getValueOrDefault(), tablename);
//                    window = ZKM.showModal("Sqlite DB View", modalCom, ZKM.WH100_MIN);
//                    break;
//            }
//            if (window == null) {
