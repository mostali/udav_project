...
public class MenuPicker<T> extends ItemsPicker<T> {
	...
	public static LmMenuPicker ofAllForms(String sd3, String page) {
		List<String> fileNames = UFS.toFileNames(SpaceType.NODES.ls(Sdn.of(sd3, page), UO.VIEW));
//		List<String> fileNames = UFS.toFileNames(AFC.FORMS.DIR_FORMS_LS_CLEAN(sd3, page));
		return new LmMenuPicker(Sdn.of(sd3, page), ItemPath.filterNoIndexPath(fileNames), SpaceType.NODES);
	}

	public static LmMenuPicker ofAllPages(String sd3) {
		List<String> fileNames = UFS.toFileNames(AFC.PAGES.DIR_PAGES_LS_CLEAN(sd3));
		return new LmMenuPicker(Sdn.of(sd3, NodeID.PAGE_INDEX_ALIAS), ItemPath.filterNoIndexPath(fileNames), SpaceType.PAGES);
	}

	public static LmMenuPicker ofAllSd3() {
		List<String> fileNames = UFS.toFileNames(AFC.PLANES.DIR_PLANES_LS_CLEAN(false));
		return new LmMenuPicker(Sdn.of(NodeID.PLANE_INDEX_ALIAS, NodeID.PAGE_INDEX_ALIAS), ItemPath.filterNoIndexPath(fileNames), SpaceType.SPACES);
	}
