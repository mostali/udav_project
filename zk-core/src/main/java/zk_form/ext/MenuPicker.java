...
//				ZKS.BORDER_GRAY(modalCom);
	...
	public static Component ofAllForms(String sd3, String page) {
		List<String> fileNames = UFS.toFileNames(AFC.FORMS.DIR_FORMS_LS_CLEAN(sd3, page));
		return new LmMenuPicker(fileNames, LeftMenu.SpaceType.NODES);
	}

	public static Component ofAllPages(String sd3) {
		return new LmMenuPicker(UFS.toFileNames(AFC.PAGES.DIR_PAGES_LS_CLEAN(sd3)), LeftMenu.SpaceType.PAGES);
	}

	public static Component ofAllSd3() {
		return new LmMenuPicker(UFS.toFileNames(AFC.PLANES.DIR_PLANES_LS_CLEAN(false)), LeftMenu.SpaceType.SPACES);
	}
