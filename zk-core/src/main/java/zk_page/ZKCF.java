...
//		}
	...
	public static <T> T findFirstIn(boolean inPage_or_inWin, Class<T> clazz, boolean recursive_or_first, T... defRq) {
		List<T> all = inPage_or_inWin ? rootsByClass((Class) clazz, recursive_or_first, (List) null) : findAllInWin((Class) clazz, recursive_or_first, (List) null);
		if (X.notEmpty(all)) {
			return all.get(0);
		}
	...
	public static <C extends Component> List<C> rootsByClass(Class<C> comClazz, boolean recursive_or_first, List<C>... defRq) {
...
//		Iterator<Component> it = src instanceof Page ? ((Page) src).getRoots().iterator() : ((Component) src).getChildren().iterator();
		Iterator<Component> it = src.getRoots().stream().iterator();
		...
		return ARG.toDefThrow(() -> new RequiredRuntimeException("Component '%s' (recursive=%s) not found from src '%s'", comClazz, recursive_or_first, src), defRq);
	}
	...
	public static <C extends Component> C removeMeFirst(Class<C> comClass, boolean... recursive) {
		C c = ZKCF.findFirstIn_Win(comClass, ARG.isDefEqTrue(recursive), null);
		if (c != null) {
	...
	public static <C extends Component> List<C> removeMeAllByFilenameInWin(Class<C> comClass, IPath path, boolean... recursive) {
		List allByFilename = ZKCF.findAllByFilename((Class) comClass, path, ARR.EMPTY_LIST);
...
//		ZKComFinder.findAllInPage(Window.class,true).stream().flatMap(l->l.getChildren().stream()).collect(Collectors.toList())
	...
	public static <C extends Component> List<C> removeMeAllInWin(Class<C> comClass, boolean checkWraperWin, boolean... recursive) {
		List<C> all = ZKCF.findAllInWin(comClass, ARG.isDefEqTrue(recursive), ARR.EMPTY_LIST);
		all.forEach(i -> {
			if (checkWraperWin) {
	...
	public static <V extends IPath> List<V> findAllByFilename(Class<V> openViewClass, IPath fname, List<V>... defRq) {
		List<V> all = ZKCF.rootsByClass((Class) openViewClass, true, null);
		if (all != null) {
			return all.stream().filter(i -> fname.equals(i.fPath())).collect(Collectors.toList());
		}
	...
	public static <V extends IPath> V findByFilename(Class<V> openViewClass, Predicate<IPath> filter, V... defRq) {
		List<V> all = ZKCF.rootsByClass((Class) openViewClass, true, ARR.EMPTY_LIST);
