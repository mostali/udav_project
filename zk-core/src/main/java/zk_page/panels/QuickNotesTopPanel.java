...
public class QuickNotesTopPanel extends Div0 implements IReRender {
	...
	public static void addPhrases(String prevKeyPhrases, String keyPhrases, String phrases) {
		if (!keyPhrases.equals(prevKeyPhrases)) {
			AppCoreNotes.TREE_QUICK_NOTES_HISTORY().removeByKeyIfExist(prevKeyPhrases);
		}
		AppCoreNotes.TREE_QUICK_NOTES_HISTORY().put(keyPhrases, phrases);
		rerender(null);
	}
	...
	public static void addLogView(String file, String name) {
		AppCoreNotes.TREE_QUICK_NOTES_HISTORY().put(file, name);
		rerender(null);
	}
...
//		ZKS.MARGIN(this, "-30px 0 0 0"); //top
	...
	private void appendRight_StopPhrasesPanel() {
		...
		List<Pare3<String, String, String>> pares = AppCoreNotes.TREE_QUICK_NOTES_HISTORY().getModelsAsPare3();
		for (Pare3<String, String, String> phrases : pares) {
			...
			DefAction act = (e) -> {
				AppCoreNotes.TREE_QUICK_NOTES_HISTORY().removeByKeyIfExist(keyShortName);
...
//			ZKS.COLOR(lnToDirOrFile, UColorTheme.RED[0]);
	...
	private static void addMI_RemoveFromPanel(Menupopup0 menu, String keyFile) {
		menu.addMI("Remove from panel", e -> AppCoreNotes.TREE_QUICK_NOTES_HISTORY().removeByKeyIfExist(keyFile));
	}
