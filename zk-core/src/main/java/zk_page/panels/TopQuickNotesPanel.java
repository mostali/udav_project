...
public class TopQuickNotesPanel extends Div0 implements IReRender {

	public static void openSimple() {
		ZKC.getFirstWindow().appendChild(new TopQuickNotesPanel());
	}

	public static TopQuickNotesPanel rerender(TopQuickNotesPanel... defRq) {
		TopQuickNotesPanel first = removeMeFirst(defRq);
		if (first != null) {
			first.rerender(true);
		}
		return first;
	...
	public static TopQuickNotesPanel removeMeFirst(TopQuickNotesPanel... defRq) {
		return ZKC.removeMeFirst(TopQuickNotesPanel.class, true, defRq);
	}
	...
	private void appendRight_StopPhrasesPanel() {
		...
		for (Pare3<String, String, String> phrases : pares) {
			...
			DefAction act = (e) -> {
				...
				TopQuickNotesPanel.rerender(null);
			};
