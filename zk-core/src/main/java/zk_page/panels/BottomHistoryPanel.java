...
public class BottomHistoryPanel extends Div0 implements IReRender {
	...
	public static void addItem(BottomHistoryPanel firstPanel, String file, boolean open, boolean withInfo) {
		...
		String btName = ALI.LOGVIEW + QDate.now().f(QDate.F.HH_mm) + "*" + Hu.sizeOfFile(filePath);
		...
		ZKS.BGCOLOR(bt, EColor.GREEN.nextColor());
		ZKS.COLOR(bt, EColor.BLACK.nextColor());
...
//			DefAction defAction = a -> ZKR.restartPage();
		...
		Bt clearAll = appendBt(new CfrmRemoveFileSEL(TMP_STORE_DIR.toString(), defAction), "Clear all");
//		}
