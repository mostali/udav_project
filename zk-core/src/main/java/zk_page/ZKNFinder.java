...
public class ZKNFinder {
	...
	public static List<Component> findAllNodeCom(NodeDir nodeDir, boolean isForm, boolean unwrapIfInWindow) {
		...
		finded = S_.filterToList(finded, n -> {
			if (isForm) {
		...
		if (unwrapIfInWindow) {
			...
			finded = S_.mapToList(finded, c -> (c.getParent() instanceof Window && c.getParent() != firstWindow) ? c.getParent() : c);
		}
		return finded;
	}
