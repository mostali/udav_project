...
public class SecPSP extends PageSP implements IPerPage, IZState, WithLogo, WithMainTbx, WithSearch {
	...
	static class SecPlanes extends Div0 {
		...
		protected void init() {
			...
			List<Pare3<String, Path, ObjState>> planeStates = PlaneWalker.doWalkToList(withIndex, withSysPlanes, withUserDomains);

			for (Pare3<String, Path, ObjState> planeState : planeStates) {
...
//				String titlePlane = plane.equals(NodeID.PAGE_INDEX_ALIAS) ? SYMJ.HOME2 + " " + plane : plane;
				...
				List<Pare<String, ObjState>> pageStates = PagesWalker.doWalkToList(plane, withSysPages);

				for (Pare<String, ObjState> pageState : pageStates) {
