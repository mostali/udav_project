...
//						return true;
//					}
					map.put(new PageTabpanel0(pagename, Pare.of(sd3, pagename), false), null);
					return true;
				}
		...
		for (Component child : tabs.getChildren()) {
			...
			tab0.addEventListener(Events.ON_DROP, e -> {
				...
				ZKI_Messagebox.showMessageBoxBlueYN("Moving note", msg, (yes) -> {
					if (yes) {
...
//						ZKI.infoRef("Ok - " + msg);
			...
			ANM.applyPageLink(menu, sdn, true);
...
//				String link = new LocalNoteApi().zApiUrl.GET_toPage(sdn);
					String link = RSPath.PAGE.toPlanPage(sdn);
