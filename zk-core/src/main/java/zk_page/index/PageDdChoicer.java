...
public abstract class PageDdChoicer extends BaseDdChoicer {
	...
	public abstract void onChoicePage(Event onPageSubmitEvent, String pagename);
	...
	protected void init() {
		...
		List<String> choices = S_.mapToList(allPages, Pare::key);

		normalizeList(choices);

		Dd dd = new Dd(setPage, choices);
		dd.onSELECTION(onPageSubmitEvent -> {
			String value = dd.getValue();
			if (!value.equals(setPage)) {
				try {
					onChoicePage(onPageSubmitEvent, value);
				} catch (Exception ex) {
