...
//		absolute();
		...
		{ //leftMenu Cb
			...
			if (!SpVM.get().getQuery().hasParam(PageState.TabsMode.tbf.name())) {
				Boolean isEnableMenu = (Boolean) getPageState().getAs(AppNotesProps.NAV_MENU_OPENED, Boolean.class, false);
				lmCb.setChecked(isEnableMenu);
	...
	private Paree3<Boolean, Boolean, Boolean> getDefaultState() {
		RSPath pathType = Sdn.get().getPathType();
		switch (pathType) {
			case ROOT:
