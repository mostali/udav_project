...
//			appendChild(hSep.get());
...

		{
			...
			OptLazySupplier lazySupplier = new OptLazySupplier(() -> appendChild(hSep.get()));

			FunctionV2<String, String> adderNewIcon = (sd3, pagename) -> {
			...
			Function3<String, String, ObjState, Object> applierPage = (plane, pagename, state) -> {
				if (state.fields().get_FIXED(false)) {
					adderNewIcon.apply(sdn.key(), pagename);
				}
				return null;
			};
