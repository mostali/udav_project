...
public class TopSpacePanel extends Win0 implements ISpCom {
	...
	public static void replaceFirst() {
		findFirst().replaceWith(new TopSpacePanel());
	}
...
//		SecState ss = SecState.getSecState();
		...
		{
			Cb cbNav = (Cb) new Cb("NAV").moldToggle().onCLICK(e -> {
...
//                    LeftMenu.detachFirst(null);

					NotesSpace.rerenderFirst();//<-- here top spce panel
//                    TopSpacePanel.replaceFirst();
					NoteLogo.replaceFirst();


				};

				cbRefresh = (Cb) new Cb().moldSwitch().onCLICK(eventListener);
//                cbRefresh = (IZCom) Ln.of(SYMJ.ARROW_REPEAT_RIGHT, eventListener).decoration_none().padding("10px 10px 0px 0px");
				cbRefresh.title("Refresh page");
				cbRefresh.scale(0.6);
//                Ln ln = Ln.of(SYMJ.ARROW_REPEAT_TRIANGLE_BW, eventListener);
//                ln.decoration_none();
//                appendChild(ln);
			}
...
//				if (checkViewEnable && ss.isPlaneHolder_Or_AdminOrOwnerRole) {
				ZKE.sendPostEventClick(cbNav);
//				}
				cbNav.setChecked(checkViewEnable);

			}

			cbNav.font_bold_nice(AxnTheme.FONT_SIZE_MENU);

//			appendChild(hSep.get());

			appendChilds(cbNav, cbRefresh.com());
		...
		if (false) {
			...
			//EDITOR
			{
				...
				appendChild(hSep.get());
				{//add note
					IconLn iconLn = new IconLn(SYMJ.FILE3, (Event e) -> NFNew.openRandomINE(sdn), "Add note", Pare.of(EColor.WHITE), MARGIN_TOP_CONTROL);
					appendChild(iconLn);
				}
				{//add special note
					if (AppZos.isDevEnable()) {

						IconLn iconLn = new IconLn(SYMJ.GENDER_SYM, Pare.of(Events.ON_MOUSE_OVER, openPopupEvent(StdType.values())), "Add special note", Pare.of(EColor.YELLOW), MARGIN_TOP_NOTETYPE);
						appendChild(iconLn);
					}
				}
			}
...

		{
			...
			SupplierLazy singlyAdderSep = new SupplierLazy(() -> appendChild(hSep.get()));
...
//				IconLn iconLn = new IconLn(pagename0, (Event e) -> RSPath.toPage_Redirect(plane0, pagename0), pagename0, iconCol.nextColor(), MARGIN_TOP_CONTROL);
	...
	private static @NotNull SerializableEventListener openPopupEvent(INodeType[] values) {
		Sdn sdn = SpVM.get().sdn();
		Popup0 popup = new Popup0();
		for (int i = 0; i < values.length; i++) {
			INodeType value = values[i];
			Ln lnToAdd = new Ln(value.stdProps().icon() + " " + value.stdTypeUC().toLowerCase());
			popup.appendChild(lnToAdd);
			if (!ARR.isLast(i, value)) {
				popup.appendChild(Xml.NBSP(2));
			}
			lnToAdd.onCLICK(e -> {
				NFNew.OptsAdd opts = NFNew.OptsAdd.newOpts();
				opts.setNodeEvalType(value);
				NFNew.openRandomINE(sdn, opts);
			});
		}
...
//			ZKS.applyNiceBg(this, iconCol.nextColor(), ZKColor.YELLOW.nextColor());
			...
			ZKS.COLOR(iconLn, EColor.BLACK.nextColor());
		}
