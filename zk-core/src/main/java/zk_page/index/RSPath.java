	...
	public String toRootLink() {
		switch (this) {
			case ROOT:
				...
				return "http://" + APP.getAppDomainName() + sfx_ska;
			default:
	...
	public String toPlaneLink(String planeName) {
		switch (this) {
			case PLANE:
				...
				String sfx_ska = SpVM.get().hasSka() ? "?" + Sec.SKA + "=" + AppZosConfig.SUPER_KEY : "";
				return "http://" + planeName + "." + APP.getAppDomainName() + sfx_ska;
			default:
				throw new WhatIsTypeException(name() + " path");
		}
	}
...
//				IT.state(!RSPath.isSd3Index(planeName), "why here plane page '%s'", planeName);
				String sfx_ska = SpVM.get().hasSka() ? "?" + Sec.SKA + "=" + AppZosConfig.SUPER_KEY : "";
				return "http://" + (RSPath.isSd3Index(planeName) ? "" : planeName + ".") + APP.getAppDomainName() + "/" + pagename + (other_path == null ? "" : "/" + other_path) + sfx_ska;
			default:
				throw new WhatIsTypeException(name() + " path");
		}
	}
