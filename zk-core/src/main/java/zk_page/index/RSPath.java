	...
	public String toRootLink() {
		switch (this) {
			case ROOT:
				...
				return "http://" + APP.getAppHost() + sfx_ska;
			default:
	...
	public String toPlaneLink(String planeName) {
		switch (this) {
			case PLANE:
				...
				String sfx_ska = SpVM.get().hasSka() ? "?" + NoteApi.SKA + "=" + AppZosConfig.SUPER_KEY : "";
				return "http://" + planeName + "." + APP.getAppHost() + sfx_ska;
			default:
				throw new WhatIsTypeException(name() + " path");
		}
	}
...
//				IT.state(!RSPath.isSd3Index(planeName), "why here plane page '%s'", planeName);
				String sfx_ska = SpVM.get().hasSka() ? "?" + NoteApi.SKA + "=" + AppZosConfig.SUPER_KEY : "";
				return "http://" + (RSPath.isSd3Index(planeName) ? "" : planeName + ".") + APP.getAppHost() + "/" + pagename + (other_path == null ? "" : "/" + other_path) + sfx_ska;
			default:
				throw new WhatIsTypeException(name() + " path");
		}
	}
