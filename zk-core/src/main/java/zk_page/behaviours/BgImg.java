...
//	}
	...
	public static String getBgImageViaNigthMode(FormState formState) {
		...
		String bgUrl = X.f("url(_bg_img/%s)", vl);
		return bgUrl;
	}
...
//		L.info("Found all Bg Images:" + ls);
		...
		if (o instanceof Path) {
			ls = (List) ls.stream().map((p) -> ((Path) p).getFileName().toString()).collect(Collectors.toList());
		} else if (o instanceof RES) {
			ls = (List) ls.stream().filter(r -> ((RES) r).fType(null) == EFT.FILE).map((p) -> ((RES) p).fName()).collect(Collectors.toList());
		} else {
