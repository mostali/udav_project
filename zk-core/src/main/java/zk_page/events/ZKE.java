...
public class ZKE {
	...
	public static final SerializableEventListener EVENT_LISTENER_INFO_ZLOG = event ->
	{
		if (event instanceof DropEvent) {
			...
			ZKI_Log.log("DropEvent:" + de.getPageX() + "x" + de.getPageY() + ":" + de.getDragged() + ">>>" + de.getTarget() + " <<< " + event);
		} else {
			ZKI_Log.log("Event:" + event);
		}
	};

	public static void addEventListenerAll_ZLOG(Component com, String... exclude) {
	...
	public static void addEventListener(Component w, EventListener eventListener, String... events) {
		List<String> eventsValues = createEvents(events, ARR.EMPTY_STR);
		for (String event : eventsValues) {
			w.addEventListener(event, eventListener != null ? eventListener : new EventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
					ZKI_Window.info("EV:" + event);
				}
			});
		}
	}

	public static void addEventListenerCtrl(XulElement xulElement, String event, EventListener eventListener) {
		xulElement.addEventListener(event, eventListener != null ? eventListener : (SerializableEventListener<Event>) event1 -> ZKI_Window.info("EV-CTRL:" + event1));
	}
