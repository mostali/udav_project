...
//	}
	...
	protected void init() {
		...
		final String property = appProp.name();
		final T defValue = appProp.val();
		...
		IZCom fCom;
		if (defValue instanceof Boolean) {
			fCom = _cbProp = new Cb(value == null ? (Boolean) defValue : (Boolean) value).moldToggle();
		} else if (defValue instanceof Enum) {
			fCom = _ddProp = new Dd((Enum) defValue);
		} else {
			String tbxVl = value == null ? X.toString(defValue, "") : value.toString();
		...
		SerializableEventListener updaterProp = event -> {
			try {
				...
				ZKI_Log.log("Update '%s' value '%s'", property, appProp.getValue());
				ZKR.restartPage();
			} catch (Exception ex) {
		...
		fCom.title(appProp.get_desc());

		if (fCom instanceof InputElement) {
			fCom.placeholder(appProp.name());
		}
		...
		SerializableEventListener event_PropChanged = event -> {
			if (event instanceof InputEvent) {
				...
				changedValue(appProp, (T) (X.equals(appProp.getValue(), false) ? (Boolean) true : (Boolean) false));
			}
		};
