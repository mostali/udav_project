...
public class FileState<PATH> extends EntityState implements IPath {
	...
	public static Path partPath(Path file, int... ind) {
		String fn = partName(file.getFileName().toString(), ind);
		return file.getParent().resolve(fn);
	}

	public static String partName(String fn, int... ind) {
	...
	public Path pathFc(int ind) {
		...
		return pathFc.getParent().resolve(partName(fn, ind));
	}
	...
	public EntityState writeFcDataOk(String data) {
		return writeFcData(data, 1);
	}

	public EntityState writeFcDataError(String data) {
		return writeFcData(data, 2);
	}
	...
	public void updateProp_STATE(Object state) {
		update(PK_STATE, state);
	}

	public void updateProp_TITLE(Object state) {
		update(PK_TITLE, state);
	}
	...
	public String readFcData(int index, String... defRq) {
		return RW.readContent(pathFc(index), defRq);
	}
	...
	public String readPropsData(String... defRq) {
		return RW.readContent(pathProps(), defRq);
	}
	...
	public static boolean isPropsPathFn(String fileName) {
	...
	public String nodeDataCached(boolean... fresh) {
		...
		return nodeDataCached != null ? nodeDataCached.orElse(null) : (nodeDataCached = Optional.ofNullable(nodeData(null))).orElse(null);
	}
