...
//	public FormState stateSwap() {
////		return isFormOrCom && isForm ? this : new FormState(sdn, pathFcStr, isFormOrCom);
//		FormState.ofPathComFile_OrCreate(sdn,);
//		return new FormState(sdn, pathFcStr, !isForm);
//	}

	public void updateProp_SECE(String vl) {
		updatePropSingle(ISecState.SECE, vl);
	}

	public String getProp_SECE(String... defRq) {
		return get(ISecState.SECE, defRq);
	}

	public String getProp_SECV(String... defRq) {
	...
	public void updateProp_VIEW(NodeDir.NVT noteViewType) {
		updatePropSingle(PK_VIEW, noteViewType.name());
	}
...
//			}
		...
		fCom.updatePropSingle("width", w);
		fCom.updatePropSingle("height", h);
	}
...
//		MoveEvent me = (MoveEvent) e;
	...
	public static void addEventListenerMoveAndResize(HtmlBasedComponent movableParentCom, HtmlBasedComponent resizableChildCom, FormState props) {
		...
		EventListener updateEvent = e -> {
			...
			} else if (resizableIsWin ? e instanceof SizeEvent : e instanceof MouseEvent && aClass.isAssignableFrom(e.getTarget().getClass())) {
				FormState.updateResizableEvent(e, props);
			}
		};
		resizableChildCom.addEventListener(resizableIsWin ? Events.ON_SIZE : Events.ON_CLICK, updateEvent);
