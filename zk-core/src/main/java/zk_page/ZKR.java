...
//				ZKC.removeMeReturnParent()
	...
	public static void invalidatePage() {
		...
		ZKCFinder.rootsByClass(GenericViewPageComponent.class, true).get(0).invalidate();
	}
...
//		ZKComFinder.findAll(page, Window.class, false).get(0);
			...
			if (false) {
				...
				ZKCFinder.findAll(page, GenericViewPageComponent.class, true, Collections.EMPTY_LIST).forEach(c -> ZKC.removeMeReturnParent((Component) c));
...
//				SeWin.findAll(Collections.EMPTY_LIST).forEach(c -> ZKC.removeMeReturnParent((Component) c));
				...
				ZKCFinder.findAll(page, TopAdminMenu.class, true, Collections.EMPTY_LIST).forEach(c -> ZKC.removeMeReturnParent((Component) c));
				ZKCFinder.findAll(page, QuickCmdRunner.class, true, Collections.EMPTY_LIST).forEach(c -> ZKC.removeMeReturnParent((Component) c));
			}
