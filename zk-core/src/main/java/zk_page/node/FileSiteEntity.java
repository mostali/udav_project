...
public abstract class FileSiteEntity<FORMSTATE extends FileState> extends SiteEntity<FORMSTATE> implements IPath {
	...
	public Long formSize(Long... defRq) {
		return X.sizeOfFile(getTargetPathDataFc(), defRq);
	}

//	public Path getTargetPathDataFc_asCom() {
//		return getTargetPathDataFc().getParent().getParent().resolve(ANFS.DIR_COMS);
//	}

	public Path getTargetPathDataFc() {
		//return targetFormFile != null ? targetFormFile : (targetFormFile = formState().pathFc());
		return state().pathFc();
	}
