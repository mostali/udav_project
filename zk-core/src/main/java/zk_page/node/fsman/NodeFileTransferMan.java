...
//			_moved1 = UFS_BASE.MV.moveIn(src_parent, dst_forms, true);
	...
	public static class AddNewForm {
		...
		public static Window addNewFormAndOpen(String newNotesName, String data, OptsAdd... opts_) {

			OptsAdd opts = ARG.toDefOr(OptsAdd.DEF, opts_);
			...
			props.updatePropSingle("top", RANDOM.RANGE(260, 520) + "px");
			props.updatePropSingle("left", RANDOM.RANGE(260, 520) + "px");

			props.updatePropSingle("width", RANDOM.RANGE(260, 520) + "px");
			props.updatePropSingle("height", RANDOM.RANGE(260, 520) + "px");

			props.updatePropSingle("height", RANDOM.RANGE(260, 520) + "px");
			//		props.updatePropSingle(BG, RANDOM.RANGE(260, 520) + "px");

			props.updatePropSingle(FormState.OPEN, true);
			if (opts.isWysiwygView()) {
				props.updatePropSingle(FormState.PK_VIEW, NodeDir.NVT.SE_WIN);
			} else if (opts.isHttpCallForm()) {
				if(props.emptyData()){
					props.writeFcData("GET http://github.com\n--#header comment\n--#Authorization:Bearer");
				}
				props.updatePropSingle(FormState.PK_VIEW, NodeDir.NVT.TEXT_WIN);
			} else {
				props.updatePropSingle(FormState.PK_VIEW, NodeDir.NVT.TEXT_WIN);
			}
			WebUsr user = Sec.getUser();
			props.updatePropSingle(SecFileState.PK_USER, user.getAliasOrLogin());
			props.updatePropSingle(SecFileState.SECE, "");
			props.updatePropSingle(SecFileState.SECV, "");

			props.updatePropSingle(FormState.BG_COLOR, RANDOM.ARRAY_ITEM(UColorTheme.WHITE));
