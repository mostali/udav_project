		...
		public static Window addNewFormAndOpen(String newNotesName, String data, OptsAdd... opts) {
			...
			{//UPDATE VIEW
				...
				if (opts0.isWysiwygView()) {
					...
					if (props.emptyData()) {
//						size = 3;
						props.writeFcData("PUT http://localhost:9092/kafka_topic\n--#kafka/\n#header comment\n--#key:msgkey\nmsgvalue");
					}
				} else if (opts0.isSqlCallForm()) {
					if (props.emptyData()) {
//						size = 3;
						props.writeFcData("--jdbc:postgresql://domain.com:5432/schema_name?currentSchema=cur_schema&searchpath=cur_schema" + //
								"\n--login" +  //
								"\n--pass" + //
