...
//		ZKS.addStyleAttr(window, "width", win_px_pct == null ? null : toPxPct(win_px_pct));
    ...
    public static Integer px(String vlPx, Integer... defRq) {
        ...
        return ARG.throwMsg(() -> X.f("Except pattern with px '%s'", vlPx), defRq);
    }
    ...
    public static <C extends HtmlBasedComponent> C MARGIN(C com, Object t_px_pct, Object r_px_pct, Object b_px_pct, Object l_px_pct) {
        return applyStylePropertyPxPct(com, "margin", X.f("%s %s %s %s", toPxPctero(t_px_pct), toPxPctero(r_px_pct), toPxPctero(b_px_pct), toPxPctero(l_px_pct)));
    }
    ...
    public static String toPxPctero(Object value) {
    ...
    public static void applyNiceBg(HtmlBasedComponent com, Pare bgColor) {
        String first = bgColor.key() instanceof EColor ? ((EColor) bgColor.key()).nextColor() : (String) bgColor.key();
        String sec = bgColor.val() == null ? null : (bgColor.val() instanceof EColor ? ((EColor) bgColor.val()).nextColor() : (String) bgColor.val());
    ...
    public static void appNiceFlex(HtmlBasedComponent com) {
        ZKS.addStyleAttr(com, "display", "flex");
        ZKS.addStyleAttr(com, "flex-wrap", "wrap");
        ZKS.addStyleAttr(com, "gap", "10px");
    }
    ...
    public static <T extends XulElement> T toggleDnoneFirst(Class<T> clazz, boolean recursive_or_first, T... defRq) {
        ...
        return first != null ? toggleDnone(first) : ARG.throwErr(() -> new RequiredRuntimeException("Recursive(%s) com '%s' not found", recursive_or_first, clazz.getSimpleName()), defRq);
    }
...
//		}
    ...
    public static <C extends HtmlBasedComponent> C SCALE(C com, double scale) {
        addStyleAttr(com, "scale", scale+"");
        return com;
    }
