...
public class UPageSP {
	...
	private static List<FormState> getAllFormComStates(Pare<String, String> sdn) {
		return getAllFormStates(sdn).stream().map(FormState::stateCom).collect(Collectors.toList());
	}

	private static List<FormState> getAllFormStates(Pare<String, String> sdn) {
		return AFC.FORMS.DIR_FORMS_LS_CLEAN(sdn).stream().//;
				map(p -> AppStateFactory.ofFormDir_orCreate(sdn, p)).collect(Collectors.toList());
	}

	private static Map<int[], FormState> getAllFormComStatesAsGrid(Pare<String, String> sdn) {
		Map<int[], FormState> comsCoors = getAllFormComStates(sdn).stream().map(s -> Pare.of(s, s.fields().get_TOP_LEFT(null))).filter(p -> p.val() != null).collect(Collectors.toMap(k -> k.val(), v -> v.key()));
		return comsCoors;
	}

	public static void doAlignPageGrid() {
		Map<int[], FormState> comsCoors = getAllFormComStatesAsGrid(SpVM.get().ppi().sdnRq());
