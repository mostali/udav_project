...
public class PagePathInfo implements Serializable {
	...
	public Ns ns(Ns... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("except some one, path '%s'", path()), defRq);
	}


	public String pathStr(int index, String... defRq) {
		Path item = UPath.item(Paths.get(servlet_path), index, null);
		return item != null ? item.toString() : (ARG.throwErr(() -> new RequiredRuntimeException("Except path item by index"), defRq));
	}
