...
import zk_old_core.apis.impl.ApiCallPage_OLD;
...
public class FinderPSP {
	...
	private PageSP findPageImpl() {
		...
		pageSP = findPage_TreeRestApiCall_OLD();
		if (pageSP != null) {
			return pageSP;
		}
...
//			}
	...
	@Deprecated
	private PageSP findPage_TreeRestApiCall_OLD() {
		boolean isApiPath = ApiCallPage_OLD.is(pagename);
		if (!isApiPath) {
			return null;
		}
		return new ApiCallPage_OLD(window, spVM).cleanResponse();
	}
