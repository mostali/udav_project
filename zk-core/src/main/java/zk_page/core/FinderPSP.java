...
public class FinderPSP {
	...
	public PageSP findPage(PageSP... defRq) {
		...
		if (ex != null) {
			...
			return ARG.throwErr(() -> new PspNotFoundException(finalEx, "PageSP '%s' not found (has errors)", ppi), defRq);
		} else {
			return ARG.throwErr(() -> new PspNotFoundException("PageSP '%s' not found", ppi), defRq);
		}
	}
