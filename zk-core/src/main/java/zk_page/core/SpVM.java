...
//		return (SpVM) Executions.getCurrent().getDesktop().getFirstPage().getAttribute("vm");
	...
	public void afterCompose(@ContextParam(ContextType.VIEW) Window window) throws IOException {
		...
		if (endRequest_FINISH == null) {
			...
			UWeb.sendResponse(ZKR.getResponse(), (IResponseStatusException) endRequest_FINISH);
			if (L.isInfoEnabled()) {
			...
			UWeb.sendResponseWithCleanDataAndClose(ZKR.getResponse(), ((CleanDataResponseException) endRequest_FINISH).getCleanData());
			if (L.isInfoEnabled()) {
	...
	public Pare<String, String> sdn() {
		return Pare.of(subdomain3(), pagename());
	}
	 ...
	 */
	@SneakyThrows
	public void afterComposeImpl(@ContextParam(ContextType.VIEW) Window window) throws Sd3EE {
