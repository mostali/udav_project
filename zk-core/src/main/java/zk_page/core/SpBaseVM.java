...
public class SpBaseVM implements Serializable {
	...
	public Pare3<Sb, Sb, Sb> showDebugLog() {
		...
		Sb sbRequest = new Sb();
		{
			...
			sbRequest.TABNL(1, "sec-level:" + ARR.as(ROLE.USER.has(roles), ROLE.EDITOR.has(roles), ROLE.ADMIN.has(roles), ROLE.OWNER.has(roles)));
	...
	public boolean hasSkaStrict(boolean... any) {
		return Sec.hasSkaStrict(getQuery(), any);
	}
...
//			case PLANE:
//				return rsPath.toPlaneLink(ppi0.plane());
//			case PAGE:
//				return rsPath.toPageLink(ppi0.plane(), ppi0.pagename0(), path2_nodeName);
//			default:
