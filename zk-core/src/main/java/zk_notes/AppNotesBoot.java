...
import mpu.core.ARR;
import mpu.str.Sb;
import zk_notes.node_srv.quartz.QzTrm;
import zk_os.AppZos;
...
public class AppNotesBoot extends AppBoot {
	...
	public static void initTrm(String... customPackets) {
		...
		try {
			TRM.run_scan(NetUsrId.def(), true, true, ARR.addArrayElements(trm_routes, customPackets));
		} catch (Exception ex) {
...
//		};
		...
		FinderPSP.regPageEntity(ARR.addArrayElements(basePackets, customPackets));
