...
public class NotesPageHeader extends Div0 implements IReRender {
	...
	public static void addPhrases(String prevKeyPhrases, String keyPhrases, String phrases) {
		if (!keyPhrases.equals(prevKeyPhrases)) {
			AppNotesCore.TREE_QUICK_HISTORY().remove(prevKeyPhrases);
		}
		AppNotesCore.TREE_QUICK_HISTORY().put(keyPhrases, phrases);
		rerender(null);
	}
	...
	public static void addLogView(String file, String name) {
		AppNotesCore.TREE_QUICK_HISTORY().put(file, name);
		rerender(null);
	}
...
//		ZKS.BGCOLOR(this, "#f08080");
	...
	private void appendRight_StopPhrasesPanel() {
		...
		List<Pare3<String, String, String>> pares = AppNotesCore.TREE_QUICK_HISTORY().getPares();
		for (Pare3<String, String, String> phrases : pares) {

			String keyShortName = phrases.getKey();
			String stopPhrasesString = phrases.getVal();

			Tbx tbxKey = new Tbx(keyShortName);
			DefAction act = (e) -> {
				AppNotesCore.TREE_QUICK_HISTORY().remove(keyShortName);
			...
			DefAction defAction = e -> ZKME.openEditorWithContent(() -> stopPhrasesString, newSaveCallback(tbxKey), Span0.of(tbxKey, btRmList), false);
	...
	private Ln newBtAddNewPhrases() {
		...
		DefAction defAction = e -> ZKME.openEditorWithContent(() -> "new-stop-phrase", newSaveCallback(tbxKey), tbxKey, false);
	...
	private static Function<String, Boolean> newSaveCallback(Tbx tbxKey) {
		...
		Function<String, Boolean> saveCallback = newString -> {
			...
			if (X.empty(keyPhrases)) {
				ZKI_Log.alert("set key for phrases list");
				return false;
			}
...
//			ZKS.COLOR(lnToDirOrFile, UColorTheme.RED[0]);
	...
	private static void addMI_RemoveFromPanel(Menupopup0 menu, String keyFile) {
		menu.addMenuitem("Remove from panel", e -> AppNotesCore.TREE_QUICK_HISTORY().remove(keyFile));
	}
