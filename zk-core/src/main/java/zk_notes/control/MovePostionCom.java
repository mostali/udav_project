...
public class MovePostionCom extends Div0 {
	...
	protected void init() {
		super.init();
		_sizable()._modal(Window.Mode.POPUP)._closable()._pos(WinPos.center)._sizable();
		...
		ZKS.WIDTH_HEIGHT(this, 180, 80);
		for (Pos value : Pos.values()) {
			appendChild(new Bt(value.name()).onCLICK(e -> movePostition(value)));
		}
	}
