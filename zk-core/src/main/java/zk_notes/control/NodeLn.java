...
//		appendChild(Xml.ofXml(nodeDir.nodeName(), EHtml5.s));
	...
	protected void initBeaviours() {
		...
		if (checkBusinessLogic_HPE) {
			...
			NodeFactory.on_checkBusinessLogic_HPE(nodeDir.getPathFc());
		}

		if (checkAndOpen_IfOpened) {
			NodeFactory.openFormIdentitySinglyAsWin0_ifNotClosed(nodeDir, onQView);
		}
			...
			onCLICK(e -> {
				...
				switch (keys) {
					...
					case ZKE.ZKE_CTRL_CODE:
						nodeEvalType.doEventAction(nodeDir, this, keys);
						break;
					case ZKE.ZKE_ALT_CODE:
						...
						if (X.empty(rslt)) {
							...
							ZKM_Editor.openEditorText("OK data", rslt, true);
						}
						break;
					case ZKE.ZKE_SHIFT_CODE:
						...
						if (X.empty(rsltErr)) {
							...
							ZKM_Editor.openEditorText("ERR data", rsltErr, true);
						}
						break;

					default:

						NodeFactory.openNoteWin(nodeDir);
