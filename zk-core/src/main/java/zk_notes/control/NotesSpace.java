...
public class NotesSpace extends Div0D implements IReRender {
	...
	protected void init() {
		...
		Map<NodeDir, Component> comMap = NodeFactory.buildPageComMap(sd3pn, this);

//		comMap = (Map) comMap.entrySet().stream().collect(Collectors.toMap(k -> k.getKey(), v -> v.getValue(), (k, v) -> v, LinkedHashMap::new));
