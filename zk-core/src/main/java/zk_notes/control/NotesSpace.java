...
import zk_notes.factory.NFPage;
...
//		ZKC.printAll();
	...
	public static NotesSpace newNotesSpaceIn_NoAdd(Window window) {
		return initNotesSpaceOnPage(window, false);
	}

	public static NotesSpace newNotesSpaceIn(Window window) {
		return initNotesSpaceOnPage(window, true);
	}

	private static NotesSpace initNotesSpaceOnPage(Window window, boolean add) {
		...
		NotesSpace notesSpace = new NotesSpace(window, SpVM.get().sdn());
	...
	protected void initDND() {
		if (tabMode == false && getPageState().isAllowedAccess_EDIT()) {
			IZDnd.initDND(window, ARG.toObjNN(defaultAfterUpdateClb));
		}
	}
	...
	protected void init() {
		...
		ZKS.appNiceFlex(this);

		if (isFirstRun) {
			ZPage.of(sdn, window).addBreadDiv();
		}
	...
	private void initTabsMode(Pare<PageState.TabsMode, String[]> tabsMode) {
		...
		if (isAll) {
			...
			List<String> pageNames = S_.mapToList(items, UF::fn);
		...
		if (activeTab != null) {
			Optional<PageTb> first = S_.findFirstOpt(initTabs.keySet(), tb -> activeTab.equals(tb.getName()));
	...
	private void initTabsWithFormView(String[] tabsForms) {
		...
		Map<NodeDir, Component> comMap = NFPage.buildPageComMap(sdn, this);
