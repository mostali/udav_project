...
import zk_page.ZKC;
import zk_page.ZKS;
import zk_page.node_state.PropFormItem;
...
public class NotesHeaderProps extends Div0 {
	...
	protected void init() {
		...
		List<AppPropDef> allProps = AppPropDef.getAllProps(AppZosProps.class, AppZosCore_Old.TREE_PROPS, AppNotesProps.class, AppNotesCore.TREE_QUICK_HISTORY());

		allProps.forEach(i -> appendChild(new PropFormItem(i).posRightTop(true)));
