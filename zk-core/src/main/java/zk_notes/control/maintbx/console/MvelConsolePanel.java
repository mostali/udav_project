...
public class MvelConsolePanel extends AbstractConsolePanel {
	 ...
	 */
	@Override
	protected void onClickButtonRun() {
	 ...
	 */
	@Override
	protected void onClickButtonContext() {
		if (task == null) {
			out_show_code = 1;
		}
		switch (++out_show_code) {
			case 0:
				...
				break;
			case 1:
				setOutputValues("result", buildStringContextResult());
				break;
			case 2:
				setOutputValues("init", buildStringContextInit(getContextInit()));
				out_show_code = -1;
				break;
		}
	}
	...
	private void renderResultInfo(MvelTask task) {
		...
		super.setOutputValues(clas.getName(), val);
	}
	...
	private void renderResultError(MvelTask task) {
		...
		super.setOutputValues(task.getException().getClass().getName(), strStackTrace);
	}
