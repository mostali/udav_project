...
//			tbxmOut.setCLASS(RFL.scn(MainTbx.class) + "Out");
	...
	private void run() throws IOException {
		...
		switch (value) {
			...
			default:
				...
				if (isShMode) {
					runCommand_ShMode(sdn(), value);
				} else {
					runCommand_AppMode(sdn(), value);
				}
				break;
		}
	...
	private static void runCommand_ShMode(Sdn sdn, String cmd) {
		String apply = (String) AppZosBashExecView.funcExecCmdAndShowResult.apply(cmd);
		BottomHistoryPanel.addItemAsData(apply, false);
	}

	private static void runCommand_AppMode(Sdn sdn, String cmd) throws IOException {
		switch (cmd) {
			case "+":
			...
			default:
				...
				} else if (cmd.startsWith("#") && cmd.length() > 4) {
					String topPos = RANDOM.range(10, 60) + "%";
					String leftPos = RANDOM.range(10, 60) + "%";
					Lb lb = (Lb) Lb.of(cmd).addSTYLE(X.f_("display:block;width:30%;height:30%;position:absolute;top:%s;left:%s;z-index:10000;background-color:%s", topPos, leftPos, cmd.substring(1)));
					lb.onDBLCLICK(e -> {
						Thread.sleep(2000);
						e.getTarget().detach();
					});
