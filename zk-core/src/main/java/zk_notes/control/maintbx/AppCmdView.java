...
//				ZKR.restartPage();
			...
			Pare<NodeDir, Window> nodeDirWindowPare = NFNew.openRandomINE(sdn);
...
//				ZKR.restartPage();
	...
	private static AppCmd run_AddNode_AsDldLogfileUrl(Sdn sdn, String cmd) throws IOException {
		...
		return AppCmd.forOwnerOrAdmin(() -> {
			...
			Pare<NodeDir, Window> nodeDirWindowPare = NFNew.openRandomINE(sdn, opts);
			if (nodeDirWindowPare != null) {
				NodeDir node = nodeDirWindowPare.key();
	...
	private static AppCmd run_OpenManualAdmin(String cmd) {
		if (cmd.equals("??")) {
			return AppCmd.forOwnerOrAdmin(() -> {
				ZKC.getFirstWindow().appendChild(Xml.asRsrcXml(MainTbx.MANUAL_RCRS_ADMIN)._modal()._title(SYMJ.INFO_SIMPLE + " Help for admin")._closable()._showInWindow());
				return true;
			});
		}
		return null;
	...
	public static void runImplMVD(String mvu_cmd) {
		...
		NoteApi znApi = AppCoreZos.createNoteApiByAlias(hst);
	...
	public static void runImplMVU(String mvu_cmd) {
		...
		NoteApi noteApi = AppCoreZos.createNoteApiByAlias(hst);
...
//		bodyLines = bodyLines.replace("\n", "&nbsp;");
	...
	static AppCmd getAppCommand_NoSec(Sdn sdn, String cmd) {
		...
		switch (cmd) {
			...
			case "?":
				return () -> ZKC.getFirstWindow().appendChild(Xml.asRsrcXml(MainTbx.MANUAL_RCRS)._modal()._title(SYMJ.INFO_SIMPLE + " Help")._closable()._showInWindow());
