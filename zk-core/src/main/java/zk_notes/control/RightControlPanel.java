...
//		appendChild(Xml.ofJs("function lightRCP(){ highlightElement()alert ('ok:'+ %s);return %s; }", getUuid(), getUuid()));
...

		{
			...
			Pare<String, SerializableEventListener> eventShowPopup = Pare.of(Events.ON_MOUSE_OVER, Popup0.openPopupEventWith(lnSize, lnPretty, lnWYSIWIG));
...
//				List<String> allEmojs = SYMJ.getAllEmojs();
			...
			Pare<String, SerializableEventListener> eventShowPopup = Pare.of(Events.ON_MOUSE_OVER, Popup0.openPopupEventWith(lnClean, lnAutoGrid, lnShowColor, lnShowEmoji));
