...
//			FIDP fidp = FIDP.of(CN.HREF, CN.TARGET);
		...
		boolean allowedRunNode = SecMan.isAllowedRunProtectedMode(nodeDir, false);

		if (!allowedRunNode) {
//			if(SecMan.isPlaneOwnerOrAdmin())
//			boolean allowedRunNodeHolder = SecMan.isAllowedRun(nodeDir, false);
			if (SecMan.isPlaneOwner()) {
