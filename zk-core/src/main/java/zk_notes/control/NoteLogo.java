...
import zk_pages.DemoManualPageSP;
import zk_pages.DemoNodePageSP;
...
public class NoteLogo extends WithLogo.LogoCom {
    ...
    public static void replaceFirst() {
        findFirst().replaceWith(new NoteLogo());
    }
    ...
    protected void init() {
        ...
        if (isAlllowEdit) {
            ...
            { // ADD
                Function2<String, String, Object> successCallback = (String newNotesName, String val) -> NFNew.openNewAlertINE_inCurrentSdn(newNotesName, val);
            ...
            { //Page Props
                if (isAlllowEdit) {
                    ...
                    Menupopup0 pageMenu = menu.addInnerMenu(SYMJ.GLOB_GRID + " Page Props");
                    ANMP.applyPageLink(pageMenu, sdn());
...
//            menu.add_______();
        ...
        SupplierLazy<Menupopup0> getPanelsMenuRef = new SupplierLazy(() -> menu.addInnerMenu(ANI.SETTINGS + " Panel's"));
...
//					ZKR.restartPage();
        ...
        if (isAdmin) {

            {
                ...
                AppEventsFD.applyEvent_OPENTREE(menuTrees, APP.TREE_VERSIONED());
                menuTrees.add_______();
                ...
                AppEventsFD.applyEvent_OPENTREE(menuTrees, AppCoreNotes.TREE_NOTIFY_GLOB().getDbFilePath());
...
//					innerMenuAbout.addMI_SimpleLabel(APP.getAppName());
    ...
    public enum BasePageViews {
        HTML, XSD, JSON, __TASKS, __ACTUATOR, __DOCKER;
...
//                    return UUrl.normUrl(APP.HOST.getAppHostWithProtocol(), "@@tasks");
    ...
    private void applyMainViews(Menupopup0 viewsMenu) {
        viewsMenu.addMI(SYMJ.BOOK_YELLOW + " Manual", (Event e) -> ZKR.toRedirectWithPath.apply(DemoManualPageSP.PAGENAME));

        viewsMenu.add_______();
        viewsMenu.addMI(SYMJ.SQUARE_BLACK + " Demo Notes", (Event e) -> ZKR.toRedirectWithPath.apply(DemoNodePageSP.PAGENAME));
        ...
        Menupopup0 devMenu = viewsMenu.addInnerMenu(SYMJ.GEAR + " Dev");
        {
            devMenu.addMI_Href_in_Self(" Index", "/@@index", null);
            devMenu.add_______();

            devMenu.addMI(SYMJ.SQUARE_BLACK + " Dev Script generator", (Event e) -> ZKR.toWindowByUrl.apply(APP.HOST.getAppUrlWithPlaneAndPath("view", "dsrc")));
            devMenu.addMI(SYMJ.SQUARE_BLACK + " Class generator", (Event e) -> ZKR.toWindowByUrl.apply(APP.HOST.getAppUrlWithPlaneAndPath("view", "class")));
            devMenu.addMI(SYMJ.SQUARE_BLACK + " Log Analyzer", (Event e) -> ZKR.toRedirectWithPath.apply("@@log"));
            AppEventsFD.applyEvent_LOG_SIMPLE_VIEW(devMenu, APP.getPathServerLog());

            devMenu.add_______();

            devMenu.addMI(SYMJ.SQUARE_BLACK + " Demo Com (Dev)", (Event e) -> ZKR.toRedirectWithPath.apply("demo-com"));

        }

        {
            Menupopup0 specViewMenu = viewsMenu.addInnerMenu(SYMJ.GEAR + " View other..");
            String currentUrl = Sdn.get().toCurrentUrl("?tbf=*");
            specViewMenu.addMI(SYMJ.SQUARE_BLACK + " Single note tabs view", (Event e) -> ZKR.redirectToPage(currentUrl));
        }
