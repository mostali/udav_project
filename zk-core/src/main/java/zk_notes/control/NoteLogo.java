...
public class NoteLogo extends WithLogo.LogoCom {
	...
	protected void init() {
		...
		if (secAllowedEdit) {
			menu.add_______();
			menu.addMI(ANI.SETTINGS + " Quick Panel" + (headOpenState ? SYMJ.OK_GREEN : ""), (Event e) -> {
				if (headOpenState) {
		...
		if (secAdminOrOwner) {
...

			{
				...
				menu.addMI(ANI.SETTINGS + " Config Panel" + (openStateCgfPanel ? SYMJ.OK_GREEN : ""), (Event e) -> {
...
//			}
		...
		if (secAdminOrOwner) {
			...
			if (isOwner) {
				...
				menu.addMI(ANI.REBOOT + " Reboot App", e -> ZKI.infoEditorBw(Sys.exec_rq("./r.sh", "--R").getOutMerged()));
				menu.addMI(ANI.REBOOT + " Open FS", e -> SimpleDirView.openWithSimpleMenu(APP.LOCATION.getAppDataDir(true)));
				AppEventsFD.applyEvent_OPENTREE(menu, APP.TREE_GNC());
				AppEventsFD.applyEvent_OPENTREE(menu, APP.TREE_GND_V1());
				AppEventsFD.applyEvent_OPENTREE(menu, APP.TREE_GNDD_V1());
				menu.addMI(SYMJ.CLEAR + " Clear All Ctx Db", e -> {
					UTree.clearAll( //
//							APP.GNC_TREE(), //
							APP.TREE_GND_V1(), //
							APP.TREE_GNDD_V1() //
...
//				menu.addMenuItem(SYMJ.CLEAR + " Clear Page From Empty Notes", e -> NodeFileTransferMan.clearPageFromEmptyNotes(sdn()));
			...
			{
				Menupopup0 innerMenuAbout = menu.addInnerMenu("Pages");
				innerMenuAbout.addMI_Href("Show Index", "/@@index", null);
				innerMenuAbout.addMI_Href("Logs", "/@@logs", null);
			}
			{
				Menupopup0 innerMenuAbout = menu.addInnerMenu("About");
				if (SecMan.isAdminOrOwner()) {
					innerMenuAbout.addMI_SimpleLabel(APP.PROP.getAppName());
					innerMenuAbout.addMI_SimpleLabel(APP.PROP.getAppHost());
				}
				innerMenuAbout.addMI_SimpleLabel("v." + BootRunUtils.getVersionFromAny(WithLogo.class, "0"));
				innerMenuAbout.addMI("Debug border on component", e -> EventHighlightForm.applyOnOff_MouseOverOut((List) ZKCFinder.findAllInWin(HtmlBasedComponent.class, true)));
		...
		menu.addMI(ANI.HELP + " Help", (Event e) -> ZKI.infoEditorBw(RES.readString("/help.txt")));
