...
public class NoteLogo extends WithLogo.LogoCom {
	...
	protected void init() {
		...
		if (SecMan.isAllowedEdit()) {
			...
			Menupopup0 viewsMenu = menu.addInnerMenu("View's..");
...
//			}
		...
		if (secAdminOrOwner) {
			...
			if (isOwner) {
				...
				AppEventsFD.applyEvent_LOGVIEW(menu, APP.getPathServerLog());

				menu.addMI_Href_v1("App Logs", PageSP.PAGENAME_LOGS);

				menu.addMI_EDITOR("App Logger's", APP.getPathLogbackXml(), true, EXT.XML);
				...
				menuTrees.addMI(SYMJ.CLEAR + " Clear All Ctx Db", e -> {
					UTree.clearAll( //
//							APP.GNC_TREE(), //
...
//				menu.addMenuItem(SYMJ.CLEAR + " Clear Page From Empty Notes", e -> NodeFileTransferMan.clearPageFromEmptyNotes(sdn()));
			...
			}
			{
				...
				if (SecMan.isAdminOrOwner()) {
					innerMenuAbout.addMI_SimpleLabel(APP.MAIN.getAppName());
					innerMenuAbout.addMI_SimpleLabel(APP.MAIN.getAppHost());
				}
				innerMenuAbout.addMI_SimpleLabel("v." + BootRunUtils.getVersionFromAny(WithLogo.class, "SNAPSHOT"));
			}
		}
