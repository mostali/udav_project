	...
	protected void init() {
		...
		if (secAllowedEdit) {
			...
			menu.addMenuitem(ANI.SETTINGS + " Quick Panel" + (headOpenState ? SYMJ.OK_GREEN : ""), (Event e) -> {
				if (headOpenState) {
					pagetate.updatePropSingle(AppZosProps.LOGO_HEADER_OPEN, false);
					NotesPageHeader.removeMeFirst();
				} else {
					NotesPageHeader.openSimple();
					pagetate.updatePropSingle(AppZosProps.LOGO_HEADER_OPEN, true);
				}
				ZKR.restartPage();
			});
		}
		...
		if (secAdminOrOwner) {
			...
			menu.addMenuitem(ANI.SETTINGS + " Config Panel" + (openStateCgfPanel ? SYMJ.OK_GREEN : ""), (Event e) -> {
				if (openStateCgfPanel) {
					pagetate.updatePropSingle(AppNotesProps.PAGE_CONFIG_OPEN, false);
					NotesHeaderProps.findFirst().removeMe();
				} else {
					NotesHeaderProps.openSimple();
					pagetate.updatePropSingle(AppNotesProps.PAGE_CONFIG_OPEN, true);
				}
				ZKR.restartPage();
			});
		...
		if (secAdminOrOwner) {
			...
			if (isOwner) {
				...
				menu.addMenuitem(ANI.DIRVIEW + " Clear Page From Empty Notes", e -> NodeFileTransferMan.clearPageFromEmptyNotes(sdn()));
