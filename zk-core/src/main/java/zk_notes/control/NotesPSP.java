...
//		}
	...
	public static void checkAndOpenStatablePanels(FormState pageState) {
		if (SecMan.isAdminOrOwner()) {
			if (pageState.hasPropEnable(AppNotesProps.PAGE_PANEL_CONFIG_OPENED, false)) {
				AppConfigPropsTopPanel.openSimple();
			}
			if (pageState.hasPropEnable(AppNotesProps.LOGO_HEADER_OPEN, false)) {
				QuickNotesTopPanel.openSimple();
			}

			if (pageState.hasPropEnable(AppNotesProps.PAGE_PANEL_BOTTOM_HISTOTY_PANEL_OPEN, false)) {
				BottomHistoryPanel.openSimple();
			}
			if (pageState.hasPropEnable(AppNotesProps.PAGE_PANEL_TASKS_OPENED, false)) {
...
//		boolean isIndexPage = ItemPath.PAGE_INDEX_ALIAS.equals(ppi.pagename0());
		...
		if (ppi.isEmptySd3()) {
			...
			new IndexNotesPSP(window, ppi.plane()).buildPage();
		}
		return true;
	}
