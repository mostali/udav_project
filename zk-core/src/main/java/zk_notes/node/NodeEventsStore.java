...
public class NodeEventsStore {
	...
	public UTree getDb() {
		...
		UTree tree = (UTree) UTree.tree(dbFile).setAutoCleanCfg(AUTOCLEAN_CTR_EVERY_MIN_MAX_PACKET_FIRSTEND);
		return tree;
	}

	public void store(String src, String head, Exception e) {
		getDb().add(src, head, e == null ? null : ERR.getMessagesAsStringWithHead(e, head));
	}
