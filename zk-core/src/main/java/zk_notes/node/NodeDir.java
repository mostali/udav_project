...
public class NodeDir extends SiteDir<FormState> {
	...
	public static NodeDir ofNodeId(NodeID nodeID) {
		return ofNodeName(nodeID.toSdn(), nodeID.item());
	}
	...
	public String nodeNameWithSdn() {
		Pare<String, String> sdn = sdn();
		return sdn.key() + "/" + sdn.val() + "/" + nodeName();
	}
...
//	}
	...
	public static Pare<NodeDir, Component> buildSingleCom(HtmlBasedComponent parent, NodeDir nodeDir) {
		...
		try {
			Pare<NodeDir, Component> nodeDirComponentPare = NodeFactory.buildSingleComImpl(parent, nodeDir);
			if (L.isDebugEnabled()) {
...
//				return Pare.of(NodeDir.ofDir(path, sdn()), new ErrLb("Unexcepted err:" + nodeName(), ex));
	...
	public enum NVM {
		...
		public static NVM ofNodeFirst(NodeDir nodeDir, NVM... defRq) {
			Map<GEXT, List<Path>> map = nodeDir.fMapGExt();
