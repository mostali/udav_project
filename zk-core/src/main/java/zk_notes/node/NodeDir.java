...
public class NodeDir extends SiteDir<FormState> implements INode<NodeDir> {
	...
	public NVM nvm_define() {
		return nvm != null ? nvm : (nvm = NVM.defineType(this, null));
	}

	private INodeType nodeStdType;

	public INodeType stdType(INodeType... defRq) {
		if (nodeStdType != null) {
			return nodeStdType;
		}
		return this.nodeStdType = defineNodeType(false, defRq);
	}

	public <S extends ZService> S stdSrvAny() {
		NodeSrv nodeSrv = stdSrv(null);
		return nodeSrv != null ? (S) nodeSrv : (S) StdZSrv.DEFAULT;
	}

	public <S extends NodeSrv> S stdSrv(S... defRq) {
		INodeType iNodeType = stdType(null);
		return iNodeType != null ? iNodeType.stdSrv(defRq) : ARG.throwMsg("except stdtype", defRq);
	}
	...
	public String injectStr(boolean... fresh) {
		resetCache(fresh);
		return inject(fresh).nodeDataStrOrNull();
	}
