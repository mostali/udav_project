...
public class InAction {
    ...
    private final @Getter INodeType evalStdType;

    public static InAction of(INodeType nodeType) {
        return new InAction(nodeType);
    }

    public FunctionIO<NodeDir, Object> srvIn(TrackMap.TrackId trackId) {

        ZService nodeSrv = evalStdType.stdSrv(null);

        if (nodeSrv != null) {

            NodeSrv.EvalOpts stdOpts = new NodeSrv.EvalOpts();
            stdOpts.track = trackId;
//            stdOpts.trackContext = ARR.EMPTY_MAP;


            return (node) -> nodeSrv.evalAsString(node, stdOpts);
        }

        return stdIn(trackId);

    }

    public FunctionIO<NodeDir, Object> stdIn(TrackMap.TrackId trackId) {
        ...
        StdType stdType0 = evalStdType.stdType();
