...
import zk_notes.node_srv.HttpCallService;
import zk_notes.node_srv.KafkaCallService;
import zk_notes.node_srv.SendMsgService;
...
public class NodeActionIO {
	...
	public static FunctionT<NodeDir, Object> in(NodeEvalType nodeEvalType, Map... context) {
...
//				evalFunction = evalFuncOut;
			...
			case HTTP:
				evalFunction = node -> HttpCallService.doHttpCall_STRING(node.nodeDataInjected(), false, false, false);
				break;
			...
			case PYTHON:
				evalFunction = (node0) -> Sys.exec_filetmp("python3", node0.nodeDataInjected().nodeData, null, false);
				break;
			...
			case MVEL:
				...
				break;
			case GROOVY:
				evalFunction = (node0) -> new GroovyShell().evaluate(node0.nodeDataInjected().nodeData);
				break;

			case SHTASK:
				evalFunction = (node) -> {

					String nodeData = node.nodeDataInjected().nodeData;

					BashCallMsg callMsg = BashCallMsg.of(nodeData);
					callMsg.setWorkDir(node.toPath());
					callMsg.setFileData(nodeData);
			...
			case SENDMSG: {
				FunctionT<String, Object> evalFuncOut = (rsltOut) -> {
					if (X.empty(rsltOut)) {
						...
						ZKM_Editor.openEditorLog("SendMsg response node ", rsltOut);
					}
					return null;
				};
				return evalFuncOut;
				...
				FunctionT<NodeDir, Object> evalFuncOut = node -> {
					...
					switch ((Integer) keys) {
						case ZKE.ZKE_2_CTRL_SHIFT_CODE:
							return ZKM.showModal(title, Xml.ofMd(node.getPathFormFc()), ZKC.getFirstWindow(), ZKM.WH100);
						case ZKE.ZKE_2_CTRL_ALT_CODE:
							return ZKM_Editor.openEditorHTML(title, node.getPathFormFc());
						default:
							return ZKM_Editor.openEditorTEXT(title, node.getPathFormFc());
					}
				};
				return evalFuncOut;
//				return NodeEvalAction.DEFAULT_OUT_VOID;
			}
			default:
	...
	public static void applyFormMenu(Menupopup0 menu, NodeDir node) {
		...
		menu.addMenuItem(nodeEvalType.titleWithIcon(), e -> doEventAction_ActiveWeb(node));
	}
	...
	public static <T> Pare3<NodeDir, Optional<T>, Object> doEventAction_ActiveWeb(NodeDir node, Integer keys) {
		...
		return NodeEvalAction.doEvalNodeAction(node, nodeEvalType.evalIn(webContext), nodeEvalType.evalOut(webContext));
	}
