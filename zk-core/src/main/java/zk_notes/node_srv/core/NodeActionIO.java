...
public class NodeActionIO {
	...
	public static FunctionT<NodeDir, Object> in(NodeEvalType nodeEvalType, Map... context) {
		...
		switch (nodeEvalType) {
			...
			case NODE:
				...
				evalFunction = node -> {
					String title = node.evalType(false).icon() + " " + node.nodeName();
					//if (nodeDir.nvt(null) == NodeDir.NVT.WYSIWYG) {
					Map def = ARG.toDef(context);
					Object keys = def.getOrDefault("keys", 0);
					switch ((Integer) keys) {
	...
	public static <T> Pare3<NodeDir, Optional<T>, Object> doEventAction_ActiveWeb(NodeDir node, Integer keys) {
		NodeEvalType nodeEvalType = node.evalType(false);
		return NodeEvalAction.doEvalNodeAction(node, nodeEvalType.evalIn(MAP.of("keys", keys)), nodeEvalType.evalOut());
	}
