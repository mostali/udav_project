...
//		NodeSrv service = INodeType.toNodeSrv(evalType, null);
		NodeSrv service = evalType.stdSrv( null);
		...
		switch (evalType) {
			...
			case SQL: {
				FunctionIO<List<List<AbsType>>, Object> evalFuncOut = (tableRows) -> {
					HtmlBasedComponent modalCom = Listbox0.fromListList_AT(tableRows, null);
			...
			case SENDMSG: {
				{
					FunctionIO<String, Object> evalFuncOut = (rsltOut) -> {
						if (X.empty(rsltOut)) {
							...
							ZKME.log("SendMsg Response ... ", rsltOut);
						}
						return null;
					};
					return evalFuncOut;
			...
			case NODE: {
				{
					FunctionIO<NodeDir, Object> evalFuncOut = node -> {
						String title = node.stdType(StdType.NODE).stdProps().icon() + " " + node.nodeName();
						...
						switch ((Integer) keys) {
							case ECtrl.ZKE_2_CTRL_SHIFT_CODE:
								return ZKM.showModal(title, Xml.asMd(node.getPath_FormFc_Data()), ZKC.getFirstWindow(), ZKM.WH100_MIN);
