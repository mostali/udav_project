	...
	public static class QzRmLn extends Ln {
		public QzRmLn(NodeDir node) {
			...
			addEventListener(e -> {
				...
				String msg = X.f("Remove mode -> Single (YES) | ALL (NO) <- (%s/%s) ?", X.sizeOf(QzEvalService.findAllJobKeys(node)), X.sizeOf(QzApiEE.getAllJobKeys()));
	...
	public static class QzShowAll extends Ln {
		public QzShowAll(NodeDir node) {
			...
			FunctionV1<Set<JobKey>> printer = jobs -> {
				...
				ZKME.openEditorTextReadonly("All founded Job's", rp);
			};
			addEventListener(e -> {
				FunctionV1<Boolean> f = r -> {
