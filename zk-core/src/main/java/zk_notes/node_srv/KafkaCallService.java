...
public class KafkaCallService {
	...
	private static Pare3<KafkaCallMsg, Object, Throwable> doKafkaCall(NodeDir node, Component activeExecutionCom, boolean... RETURN) {
		...
		if(isExecutionEnable){
			String threadName = KafkaCallService.getKafkaConsumerThreadName(node);
			if (KafkaCallMsg.of(node).kafka_method == KafkaCallMsg.KafkaMethodType.KGET && !Sys.isThreadActive(threadName)) {
		...
		try {
			...
			switch (kafkaCallMsg.kafka_method) {
				case KPUT: {
					...
					Thread0<String> objThread = new Thread0("KafkaProducer-" + node.id(), true) {
						@Override
						public void run() {
...
//							ZKRPush.activePushCom(KafkaPlayLn.this.getDesktop());
							threadState = Sys.getThreadState(threadName, null);
							if (threadState == null) {
...
//											node.state().appendFcData(msg, 1);
					...
					if (rslt.val() != null) {
						...
						if (isExecutionEnable) {
							ZKM_Editor.openEditorText(kafkaCallMsg.kafka_method + ":" + node.id(), content, true);
						}
					}
					if (rslt.ext() != null) {
						state.writeFcDataErr(rslt.ext());
			...
			String msg = X.f("Kafka Call error with node '%s'", node.id());
			L.error(msg, ex);
