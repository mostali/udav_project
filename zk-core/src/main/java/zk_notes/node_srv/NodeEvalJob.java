...
//		Sys.p("Job1End" + CTR++);
        ...
        X.p("NodeEvalJob:" + QDate.now());
        X.p("NodeEvalJob:" + jobExecutionContext.getMergedJobDataMap());

        JobDataMap context = jobExecutionContext.getMergedJobDataMap();
        String sayStr = context.getString("say");
        if (X.notEmpty(sayStr)) {
            Sys.say("" + sayStr);
        }

        String nodeIdSrc = IT.NE(context.getString(CN0.NODEID));
        ...
        try {
            ...
            String nodeIdDst = QzCallMsg.of(nodeDir.nodeDataStr()).getArgs();
            ...
            rslt = EvalNodeService.evalNodeNoWeb__std(nodeDir1, wrappedMap);
