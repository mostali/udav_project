...
//		FunctionV2<Throwable, CharSequence> writerAlert = !isExecutionEnable ? ZKRPush.getFuncError(isReturn ? ZKI.Type.LOGBACK : ZKI.Type.ERR_EXT) : ZKRPush.getFuncError(activeExecutionCom, ZKI.Type.ERR_MB);
	...
	public static JarOutWriter buildSyncWeb(FormState state) {
		FunctionV2<String, String> writerState = ZKRPush.getFuncWriteState(state, ZKI.Type.VOID);
	...
	public static JarOutWriter buildAsyncWeb(FormState state, Component activeExecutionCom) {
		FunctionV2<String, String> writerState = ZKRPush.getFuncWriteState(activeExecutionCom, state, ZKI.Type.LOG);
	...
	@SneakyThrows
	public static JarOutWriter buildSyncRest(FormState state) {
		FunctionV2<String, String> writerState = ZKRPush.getFuncWriteState(state, ZKI.Type.LOGBACK);
