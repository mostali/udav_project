...
public class QzECS {
	...
	public static int[] QDELETE(NodeDir nodeDir, boolean interrupt) {
	...
	public static void QADD(NodeDir srcNode) {
		QADD(srcNode, QzCallMsg.of(srcNode.state().nodeData()));
	}

	public static void QADD(NodeDir srcNode, QzCallMsg qzCallMsg) {
	...
	private static void addJob(Class<? extends Job> jobClass, NodeDir srcNode, Map jobConfigureContext) {
		...
		jobData.put(CN0.NODEID, srcNode.nodeId());
		...
		QzCallMsg qzEvalMsg = QzCallMsg.of(srcNode.nodeDataStr());
