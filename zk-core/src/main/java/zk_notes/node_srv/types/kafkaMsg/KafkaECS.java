...
//											node.state().appendFcData(msg, 1);
					...
					if (rslt.val() != null) {
						List<GsonMap> jsonMsgs = S_.mapToList(recivedMsgs, ksfakMsg -> GsonMap.ofKV("key", ksfakMsg.key(), "value", ksfakMsg.val()));
						String content = JOIN.allByNL(S_.mapToList(jsonMsgs, GsonMap::toStringPrettyJson));
