...
public class KafkaECS {
	...
	private static Pare3<KafkaCallMsg, Object, Throwable> doKafkaCall(NodeDir node, Component activeExecutionCom, boolean... RETURN) {
		...
		if(isExecutionEnable){
			...
			if (KafkaCallMsg.of(node).method == KafkaCallMsg.Method.KGET && !SysThreads.isThreadActive(threadName)) {
...
//											node.state().appendFcData(msg, 1);
					...
					if (rslt.val() != null) {
						...
						if (isExecutionEnable) {
							ZKME.textReadonly(kafkaCallMsg.method + ":" + node.nodeId(), content, true);
						}
					}
					if (rslt.ext() != null) {
						state.writeFcDataErr(rslt.ext());
				...
				default:
					throw new WhatIsTypeException(kafkaCallMsg.method);
