...
import mpc.map.MAP;
import mpe.NT;
import mpe.cmsg.std.SendCallMsg;
...
//
//		StdType nodeEvalType = (StdType) node.evalType( null);
//		switch (nodeEvalType) {
//			case SENDMSG:
//				break;
//			default:
...
//		String nodeDataVal = inject.newInjected(track).nodeDataStr;
		...
		SendCallMsg.TYPE type = sendCallMsg.subtype();
		...
		switch (type) {
			...
			default:
				throw new FIllegalStateException("SendMessage not support '%s'", type);
		}
	...
	public static Object EMAIL_send(NodeDir node, SendCallMsg sqlCallMsgPrepared) {
		ZBin.JarCall jarCall = ZBin.MAIL.newJarCall();
		...
		Object rspInvoke = jarCall.invokeStringArgs(headersSeqargs);
		return rspInvoke;
	}
...
//		WebUsr webUsr = WebUsrService.get().loadUserById(userId);
		...
		return ARG.throwMsg(() -> X.f("Set Tg App"), defRq);
//		Sys.say("tgsenddd");
