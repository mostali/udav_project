...
//		TaskPanel.removeTask(nodeDir);
	...
	public static String evalNodeNoWeb(NodeData inject, Map trackContext) {
		return evalNode_NULL_QUERY(inject, false, false, trackContext);
	}

	public static String evalNode_NULL_QUERY(NodeData inject, boolean withOuterJp, boolean withOuterXp) {
		return evalNode_NULL_QUERY(inject, withOuterJp, withOuterXp, SpVM.getTrackContext(ARR.EMPTY_MAP));
	}

	public static String evalNode_NULL_QUERY(NodeData inject, boolean withOuterJp, boolean withOuterXp, Map initTrackContext) {
	...
	private static String evalNodeByTrackId(NodeData inject, boolean withOuterJp, boolean withOuterXp, TrackMap.TrackId track) {
...
//				throw NodeApiCallType.toCleanDataResponseException((Pare3) rsp, nodeDir);
			...
			case JARTASK:
				return String.valueOf(JarCallService.EVAL.doJarCallSyncRest_VALUE(inject));
	...
	private static String evalNode_undefined(NodeDir nodeDir, PagePathInfoWithQuery curPPI) {
		...
		switch (exe) {
			case "bash":
			case "python3":
				Pare<Integer, List<String>> rslt = SysExec.exec_filetmp(exe, nodeDir.nodeData(), null, false);
				throw NodeApiCallType.sendCleanResponse(rslt);

			case "bash*":
			case "python3*":
				String nodeName = nodeDir.nodeName();
				String vl = new NoteApi0().zApiUrl.GET_toItem(ItemPath.of(curPPI.sdn0(), nodeName), Pare.of(CN.EXE, exe));
