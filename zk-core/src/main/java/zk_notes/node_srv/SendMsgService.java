...
public class SendMsgService {
	...
	public static final Logger L = LoggerFactory.getLogger(SendMsgService.class);
	...
	public static String doSendMsg_AsyncLog_Simple(NodeDir node) {
		...
		Pare3<Object, Throwable, String> rsp = doSendMsg_AsyncLog(NodeData.of(node, null), null);
...
//		return X.blank(rsp.ext()) ? rsp.key() + "" : rsp.ext();
	...
	public static Pare3<Object, Throwable, String> doSendMsg_AsyncLog(NodeData inject, TrackMap.TrackId track) {

		NodeDir nodeDir = inject.nodeDir;

		String nodeDataVal = inject.fillIfEmpty(track).nodeData;
		...
		switch (sendCallMsg.type()) {
			case EMAIL:
				return LogTailReader.doAsyncLogOperation(() -> MAIL_send(nodeDir, SendCallMsg.of(nodeDataVal)));
			case TG:
		...
		Supplier tgSenderFunc = () -> {
			...
			for (String toChat : toChats) {
				try {
					...
					Integer msgId = tg_sendMsgFromBeaUser(toChat, sendCallMsg.getMsgOrBody(), parseMode);
					l.add(msgId);
				} catch (Exception ex) {
	...
	@SneakyThrows
	public static Object MAIL_send(NodeDir node, SendCallMsg sqlCallMsgPrepared) {
...
//	}
	...
	public static Integer tg_sendMsgFromBeaUser(Long userId, String msgVal, Integer... defRq) {

		WebUsr webUsr = WebUsrService.get().loadUserById(userId);

		String chatId = webUsr.isOwner() ? APP.getTgBotOwnerId() : webUsr.getNidByNet(NT.TG) + "";
...
//		Sys.say("tgsenddd");
	...
	public static Integer tg_sendMsgFromBeaUser(String chatId, String msgVal, String parseMode, Integer... defRq) {
		TgApp appBot = AppZos.TgApp;
		if (appBot != null) {
