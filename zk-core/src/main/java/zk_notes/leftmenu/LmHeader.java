...
public class LmHeader extends Div0 {
	...
	public static class LmCb extends Cb {
		...
		public LmCb(SpaceType spaceType) {
		...
		private static void onUpdateLeftMenu(Event e, SpaceType spaceType) {
	...
	public LmHeader(Paree3<Boolean, Boolean, Boolean> state) {
		...
		Function<SpaceType, Boolean> hasType = (spaceType) -> {
			if (spaceType == null) {
				return false;
			}
			switch (spaceType) {
		...
		Arrays.stream(SpaceType.values()).forEach(spaceType -> {
