		...
		if (nameCount == 2) {
			...
			switch (oper) {
				...
				case lsi:
					...
					String si = allImages.stream().flatMap(c -> ARR.as(c.getRepoTags()).stream()).filter(X::NE).collect(Collectors.joining(STR.NL));
...
//							STREAM.mapToList(allImages, c -> STREAM.filterToAll(ARR.as(c.getRepoTags()), X::NE) + "" + STR.NL) + "";
		...
		if (nameCount == 3) {
			...
			String cid = path.getName(2).toString();
			try {
				String apply = oper.apply2(cid);
