...
//	private static DockerClient dockerClient;
	...
	public enum Oper {
		create, ls, lsi, start, stop, rm, logs, state, build;

		public static Oper valueOf(String name, Oper... defRq) {
			return ENUM.valueOf(name, Oper.class, defRq);
		}

		public String apply2(String cid) {
			switch (this) {
				case state:
					return DockerSrv.getContainerStatus(cid).ext();
				case create:
					throw new FIllegalStateException("support only container operation");
				case start:
					return DockerSrv.startContainer(cid);
				case stop:
					return DockerSrv.stopContainer(cid);
				case rm:
					return DockerSrv.removeContainer(cid);
				case logs:
					return JOIN.allByNL(DockerSrv.printContainerLogs(cid));
				default:
