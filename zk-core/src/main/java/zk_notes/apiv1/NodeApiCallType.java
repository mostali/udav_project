...
import zk_notes.node_srv.EvalNodeService;
...
public enum NodeApiCallType {
	...
	public static NodeApiCallType valueOf(String key, NodeApiCallType... defRq) {
		...
		return ARG.throwOpt(() -> new RequiredRuntimeException("NodeApi not found by key %s", key), findFirst, defRq);
	}
...
//					throw CleanDataResponseException.OK("Downloaded: " + path);
	...
	public static void checkExeParam(Pare sdn, NodeDir nodeDir, PagePathInfoWithQuery curPPI) {
		...
		boolean hasEvalType = nodeDir.evalType( null) != null;
		if (!hasEvalType) {
...
//		NodeData injected = nodeDir.inject(true);
		...
		if (!isAsync) {
			...
			String rsp = EvalNodeService.evalNode_NULL_QUERY(nodeDir, withOuterJp, withOuterXp);
		...
		TaskManager.addTaskAsync(taskName, () -> EvalNodeService.evalNode_NULL_QUERY(nodeDir, withOuterJp, withOuterXp, trackContext));
