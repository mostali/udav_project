...
public abstract class TreeOper {
	...
	public @NotNull Path getAtiTreePath() {
		...
		switch (level) {
			case 0: //sd
				return AFC.EVENTS.getStatePath(sd3, ItemPath.PAGE_INDEX_ALIAS, ItemPath.ITEM_INDEX_ALIAS, EXT.SQLITE);
			case 1: //page
			{
				String page = path(0).toString();
				return AFC.EVENTS.getStatePath(sd3, page, ItemPath.ITEM_INDEX_ALIAS, EXT.SQLITE);
			}
			case 2: //item
			{
				String page = path(0).toString();
