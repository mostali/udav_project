...
//		boolean anonim = SecMan.isAnonim();
		...
		if (allowedEdit) {
			FunctionV1<String> stringFunctionV1 = (String newName) -> {
				UFS_BASE.MV.rename(AppNotes.getPathOfComNote(nodeName).getParent(), newName, null);
				ZKI.infoSingleLine("File '%s' renamed successfully to '%s'", nodeName, newName);
...
//			window.setStyle("position:absolute;left:5%;top:9%;width:90%;height:95%;z-index:1000");
//
//			Win0 window = Win0.of(child);
//			Caption childC = new Caption();
//			childC.appendChild(new Label(title));
//			window.appendChild(childC);
//			window.ovl();
//			window.popup();
////			window.modal();
//			window.setStyle("position:absolute;left:5%;top:9%;width:90%;height:95%;z-index:1000");
//			window.openInFirstWindow();
