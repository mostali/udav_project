...
//	private AppPropDef.ViewType _viewType = AppPropDef.ViewType.AUTO;
//
//	public PropFormItem<T> viewType(AppPropDef.ViewType viewType) {
...
//		} else
	...
	protected void init() {
		...
		if (defValueWithType instanceof Boolean) {
			...
			String tbxVl = targetValue == null ? X.toString(useValue, "") : targetValue.toString();
...
//				ZKI_Log.log("Update '%s' with new value '%s'", property, newValue);
		...
		SerializableEventListener event_PropChanged = event -> {
			if (event instanceof InputEvent) {
				...
				T valueOrDefault = appProp.getValueOrDefault();
