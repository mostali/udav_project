...
//	}
	...
	public class Fields {
		...
		public int[] get_TOP_LEFT(int[]... defRq) {
			...
			return top != null && left != null ? new int[]{top, left} : ARG.throwErr(() -> new RequiredRuntimeException("Except top(%s) left(%s) args ", top, left), defRq);
		}

		public Integer get_TOP(Integer... defRq) {
//			return ZKS.px(get(CN.TOP, ""), defRq);
			String vl = get(CN.TOP, null);
			return vl != null ? ZKS.px(vl, defRq) : (ARG.throwErr(() -> new RequiredRuntimeException("Except value from TOP property"), defRq));
		}

		public Integer get_WIDTH(Integer... defRq) {
			String vl = get(CN.WIDTH, null);
			return vl != null ? ZKS.px(vl, defRq) : (ARG.throwErr(() -> new RequiredRuntimeException("Except value from WIDTH property"), defRq));
		}

		public Integer get_HEIGHT(Integer... defRq) {
			String vl = get(CN.HEIGHT, null);
			return vl != null ? ZKS.px(vl, defRq) : (ARG.throwErr(() -> new RequiredRuntimeException("Except value from HEIGHT property"), defRq));
		}
...
//		}
		...
		public Integer get_SIZE(Integer... defRq) {
			return getAs(NOTE_SIZE, Integer.class, defRq);
		}
	...
	public NVT viewType(NVT... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("NodeViewType not found from '%s'", objName()), defRq);
	}
	...
	public boolean apply(HtmlBasedComponent com, String... props) {
		return apply(com, pathFc(), null, isForm, props);
	}

	public boolean applyChild(HtmlBasedComponent com, String childKey, String... props) {
		return apply(com, pathFc(), childKey, isForm, props);
	}
