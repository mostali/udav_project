...
//	public static PageState forUser(Pare<String, String> sdn, boolean... create) {
//		Path pageState = AppStatePath.getUserPropsPath(sdn.key(), sdn.val());
...
//	}
	...
	public static <T extends ObjState> T ofFormName_WithContent(Pare sdn, String nodeName, String content, boolean... create) {
	...
	public static <T extends ObjState> T ofFormName_WithContent(Pare sdn, String nodeName, String content, String dataProps, boolean... create) {
		Path pathOfFormNote = sdn != null ? AppStatePath.getFormDataPath(sdn, nodeName) : AppStatePath.getFormDataPath_PPI(nodeName);
		if (ARG.isDefEqTrue(create)) {

			boolean existedState = UFS.MKFILE.createFileIfNotExistWithContentMkdirs(pathOfFormNote, content);
			FunctionV s1 = null;
//			if (!created) {
//				s1 = () -> IT.state(UFS.MKFILE.createFileIfNotExistWithContentMkdirs(AppStatePath.getFormDataPath_PPI(nodeName), content));
//			}

			FunctionV s2 = null;
			Path pathFormState = FileState.toPathPropsFromPathFc(pathOfFormNote, false);
			boolean existedState2 = UFS.MKFILE.createFileIfNotExistWithContentMkdirs(pathFormState, dataProps);
//			if (!createdState) {
//				s2 = () -> IT.state(UFS.MKFILE.createFileIfNotExistWithContentMkdirs(pathFormState, content));
//			}


			//wth
//			if (X.nullAnyObj(s1, s2)) {
