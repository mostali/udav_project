...
public class NodeProxyRW extends StateProxyRW {
	...
	public IPath getTargetPathFile(IPath... defRq) {
		...
		return targetPathDir != null ? IPath.of(targetPathDir, EFT.FILE) : ARG.throwMsg(() -> X.f("Except dir [%s] from FILE", nodeDir.nodeId()), defRq);
	}
	...
	public IPath getTargetPathDir(IPath... defRq) {
		...
		return targetPathDir != null ? IPath.of(targetPathDir, EFT.DIR) : ARG.throwMsg(() -> X.f("Except dir [%s] from DIR", nodeDir.nodeId()), defRq);
	}
