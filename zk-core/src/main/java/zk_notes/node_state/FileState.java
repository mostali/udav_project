...
public abstract class FileState<PATH> extends EntityState implements IPath {
	...
	public static final int FILE_OUT_OK = 1;
	public static final int FILE_OUT_ERR = 2;
	...
	public EntityState writeFcProps(String data) {
		Path file = pathProps();
		RW.write(file, data, true);
		L.info("Write FileSate#Props | {}", UF.ln(file));
		return this;
	}
	...
	public EntityState writeFcData(String data, int index) {
		Path file = pathFc(index);
		RW.write(file, data, true);
		L.info("Write FileSate#{} | {}",index, UF.ln(file));
		return this;
	}
	...
	public boolean existPropsFile(boolean... invert) {
		return existPropsFile(pathFc(), ARG.isDefEqTrue(invert) ? !isForm : isForm);
	}
	...
	public GsonMap readFcDataAsGsonMap(GsonMap... defRq) {
		...
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Except state json from file '%s'", fileCom), defRq);
		}
	}
	...
	static boolean apply(HtmlBasedComponent com, Path fileCom, String childKey, boolean isForm, String... props) {
		...
		if (childKey != null) {
			applyGsonMap = applyGsonMap.getAsGsonMap(childKey, null);
			if (X.empty(applyGsonMap)) {
				return false;
			}
		}
		...
		for (String prop : props) {
			...
			boolean rslt = BeType.applyProp(com, applyGsonMap, prop);
	...
	public static Boolean has(Path fileCom, boolean isForm, String key, Boolean... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required has", fileCom, key), defRq);
	}
	...
	public static Boolean hasEnable(Path fileCom, boolean isForm, String key, Boolean... defRq) {
		...
		if (o != null) {
			...
			return b != null ? b : ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required boolean", fileCom, key), defRq);
		}
		return ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required boolean(not null)", fileCom, key), defRq);
	}
	...
	public static Integer getPX(Path fileCom, boolean isForm, String key, Integer... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required px value", fileCom, key), defRq);
	}
	...
	public static <T> T getAs(Path fileCom, boolean isForm, String key, Class<T> asType, T... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required not nul object", fileCom, key), defRq);
	}
	...
	public static Object get0(Path fileCom, boolean isForm, String key, Object... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Com '%s' GsonMap by key '%s' is required", fileCom, key), defRq);
	}
	...
	public static GsonMap readGsonMapOrCreate(Path fileCom, boolean isFormOrProps) {
		return GsonMap.of(toPathPropsFromPathFc(fileCom, isFormOrProps), true);
	}

	public static Path toPathPropsFromPathFc(Path fileCom, boolean isFormOrProps) {
		return !isFormOrProps ? fileCom : fileCom.getParent().resolve(fileCom.getFileName() + AFCC.PROPS_FILE_EXT);
	}
