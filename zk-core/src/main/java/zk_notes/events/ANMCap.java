...
import zk_notes.node_srv.INodePlayProps;
...
//			cap.appendChild(Ln.uploadTo(SYMJ.UPLOAD, node.toPath()));
		...
		if (isEditorAdminOwner) {
			...
			INodeType nodeEvalType = node.evalType(null);

			if (nodeEvalType != null) {

				PlayContainer.PlayLn playLn = PlayContainer.PlayLn.of(node);

				PlayContainer playC = INodePlayProps.toPlayContainer(nodeEvalType, playLn);
		...
		if (isEditorAdminOwner) {
			...
			if (line0 != null && line0.startsWith(NT.TG.HOST_URL()) && UST.URL0(line0, null) != null || NodeCapsCom.SwapTgPostLn.isScriptPattern(line0)) {
				cap = (Cap0) Win0.getCap0OrCreate(comInWin);
	...
	private static @NotNull ArrayList<HtmlBasedComponent> getLinksFromData(ObjState state) {
		...
		List<String> urls = nodeData.stream().filter(u -> UST.URL0(u, null) != null).collect(Collectors.toList());
