...
public class AppEventsFD extends AppEvents {
	...
	public static Pare<String, SerializableEventListener> applyEvent_LOGVIEW(Component clickableSrc, Path path, String... event) {
		if (!UFS.exist(path)) {
			return null;
		}
	...
	public static Pare<String, SerializableEventListener> applyEvent_OPENDIR_VIEW(Component clickableSrc, Path path, String... event) {
		...
		return apply(Pare.of(ANI.OS_OPEN + " Open Dir - View", DirView0.getEventOpenDirViewWithSimpleMenu(path)), clickableSrc, event);
	}
	...
	public static Pare<String, SerializableEventListener> applyEvent_OPENDIR_OS(Component clickableSrc, Path path, String... event) {
		...
		return apply(Pare.of(ANI.OS_OPEN + " Open Dir - OS Manager", DirView0.getEventOpenSimpleMenu_OS(path)), clickableSrc, event);
	}
	...
	public static Pare<String, SerializableEventListener> applyEvent_OPENDIR_TERMINAL(Component clickableSrc, Path path, String... event) {
		...
		return apply(Pare.of(ANI.OS_OPEN + " Open Dir - OS Terminal", DirView0.getEventOpenSimpleMenu_Terminal(path)), clickableSrc, event);
	}
