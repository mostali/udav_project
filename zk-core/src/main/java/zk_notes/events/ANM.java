...
//		menu.addMI_EDITOR(ANI.EDIT_MODE + " Edit Form Props*", nodeDir.state().pathPropsCom(), false, EXT.JSON);
		menu.addMI_EDITOR(ANI.EDIT_MODE + " Edit Form Props*", nodeDir.getPath_FormFc_Props(), false, EXT.JSON);
	}
...
//		menu.addMI(SYMJ.LINK + " Copy link to rest", e -> ZKJS.eval(X.f("copyToClb('%s')", url2item)));

		{
			if (Sec.isAdminOrOwner()) {
				if (node.getPathFcParentLs().size() > 2 || IPath.ofd(node.getPathFc().getParent()).dMapExt(ARR.EMPTY_MAP).hasOnly(EXT.PROPS, EXT.PROPS$$)) {
					...
					String dldUrl = APP.HOST.getAppHost0() + "/" + ApiCase._adi + "/.planes" + AFCC.relativizeAppFile(pathFc);
		...
		menu.addMI(SYMJ.LINK + " Copy node data injected", e -> ZKME.textReadonly(node.nodeId(), NodeData.of(node).newInjected(null).nodeData));
		menu.add_______();
	...
	public static class ANM_Mark {
		public static void applyMenu_Mark(Menupopup0 mainMenu, NodeDir nodeDir) {
			...
			Arrays.stream(NVT.values()).forEach(nvt -> {
				...
				m.addMI(LBL_PROP(FormState.PK_VIEW, lbl), (SerializableEventListener) event -> {
...
//			markMenu.addSeparator();
		...
		private static void addEnableDisableProp_USER(Menupopup0 markMenu, FormState formState, String prop) {
			...
			markMenu.addMI(LBL_PROP(FormState.PK_USER, lbl), buildEventPropChanger_REMOVE_or_SET_STATIC_VALUE(formState, prop, Sec.login()));
		}
		...
		private static void addEnableDisableProp_SEC(Menupopup0 markMenu, FormState formState, String prop) {
			...
			markMenu.addMI(LBL_PROP(prop, lbl), buildEventPropChanger_REMOVE_or_SET_STATIC_VALUE(formState, prop, SecMan.SECFORALL));
		}

		public static EventListener buildEventPropChanger_BOOL_SWAP(FormState state, String prop, boolean defIfNotPresent) {
