...
public class ANM extends ANMFd {
	...
	public static void applyMenu_EditAs(Menupopup0 menu, NodeDir nodeDir, boolean innerMenu) {
		...
		menu.addMI_EDITOR(SYMJ.PUZZLE + " Edit Form Data", nodeDir.getProxyRW(false).getTargetPath_FILE(), true, null);
//		menu.addMI_EDITOR(ANI.EDIT_MODE + " Edit Form Props*", nodeDir.state().pathPropsCom(), false, EXT.JSON);
	...
	public static void applyMenu_SimpleViewAs(Menupopup0 menu, NodeDir nodeDir) {
		...
		menuViewAs.addMI(ANI.OPEN_SE_EDITOR + " View As - HTML", e -> ZKM.showModal(nodeDir.nodeName(), Xml.asXmlOf(nodeDir.getPathFc())));
		menuViewAs.addMI(ANI.OPEN_SE_EDITOR + " View As - Markdown", e -> ZKM.showModal(nodeDir.nodeName(), Xml.asMd(nodeDir.getPathFc())));
	}
...
//		menu.addMI(SYMJ.LINK + " Copy link to rest", e -> ZKJS.eval(X.f("copyToClb('%s')", url2item)));
		...
		menu.addMI(SYMJ.LINK + " Copy node absolute file pth", e -> ZKJS.eval(X.f("copyToClb('%s')", node.getPath_FormFc_Data())));
