...
public class ANM extends ANM0 {
	...
	public static void applyFormLink(Menupopup0 menu, NodeDir nodeDir) {
		...
		if (allowedEdit) {
			...
			menu.addMenuItem(SYMJ.FILE_IMG2 + " Choice Color", (Event e) -> ZKC.getFirstWindow().appendChild(new ColorPicker() {
				@Override
			...
			{// ACTUAL
				...
				menu.addMenuItem(LBL(prop, lbl0), ANM_Mark.buildEventPropChanger_BOOL_SWAP(nodeDir, prop, defIfNotPresent));
			}
			{// ACTUAL
				String prop = CN.BIG;
				boolean defIfNotPresent = true;
...
//		boolean userEqualsByLoginOrAlias = SecMan.isUserEqualsByLoginOrAlias(sdn.key());
	...
	public static void applyMenu_EditAs(Menupopup0 menu, NodeDir nodeDir) {
		menu.add_______();
		menu.addMenuItem(ANI.EDIT_MODE + " Edit Data", e -> ZKI.infoEditorBw(nodeDir.getPathFc()));
		menu.addMI_EDITOR(ANI.EDIT_MODE + " Edit Props", nodeDir.state().pathProps(), true, EXT.JSON);
		menu.addMI_EDITOR(ANI.EDIT_MODE + " Edit Props (Com)", nodeDir.state().pathPropsCom(), false, EXT.JSON);
	}

	public static void applyMenu_OpenAs(Menupopup0 menu, NodeDir nodeDir) {
		menu.add_______();
		menu.addMenuItem(ANI.OPEN_SE_EDITOR + " Html View (Modal)", e -> ZKM.showModal(nodeDir.nodeName(), Xml.ofFile(nodeDir.getPathFc())));
	}
	...
	private static void addOpenToClipboardMenu(Menupopup0 menu, NodeDir node) {
		...
		menu.addMenuItem(SYMJ.LINK + " Copy link to rest event", e -> ZKJS.eval(X.f("copyToClb('%s')", url2event)));
		menu.addMenuItem(SYMJ.LINK + " Copy nodeID", e -> ZKJS.eval(X.f("copyToClb('%s')", node.nodeId())));
		menu.addMenuItem(SYMJ.LINK + " Copy node data injected", e -> ZKM_Editor.openEditorText(node.nodeId(), NodeData.of(node).fillIfEmpty(null).nodeData));

		menu.add_______();

		menu.addMenuItem(ANI.DOWNLOAD + " Download As File", e -> ZKR.download(node.state().pathFc()));
