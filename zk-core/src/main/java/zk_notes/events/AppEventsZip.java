		...
		return apply(Pare.of(SYMJ.FILE_ARCHIVE + " Archive Page", (Event e) -> {
			ZKI_Messagebox.showMessageBoxBlueYN("Zip page...", "Zip page?", (yes) -> {
				if (yes) {
					...
					UFS_BASE.MKDIR.mkdirParentIfNotExist(dstZip, 2);
					...
					ZKI.infoAfterPointer(X.f("Page was archive (%s) to file '%s'", X.sizeOfHu(dstZip), dstZip), ZKI.Level.INFO);
				}
			});
		}), com, event);
	}
