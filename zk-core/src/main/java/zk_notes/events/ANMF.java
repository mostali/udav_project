...
//		boolean isAdminOrOwner = SecMan.isOwnerOrAdmin();
		...
		if (allowedEdit) {
			...
			INodeType nodeEvalType = nodeDir.stdType( null);
			...
			Mi miCopyNotes = CfrmTbx_SEL.toMenuItemComponent("Do copy '" + nodeName + "'", SYMJ.COPY + " Copy item", nodeName, "Set new Notes name", (String newNotesName) -> {
...
//			menu.addMI(ANI.LOGVIEW + " Log View", e -> LogFileView.openSingly(.toString()));
		...
		if (isOwner) { // EXEC HOST PROFILE
			...
			if (hpc != null) {
				...
				Mi menuItemComponent = CfrmTbxm2_SEL.toMI(ANI.VIEW_MODE + "Host Exec", "open session", ARR.of("", ""), ARR.of("", ""), func);
...
//		menu.add_______();
		...
		SupplierLazy<Menupopup0> getterVerisonPanel = new SupplierLazy(() -> innerSpecify.addInnerMenu(SYMJ.MENU_V + " Versioning.."));
	...
	public static void applyFormMenu(Menupopup0 menu, NodeDir node) {
		INodeType nodeEvalType = node.stdType();
