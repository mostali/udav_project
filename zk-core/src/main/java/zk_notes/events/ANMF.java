...
//		FormState formState = AppStateFactory.ofFormDir_orCreate(sdn, nodeDirPath);
		...
		if (allowedEdit) {
			...
			//MOVE ITEM
			AppEventsPage.applyEvent_MoveNode2Page(menu, sdn, formState.formName());
...
//			menu.addMI(ANI.LOGVIEW + " Log View", e -> LogFileView.openSingly(.toString()));
	...
	private static void applyMenu_Behaviours(Menupopup0 menu, NodeDir nodeDir) {
		...
		innerSpecify.addMI(SYMJ.FILE_IMG2 + " Choice Color - Link", (Event e) -> ZKC.getFirstWindow().appendChild(new ColorPicker() {
			@Override
			public void onCLICK_COLOR(Event event, String parentName, String colorCode) {
				AppStateFactory.ofComName_orCreate(sdn, nodeName).set(FormState.BG_COLOR, colorCode);
				removeMe();
				NotesSpace.rerenderFirst();
			}
		}));
...
//				innerSpecify.addMI_Position(nodeDir);
...

			{
				FormState.Position curVal = formState.fields().get_POSITION(FormState.Position.ABS);
				String curValStr = ANM.LBL_PROP("Position - Form", curVal.name());
				innerSpecify.addMI_Cfm1(curValStr, "Set Position - Form " + ARR.as(FormState.Position.values()) + " | Absolute / Fixed / Relative ", curVal.name(), (in) -> {
					formState.fields().set_POSITION(FormState.Position.valueOf(in));
					NotesSpace.rerenderFirst();
					return null;
				});
			}
			...
			{
				FormState.Position curVal = formStateCom.fields().get_POSITION(FormState.Position.ABS);
				String curValStr = ANM.LBL_PROP("Position - Link", curVal.name());
				innerSpecify.addMI_Cfm1(curValStr, "Set Position - Link " + ARR.as(FormState.Position.values()) + " | Absolute / Fixed / Relative ", curVal.name(), (in) -> {
					formStateCom.fields().set_POSITION(FormState.Position.valueOf(in));
					NotesSpace.rerenderFirst();
					return null;
				});
			}
