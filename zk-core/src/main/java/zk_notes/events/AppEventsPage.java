...
public class AppEventsPage extends AppEvents {
	...
	public static Pare<String, SerializableEventListener> applyEvent_EditPageProps(Component com, Path pageState, String... event) {
		String pathPageState = pageState.toString();
		return applyTo(Pare.of(ANI.PROPS_ZZZ + " Page Props", (Event e) -> ZKI.infoEditorDark(Paths.get(pathPageState), EXT.JSON)), com, event);
	}
	...
	public static Pare<String, SerializableEventListener> applyEvent_MovePageToOtherPlane(Component com, Pare<String, String> sdn, String... event) {
		...
		return applyTo(Pare.of(ANI.MOVE_FILE + " Move to other plane", (Event e) -> {
			...
			PlaneDdChoicer sd3Choicer = new PlaneDdChoicer() {
				@Override
				public void onChoicePlane(Event onPlaneSubmitEvent, String dstPlane) {
					String sd3 = sdn.key();
					String pagename = sdn.val();
					FunctionV1<Boolean> func = (rslt) -> {
						if (rslt) {
							NFTrans.movePageToSd3(sd3, pagename, dstPlane);
							if (Sdn.isEmptyPlane_WoCheckIndex()) {
								RSPath.toPlane_Redirect(dstPlane);
							} else {
								RSPath.toPage_Redirect(dstPlane, pagename);
							}
						}
					};
					String message = X.f("Move page '%s' to plane '%s'", pagename, dstPlane);
	...
	public static Pare<String, SerializableEventListener> applyEvent_MoveNode2Page(Component com, Pare<String, String> sdn, String formname, String... event) {
		...
		return applyTo(Pare.of(ANI.MOVE_FILE + " Move to other page", (Event e) -> {
				...
				public void onChoicePlane(Event onPlaneSubmitEvent, String dstPlane) {
					...
					FunctionV1<Boolean> func = (yesPlane) -> {
						if (yesPlane) {
							...
							PageDdChoicer choicerPage = new PageDdChoicer(dstPlane) {
								...
								@Override
								public void onChoicePage(Event onPageSubmitEvent, String dstPagename) {

									NodeDir dstNode = NodeDir.ofNodeName(Sdn.of(dstPlane, dstPagename), formname);
									FunctionV1<Boolean> func = (yesPage) -> {
										if (yesPage == null) {
											return;
										}
										IT.state(!dstNode.existNode(false), "Note %s already exited", dstNode);
										NFTrans.moveItemNote(srcNode, dstNode);
