	...
	public static Window openNoteWin(NodeDir nodeDir, OptsOpenNode opts, Component... parent) {
		...
		switch (nvt) {
			...
			case CODE:
				...
				IZWin comText = NFCreate.createForm(nodeDir, nvt);
				...
				if (nodeDir.state().fields().get_FIXED(true)) {
					...
					comText.absolute();
					formState.apply_TOP_LEFT(targetCom);
					formState.apply_WIDTH_HEIGHT(targetCom);

				}


				formState.apply_TITLE(targetCom);
				formState.apply_TITLEX(targetCom);
				...
				if (SecMan.isAllowedEdit(nodeDir)) {
					ANMF.applyNolCom(comText.getOrCreateMenupopup(firstWindow), nodeDir);
				}
