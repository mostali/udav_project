...
public class NFOpen extends NFCreate {
	...
	public static Window openNoteWinAsNotify(NodeDir nodeDir, Component... parent) {
		Window window = NFOpen.openNoteWin(nodeDir);
		Notification.show(nodeDir.nodeData(), parent.length == 0 ? ZKC.getFirstWindow() : IT.NN(parent[0]));
		return openNoteWin(nodeDir, true, false, false, parent);
	}
	...
	public static Window openNoteWin(NodeDir nodeDir, OptsOpenNode opts, Component... parent) {
		...
		switch (nvt) {
			...
			case CODE:
				...
				IZWin comText = NFCreate.createForm(nodeDir, nvt, NodeDir.Beahviour.Absolute);

				HtmlBasedComponent targetCom = (HtmlBasedComponent) comText;

				if (nodeDir.state().fields().get_FIXED(true)) {

					comText.absolute();

					formState.apply(targetCom, FormState.TOP_LEFT);

					formState.apply(targetCom, FormState.WIDTH_HEIGHT);

				}


				formState.apply(targetCom, FormState.PK_TITLE);
				formState.apply(targetCom, FormState.PK_TITLEX);
	...
	public static void openOrCloseToggle(NodeDir nodeDir, Component... parent) {

		IZCom detachedCom = closeCom(nodeDir);
		if (detachedCom != null) {
			return; //closed
		}
...
//		NFOpen.openNoteWin(nodeDir, parent);
	...
	public static @Nullable IZCom closeCom(NodeDir nodeDir) {
		IZCom node = nodeFormAny(nodeDir);
		if (node != null) {
			if (node instanceof NoteTbxm) {
				((NoteTbxm) node).detachNodeCom();
			} else {
