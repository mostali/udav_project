...
public class NFOpen extends NFCreate {
	...
	public static Window openFormWithNotify(NodeDir nodeDir, Component... parent) {
//		Window window = NFOpen.openNoteWin(nodeDir);
		Notification.show(nodeDir.nodeDataStr(), parent.length == 0 ? ZKC.getFirstWindow() : IT.NN(parent[0]));
	...
	public static Window openNoteWin(NodeDir nodeDir, OptsOpenNode opts, Component... parent) {
		...
		switch (nvt) {
			...
			case CODE:
				...
				FormState.Position pos = nodeDir.state().fields().get_POSITION(FormState.Position.ABS);
