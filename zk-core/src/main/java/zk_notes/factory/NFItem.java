...
public class NFItem {
	...
	public static Collection<Class<Component>> COMS_CLASSES = (Collection) ARR.asAD(
			NodeLn.class
	);
	public static Collection<Class<Component>> FORM_CLASSES = (Collection) ARR.asAD( //
			NoteTbxm.class, //
			SeNoteTbxm.class, //
	...
	public static IZWin createAuto(NodeDir nodeDir, NVT def) {

		NVM nvm = nodeDir.nvm_define();

		if (nvm != null) {
			return (IZWin) createForm_MEDIA(nodeDir, nvm);
		}

		NVT nvt = def != null ? def : nodeDir.nvt(NVT.TEXT);

//        return NFItem.createForm(nodeDir, nvt);
		return nodeDir.createForm(nvt);
	...
	public static IZWin createForm_XTABLE(NodeDir node) {
		//CtxTreeView view = CtxTreeView.createView(node.toPathEventTree());
		//return (IZWin) view.borderSilver().inlineBlock();
		NodeProxyRW proxyRW = node.getProxyRW();
		Path targetAnyPathRead = proxyRW.getTargetPathFileRead();
...
//				Xml xml = Xml.ofFile(pathForm.toString());
				Xml xml = Xml.asXml(node.injectStr());
			...
			case MD_WIN:
				return Xml.asMd(node.injectStr());
...
//			NoteTbxm nodeTbx0 = (NoteTbxm) new NoteTbxm(nodeDir, pathForm, Tbx.DIMS.BYCONTENT).prettyjson(stateForm.isJson());//.eventBusable(Pare.of(QzNoteService.SaveTbxListener.BUSEVENT_NOTE_SAVE, nodeDir));
	...
	public static IZCom createForm_MEDIA(NodeDir node) {
		NVM nvm = node.nvm_define();
		if (nvm == null) {
			return null;
		}
		return createForm_MEDIA(node, nvm);
	}
	...
	public static IZCom createForm_MEDIA(NodeDir node, NVM nmt) {
		switch (nmt) {
			...
			case BI:
				return createForm_BI(node);
			default:
				throw new WhatIsTypeException(nmt);
		}
	}

	private static IZCom createForm_BI(NodeDir node) {
		return new BiVF(node);
	}
