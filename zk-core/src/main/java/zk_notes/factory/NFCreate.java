...
//		return (IZWin) view.borderSilver().inlineBlock();
	...
	public static IZWin createForm(NodeDir node, NVT nvt, NodeDir.Beahviour... openBeahviour) {
...
//				return (IZWin) view.borderSilver().inlineBlock();
			...
			case HTML:
				return XmlHtml.ofWithFormName(node.nodeDataInjected().nodeData, node.nodeName());

			case MD:
				return XmlMd.ofWithFormName(node.nodeDataInjected().nodeData, node.nodeName());
	...
	public static SeNoteTbxm createForm_WYSIWYG(NodeDir nodeDir) {
		...
		boolean isEditorAdminOwner = Sec.isEditorAdminOwnerRole();
...
//			NoteTbxm nodeTbx0 = (NoteTbxm) new NoteTbxm(nodeDir, pathForm, Tbx.DIMS.BYCONTENT).prettyjson(stateForm.isJson());//.eventBusable(Pare.of(QzNoteService.SaveTbxListener.BUSEVENT_NOTE_SAVE, nodeDir));
	...
	static @NotNull Pare initBehavioursNVT(NodeDir node, IZCom izCom, NodeDir.Beahviour... openBeahviour) {
...
//						throw new WhatIsTypeException(nvt);
		...
		boolean adminOrOwner = Sec.isAdminOrOwnerRole();

		if (!ARR.contains(openBeahviour, NodeDir.Beahviour.Relative)) {
			com.absolute();
		}

		NFBe.applyDnd_PosWh_Behaviours(com, node.nodeName(), adminOrOwner, true);

		if (ARR.contains(openBeahviour, NodeDir.Beahviour.Inline)) {
			izCom.inlineBlock();
		}
