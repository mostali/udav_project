...
public class NFPage {
	...
	public static final Logger L = LoggerFactory.getLogger(NFPage.class);
...
//		Set<Path> set = AFC.FORMS.DIR_FORMS_LS_CLEAN(sd3pn);
		Set<Path> set = SpaceType.NODES.ls(sdn, UO.VIEW, UO.EDIT, UO.RUN);
...
//			}
			return Pare.of(nodeDir, val);
				...
				Collectors.toMap(nd -> nd.key(), //
						...
						(v1, v2) -> X.throwIllegalStateException("who is master? %s vs %s", v1, v2), //
...
//			if (AppZosProps.APD_IS_PROM_ENABLE.getValueOrDefault(false)) {
		...
		Pare<NodeDir, Component> extCom = NFItem.createFormNOL(node, true);
		if (extCom != null) {
			return extCom;
		}
