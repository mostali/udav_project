...
public class NFPageCom {
	...
	public static Map<NodeDir, Component> buildPageComMap(Pare sd3pn, HtmlBasedComponent parent) {
		...
		return set.stream().map(nodePath -> {
			...
			if (fileType != null) {
				String fileData = AppStateFactory.ofFormDir_orCreate(sd3pn, nodePath).readFcData(null);
				if (X.empty(fileData)) {
...
//				return Pare.of(NodeDir.ofDir(path, sdn()), new ErrLb("Unexcepted err:" + nodeName(), ex));
	...
	private static @NotNull NodeLn buildNodeLn(HtmlBasedComponent parent, NodeDir node) {
		...
		boolean isDeprecated = stateCom.hasPropEnable(NodeCom.DEPRECATED, false);

		NodeLn nodeLn = isDeprecated ? new NodeLn(parent, node, (Boolean) null) : new NodeLn(parent, node);

		NFStyle.applyDefaultStyle(nodeLn, node);

		NFStyle.applyState_BgColor(nodeLn, node.state());
