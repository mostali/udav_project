...
public class NFNew {
	...
	public static Pare<NodeDir, Window> openRandomINE(Pare sdn, OptsAdd... opts) {
		...
		return openNewAlertINE_inCurrentSdn(NFTrans.NEW_NOTE_PFX + STR.randAlpha(3), "", opts);
	}

	public static Pare<NodeDir, Window> openNewAlertINE_inCurrentSdn(String newNotesName, String content, OptsAdd... opts) {
		return openNewAlertINE(null, newNotesName, content, opts);
	}

	public static Pare<NodeDir, Window> openNewAlertINE(Pare<String, String> sdn_OrNull, String newNotesName, String content, OptsAdd... opts) {
		...
		if (UFS.exist(newNotesPath)) {
			...
			return openNewForce(sdn_OrNull, newNotesName, content, opts);
		}
	}

	public static Pare<NodeDir, Window> openNewForce(Pare<String, String> sdn, String nodeName, String data, OptsAdd... opts) {

		sdn = sdn == null ? Sdn.get() : sdn;

		OptsAdd opts0 = ARG.toDefOrGet(() -> OptsAdd.newOpts(), opts);
		...
		if (opts0.isAllowedAutoPfx()) {
			nodeName = pfxName.apply(nodeName);
		}

		String content = X.toStringNN(data, "" + RANDOM.uuid(3));

		FormState props = AppStateFactory.ofFormName_WithContent(sdn, nodeName, content, true);
		...
		NVT viewType = NVT.TEXT;
		{
			...
			if (opts0.nodeViewType != null) {
				...
				if (defResData == null) {
					...
					props.set(ObjState.BG_COLOR, nodeEvalType.stdColor());
					props.writeFcData(defResData);

				}
			}

			opts0.applyProp(props, BeType.note_size);
		...
		{//try open new
			NodeDir nodeDir = NodeDir.ofNodeName(SpVM.get().sdn(), nodeName);
