...
//		}
		...
		FormState.Position position = stateForm.fields().get_POSITION(FormState.Position.ABS);
		String prop = null;
		switch (position) {
			case ABS:
				break;
			case FIX:
				stateForm.apply((HtmlBasedComponent) comTextNoteTbxm_OrComTextWin.getParent(), FormState.PK_FIXED);
				break;
			case REL:
				break;
			default:
	...
	public static void applyComDefaultStyle(HtmlBasedComponent nodeLn, NodeDir node) {
		...
		if (!stateCom.apply(nodeLn, "position")) {
			FormState.Position position = stateCom.fields().get_POSITION(FormState.Position.ABS);
			switch (position) {
				case ABS:
					ZKS.ABSOLUTE(nodeLn);
					break;
