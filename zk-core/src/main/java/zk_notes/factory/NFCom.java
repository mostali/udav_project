...
public class NFCom {
	...
	public static IZWin createForm(NodeDir nodeDir, NVT nvt) {
		...
		switch (nvt) {
			...
			case DIR:
				return (IZWin) DirView0.openWithSimpleMenuAsForm(nodeDir.toPath()).borderSilver().inlineBlock();
	...
	public static PrettyCodeXml createForm_PRETTYCODE(NodeDir nodeDir) {
		return PrettyCodeXml.of(nodeDir.nodeData(), true, nodeDir.nodeName());
	}
	...
	public static IZCom createForm_Dir(NodeDir node) {
		return new DirView0(node.toPath(), true);
	}
