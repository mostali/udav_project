	...
	public Collection<T> searchImpl(String searchPart, int count) {
		...
		switch (searchMode) {
			...
			case AP: {
				...
				Map<Pare<APType, String>, Pare<String, List<String>>> mapKeys;
				...
				if (X.empty(mapKeys)) {
					...
					List<Pare3<APType, String, String>> collect = mapKeys.entrySet().stream().map(kv -> {
						Pare<APType, String> key = kv.getKey();
						Pare<String, List<String>> value = kv.getValue();
						List<Pare3<APType, String, String>> pares = STREAM.mapToList(value.val(), v -> Pare3.of(key.key(), value.key(), v));
						return pares;
