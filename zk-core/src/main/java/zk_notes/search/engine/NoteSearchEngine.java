	...
	public Collection<T> searchImpl(String searchPart, int count) {
		...
		switch (searchMode) {
			...
			case AP: {
				...
				Predicate<String> stringPredicate = k -> searchPart == null ? true : k.contains(searchPart);
				Map<Pare<BootContext.ApType, String>, Pare<String, List<String>>> mapKeys = bootContext.findValuesMapWithKey(stringPredicate, false);
				if (X.empty(mapKeys)) {
					founded = ARR.as();
				} else {
