...
public class DirView extends FdView implements IReRender {
	...
	public static DirView findFirst() {
		return IZCom.findFirstInPage(DirView.class, true);
	}
	...
	protected void init() {
		...
		Lb dirRoot = appendLb(filePfx(level) + FD_ICON.toNameDir(dirPath));

		if (applierDirMenu != null) {
		...
		if (filterFiles != null) {
			files = ST.filter(files, filterFiles);
		}
		for (Path file : files) {
...
//				Menupopup0 menupopup = Menupopup0.createMenupopup(this, fileViewChild, null);
//				applierFileMenu.apply(menupopup, dirPath);
//			}
		}
	}
	...
	public <T extends FdView> List<T> getFdViews(EFT eft) {
		...
		List all = ZKCFinder.findAllFromParent(this, fdVieClass, false, ARR.EMPTY_LIST);
		return all;
	}
