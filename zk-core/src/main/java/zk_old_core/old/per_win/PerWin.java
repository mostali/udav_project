...
public class PerWin extends Window implements IPerDndEvent {
	...
	public static <W extends Component> List<W> findAll(Class<W> clazz, List<W>... defRq) {
		return ZKCFinder.rootsByClass(clazz, true, defRq);
	}

	@Override
	public IPerWinStateRw getStateRw() {
		return IPerWinStateRw.loadStateRw(stateAsPath());
	}

	private final String stateAsFile;
	private transient Path stateAsPath;

	public Path stateAsPath() {
		return stateAsPath == null ? (stateAsPath = Paths.get(stateAsFile)) : stateAsPath;
	}

	public String nameState() {
		return stateAsPath().getFileName().toString();
	}
	...
	public PerWin() {
		if (this instanceof IPerState) {
			...
			this.stateAsPath = iPerState.getPathState(false, null, null, null);
			this.stateAsFile = stateAsPath.toString();
		} else {
	...
	public PerWin(Path fileState) {
		this.stateAsFile = fileState.toString();
		this.stateAsPath = fileState;
	}
...
//		setTitle("goo");
		...
		IPerDndEvent.recoveryState(this, stateAsPath());
			...
			return new EnumSwitcher(EyesOptions.class) {
				...
				protected void applyPosition(Enum typeValue) {
					switch ((EyesOptions) typeValue) {
						case RESET:
							reset(PerWin.this.getClass(), stateAsPath());
							if (pageDirModel != null) {
								ZKR.rebuildPage(pageDirModel);
							}
							break;
						case PROPS:
							ZKME.openEditorImgOrText(stateAsPath());
							break;
						case NOTHING:
							break;
						default:
