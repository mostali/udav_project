...
public class SdRsrc {
	...
	public enum LocRsrc {
		...
		public static LocRsrc of(String requestRsrcPath, LocRsrc... defRq) {
			...
			} else if (requestRsrcPath.startsWith(AppZosCore.rpaAssetsDir_)) {
			...
			} else if (requestRsrcPath.startsWith(AppZosCore.rpaUploadsDir_)) {
		...
		public static Path getParentOfStdLocation(LocRsrc locRsrc, PagePathInfo ppi, String sd3) {
			switch (locRsrc) {
				...
				case SITE_ASSETS:
					return AppZosCore.getRpaAssetsPath();
				case SITE_UPLOADS:
					return AppZosCore.getRpaUploadPath();
				default:
