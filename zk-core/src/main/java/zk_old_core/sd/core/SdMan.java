...
////		URx.exit("^\\d{1,12}$", "123", "123.0", "123,0", "123,09");
	...
	public static Pare<RepoPageDir, Path> findPageIndex() {
		return Pare.of(RepoPageDir.of(AppZosCore_Old.getMasterRepo()), AppZosCore_Old.getMasterPage());
	}

	public static Pare<RepoPageDir, Path> findPage(String pagename) {
		return RepoPageDir.getPageWithRepo(AppZosCore_Old.getMasterRepo(), pagename, true);
	}
...
//	}
	public static List<String> getAllSubdomainNames() {
		...
		return UF.fn(UDIR.ls(AppZosCore_Old.getDomainsRepo(), EFT.DIR));
	}
