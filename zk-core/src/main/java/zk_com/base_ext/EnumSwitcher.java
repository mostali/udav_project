...
public abstract class EnumSwitcher<T extends Enum> extends SpanSwitch implements ITouchEvent {

	final Class<T> ddType;

	public EnumSwitcher(Class<T> ddType) {
	...
	public EnumSwitcher(Class<T> ddType, String eventToShowDd, IZComExt holderMenu, boolean visibleOnMove) {
		super();
		...
		if (holderMenu != null) {
			...
			if (X.empty(eventToShowDd)) {
				addToMenu.addMenuItem(getIcon(), (SerializableEventListener) -> onSwitchEvent());
			} else {
				addToMenu.addMenuItem(getIcon(), eventToShowDd, (SerializableEventListener) -> onSwitchEvent());
			}
		} else {
