...
//		addMI(label, new UploaderFileOrPhotoEvent(formPath.toString(), isImg, null));
	...
	public void addMI_DeleteFile_WithSec(String label, Path file, boolean... isFormPathComponent_orPage_orNot) {
		...
		boolean editorAdminOwner = Sec.isEditorAdminOwnerRole();
		if (!editorAdminOwner) {
			return;
		}
	...
	public void addMI_Position(NodeDir nodeItem, boolean... com) {
		...
		addMI(SYMJ.MOVE + " Set Position - " + (ARG.isDefEqTrue(com) ? "Link" : "Form"), e -> {
			...
			FormState formState = ARG.isDefEqTrue(com) ? nodeItem.stateCom() : nodeItem.state();
			FormState.Position position = formState.fields().get_POSITION(FormState.Position.ABS);
			Dd dd = new Dd(position);
			dd.onOK(onChangeDd -> {
				formState.fields().set_POSITION((FormState.Position) dd.getValueAsEnum(FormState.Position.class));
