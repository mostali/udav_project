...
import zk_form.events.cfrm.CfrmRemoveFileSEL;
import zk_form.events.cfrm.CfrmTbx_RenameFileSEL;
import zk_form.events.cfrm.CfrmTbx_SEL;
...
//			return menuPopup;
	...
	public Pare<Menuitem, Menupopup0> addMI_Cfm1(String label, String placeholder, String initValue, Function<String, Object> hiv) {
		Mi menuItemComponent = CfrmTbx_SEL.toMenuItemComponent("", label, initValue, placeholder, hiv);
...
//		addMI(label, new UploaderFileOrPhotoEvent(formPath.toString(), isImg, null));
	...
	public void addMI_UploadDdTo(String label, Path formPath) {
		...
		addMI(label, e -> DDFileUploader.open(Paths.get(formPathStr)));
	}

	public void addMI_UploadTo(String label, Path formPath, boolean isImg, Function<String, Boolean> saveCallback) {
		String formPathStr = formPath.toString();
		addMI(label, new UploaderFileOrPhotoEvent(formPathStr, isImg, saveCallback));
	}

	public void addMI_Download(String label, Path file) {
		String fileStr = file.toString();
		addMI(label, (Event e) -> ZKR.download(Paths.get(fileStr)));
	}

	public void addMI_DownloadAsZip(String label, Path downloadDir) {
		String dirStr = downloadDir.toString();
		addMI(label, (Event e) -> {
	...
	public void addMI_DeleteFile_WithSec(String label, Path deleteFile, boolean... isFormPathComponent_orPage_orNot) {
		if (!UFS.exist(deleteFile)) {
			return;
		}
		...
		String fileStr = deleteFile.toString();
		addMI(label, new CfrmRemoveFileSEL(fileStr, (e) -> {
			Path pageDir = Paths.get(fileStr);
			if (ARG.isDefEqTrue(isFormPathComponent_orPage_orNot)) {
				NFTrans.checkEmptyDir_Sd3_Page_andRemove(AFCC.getPageOfNodeFileCom(pageDir));
			} else if (ARG.isDefEqFalse(isFormPathComponent_orPage_orNot)) {
				NFTrans.checkEmptyDir_Sd3_Page_andRemove(pageDir);
			}
			ZKR.restartPage();
		}));
	}
	...
	public void addMI_DeleteFile(String pathStr, DefAction successCallback_orNull) {
		addMI_Href_in_Self(CfrmRemoveFileSEL.toMenuItemComponent(pathStr, successCallback_orNull));
	}
	...
	public void addMI_RenameFile_Cfrm(String pathStr, FunctionV1 successCallback_orNull, boolean... withRenameChildsDir) {
		...
		addMI_Href_in_Self(CfrmTbx_RenameFileSEL.toMenuItemComponent(pathStr, successCallback_orNull, ARG.isDefEqTrue(withRenameChildsDir)));
	}

	public void addMI_RenameFileDirect(Path renamedFile, FunctionV1 successCallback_orNull, boolean... withRenameChildsDir) {
		if (!UFS.exist(renamedFile)) {
			return;
		}
		String fileStr = renamedFile.toString();
		...
		addMI(SYMJ.EDIT_PENCIL + " Rename " + UF.fn(renamedFile), eventListener);
	}
	...
	public void addMI_MoveNode2Plane_WithSec(NodeDir nodeItem) {
		...
		PlaneDdChoicer child = new PlaneDdChoicer() {
			...
			public void onChoicePlane(Event onPlaneSubmitEvent, String plane) {
				FunctionV1<Boolean> func = (rslt) -> {
					if (rslt) {
						nodeItem.fsMan().moveItemToSd3(plane);
						RSPath.toPage_Redirect(plane, nodeItem.sdnPare().valStr());
//															ZKL.alert("Move %s ot %s", srcCom, dstCom);
					}
				};
				String title = X.f("Move to plane '%s'", nodeItem.nodeName());
				String message = X.f("Move to plane '%s'", plane);
	...
	public void addMI_Tbx2_Cfrm(String label, String[] initValues, String[] placeholders, Function2<String, String, Object> successCallback) {
		Mi miNewNotes = CfrmTbxm2_SEL.toMI(label, label, initValues, placeholders, successCallback);
		addMI_Href_in_Self(miNewNotes);
	}

	public Pare<Menuitem, Menupopup0> addMI_OpenDirView(String label, Path openDir, boolean... mkdirsIfNotExist) {
		String fileStr = openDir.toString();
		return addMI(label, e -> DirView0.openWithSimpleMenu(Paths.get(fileStr), mkdirsIfNotExist));
	}
