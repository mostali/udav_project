...
import zk_old_core.old.fswin.core.SimpleDirView;
...
//			return menuPopup;
	...
	public Pare<Menuitem, Menupopup0> addMenuitem_Cfm1(String label, String placeholder, Function<String, Object> hiv) {
		Mi menuItemComponent = Tbx_CfrmSerializableEventListener.toMenuItemComponent("", label, "", placeholder, hiv);
...
//		onCLICK((e) -> Clients.evalJavaScript("window.location.replace('q.com:8080');"));
	...
	public Menupopup0 addInnerMenu(String label) {
		Menu menuPageMode = new Menu(label);
		appendChild(menuPageMode);
		Menupopup0 menuPageMode_INNER = new Menupopup0();
		menuPageMode.appendChild(menuPageMode_INNER);
		return menuPageMode_INNER;
	}
	...
	public void addMenuitem_DeleteNode_WithSec(String label, NodeDir nodeDir) {
		...
		EventListener listen = (SerializableEventListener) event -> {
			...
			String title = X.f("Delete item '%s'", nodeDir.nodeName());
...
//															ZKL.alert("Move %s ot %s", srcCom, dstCom);
		...
		addMenuitem_DeleteNode_WithSec("Delete Item", nodeItem);
	}

	public void addMenuitemClpAndRunViaBash(String formName) {
		if (Sec.isOwner()) {
			String label = SYMJ.CLIPBOARD + " Copy & Execute Bash Script(NI)";
