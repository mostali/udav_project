...
public class ThemeColorPicker extends Div0 {
	...
	protected void init() {
		...
		appendChild(new ThemeColorPickerContainer() {
			...
			public void onCLICK_COLOR(Event event, String parentName, String colorCode) {
				ThemeColorPicker.this.onCLICK_COLOR(event, parentName, colorCode);
			}
		});
	...
	public static abstract class ThemeColorPickerContainer extends Div0 {
		...
		protected void init() {
			...
			List<Field> colorsNames = RFL.fields(UColorTheme.class, String[].class);
			List<String[]> colors = RFL.fieldValuesSt(UColorTheme.class, String[].class, false);
			if (totalColors == null) {
