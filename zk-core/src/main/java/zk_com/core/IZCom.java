...
public interface IZCom<T extends XulElement> extends IZStyle, IZState, IRfl {
	...
	default void applyState_BgColor(FormState... comState) {
		NFBe.apply_BgColor(comH(), comState.length == 1 ? comState[0] : getComStateDefault());
	}

//	default void positionPersistenceForm(String name) {
//		absolute();
//		FormState.apply_or_RANDOM_TOP_LEFT(comH(), getFormState_PROPS(name, false));
//	}
//
//	default void draggablePersistenceForm(String name, boolean enableDndEvent) {
//		absolute();
//		if (enableDndEvent) {
...
//	}
	...
	static <T> T findFirstInPage(Class<T> clazz, boolean recursive_or_first, T... defRq) {
		return ZKCFinderExt.findFirst_inPage0(clazz, recursive_or_first, defRq);
	}

	static <T> T findFirstInWin(Class<T> clazz, boolean recursive_or_first, T... defRq) {
		return ZKCFinderExt.findFirst_inWin0(clazz, recursive_or_first, defRq);
	}
...
//		applyState_RandomOrTopLeft();
		NFBe.applyState_RandomOrTopLeft_ForNode(this);
		absolute();
...
//	default void appendChildStyle(String tagContent, Object... args) {
//		com().appendChild(Xml.ofStyle(tagContent, args));
//	}
//
//	;
