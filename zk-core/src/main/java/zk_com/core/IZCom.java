...
public interface IZCom<T extends XulElement> extends IZStyle, IZState, IRfl {
	...
	default <T> T findSingleParent(Class<T> clazz, T... defRq) {
		Component singleParent = ZKCF.findSingleParent(com(), (Class) clazz, null);
	...
	default <T> List<T> findAll(Class<T> clazz, boolean recirsive_or_first, List... defRq) {
		return ZKCF.findAllFromParent(com(), (Class) clazz, recirsive_or_first, defRq);
	}
	...
	static <T> T removeMeFirst(Class<T> clazz, boolean recursive_or_first, T... defRq) {
		...
		if (first != null) {
			ZKC.removeMeReturnParent((Component) first);
			return first;
	...
	static <T> T findFirstInPage(Class<T> clazz, boolean recursive_or_first, T... defRq) {
		return ZKCF.findFirstIn_Page(clazz, recursive_or_first, defRq);
	}

	static <T> T findFirstInWin(Class<T> clazz, boolean recursive_or_first, T... defRq) {
		return ZKCF.findFirstIn_Win(clazz, recursive_or_first, defRq);
	}
...
//		parent.appendChild(it);
		...
		if (this instanceof IHeadCom) {
			ZkPage.renderHeadRsrc_Form((PageCtrl) getFirstWindow().getPage(), (IHeadCom) this);
		}
		return it;
	}
...
//		return true;
	...
	default Object attr_put(String key, Object newValue) {
		return UMap.update(com().getAttributes(), key, newValue, UMap.PutGetRemove.PUT);
	}

	default Object attr_rm(String key) {
		return UMap.update(com().getAttributes(), key, null, UMap.PutGetRemove.REMOVE);
	}

	default Object attr(String key, Object... defRq) {
		return UMap.get(com().getAttributes(), key, defRq);
	}

	default <T> T attr_as(String key, Class<T> asType, T... defRq) {
		return UMap.getAs(com().getAttributes(), key, asType, defRq);
	}
	...
	default IZCom placeholder(String placeholder) {
		InputElement inputElement = IT.isType0(this, InputElement.class);
	...
	default Menupopup0 getOrCreateMenupopup(HtmlBasedComponent holder, String eventForOpenMenu_orNullIfRightClick) {
		...
		if (X.notEmpty(popup)) {
			...
			String uuid = STR.substrStartEnd(comX().getContext(), 5, 1);
	...
	default IZCom randomAbs(String... bgColors) {
