...
public class Listbox0 extends Listbox {
	...
	public static Listbox0 fromSqliteDb(Path treeDb, int limit, String... tablename) {
		...
		Listbox0 modalCom = Listbox0.fromListList_AT(lists, treeDb);
		return modalCom;
	}

	@SneakyThrows
	public static Listbox0 fromXlsx(Path fileXlsx) {
//		String tablename0 = ARG.toDefOrNull(tablename);
//		ICtxDb tree = tablename0 != null ? ICtxDb.of(fileXlsx, tablename0) : ICtxDb.of(fileXlsx);
//		List<List<AbsType>> maps = tree.getModelsAsMapRow(tablename0);
//		XlsCallMsg.of_TREAD(fileXlsx);
		XlsCallMsg xlsCallMsg = XlsCallMsg.of(XlsCallMsg.Method.TREAD_AT, fileXlsx);
		List<List<AbsType>> maps = (List<List<AbsType>>) ZBin.POI.invokeMsg(xlsCallMsg);
		Listbox0 modalCom = Listbox0.fromListList_ATXY(maps, fileXlsx);
		return modalCom;
	}
	...
	public static Listbox0 fromCtxDb(Path treeDb, String... tablename) {
		...
		List<List<AbsType>> maps = tree.getModelsAsMapRow(tablename0);
		Listbox0 modalCom = Listbox0.fromListList_AT(maps, treeDb);
		return modalCom;
	}
...
//		}
	...
	public static Listbox0 fromListList_ATXY(List<List<AbsType>> rows, Path treeDb) {
		IT.notEmpty(rows, "set data");

		Listbox0 listbox = new Listbox0();

		enablePaging(listbox, rows.size(), AppZosProps.DEF_TREE_LIMIT.getValueOrDefault());
...
//			if (firstCell != null) {
//			listitem.setAttribute(CN.I, cell.name());
//			}
			int ri = 0;
			for (AbsType vl : row) {
	...
	public static Listbox0 fromListList_AT(List<List<AbsType>> rows, Path treeDb) {
		IT.notEmpty(rows, "set data");
...
//			//listbox.setMold("default");
	...
	public static SortListcell newListCell(AbsType vl, Path treeDb, Integer[] xyIndex) {
		...
		listCell.appendChild(new CellDbEditableValue(vl, treeDb, xyIndex));
		return listCell;
	}
