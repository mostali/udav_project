...
//		return this;
//	}
	public Caption getCaptionOrCreate(boolean... createIfNotExist) {
		return getCaptionOrCreate(this, createIfNotExist);
	}

	public static Caption getCaptionOrCreate(Window com, boolean... createIfNotExist) {
		if (com.getCaption() != null) {
			return com.getCaption();
		}
		if (ARG.isDefEqTrue(createIfNotExist)) {
			Caption child = new Caption();
	...
	public Win0 caption(Object captionTitleOrCom) {
		if (captionTitleOrCom instanceof CharSequence) {
			getCaptionOrCreate(true).setLabel(captionTitleOrCom.toString());
