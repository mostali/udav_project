...
public class CWindowComposer extends GenericForwardComposer {
	...
	public static Window show(String title, Component child, Component... parent) {
		...
		Window com = ZKCF.findSibling(component, Window.class, true).get(0);
		com.doModal();
		return com;
	}
...
//		Window pn = secondModalWindow;
		...
		if (title != null) {
			Win0.getCaptionOrCreate(comWin, true).appendChild(new Label(title));
		}
...
//		isCloasable=true;
		...
		if (_captionCom != null) {
			...
			Win0.getCaptionOrCreate(comWin, true).appendChild(_captionCom);
