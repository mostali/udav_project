...
//		}
	...
	protected void init() {
		...
		if (heightForm != null && heightForm.endsWith("px")) {
			String h = TKN0.firstNum(heightForm, null);
			if (h != null) {
...
//		setAttribute(FileState.NOTE_SIZE, i);

		for (int i = 0; i < paths0.size(); i++) {

			Path path = paths0.get(i);

			TbxmChild tbxm = (new TbxmChild(path, i));

//			TbxmChild tbxm;
//			if (i == 0) {
//				fn = Tbxmy.this.getFormName();
//			} else {
//				String finalFn = fn;
//				tbxm = new TbxmChild(path, i) {
//					@Override
