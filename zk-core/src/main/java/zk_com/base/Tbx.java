...
import zk_notes.node_state.proxy.ProxyRW;
import zk_os.AppZos;
...
public class Tbx extends Textbox implements IZCom {
	...
	protected void init() {
		...
		IT.isNull(submitEvent, "wth");

		if (submitEvent == null || saveOnShortCut) {
			ZKE.SKeys.CTRL_S.add(this, submitEvent = new SubmitEvent());
		}
	...
	public void save() {
		...
		if (doWrite) {
			...
			if (securityAllow) {
				...
				ProxyRW proxyRW = ProxyRW.of(this);

				proxyRW.writeContent(value);
			}
		}
		...
		if (L.isDebugEnabled()) {
			...
			msg += X.f(". Path '%s', length '%s'", ProxyRW.of(this).getTargetPath_FILE(), X.sizeOf(value));
			L.debug(msg);
		}
	}

	private Tbx getThisObjId() {
		if (this instanceof TbxmChild) {
			Component first = ARRi.first(getParent().getChildren(), null);
			if (first != null) {
				return (Tbx) first;
			}
		}
