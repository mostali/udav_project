...
public class EditableValue extends Span implements IZStyle {
	...
	public EditableValue isLabelVew(boolean isLabelVew) {
		...
		swapVisible();
		onChangeStateToEditableOrLabel(!isLabelVew);
		return this;
	}
	...
	public EditableValue(String value, Class<? extends Component> classCom) {
		...
		editCom.addEventListener(Events.ON_CANCEL, new org.zkoss.zk.ui.event.SerializableEventListener() {
			@Override
			public void onEvent(Event arg0) throws Exception {
				onChangeStateToEditableOrLabel(false, true);
			}
		});
		viewCom.addEventListener(Events.ON_DOUBLE_CLICK, new org.zkoss.zk.ui.event.SerializableEventListener() {
			@Override
			public void onEvent(Event arg0) throws Exception {
		...
		editCom.addEventListener(Events.ON_OK, new org.zkoss.zk.ui.event.SerializableEventListener() {
			@Override
			public void onEvent(Event arg0) throws Exception {
				onChangeStateToEditableOrLabel(false);
			}
...
//		});
		...
		{
			ZKE.addEventListenerCtrl(editCom, Events.ON_CTRL_KEY, ZKE.SHORTCUT_STORE_ALT_V, new SerializableEventListener() {
				@Override
				public void onEvent(Event event) throws Exception {
			...
			//		editCom.addEventListener(Events.ON_CTRL_KEY, new org.zkoss.zk.ui.event.EventListener() {
			//			@Override
			//			public void onEvent(Event arg0) throws Exception {
			//
			//			}
...
//			ZK.showLog("no save (for save use " + IZkCom.SHORTCUT_STOR_ALT_V + " )");
			...
			if (editOrLabel) {
				...
				editCom.setWidth(ZKS.AutoDims.getAutoWidth_50_100_200_300_400(valueFromView, 1));
			} else {
