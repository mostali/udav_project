...
import static mpc.str.Hu.MB1;
import static mpc.str.Hu.MS;
import static mpc.core.U.L;
import static mpc.num.UDbl.pct2dbl1;
import static mpc.num.UDbl.scale2;
...
//		typ_ctr_ms_time_mem
	...
	public static void main(String[] args) {
		U.RUN_TIMER(0, 1000, new TimerTask() {
			@Override
			public void run() {
				Sys.p(print(0));
			}
...
//			return "G" + typ_ctr_ms_time_mem[0];
	...
	public static void RUN(int period) {
		U.RUN_TIMER(0, period, new TimerTask() {
			@Override
			public void run() {
				Sys.p(print(0));
			}
	...
	public enum YoGc {
		...
		public static YoGc of(GarbageCollectorMXBean bean, YoGc... defRq) {
			if (STR.contains(bean.getName(), "Young", false)) {
				return YGC;
			} else if (STR.contains(bean.getName(), "Old", false)) {
				return OGC;
