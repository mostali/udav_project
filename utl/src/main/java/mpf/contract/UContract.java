...
public class UContract {
	...
	public static <T> T getRequiredContractValue_LAST_DEFRQ(Class<T> returnType, Object[] methodArgs) {
		...
		Object last = ArrItem.last(methodArgs);
		if (last == null) {
		...
		if (class0.isArray() && class0.getComponentType() == returnType) {
			...
			Object arg = ArrItem.first(lastArr);
	...
	public static <T> Pare<Boolean, T> getRequiredContractValue_DefRq(Method method, Object[] methodArgs) {
		...
		Class lastParamType = ArrItem.last(params);
		if (!lastParamType.isArray()) {
			return null;
		}
		ERR.state(returnType.isAssignableFrom(lastParamType.getComponentType()), "Method [%s] has last PARAM type [%s], but except type assignable from defRq type as [%s]", method.getName(), returnType, lastParamType.getClass().getComponentType());
		Object lastArg = ArrItem.last(methodArgs);
		if (lastArg == null) {
			return (Pare<Boolean, T>) Pare.of(false, null);
		}
		ERR.state(lastArg.getClass().isArray(), "last method params except array type");
		ERR.state(lastArg.getClass().getComponentType().isAssignableFrom(lastParamType.getComponentType()), "Method [%s] has last ARG type [%s], but except type assignable from defRq type as [%s]", returnType, lastParamType.getClass().getComponentType());
	...
	public static Object getObjectValue_DefRq(Map<String, Object> data, Method method, Object[] methodArgs) {
		String methodName = method.getName();

		String key = getKeyNameFromGetMethodName(methodName);
...
//			return val;
	...
	public static Object getObjectValue_MarkNotRq(Map<String, Object> data, Method method, Object[] methodArgs) {
		String methodName = method.getName();
		Object val;
	...
	public static String getKeyNameFromGetMethodName(String methodName) {
