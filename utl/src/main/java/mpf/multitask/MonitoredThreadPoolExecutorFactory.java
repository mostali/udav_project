...
public class MonitoredThreadPoolExecutorFactory {
	...
	private static ExecutorService createFJP() {
		...
		try {
			Class<?> fjpClazz = Class.forName("com.otr.util.concurrent.MonitoredForkJoinPool");
			ExecutorService instance = (ExecutorService) fjpClazz.newInstance();
			ManagementFactory.getPlatformMBeanServer().registerMBean(instance, new ObjectName("sufd:type=ForkJoinPool"));
			return instance;
		} catch (Exception var2) {
			log.error("Ошибка инициализации ForkJoinPool-а - он будет недоступен", var2);
			return null;
		} catch (NoClassDefFoundError var3) {
			log.error("Ошибка инициализации ForkJoinPool-а - он будет недоступен", var3);
			return null;
		}
	}
	...
	static {
		...
		if ("true".equals(System.getProperty("disableFJP", "false"))) {
			FJP = null;
		} else {
			FJP = createFJP();
		}
