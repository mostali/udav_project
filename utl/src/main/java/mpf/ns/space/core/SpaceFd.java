...
public abstract class SpaceFd implements IPath, ISs {
	...
	public SpaceHomeMap homeMap(SpaceHomeMap... defRq) {
		try {
			return _spaceHomeMap = srcType().homeMap(fPath());
		} catch (Exception ex) {
			return ARG.toDefThrow(() -> new RequiredRuntimeException(ex, "SpaceSrcMap invalid from path '%s'", fPath()), defRq);
		}
	}

	public Ns ns() {
		return Ns.ofUnsafe(fPath());
	}
