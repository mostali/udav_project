...
//		Sys.exec_UnsafeSpace("ssh HOST tail -100 /opt/tomcat-9/logs/catalina.out");
	 ...
	 * *************************************************************
	 */
	public static void p(Throwable err) {
		if (err != null) {
	...
	public static Timer RUN_TIMER(int beforeDelayMs, int periodMs, TimerTask runnable, boolean isDaemon) {
		Timer timer = new Timer("LifePrinter-Daemon", isDaemon);
	...
	public enum PKS {
		...
		public static Pare<PKS, String> getValueFirst(String key, Pare<PKS, String>... defRq) {
			...
			return ARG.throwErrOpt(() -> new RequiredRuntimeException("Except pks key '%s'", key), opt, defRq);
		}

		public <T> T getValueAs(String key, Class<T> asType, T... defRq) {
			String value = getValue(key, null);
			return value != null ? UST.strTo(value, asType, defRq) : ARG.throwMsg(() -> X.f("Except not null value by key '%s'", key), defRq);
		}
		...
		public String getValue(String key, String... defRq) {
			switch (this) {
				...
				case BOOT_CONTEXT:
					...
					return bootContext != null ? bootContext.get(key, defRq) : ARG.throwMsg(() -> X.f("Set boot context"), defRq);
				default:
					throw new WhatIsTypeException(this);
			}
		}
	...
	public static String getSysProp(String syskey, String... defRq) {
		...
		return s != null ? s : ARG.throwMsg(() -> X.f("Except value by sysKey '%s'", syskey), defRq);
	}

	public static String getEnvProp(String envkey, String... defRq) {
		String s = System.getenv(envkey);
		return s != null ? s : ARG.throwMsg(() -> X.f("Except value by envKey '%s'", envkey), defRq);
	}
...
//	}
	...
	public static String getEnvSysWoPfxFirst(String pfx, String key, String... defRq) {
		...
		return ARG.throwErrOpt(() -> new RequiredRuntimeException(X.f("Except env&sys prop '%s'", key)), first, defRq);
	}
