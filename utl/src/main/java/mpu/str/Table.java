...
public class Table {
	public static final Logger L = LoggerFactory.getLogger(Table.class);

	public static void main(String[] args) {
		Table tl = new Table(3, "ID", "String 1", "String 2").sortBy(0).withUnicode(true);
// from a list
...
// or manually
	...
	public static Table of(String... cols) {
		return new Table(cols);
	}

	public static Table toStringFromMap(Map map, boolean print) {
//		TableList tableList = new TableList(2, "Key", "Value");
		Map.Entry first = ARRi.first(map);
		Table tableList = new Table(2, first.getKey().toString(), first.getValue().toString());
	...
	public static String toStringFromListMap(List<Map<String, AbsType>> rows, String returnIfEmpty) {
		...
		Table printTable = Table.of(cols.toArray(new String[0]));
	...
	public Table align(EnumAlignment align) {
		for (int i = 0; i < aligns.length; i++) {
			align(i, align);
		}
		return this;
	...
	public Table addRowAs(Object... elements) {
