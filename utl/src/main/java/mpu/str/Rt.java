...
public class Rt {
	...
	public static final Logger L = LoggerFactory.getLogger(Rt.class);//print in rserver.log

	public static void main(String[] args) {
		X.p(buildReportForArray(ARR.of(1, 2)));
		X.p(buildReport(ARR.as(1, 2)));
//			Sys.p(buildReport((Collection) null, "asd", 0));
//			Sys.p("ok");
	}

	public static Sb buildReportForArray(Object array, Logger... logger) {
	...
	public static Sb buildReport(Collection collection, String[] headFoot, int tabLevel, Logger... logger) {
		...
		if (size > 0) {
			...
			sb.append(TAB).append(collection.getClass().getSimpleName()).append("*").append(X.sizeOf(collection)).NL();
		}
		for (Object o : collection) {
			sb.append(TAB2).append(o).NL();
		}
		if (size > 1) {
	...
	public static String toStringLine(Sb buildReport) {
		return STR.no(buildReport.toString(), true, true, true, STR.COL_DEL);
	}
