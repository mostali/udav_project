...
public class PrintTable {
	public static final Logger L = LoggerFactory.getLogger(PrintTable.class);

	public static void main(String[] args) {
		PrintTable tl = new PrintTable(3, "ID", "String 1", "String 2").sortBy(0).withUnicode(true);
// from a list
...
// or manually
	...
	public PrintTable(String... descriptions) {
		this(descriptions.length, descriptions);
	}

	public PrintTable(int columns, String... descriptions) {
	...
	public static PrintTable of(String... cols) {
		return new PrintTable(cols);
	}

	public static PrintTable toStringFromMap(Map map, boolean print) {
//		TableList tableList = new TableList(2, "Key", "Value");
		Map.Entry first = ARRi.first(map);
		PrintTable tableList = new PrintTable(2, first.getKey().toString(), first.getValue().toString());
	...
	public static String toStringFromListMap(List<Map<String, AbsType>> rows, String returnIfEmpty) {
		...
		PrintTable printTable = PrintTable.of(cols.toArray(new String[0]));
		for (Map<String, AbsType> row : rows) {
			printTable.addRowAs(row.values().stream().map(AbsType::getValue).toArray());
		}
		return printTable.getStringBuilder(true).toString();
	}
	...
	public PrintTable compareWith(Comparator<String[]> c) {
		this.comparator = c;
		return this;
	}

	public PrintTable sortBy(int column) {
		return this.compareWith(Comparator.comparing(o -> o[column]));
	}

	public PrintTable align(EnumAlignment align) {
		for (int i = 0; i < aligns.length; i++) {
			align(i, align);
		}
		return this;
	...
	public PrintTable align(int column, EnumAlignment align) {
		aligns[column] = align;
		return this;
	}

	public PrintTable withSpacing(int spacing) {
	...
	public PrintTable addRowAs(Object... elements) {
		return addRow(Arrays.stream(elements).map(String::valueOf).toArray(String[]::new));
	}

	public PrintTable addRow(String... elements) {
	...
	public PrintTable filterBy(int par0, String pattern) {
		this.findex = par0;
		this.filter = pattern;
		return this;
	}

	public PrintTable withUnicode(boolean ucodeEnabled) {
		this.ucode = ucodeEnabled;
		return this;
	}

	public void printConsole(boolean... woUnicode) {
		X.p(getStringBuilder(woUnicode));
	}

	public Sb getStringBuilder(boolean... woUnicode) {
