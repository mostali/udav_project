...
public class TKN0 {
	...
	static <T> T toType(boolean firstLast, String str, Object splitDelStr, Function<String, T> converter, String first, T... defRq) {
		return toType(firstLast, str, splitDelStr, -1, converter, first, defRq);
	}

	static <T> T toType(boolean firstLast, String str, Object splitDelStr, int ind, Function<String, T> converter, String first, T... defRq) {
		Exception err = null;
		if (first != null) {
			try {
				T apply = converter.apply(first);
				if (apply != null) {
					return apply;
				}
			} catch (Exception ex) {
		...
		flMark += ind == -1 ? "" : "*" + ind;
		String msg = X.f("Wrong convert '%s' from %s/'%s' is %s, string '%s'", converter, flMark, splitDelStr, err == null ? "NULL" : "ERROR", str);
