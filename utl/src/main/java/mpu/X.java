...
//		List<Integer> integers = ARR.as(new Integer[]{1, 2, 3});
	...
	public static boolean nullAny(Object... args) {
	...
	public static boolean emptyLL(List<List> ll) {
		return empty(ll) || ll.stream().noneMatch(X::notEmpty);
	}
...
//	public static boolean isLength(List l,int index) {
//		return notEmpty(str);
//	}
	...
	public static <N extends Number> boolean isNegOrZero(N number) {
		return number != null && number.doubleValue() <= 0;
	}

	public static <N extends Number> boolean isNegNotZero(N number) {
		return number != null && number.doubleValue() < 0;
	}

	public static <N extends Number> boolean isPosNotZero(N number) {
		return number != null && number.doubleValue() > 0;
	}

	public static <N extends Number> boolean isPosOrZero(N number) {
		return number != null && number.doubleValue() >= 0;
	}
	...
	public static long sizeOfLines(Path path, Long... defRq) {
		return Files.isRegularFile(path) ? UFS.getSizeLinesOfFile(path) : ARG.throwMsg(() -> X.f("Error read file size '%s' size", path), defRq);
	}

	@SneakyThrows
	public static Long sizeOf(Path path, Long... defRq) {
		return path != null && Files.isRegularFile(path) ? Files.size(path) : ARG.throwMsg(() -> X.f("Error read file length '%s' size", path), defRq);
	}
...
//		System.out.println(t);
	...
	public static String toStringSE(Object obj, int len) {
		String str = obj + "";
		return STR.toStringSE(str, len, str);
	}
	...
	public static <T> T toObjStr(CharSequence obj, Class<T> asType, T... defRq) {
	...
	public static String toString0(InputStream inputStream, String... defRq) {
		try {
			...
			return ARG.throwErr(new RequiredRuntimeException(e, "Error read InputStream. Cause:%s"), defRq);
		}
	}
...
//	}
	...
	public static Boolean toObjOrBool(Object obj, Boolean def) {
	...
	public static <T> T toObj(Object obj, Class<T> asType, T... defRq) {
		return ObjTo.objTo(obj, asType, defRq);
	}

	public static <T> T toObjArgs(Object[] args, int index, Class<T> asType, T... defRq) {
		return ARGS.argsAsType(args, index, asType, defRq);
	}
	...
	public static <T> T throwIllegalStateException(String msg, Object... args) {
		_throwException(new FIllegalStateException(msg, args));
		return null;
	}

	public static <T> T throwRequiredException(String msg, Object... args) {
		_throwException(new RequiredRuntimeException(msg, args));
		return null;
	}
	...
	public static boolean equals(CharSequence obj1, CharSequence obj2, boolean ic) {
		return ic ? equalsIgnoreCase(obj1, obj2) : equalsIgnoreCase(obj1, obj2);
	}
	...
	public static boolean emptyAllObjStr(List<Object> objects) {
		return objects.stream().allMatch(X::emptyObj_Str);
	}
}
