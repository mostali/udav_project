...
//		X.exit(qDate.diff_WRONG_MINUS(now) + "|" + qDate.diff(now));
	...
	public static boolean equals(QDate d1, QDate d2, int precission_calendar) {
		if (d1 == null || d2 == null) {
			return false;
		}
		return d1.equals(d2, precission_calendar);
	}
...
//		return mono6_y2d2() + mono6_h2m2s2();
//	}

	/**
	 * 005959
	 */
	public String mono6_h2m2s2() {
...
//	}
	...
	public String mono14_y4s2() {
		return f(QDate.F.YYYYMMDD) + mono6_h2m2s2();
	}
...
//    public boolean equalsAll(QDate date, Integer... calendar_period) {
//        IT.notEmpty(calendar_period);
//        return Arrays.stream(calendar_period).noneMatch(cal -> !equals(date, cal));
//    }

	public boolean equals(QDate date, int precission_calendar) {
		switch (precission_calendar) {
			case Calendar.YEAR:
			...
			default:
				throw new WhatIsTypeException(precission_calendar);
		}
	}
	...
	public QDate[] border(int dayOfMonth, ZoneId... zoneId) {
		if (true) {
			switch (dayOfMonth) {
				...
				case Calendar.MONTH:
				case Calendar.YEAR:
					return QDateBetween.rangeBetween(this, dayOfMonth, ARG.toDefOrNull(zoneId), QDate.class);
				default:
		...
		switch (dayOfMonth) {
			case Calendar.DAY_OF_MONTH:
				return QDateBetween.rangeBetween(this, dayOfMonth, ARG.toDefOrNull(zoneId), QDate.class);
			case Calendar.DAY_OF_WEEK:
				return QDateBetween.rangeBetween_WeekAsQDate(this);
			case Calendar.MONTH:
				return QDateBetween.rangeBetween_MonthAsQDate(this);
			case Calendar.YEAR:
				return QDateBetween.rangeBetween_YearAsQDate(this);
			default:
				throw new WhatIsTypeException(dayOfMonth);
		}
	}
	...
	public enum F {
		...
		MONO15NF_ICO("⏱%s%s%s|%s:%s", "⏱yyyyMMdd|HH:mm"),
		MONO17NF_ICO("⏱%s-%s-%s %s:%s", "⏱yyyy-MM-dd HH:mm"),
		...
		MONO20NF_ICO("⏱%s-%s-%s %s:%s:%s", "⏱yyyy-MM-dd HH:mm:ss"),
	...
	public QDate add(int calendarUnit, int count) {
		...
		Date d = add(toDate(), calendarUnit, count);
		return of(d);
	}

	public static Date add(Date date, int calendarUnit, int count) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		cal.add(calendarUnit, count); // number of days to add
	...
	public static QDate of(Long ms, QDate... defRq) {
		return ms != null ? of((long) ms) : ARG.throwMsg(() -> X.f("Date value is illegal '%s'", ms), defRq);
	}
	...
	public static QDate of(String date, String[] format, QDate... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Parse string '%s' with format's '%s'", date, ARR.of(format)), defRq);
	}
...
//			return of(new SimpleDateFormat(format).parse(date));
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Parse string '%s' with format '%s'", date, format), defRq);
		}
	}
