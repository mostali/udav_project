...
//	}
	...
	public long diff_WRONG_MINUS() {
		return diff_WRONG_MINUS(QDate.now());
	}

	public long diff_WRONG_MINUS(QDate with) {
		return with.ms() - ms();
	}

	public Number diff_WRONG_MINUS(TimeUnit timeUnit) {
		return diff_WRONG_MINUS(now(), timeUnit);
	}

	public BigDecimal diff_WRONG_MINUS(QDate with, TimeUnit timeUnit, Integer... scale) {
		long nano = diff_WRONG_MINUS(with, TimeUnit.NANOSECONDS).longValue();
		BigDecimal hourStr = UTime.toTime(nano, TimeUnit.NANOSECONDS, timeUnit, scale);
		return hourStr;
	}

	public Number diff_WRONG_MINUS(QDate with, TimeUnit timeUnit) {
		return QDate.toTime(timeUnit, with.ms() - ms());
	}
