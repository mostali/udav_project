...
public class RW {
	...
	public static byte[] readBytes(Path path, byte[]... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "read bytes from file '%s'", path), defRq);
		}
	}
	...
	public static List<String> readLinesFast(Path file, int countStartOrEnd, boolean... reverseResult) {
		return readLines(file, countStartOrEnd, reverseResult);
	}

	@SneakyThrows
	public static List<String> readLines(Path file, int countStartOrEnd, boolean... reverseResult) {
	...
	public static Path write_(Path file, byte[] bytes, OpenOption... openOption) throws IOException {
		Path file1 = Files.write(file, bytes, openOption);
		if (L.isDebugEnabled()) {
			L.debug("Write file {} with bytes*{}", UF.ln(file), bytes.length);
		}
		return file1;
	}
	...
	@SneakyThrows
	public static List<String> readLinesCleanTrim(Path publistPath, List<String>... defRq) {
		try {
			return Files.readAllLines(publistPath).stream().map(String::trim).filter(X::NE).collect(Collectors.toList());
		} catch (Exception ex) {
			return ARG.throwErr(ex, defRq);
		}
	}
