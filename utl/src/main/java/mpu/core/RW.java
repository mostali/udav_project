...
public class RW {
	...
	public static List<String> readLines(Path file, int count) {
	...
	public static String readContent(Path file, String... defRq) {
	 ...
	 * *************************************************************
	 */
	public static void write_Append_(Path file, Path appendFileContent, boolean... createIfNotExists) throws IOException {
		if (ARG.isDefEqTrue(createIfNotExists)) {
			UFS_BASE.MKFILE.createFileIfNotExist(file, true);
		}
		write_(file, FileUtils.readFileToByteArray(appendFileContent.toFile()), StandardOpenOption.APPEND);
	}

	public static void write_Append_(Path file, CharSequence content) throws IOException {
		write_(file, content, (Charset) null, StandardOpenOption.APPEND);
	}

	public static Path write_AppendOrCreateNew_Mkdirs(Path file, CharSequence seq) throws IOException {
		UFS_BASE.MKDIR.createDirsOrSingleDirOrCheckExist(file.getParent(), true);
	...
	public static Path write_AppendOrCreateNew_(Path file, CharSequence seq) throws IOException {
		if (Files.exists(file)) {
			return write_(file, seq, StandardOpenOption.APPEND);
		} else {
			return write_(file, seq, StandardOpenOption.CREATE_NEW);
	...
	@SneakyThrows
	public static void writePrettyJson(Path file, CharSequence content, OpenOption... openOptions) {
		write(file, UGson.toStringPretty(content.toString()), openOptions);
	}

	public static void writeViaTmp(Path file, CharSequence content) {
		new TmpFileOperation() {
			@Override
			public void doOperationImpl(Path tmpFile) {
				write(tmpFile, content);
				UFS_BASE.MV.move(tmpFile, file, null);
			}
		}.doOperation();
	}

	@SneakyThrows
	public static void write(Path file, CharSequence content, OpenOption... openOptions) {
		write_(file, content, (Charset) null, openOptions);
	}

	@SneakyThrows
	public static void write(Path file, CharSequence content, Boolean mkdirs_mkdir_orNot, OpenOption... openOption_orNull) {
		write_(file, content, mkdirs_mkdir_orNot, openOption_orNull);
	}


	public static void write_(Path file, CharSequence content, Boolean mkdirs_mkdir_orNot, OpenOption... openOptions) throws IOException {
		if (mkdirs_mkdir_orNot != null && file.getParent() != null) {
			UFS_BASE.MKDIR.mkdirsIfNotExistImpl(file.getParent(), mkdirs_mkdir_orNot);
		}
	...
	@SneakyThrows
	public static List<String> readLines(Path reader) {
		return IOUtils.readLines(new FileReader(reader.toFile()));
	}
	...
	public static class Serializable2File {
		...
		public static void serialize(File file, Serializable serializable, boolean... createParentDirIfNotExist) throws IOException {
			if (ARG.isDefEqTrue(createParentDirIfNotExist)) {
				UFS_BASE.MKDIR.mkdirsIfNotExistForParent(file.getAbsolutePath());
			}
