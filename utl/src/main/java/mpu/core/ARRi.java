...
public class ARRi {
	...
	public static <T> T firstAs(Collection collection, Class<T> as, T... defRq) {
		...
		return ARG.throwMsg(() -> "Item is null", defRq);
	}
	...
	public static <K, V> K firstKey(Map<K, V> collection, K... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("First val not found from Map (Entry is null)"), defRq);
	}
	...
	public static <K, V> Map.Entry<K, V> first(Map<K, V> collection, Map.Entry<K, V>... defRq) {
		return collection == null ? ARG.throwMsg(() -> X.f("Collection is null"), defRq) : first(collection.entrySet(), defRq);
	}
	...
	public static String firstLine(Path file, String... defRq) {
		return first(RW.readLinesFast(file, 1), defRq);
	}
	...
	public static Character first(String str, Character... defRq) {
		return str.length() > 0 ? str.charAt(str.length() - 1) : ARG.throwErr(() -> new RequiredRuntimeException("Length 0", defRq));
	}
	...
	public static <T> List<T> firstMany(Iterator<T> iterator, int count, List<T>... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Item's size '%s' not equals required count '%s'", items.size(), count), defRq);
	}
	...
	public static <T> List<T> lastMany(Iterable<T> iterable, int count, List<T>... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Except string with size more that '%s', but need '%s' elements", X.sizeOf(list), count), defRq);
	}
	...
	public static String lastMany(String str, int count, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Except collection with size more that '%s', but need '%s' elements", X.sizeOf(str), count), defRq);
	}
	...
	public static <T> T itemAs(Object[] args, int index, Class<T> as, T... defRq) {
		Object item = item(args, index, null);
		if (item != null) {
			return ObjTo.objTo(item, as, defRq);
		}
		return ARG.throwMsg(() -> "Item is null", defRq);
	...
	public static <T> T item(Iterable<T> list, int index, T... defRq) {
		if (list != null) {
			if (list instanceof List) {
				...
				if (index >= 0 && index < l.size()) {
					boolean isFromStartOrFromEnd = index < l.size() / 2;
					if (list instanceof RandomAccess) {
						// ArrayList и другие с быстрым доступом
						return l.get(index);
					} else {
						// LinkedList и другие последовательные списки
						// Оптимизация: идти с ближайшего конца
						if (isFromStartOrFromEnd) {
							// Ближе к началу
		...
		return ARG.throwMsg(() -> X.f("Item by index [%s] not found", index), defRq);
	}

	public static <T> T item(T[] args, int index, T... defRq) {
		return index >= 0 && args != null && index < args.length ? args[index] : ARG.throwMsg(() -> X.f("Try get item by index '%s' from array with size '%s'", index, args.length), defRq);
	}
	...
	public static <T> Pare<Integer, T> next(Collection<T> items, Integer current, T... defRq) {
		...
		return Pare.of(current, ARG.throwErr(err, defRq));
	}
	...
	public static Integer nextIndex(Integer current, Integer size, Integer... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Next index unrecognized, current(%s), size (%s)", finalCurrent, size), defRq);
	}
	...
	public static <T> List<T> getFirstLast(List<T> els) {
		switch (els.size()) {
			case 0:
				return ARR.EMPTY_LIST;
			case 1:
				return ARR.as(els.get(0));
			default:
				return ARR.as(els.get(0), els.get(els.size() - 1));
		}
	}
}
