...
//			P.p(AR.sublist((List) null, 0, 0));
	...
	public static List<List> mergeRows(List headerRow, List<List> rows) {
	...
	public static Object[] mergeAny(Object src, Object... add) {
		Object[] all = new Object[(src != null ? 1 : 0) + (add != null ? add.length : 0)];
		int i = 0;
		if (src != null) {
			all[i++] = src;
		}
		if (add != null) {
			System.arraycopy(add, 0, all, i, add.length);
		}
		return all;
	}
...
//	}
	...
	public static <T> Set<T> asTSET(T... items) {
		return new TreeSet<>() {
			{
				for (T t : items) {
					add(t);
	...
	public static boolean containsNeedleInAny(String needle, boolean ignoreCase, String... strings) {
		return X.notEmpty(needle) && X.notEmpty(strings) && Arrays.stream(strings).anyMatch(s -> ARR.contains(s, ignoreCase, needle));
	}
	...
	public static boolean contains(String str, boolean ignoreCase, String... needle) {
		return X.notEmpty(str) && X.notEmpty(needle) && Stream.of(needle).anyMatch(n -> (ignoreCase ? StringUtils.containsIgnoreCase(str, n) : str.contains(n)));
	}
...
//		}
	...
	public static <T> List<T>[] getDiffs(List<T> first, List<T> second) {
		if (first == null) {
			first = new ArrayList<>();
		}
		if (second == null) {
		...
		List<T> onlyInFirst = first.stream().filter(e -> !secondSet.contains(e)).collect(Collectors.toList());

		List<T> onlyInSecond = second.stream().filter(e -> !firstSet.contains(e)).collect(Collectors.toList());

		return new List[]{onlyInFirst, onlyInSecond};
	}

	public static List<Object> trimEmpty(List<Object> objects) {
		trimLeft(objects, X::notEmptyObj_Str);
		trimRight(objects, X::notEmptyObj_Str);
		return objects;
	}

	public static List<String> asListWithRange(int lastIndex) {
	...
	public static String[] toArgsString(Collection keys) {
