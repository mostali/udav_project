...
//			P.p(AR.sublist((List) null, 0, 0));
	...
	public static <T> boolean contains(Collection<T> arr, T item) {
		return arr == null ? false : arr.stream().anyMatch(i -> i == item || EQ.equalsUnsafe(i, item));
	}
	...
	public static List ofMap(Map tabs) {
		List l = new LinkedList();
		tabs.entrySet().forEach(e -> l.addAll(PareEntry.ofMapEntry(e).toList()));
		return l;
	}

	public static List<List> ofDouble(Object[][] headers) {
...
//		}
	...
	public static List<String> toListString(Collection items) {
