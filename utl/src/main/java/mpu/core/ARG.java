...
//		UC.isTrue(isPredicat((int) 1));
	...
	public static <T> T toDef(Supplier<T> defIfNot, T... predicat) {
	...
	public static <T> T throwMsg(String message, T... defRq) {
		if (ARG.isDef(defRq)) {
			return ARG.toDef(defRq);
		}
	...
	public static <T> T throwMsg(Throwable ex, T... predicat) {
		return ARG.isDef(predicat) ? toDef(predicat) : X.throwException(ex);
	}

	public static <T, X extends Throwable> T throwOpt(Supplier<? extends X> supplier, Optional<T> obj, T... defRq) throws X {
	...
	public static <T, Str extends CharSequence> T throwOptMsg(Supplier<? extends Str> supplier, Optional<T> obj, T... defRq) {
		if (obj != null && obj.isPresent()) {
			return obj.get();
		}
	...
	public static <T, Str extends CharSequence> T throwMsg0(Supplier<? extends Str> supplier, T... defRq) throws RequiredRuntimeException {
		return throwMsg(supplier, defRq);
	}
	...
	public static <T, X extends Throwable> T throwErr(String msg, T... defRq) throws X {
		return isDef(defRq) ? toDef(defRq) : XX.throwException(msg);
	}

	public static <T, X extends Throwable> T throwErr(Throwable msg, T... defRq) throws X {
		return isDef(defRq) ? toDef(defRq) : XX.throwException(msg);
	}
	//
	//
	...
	public static <T> void applyVoid(FunctionV1<T> func, T... mArg) {
		if (ARG.isDef(mArg)) {
	...
	public static <T> T throwNN(Object obj, String msg, T... defRq) {
		return obj != null ? (T) obj : throwMsg(msg, defRq);
	}
}
