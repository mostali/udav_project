...
//		UC.isTrue(isPredicat((int) 1));
	...
	public static <T> T toDefOr(T defIfNotDef, T... predicat) {
		return isDef(predicat) ? toDef(predicat) : defIfNotDef;
	}
	...
	public static <T> T throwRq(T... defRq) {
		return toDefRq(defRq);
	}
	...
	public static <T> T throwErrHead(String head, Throwable ex, T... predicat) {
		if (isDef(predicat)) {
			return toDef(predicat);
		} else if (head == null && ex == null) {
			throw new RequiredRuntimeException();
		} else if (ex == null) {
			throw new RequiredRuntimeException(head);
		} else if (head == null) {
			throw new RequiredRuntimeException(ex);
		}
		throw new RequiredRuntimeException(ex, head);
	}

	public static <T, X extends Throwable> T throwErrOpt(Supplier<? extends X> supplier, Optional<T> obj, T... defRq) throws X {
	...
	public static <T, Str extends CharSequence> T throwMsgOpt(Supplier<? extends Str> supplier, Optional<T> obj, T... defRq) {
		if (obj != null && obj.isPresent()) {
			return obj.get();
		}
		return isDef(defRq) ? toDef(defRq) : (T) XX.throwException(new RequiredRuntimeException(supplier.get()));
	}
	...
	public static <T, X extends Throwable> T throwMsg(String msg, T... defRq) throws X {
		return isDef(defRq) ? toDef(defRq) : XX.throwRequiredException(msg);
	}

	public static <T, X extends Throwable> T throwErr(Supplier<? extends X> supplier, T... defRq) throws X {
		return isDef(defRq) ? toDef(defRq) : XX.throwException(supplier.get());
	}

	public static <T, Str extends CharSequence> T throwMsg(Supplier<? extends Str> supplier, T... defRq) throws RequiredRuntimeException {
		return isDef(defRq) ? toDef(defRq) : XX.throwRequiredException(supplier.get().toString());
	}

	public static <T> T throwErr(Throwable ex, T... defRq) {
		return isDef(defRq) ? toDef(defRq) : X.throwException(ex);
	}

//	public static <T, X extends Throwable> T throwErr(Throwable err, T... defRq) throws X {
//		return isDef(defRq) ? toDef(defRq) : XX.throwException(err);
//	}
	//
	//
	...
	public static <T> T toObjNN(T object) {
	...
	public static <T, R> R applyNE(Function<T, R> func, T... mArg) {
		return ARG.isDef(mArg) ? func.apply(ARG.toDef(mArg)) : null;
	}

	public static <T, R> R applyOrNull(Function<T, R> func, T... mArg) {
		return ARG.isDef(mArg) ? func.apply(ARG.toDef(mArg)) : null;
	}

	public static <T> void applyVoidIfDef(FunctionV1<T> func, T... mArg) {
		if (ARG.isDef(mArg)) {
			func.apply(ARG.toDef(mArg));
		}
	}
	...
	public static String throwNE(String str, String msg, String... defRq) {
		return str != null && str.length() > 0 ? str : throwMsg(msg, defRq);
	}

	public static <T> T throwNN0(T obj, T... defRq) {
		return obj != null ? obj : toDefRq(defRq);
	}

	public static <T> T throwNN(T obj, String msg, T... defRq) {
		return obj != null ? obj : throwMsg(msg, defRq);
	}

	public static <T> T throwNN(T obj, Supplier<String> msg, T... defRq) {
		return obj != null ? obj : throwMsg(msg.get(), defRq);
	}
}
