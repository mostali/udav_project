...
public class Tuple<T> implements IGetterAsAny {
	...
	public String toString() {
		return getClass().getSimpleName() + "*" + X.sizeOf(obs) + Arrays.toString(obs);
	}

	public T get(int i, T... defRq) {
		return ARR.isIndex(i, obs) ? obs[i] : ARG.throwMsg(() -> X.f("Except obj by index %s, but length is %s", i, obs.length), defRq);
	}

	public Boolean eq(int index, Object obj, Boolean... defRq) {
		if (ARG.isDef(defRq)) {
			return ARR.isIndex(index, obs) ? EQ.equalsUnsafe(get(index), obj) : ARG.toDef(defRq);
		}
	...
	public boolean equals(Object o) {
		...
		Tuple<?> tuple = (Tuple<?>) o;
		return Objects.deepEquals(obs, tuple.obs);
	}

	@Override
	public int hashCode() {
		return Arrays.hashCode(obs);
	}
