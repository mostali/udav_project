...
//	}
	...
	public static Pare<QDate, Path> ofEntry(Map.Entry<QDate, Path> first) {
		return of(first.getKey(), first.getValue());
	}
...
//		return "Pare[" + getKey() + "]=[" + getVal() + "]";
	...
	public Pare<K, V> cloneKey(V val) {
		return Pare.of(key(), val);
	}
