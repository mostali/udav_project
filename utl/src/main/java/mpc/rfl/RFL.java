...
//	}
	...
	public static void test(String[] args) throws InvocationTargetException, IllegalAccessException, NoSuchFieldException, NoSuchMethodException {
		Test t = new Test();
	...
	public static Method method(Class clazz, String methodName, Class[] paramTypes_Or_MbAnyPAramas, Boolean isStaticMethod, boolean isFullSearch, boolean modifyAccessibleToTrue, Method... defRq) {
		try {
			return paramTypes_Or_MbAnyPAramas == null ?
					method_(clazz, methodName, isStaticMethod, true, modifyAccessibleToTrue) :
					method_(clazz, methodName, paramTypes_Or_MbAnyPAramas, isStaticMethod, true, modifyAccessibleToTrue);
		} catch (Exception ex) {
			return ARG.toDefThrow(ex, defRq);
		}
	}
	...
	private static Field fieldFind_(Class clazz, Class fieldType, List<Class> ano, String fieldName, StringConditionPattern fieldNameStringCondition, Boolean isStatic, boolean isFullSearch, boolean modifyAccessibleToTrue) throws NoSuchFieldException {
		List<Field> first = fieldsFind_(clazz, fieldType, ano, ERR.NN(fieldName), fieldNameStringCondition, isStatic, isFullSearch, true, modifyAccessibleToTrue);
		return first.get(0);
	}
	...
	private static Field fieldFind_(Class clazz, String fieldName, Boolean isStatic, boolean isFullSearch, boolean modifyAccessibleToTrue) throws NoSuchFieldException {
		List<Field> first = fieldsFind_(clazz, null, null, ERR.NN(fieldName), null, isStatic, isFullSearch, true, modifyAccessibleToTrue);
		return first.get(0);
	}
	...
	public static AbsFieldType fieldAbsType_(Class clazz, Object from, String fieldName, Boolean isStatic_orNot_orAny, boolean ifFullSearch, boolean modifyAccessibleToTrue) throws NoSuchFieldException {
		if (from == null) {
			ERR.state(clazz != null);
			ERR.state(UBool.isTrueSafe(isStatic_orNot_orAny));
		}
	...
	public static class FieldValue<T> {
		...
		public boolean isEquals(Field f, boolean... defRq) {
			try {
				...
				return ARGi.toDefOrThrow(ex, defRq);
			}
		}
	...
	public static <T> T fieldValueSet(Object obj, String fieldname, T value, boolean fullSearch, T... defRq) {
		try {
			...
			throw new RequiredRuntimeException(e, "SET Field-Value not found from Object-Class [%s] with Field-Name [%s], FullSearch[%s", cn(obj, null), fieldname, fullSearch);
		}
	}
	...
	public static Object fieldValue(Object obj, String fieldname, boolean fullSearch, Field... defRq) {
		try {
			...
			throw new RequiredRuntimeException(e, "Field-Value not found from Object-Class [%s] with Field-Name [%s], FullSearch[%s", cn(obj, null), fieldname, fullSearch);
		}
	}
	...
	public static Object invokeJarStWith(Path jarFile, String clazzFullName, String methodName, Object... keyValues) {
		Map<Class, Object> classObjectMap = UMap.ofKeyValuesSafeType(Class.class, Object.class, keyValues);
	...
	public static Object invokeJarSt_(Path jarFile, String clazzFullName, String methodName, Class[] parameterTypes, Object... paramterObjects) throws ClassNotFoundException, NoSuchMethodException, InvocationTargetException, IllegalAccessException, MalformedURLException {
		if (parameterTypes.length != paramterObjects.length) {
			throw new ERR.CheckException("parameterTypes.length (%s) must be equals paramterObjects.length (%s)", parameterTypes.length, paramterObjects.length);
		}
		URLClassLoader child = new URLClassLoader(
				new URL[]{ERR.isFileExist(jarFile).toUri().toURL()},
				RFL.class.getClassLoader()
		);
		Class classToLoad = Class.forName(clazzFullName, true, child);
	...
	public static <T> T inst(Class<T> type, T... defRq) {
		return inst(type, true, defRq);
	}

	public static <T> T inst(Class<T> type, boolean isPrivate, T... defRq) {
		try {
			...
			throw new RequiredRuntimeException(ex, "Error create instance type '%s', private '%s'", type.getName(), isPrivate);
		}
	}

	public static <T> T inst0(Class<T> type, Class<?>[] class_arg, Object[] obj_arg, T... defRq) {
		try {
			...
			throw new RequiredRuntimeException(ex, "Error create instance type '%s', class(arg) '%s', class_obj(arg) '%s'", type.getName(), Arr.as(class_arg), Arr.as(obj_arg));
		}
	}
	...
	public static <T> T inst(Class<T> type, Class<?> class_arg, Object obj_arg, T... defRq) {
		try {
			...
			throw new RequiredRuntimeException(ex, "Error create instance type '%s', class(arg) '%s', class_obj(arg) '%s'", type.getName(), class_arg, obj_arg);
		}
	}
	...
	public static class CallToMethod<R> {
		...
		private CallToMethod(Object callToObject, Class callToClass, String method, Class... methodTypes) {
			...
			this.callToClass = callToObject != null ? callToObject.getClass() : ERR.NN(callToClass);
			this.method = method;
	...
	public static void checkInvokeTypes(Class[] methodTypes, Object[] methodObjects) {
		ERR.isEq(methodObjects.length, methodTypes.length);
