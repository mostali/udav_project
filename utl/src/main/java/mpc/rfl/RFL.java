...
//	}
	...
	public static List<Method> methods_(boolean onlyFirst, Class clazz, Class[] withAnnotation, Class[] parameterTypes, Boolean isStaticMethod, boolean isFullSearch, boolean modifyAccessibleToTrue) throws NoSuchMethodException {
		...
		while (clazz != null) {
			...
			for (Method method : methods0) {
				...
				if (X.notEmpty(withAnnotation)) {
					if (getAnoFromMethod(method, withAnnotation, null) == null) {
						continue;
					}
				}
				methods.add(method);
		...
		throw new NoSuchMethodException(X.f("Class [%s]. Ano[%s]. WithParams [%s]. FullSearch [%s]. Not found.", clazz, isStaticMethod ? "(St)" : "", ARR.as(withAnnotation), Arrays.asList(parameterTypes), isFullSearch));
	}
	...
	public static <T> T getAno(Class clazz, Class ano, T... defRq) {
		T annotation = (T) clazz.getAnnotation(ano);
		if (annotation != null) {
			return annotation;
		}
		return ARG.toDefThrow(() -> new RequiredRuntimeException("Except '%s' ano from page class '%s'", ano, clazz), defRq);
	}
...
//		Primitives.wrap()
	...
	@SneakyThrows
	public static <T> Map<String, T> fieldValuesMap(Class clazz, Object obj, Class<T> fieldType, boolean isStatic, boolean fullSearch, Map<String, T>... defRq) {
		FindFieldsPredicate findFieldsPredicate = new FindFieldsPredicate(true, fieldType, null, null, null);
	...
	public static class JarCall {
		...
		private Path jarPath() {
			return Env.getBinPath(jct.ctx[0]);
		}
	}
