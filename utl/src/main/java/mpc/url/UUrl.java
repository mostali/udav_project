...
//	public static final String HTTPS = "https://";
	...
	public static String[] getPathAndQueryString(String uri, String[]... defRq) {
		int ind = uri.indexOf('?');
		if (ind != -1) {
			return TKN.twoByPos(uri, ind);
		}
		return ARG.throwMsg(() -> X.f("Uri '%s' without query", uri), defRq);
	}
	...
	public static String getExtFromUrlPath(String path, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Extension not found from url-path '%s'", path), defRq);
	}
	...
	public static String getPathFirstItemFromUrlPath(String path, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("First Path Item not found from url '%s'", org), defRq);
	}
	...
	public static String getPathFromUrl(String url, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Path not found from url '%s'", url), defRq);
	}
	...
	public static String[] getHostAndPagename(String url, String[]... defRq) {
		...
		if (hostAndPath != null) {
			...
			hostAndPath[1] = TKN0.firstPath(path, path);
			return hostAndPath;
		}
		return ARG.throwErr(() -> new RequiredRuntimeException("Except url with any path '%s'", url), defRq);
	}
...
//	}
	...
	public static String[] getHostAndPath(String url, String[]... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Host not found from url '%s'", url), defRq);
	}
...
//	}
	...
	public static String getHostName(String url, String... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Except valid url '%s'", url), defRq);
		}
	}
	...
	public static Boolean isHttpsOrHttp(String hostWithPath, Boolean... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Http/Https Protocol not found from '%s'", hostWithPath), defRq);
