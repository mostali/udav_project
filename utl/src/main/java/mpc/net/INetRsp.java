...
//		throw NI.stop("Who need bytes? May be use toInputStream()?");
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Illegal response '%s' to convert type '%s'", any, type), defRq);
	}
...
//			}
	...
	default String getErrorJsonValue(String[] anyKey, String... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Json values [%s] not found from error", anyKey), defRq);
		}
	}
