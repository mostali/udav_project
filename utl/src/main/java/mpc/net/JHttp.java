...
//		}
	...
	public static <T> T sendPostWithStringAsFile__OLD(String url, String[][] headers, String formDataString, String filename, boolean jsonOrXml) {
		...
		String logMark = "POST" + " >> " + url + " H:" + X.sizeOf0(headers) + " " + ARR.ofDouble(headers) + " " + " D:" + STR.toStrLine(formDataString);
		if (L.isTraceEnabled()) {
			L.trace(SYMJ.ARROW_RIGHT2 + logMark + "\n" + formDataString);
		} else if (L.isInfoEnabled()) {
			L.info(SYMJ.ARROW_RIGHT2 + logMark);
...
//	public static void main_SEND_STR_AS_FILE(String[] args) {
//		String url = "https://httpbin.org/post"; // тестовый эндпоинт
//
//		String[][] headers = {
//				{"User-Agent", "Java-Multipart-Client"},
//				{"Accept", "application/json"}
//		};
//
//		String fileContent = "<enums><enum value=\"A\">Alpha</enum><enum value=\"B\">Beta</enum></enums>";
//
//		try {
//			String response = sendPostWithStringAsFile(url, headers, fileContent, "enums.export.xml");
