...
public class DLD {
	...
	public static void url2file(Map<String, String> urlBlankFiles, boolean createParentDir) {
		...
		for (Map.Entry<String, String> url2file : urlBlankFiles.entrySet()) {
			try {
				...
				writed.entrySet().forEach(f -> UFS_BASE.RM.removeFileQuickly(f.getValue()));
				throw t;
			}
	...
	public static Path url2file_WithRewriteDst(String url, String targetDir, boolean rewriteDst) throws IOException {
		...
		String name = UF.getPathFileName(url);
	...
	public static void url2file0(String url, String file, boolean createParentDir) throws IOException {
		if (createParentDir) {
			Files.createDirectories(Paths.get(file).getParent());
		}
		url2file0(url, file);
	...
	public static void url2file0(URL url, String file,String[]...headers) throws IOException {
		ERR.notNull(url, "Url is null");
		if (L.isTraceEnabled()) {
			L.trace("Start url2file [{}] to file [{}]", url, file);
		}
...
//		conn.setInstanceFollowRedirects( true);
	...
	public static void url2out(URL url, OutputStream out) throws IOException {
		...
		if (L.isTraceEnabled()) {
			if (out instanceof FileOutputStream) {
				...
				L.trace("Start url2out [{}] -> [{}]", url, RFL.cn(out));
			}
		}
	...
	public static <T> void url2httpout(URL url, ValueOutStream out, int... exceptStatus) throws IOException {
		ERR.notNull(url, "Url is null");
		if (L.isTraceEnabled()) {
			L.trace("Start url2httpout [{}] -> [{}]", url, RFL.cn(out));
		}
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.connect();
		int status = conn.getResponseCode();
		if (exceptStatus.length > 0) {
			boolean isSuccess = UN.isSeries(status, exceptStatus);
			ERR.state(isSuccess, "except success, but cam %s", status);
		}
		//	conn.setConnectTimeout(2000);
		InputStream is = conn.getInputStream();
		IOUtils.copy(is, out);
