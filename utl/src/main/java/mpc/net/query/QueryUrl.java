...
//	}
	...
	public boolean hasParam(String key, boolean... withData) {
		...
		return !map.containsKey(key) ? false : (ARG.isDefEqTrue(withData) ? X.notEmpty(map.get(key)) : true);
	}
...
//		map0.forEach((k, v) -> map.put(k.substring(withPfx.length()), v));
	...
	public List<Pare<String, String>> getQueryArgs(String[] checkQueryArgs) {
		boolean withData0 = true;
//		boolean withData0 = ARG.isDefEqTrue(withData);
		return Arrays.stream(checkQueryArgs).map(qa -> Pare.of(qa, getFirstAsStr(qa, null))).filter(qa -> withData0 ? qa.val() != null : true).collect(Collectors.toList());
