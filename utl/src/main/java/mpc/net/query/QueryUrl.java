...
public class QueryUrl implements IGetterAs {
...
//	}
	...
	public <T> T getFirstAs(String key, Class<T> asType, T... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Query Arg (First) '%s' is null", key), defRq);
	}
	...
	private String getFirst(String key, WhatIs whatIs, boolean checkAll, String... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Required value by key '%s' (%s), check-only-first-key (%s)", key, whatIs, !checkAll), defRq);
	}
...
//		boolean withData0 = ARG.isDefEqTrue(withData);
	...
	public <T> T getAs(String key, Class<T> asType, T... defRq) {
		String firstArg = ARG.isDef(defRq) ? getFirstAsStr(key, null) : getFirstAsStr(key);
		return UST.strTo(firstArg, asType, defRq);
	}
}
