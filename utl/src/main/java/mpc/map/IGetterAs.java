...
public interface IGetterAs {
    ...
    default <T> T getAs(String key, Function<String, T> str2type, T... defRq) {
        String asString = getAsString(key, null);
        if (asString != null) {
            return str2type.apply(asString);
        }
        return ARG.throwMsg(() -> X.f("Except string value by key '%s' via func", key), defRq);
    }

    default QDate getAsQDateMs(String key, QDate... defRq) {
        Long ms = getAsLong(key, null);
        if (ms != null) {
            return QDate.of(ms);
        }
        return ARG.throwMsg(() -> X.f("Date with ms not found by key '%s'", key), defRq);
    }
    ...
    default QDate getAsQDate(String key, String[] formats, QDate... defRq) {
        ...
        return ARG.throwMsg(() -> X.f("Date not found by key '%s' and formats %s", key, ARR.of(formats)), defRq);
    }
    ...
    default QDate getAsQDate(String key, String format, QDate... defRq) {
        ...
        return ARG.throwMsg(() -> X.f("Value not found by key '%s'", key), defRq);
    }
    ...
    default Long getAsTimeMarkMs(String key, Long... defRq) {
        ...
        return ARG.throwMsg(() -> X.f("TimeMark not found by key '%s'", key), defRq);
    }
    ...
    default GsonMap getAsGsonMap(String key, GsonMap... defRq) {
        return getAs(key, GsonMap.class, defRq);
    }
