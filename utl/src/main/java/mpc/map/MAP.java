...
//		l.stream().collect(Collectors.toMap(p -> p.key(), e -> e.val()));
	...
	public static <K, V> Map.Entry<K, V> getByValueFirst(Map<K, V> map, V[] keys, Map.Entry... defRq) {
		...
		return ARG.throwErr(() -> new DRQ("Value [%s] by keys [%s] is empty"), defRq);
	}
	...
	public static <K, V> V getNE(Map<K, V> map, K key, V... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Value by key '%s' is empty", key), defRq);
	}
	...
	public static <K, V> V get(Map<K, V> map, K key, V... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Value by key '%s' is null", key), defRq);
	}
	...
	public static <K, V> Map<K, V> ofKeyValuesOfStruct(Class<K> keyType, Class<V> vlType, Object... keyValues) {
	...
	public static <K, V> Map<K, V> sortRandom(Map<K, V> map) {
		List<Map.Entry<K, V>> entries = new ArrayList<>(map.entrySet());
		Collections.shuffle(entries);
// Создаем мапу со случайным порядком
