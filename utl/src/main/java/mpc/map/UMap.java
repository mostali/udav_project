...
//		l.stream().collect(Collectors.toMap(p -> p.key(), e -> e.val()));
	...
	public static <K, V> V getByKey(Map<K, V> map, Predicate whatIsKey, V... defRq) {
	...
	public static <K, V> V getByValue(Map<K, V> map, Predicate whatIsValue, V... defRq) {
	...
	public static <K, V> Map.Entry<K, V> findEntry(Map<K, V> map, Predicate whatIsKey, Predicate whatIsValue, Map.Entry<K, V>... defRq) {
		ERR.notNullAny(whatIsKey, whatIsValue);
		if (X.notEmpty(map)) {
	...
	public static <K, V> V getByValue(Map<K, V> map, K key, Predicate whatIsValue, V... defRq) {
		return getByValue(map, Arr.of(key), whatIsValue, defRq);
	}

	public static <K, V> V getByValue(Map<K, V> map, K[] keys, Predicate whatIs, V... defRq) {
		for (K key : keys) {
	...
	public static <K, V> V getAs(Map<K, ?> map, K key, Class<V> asType, Object... defRq) {
		...
		if (asType.isPrimitive()) {
			...
			asType = ObjTo.convertPrimitiveClassToWrapperClass(asType);
		}
		...
		if (primitiveClass == null) {
			throw new EmptyRuntimeException("Value by key '%s' is %s. Map:%s", key, val == null ? "null" : "empty", map);
		}
	...
	public static <K, V> V getNE(Map<K, V> map, K key, V... defRq) {
		...
		throw new RequiredRuntimeException("Value by key '%s' is empty", key);
	}
	...
	public static <K, V> V get(Map<K, V> map, K key, V... defRq) {
		...
		throw new RequiredRuntimeException("Value by key '%s' is null", key);
	}
	...
	public static <K, V> Map<K, V> ofKeyValuesSafeType(Class<K> keyType, Class<V> vlType, Object... keyValues) {
		...
		for (int i = 0; i < keyValues.length; i += 2) {
			...
			ERR.isClassOf(keyValues[i].getClass(), keyType);
			ERR.isClassOf(kvValue.getClass(), vlType);
	...
	public static Map<String, String> mapOfDeprecated(String patternMap, String delimetrPairs, String delimetrKeyValue) {
		String[] pairs = patternMap.split(delimetrPairs);
	...
	public static Map<String, String> toMapByCutPrefix(Map<String, ?> keys, String prefix, String delimter, boolean putEmptyKey, boolean checkExistKey) {
		...
		for (Map.Entry<String, ?> entry : keys.entrySet()) {
			...
			if (STR.startsWithQk(entry.getKey(), startPrefix, false)) {
	...
	public static Map<String, ?> toMapByCutPrefix(Map<String, ?> map, String prefix, String delimetr, Class<Map>... classNewMap) {
		if (X.isNullAll(prefix, delimetr)) {
		...
		ERR.notNull(map);
		Map newMap = classNewMap.length > 0 ? RFL.inst(classNewMap[0]) : RFL.inst(map.getClass());
	...
	public static Map<String, Double> sortBy(Map<String, Double> map, boolean byKeyOrByValue, boolean... ascOrDesc) {
		Boolean f = ARG.toDefBoolean(ascOrDesc);
		if (f == null) {
			return map;
		}
	...
	public static <K, V> K getKey(Map<K, V> map, V[] values, K... defRq) {
		...
		throw new RequiredRuntimeException("Key not found, by values [%s] from map [%s]", Arr.of(values), map);
	}
	...
	public static <K, V> K getKey(Map<K, V> map, V value, K... defRq) {
		...
		throw new RequiredRuntimeException("Key not found, by value [%s] from map [%s]", value, map);
	}
	...
	public static <V> void putByIndexOrAutoincrement(Map<Integer, V> map, Integer targetIndex, V value) {
		...
		int targetIndexPosition = ERR.isPosOrZero(indexPositions.indexOf(targetIndex));
