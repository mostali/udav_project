...
public class BootMap implements Map<String, String>, IContractBuilder, IGetterAs {
	...
	public String get(Object key) {
		return getRq(key, ARR.EMPTY_ARGS);
	}

	public String getRq(Object key) {
		return getRq(key, ARR.EMPTY_ARGS);
	}
	@Override
	public <T> T getAs(String key, Class<T> asType, T... defRq) {
		String strVL = ARG.isDef(defRq) ? getRq(key, null) : getRq(key, ARR.EMPTY_ARGS);
		return strVL != null ? UST.strTo(strVL, asType, defRq) : ARG.toDef(defRq);
	}

	public <T> T getExtAs(String key, Class<T> asType, T... defRq) {
		String strVL = ARG.isDef(defRq) ? getRq(key, null) : getRq(key, ARR.EMPTY_ARGS);
		return strVL != null ? UST.strToExt(strVL, asType, defRq) : ARG.toDef(defRq);
	}


	public String getRq(Object key, String... defRq) {
		...
		return X.notEmpty(all) ? ARRi.first(all) : ARG.throwMsg(() -> X.f("Not found SINGLE value by key '%s'", key), defRq);
	}
...
//		return X.notEmpty(allSPL) ? allSPL : ARG.toDefThrowMsg(() -> X.f("Not found ALL values by key '%s'", key), defRq);
	...
	public List<String> getAllSPL(String key, List<String>... defRq) {
		...
		if (root != null) {
			...
			return ARG.throwMsg(() -> X.f("Not found ROOT values '%s'", key), defRq);
		}
		return ARG.throwMsg(() -> X.f("Not found ROOT key '%s'", key), defRq);
	}

	private SPL.SplPareBoot getSPL(String key, Set<Pare3<APType, String, String>> alreadyChecked, SPL.SplPareBoot... defRq) {

		Map<Pare<APType, String>, List<String>> findedValuesMap = bootMap.findValuesMap(key, false, false);
		if (findedValuesMap == null) {
			return ARG.throwMsg(() -> X.f("Not found SPL common key '%s'", key), defRq);
		}

		SPL.SplPareBoot root = new SPL.SplPareBoot(key);

		for (Entry<Pare<APType, String>, List<String>> findedValuesMapEntry : findedValuesMap.entrySet()) {

			Pare<APType, String> apKey = findedValuesMapEntry.getKey();

			Pare3<APType, String, String> apKeyProp = Pare3.of(apKey.key(), apKey.val(), key);
		...
		for (SPL.SplPareBoot unresolvedSpl : unresolved) {
			...
			if (nextSpl == null) {
				if (!unresolvedSpl.hasDef()) {
					return ARG.throwMsg(() -> X.f("Not found SPL inner key '%s' apKey '%s'", unresolvedSpl.name(), unresolvedSpl.apKey), defRq);
				}
			} else {
	...
	public static void test1() {
		...
		X.p("BootMapContext test1 is OK");
	}
	...
	public static void test2() {
		...
		X.p("BootMapContext test2 is OK");
	}
	...
	public static void test3() {
		try {
			...
			P.warnBig(ERR.getMessagesAsStringWithHead(ex, "Happen only on build ot module?"));
		}
	}
	...
	public static void test30() {
		...
		Map ctx1 = ARR.asLMAP("file", "bb");
		Map ctx2 = ARR.asLMAP("file", "${file}");
		BootContext bootMap = BootContext.ofAll(args, ctx1, ctx2);
		BootMap bootMap0 = bootMap.asMap(true);
		List<String> all = bootMap0.getAll("file");
		IT.state(all.equals(ARR.as("v", "v2")), "test3 fail");
		X.p("BootMapContext test3 is OK");
	}
	...
	public static void test4() {
		...
		X.p("BootMapContext test4 is OK");
	}
	...
	public static void test5() {
		...
		try {
			...
			X.p("BootMapContext test5 is OK");
		}
	}
	...
	public static void test6() {
		...
		try {
			...
			X.p("BootMapContext test6 is OK");
		}
	}
	...
	public static void test7() {
		...
		X.p("BootMapContext test7 is OK");
	}
