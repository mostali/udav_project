...
//		}
	...
	public static String getKey(String key, String... defRq) {
		return INSTANCE != null ? INSTANCE.get(key, defRq) : ARG.throwMsg(() -> X.f("BC Key '%s' not found", key), defRq);
	}
	...
	public <T> T getAs(String key, Class<T> asType, T... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Not found value by key '%s'", key), defRq);
	}

	public <T> List<T> getAllAs(String key, Class<T> asType, List<T>... defRq) {
		List<String> values = getAll(key);
		if (X.notEmpty(values)) {
			return S_.mapToList(values, vl -> UST.strTo(vl, asType));
		}
		return ARG.throwMsg(() -> X.f("Not found values by key '%s' for type '%s'", key, asType), defRq);
	}
...
//		}
		...
		return ARG.throwMsg(() -> X.f("Not found values by key '%s'", key), defRq);
	}
	...
	private List<String> getAll(String key, boolean ignoreCase_NOCACHE, List<String>... defRq) {
		if (ignoreCase_NOCACHE) {
			...
			return X.notEmpty(values) ? values : ARG.throwMsg(() -> X.f("Not found values by key '%s' (ignore case)", key), defRq);
		}
		Map<String, List<String>> cacheAP = getCacheMap();
		if (cacheAP.containsKey(key)) {
			List<String> cachedVls = cacheAP.get(key);
			return X.notEmpty(cachedVls) ? cachedVls : ARG.throwMsg(() -> X.f("Not found values by key '%s'", key), defRq);
		}
		List<String> values = findValues(key);
		cacheAP.put(key, values);
		return X.notEmpty(values) ? values : ARG.throwMsg(() -> X.f("Not found values by key '%s'", key), defRq);
	}
	...
	public Map<Pare<ApType, String>, List<String>> findValuesMap(String key, Predicate<String> keyPredicate, boolean onlyFirst, boolean... ignoreCase) {
		...
		for (Map.Entry<Pare<ApType, String>, Map<String, String>> candidateMapEntry : getBootMap().entrySet()) {
			...
			String keySrc = null;
			Map<String, String> props = candidateMapEntry.getValue();
			for (Map.Entry<String, String> candidateEntryValue : props.entrySet()) {
				if (keyPredicate != null) {
					...
					if (!X.equals(candidateEntryValue.getKey(), key, ic)) {
						continue;
					}
					keySrc = key;
				}
...
//					keySrc = key;
	...
	private static Map<Pare<ApType, String>, Map<String, String>> init(String[] args, List<Pare3<ApType, String, Object>> files) {
		...
		return bootMap.entrySet().stream().map(i -> {
			Map<String, String> value = i.getValue();
			if (value != null) {
				return i;
			} else {
				if (L.isWarnEnabled()) {
					L.warn("CandidateMapEntry {} has null props, why?", value);
				}
				i.setValue(ARR.EMPTY_MAP);
				return i;
			}
