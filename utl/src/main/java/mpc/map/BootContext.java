...
public class BootContext implements IContractBuilder, IGetterAs {
	...
	private void addApLoc(Pare3<ApType, String, Object> apLoc, Integer... index) {
		IT.stateNo(initAppLocEntitys.contains(apLoc));
		if (ARG.isDef(index)) {
...
//		}
	...
	public Map<Pare<ApType, String>, Pare<String,List<String>>> findValuesMapWithKey(Predicate<String> keyPredicate, boolean onlyFirst, boolean... ignoreCase) {
		return findValuesMapWithKey(null, keyPredicate, onlyFirst, ignoreCase);
	}
	...
	public Map<Pare<ApType, String>, Pare<String, List<String>>> findValuesMapWithKey(String key, Predicate<String> keyPredicate, boolean onlyFirst, boolean... ignoreCase) {
		IT.state(X.nullOnlyOne(key, keyPredicate));
		boolean ic = ARG.isDefEqTrue(ignoreCase);
	...
	public enum ApType {
		RUN_OPTS, SYS, ENV, LOCAL_MAP, FILE_OPTS, FILE_MAP, APPDIR, RL, RSRC;
