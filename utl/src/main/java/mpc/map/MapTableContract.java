...
//	}
	...
	public static <T> T buildContract_DefRq(Map data, Class<T> contractClass) {
		return (T) Proxy.newProxyInstance(MapTableContract.class.getClassLoader(), new Class[]{contractClass}, new GetAndDefMapInvocationHandler(data, contractClass, true));
	}

	public static <T> T buildContract_DefRq(JsonObject data, Class<T> contractClass) {
		return (T) Proxy.newProxyInstance(MapTableContract.class.getClassLoader(), new Class[]{contractClass}, new GetAndDefMapInvocationHandler(data, contractClass, true));
	}
	...
	public static class GetAndDefMapInvocationHandler implements InvocationHandler {
		...
		public GetAndDefMapInvocationHandler(JsonObject data, Class contractClass, boolean isDefRqSignMode) {
			this((Map) UGson.toMapFromJO(data), contractClass, isDefRqSignMode);
			this.jsonData = data;
		}
		...
		public Object invoke(Object proxy, Method method, Object[] methodArgs) throws Throwable {
			...
			if (method.isDefault()) {
				...
				Object val = X.empty(_cleanMap) ? getObjectValue(method, methodArgs) : _cleanMap.get(UContract.getKeyNameFromGetMethodName(methodName));
				if (val == null) {
		...
		private Object getObjectValue(Method method, Object[] methodArgs) {
			return isDefRqSignMode ? UContract.getObjectValue_DefRq(data, method, methodArgs) : UContract.getObjectValue_MarkNotRq(data, method, methodArgs);
		}
			...
			if (_cleanMap == null || ARG.isDefEqTrue(reinit)) {
				...
				for (Method method : contractClass.getMethods()) {
					...
					if (method.getName().startsWith("get")) {
						String key = UContract.getKeyNameFromGetMethodName(method.getName());
