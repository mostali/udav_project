	...
	public static class GetAndDefMapInvocationHandler implements InvocationHandler {
		...
		public Object invoke(Object proxy, Method method, Object[] methodArgs) throws Throwable {
			...
			if (method.isDefault()) {
				...
				Object val;
				if (X.empty(_cleanMap)) {
					val = getObjectValue(method, methodArgs);
				} else {
					val = _cleanMap.get(UContract.getKeyNameFromGetMethodName(methodName, keyAsIs));
				}
				if (val == null) {
...
//				Map build = data;
		...
		private Object getObjectValue(Method method, Object[] methodArgs) {
			if (isDefRqSignMode) {
				return UContract.getObjectValue_DefRq(data, method, methodArgs, keyAsIs);
			}
			return UContract.getObjectValue_MarkNotRq(data, method, methodArgs, keyAsIs);
		}
