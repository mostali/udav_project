...
public enum EDayHourPeriod implements ITimeMode {
	MORNING(5, 10), DAY(11, 17), EVNING(18, 23), NIGHT(0, 4);
	public final int start, end;

	EDayHourPeriod(int start, int end) {
		this.start = start;
		this.end = end;
	}

	public static EDayHourPeriod valueOf() {
		return valueOf(QDate.now().hour);
	}

	public static EDayHourPeriod valueOfStartWith(String letter) {
		for (EDayHourPeriod val : values()) {
	...
	public static EDayHourPeriod valueOf(int hour) {
		for (EDayHourPeriod qts : values()) {
			if (qts.isTime(hour)) {
				return qts;
			}
		}
