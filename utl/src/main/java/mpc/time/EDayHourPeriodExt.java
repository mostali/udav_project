...
public enum EDayHourPeriodExt implements ITimeMode {
	...
	EDayHourPeriodExt(int start, int end) {
		this.start = start;
		this.end = end;
	}

	public static EDayHourPeriodExt valueOf() {
		return valueOf(QDate.now().hour);
	}

	public static EDayHourPeriodExt valueOfStartWith(String letter) {
		for (EDayHourPeriodExt val : values()) {
	...
	public static EDayHourPeriodExt valueOf(int hour) {
		for (EDayHourPeriodExt qts : values()) {
			if (qts.isTime(hour)) {
				return qts;
			}
		}
