...
public class QDate extends Date {
	...
	public static void checkYear(int year, int yearMin, int yearMax) {
		if (year < yearMin || year > yearMax) {
			throw new FIllegalArgumentException("Invalid Condition >> YearMin[%s] < Year[%s] < YearMax[%s]", yearMin, year, yearMax);
		}
	}

	public static void checkMonth(int month) {
		if (month < 1 || month > 12) {
			throw new FIllegalArgumentException("Invalid Condition >> MonthMin[%s] < Month[%s] < MonthMax[%s]", 1, month, 12);
		}
	}
	...
	public static void checkDay(int year, int month, int day) {
		try {
			...
			throw new FIllegalArgumentException("Invalid Day >> Year[%s],Month[%s],Day[%s]", year, month, day);
		}
	}
	...
	public QDate(Date date) {
		this(Integer.parseInt(UTime7.CURRENT_YEAR.format(date)),//
				Integer.parseInt(UTime7.CURRENT_MONTH.format(date)),//
				Integer.parseInt(UTime7.CURRENT_DAY.format(date)),//
				Integer.parseInt(UTime7.CURRENT_HOUR.format(date)),//
				Integer.parseInt(UTime7.CURRENT_MINUTES.format(date)),//
				Integer.parseInt(UTime7.CURRENT_SECONDS.format(date)),
				Integer.parseInt(UTime7.CURRENT_SECONDS.format(date))
		);
	}
	...
	public QDate(Integer year, Integer month, Integer day, Integer hour, Integer minutes, Integer seconds, Integer millis) {
		super(Date7.toDateWithManlyMonth(year, month, day, hour, minutes, seconds, millis).getTime());
		this.year = year;
		this.month = month;
		this.day = day;
		this.hour = hour;
...
//	@Deprecated
//	public QDate(int day) {
//		this(UTime.currentYear(), UTime.currentMonth(), day, UTime
//				.currentHour(), UTime.currentMinutes(), UTime.currentSeconds());
//	}

//	@Deprecated
//	public QDate(int hour, int minutes) {
//		this(UTime.currentYear(), UTime.currentMonth(), UTime.currentDay(),
//				hour, minutes, UTime.currentSeconds());
//	}

//	@Deprecated
//	public QDate(int year, int month, int day) {
//		this(year, month, day, UTime.currentHour(), UTime.currentMinutes(),
//				UTime.currentSeconds());
//	}

//	@Deprecated
//	public QDate(int year, int month, int day, int hour, int minutes) {
//		this(year, month, day, hour, minutes, UTime.currentSeconds());
//	}
...
//				m2hour(), m2minutes(), m2seconds());
	...
	public LocalDateTime toLocalDateTime(ZoneId... zoneId) {
		LocalDateTime localDateTime = LocalDateTime.of(year, month, day, hour, minutes, seconds, millis);
		return ARG.isDef(zoneId) ? localDateTime.atZone(ARG.toDef(zoneId)).toLocalDateTime() : localDateTime;
	}

//	public LocalDateTime toLocalDateTime(ZoneId... zoneId) {
//		LocalTime localDateTime = LocalTime.of(hour, minutes, seconds);
//		return ARG.isDef(zoneId) ? localDateTime.atZone(ARG.toDef(zoneId)).toLocalDateTime() : localDateTime;
//	}
	...
	public static LocalDateTime[] nowAndBefore(ChronoUnit unit, int i) {
		LocalDateTime ldt = new QDate().toLocalDateTime();
		LocalDateTime ldt2 = new QDate().toLocalDateTime().minus(i, unit);
		return new LocalDateTime[]{ldt, ldt2};
	}
...
//		QDate d = new QDate(ldt.getYear(), ldt.getMonthValue(), ldt.getDayOfMonth(), ldt.getHour(), ldt.getMinute(), ldt.getSecond());
//		return ARG.isDef(zoneId) ? d.as(ARG.toDefRq(zoneId)) : d;
	 ...
	 * @return
	 */
	public static QDate ofIso(String monodate14) {
	 ...
	 * 20190519235502
	 */
	public static QDate ofMono14(String monodate14) {
		QDate qdate = null;
		try {
			...
			qdate = new QDate(y, m, d, H, M, S);
		} catch (Exception ex) {
			throw new IllegalArgumentException("Monodate 14 invalid [" + monodate14 + "]");
		}
		return qdate;
	}
...
//	}
	...
	public enum F {
		 ...
		 * 2022-02-01 22:19:55
		 */
		MONO20NF("%s-%s-%s %s:%s:%s"),
		MONO24NF_MS("%s-%s-%s %s:%s:%s.%s"),
		/**
		 * 19991231_135959
		 */
		MONO15_SEC("%s%s%s%s%s%s"),
		MONO15_FILE_SEC("%s%s%s_%s%s%s"),
		HH_mm("%s-%s"),
		HH_mm_ss("%s-%s-%s"),
		mm_ss("%s-%s");
		final String fmt;
		...
		public String build(QDate q) {
			switch (this) {
				case HH_mm:
					return X.f(fmt, q.m2hour(), q.m2minutes());
				case mm_ss:
					return X.f(fmt, q.m2minutes(), q.m2seconds());
				case HH_mm_ss:
					return X.f(fmt, q.m2hour(), q.m2minutes(), q.m2seconds());
				case YYYY_MM_DD:
				case YYYYMMDD:
					return String.format(fmt, q.year, q.m2month(), q.m2day());

				case MONO17NF:
					return String.format(fmt, q.year, q.m2month(), q.m2day(), q.m2hour(), q.m2minutes());

				case MONO15_SEC:
				case MONO20NF:
					return String.format(fmt, q.year, q.m2month(), q.m2day(), q.m2hour(), q.m2minutes(), q.m2seconds());
				case MONO24NF_MS:
					return String.format(fmt, q.year, q.m2month(), q.m2day(), q.m2hour(), q.m2minutes(), q.m2seconds(), q.m3millis());
...
//	public static class QDateStr extends QDate {
//
//		public QDateStr(QDate qDate) {
//			super(qDate);
//		}
//
//		/**
//		 * 1999-12-31 00:59
//		 */
//		public String mono17NF_y4m2() {
//			return year + "-" + m2month() + "-" + m2day() + " " + m2hour() + ":" + m2minutes();
//		}
//	}
...
//	public String toStringFormat(String patternDate) {
//		return DateFormatUtils.format(toDate(), patternDate);
//	}
//
//	public static QDate ofStringFormat(String dateString, String... dateformat) throws ParseException {
//		Date d = DateUtils.parseDateStrictly(dateString, dateformat);
//		return of(d);
//	}

	public long diff(QDate now) {
		return now.ms() - ms();
	}
	...
	public Number diff(QDate now, TimeUnit timeUnit) {
		return QDate.toTime(timeUnit, now.ms() - ms());
	}
	...
	public long diffabs(QDate now) {
		return Math.abs(diff(now));
	}

	public Number diffabs(QDate now, TimeUnit timeUnit) {
		return QDate.toTime(timeUnit, Math.abs(diff(now)));
	}


	public BigDecimal diff(QDate now, TimeUnit timeUnit, Integer... scale) {
		long nano = diff(now, TimeUnit.NANOSECONDS).longValue();
		BigDecimal hourStr = UTime.toTime(nano, TimeUnit.NANOSECONDS, timeUnit, scale);
		return hourStr;
	}
	...
	public QDate truncate(int calendarUnit, boolean startOrEnd) {
	...
	public enum EDay {
		...
		public static boolean isToday(Date date, EDay eday) {
			return eday.equals(getDayName(date));
		}

		public static EDay getDayName(Date date) {
		...
		public static String getDayNameCapitalize(Date date) {
			return WordUtils.capitalize(getDayName(date).name().toLowerCase());
		}

		public static EDay ofIndex(int indexDay) {
			return EN.getEnum(indexDay, EDay.class);
		}

		public int index() {
			return EN.indexOf(this);
		}
	}
	...
	public boolean equals(Object o) {
		...
		QDate qDate = (QDate) o;
		return EQ.equalsUnsafe(year, qDate.year) && EQ.equalsUnsafe(day, qDate.day) && EQ.equalsUnsafe(month, qDate.month) && EQ.equalsUnsafe(hour, qDate.hour) && EQ.equalsUnsafe(minutes, qDate.minutes) && EQ.equalsUnsafe(seconds, qDate.seconds);
	}

//	@Override
//	public int hashCode() {
//		return Objects.hash(year, day, month, hour, minutes, seconds, millis);
//	}
}
