...
public class UTime {
	...
	public static String toStringTimeHumanlySec(long seconds) {
		if (seconds < TU.MIN_SEC) {
			return seconds + " seconds";
		} else if (seconds < TU.HOUR_SEC) {
			return toTime(seconds, TimeUnit.SECONDS, TimeUnit.MINUTES, 0) + " minutes";
		} else if (seconds < TU.DAY_SEC) {
	...
	public static Date parseDateCustom(String string, String format, Integer[] year_month_day_hour_min_sec, Date... defRq) {
		try {
			...
			parsedCal.setTime(dateInputFormat.parse(string));
	...
	public static void updateDate(Calendar date, Integer... year_month_day_hour_min_sec) {
		...
		Integer year = ArrItem.itemAs(year_month_day_hour_min_sec, 0, Integer.class, null);
		if (year != null) {
			date.set(Calendar.YEAR, year);
		}
		Integer month = ArrItem.itemAs(year_month_day_hour_min_sec, 1, Integer.class, null);
		if (month != null) {
			date.set(Calendar.MONTH, month);
		}
		Integer day = ArrItem.itemAs(year_month_day_hour_min_sec, 2, Integer.class, null);
		if (day != null) {
			date.set(Calendar.DAY_OF_MONTH, day);
		}
		Integer hours = ArrItem.itemAs(year_month_day_hour_min_sec, 3, Integer.class, null);
		if (hours != null) {
			date.set(Calendar.HOUR_OF_DAY, hours);
		}
		Integer minutes = ArrItem.itemAs(year_month_day_hour_min_sec, 4, Integer.class, null);
		if (minutes != null) {
			date.set(Calendar.MINUTE, minutes);
		}
		Integer seconds = ArrItem.itemAs(year_month_day_hour_min_sec, 5, Integer.class, null);
		if (seconds != null) {
			date.set(Calendar.SECOND, seconds);
		}
		Integer ms = ArrItem.itemAs(year_month_day_hour_min_sec, 6, Integer.class, null);
		if (ms != null) {
