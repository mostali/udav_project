...
public class RES extends Fd {
	...
	public static String loadFileFromRunLocationOrResources(Class resClass, String file, boolean copy, String... defRq) {
		try {
			...
			if (copy) {
				res.copyToRunLocation_(UFS_BASE.COPY.CopyOpt.FD_SKIP_IF_EXIST);
				return f.readAs(String.class);
			}
			return res.cat();
		} catch (Exception ex) {
...
//		return ARG.toDefRq(defRq);
	...
	public static RES of(Class clazz, String fileOrDir, EFT type) {
		return new RES(clazz, fileOrDir, ERR.notNull(type) == EFT.FILE);
	}
	...
	protected InputStream toInputStream(String[] child, InputStream... defRq) {
		...
		switch (type()) {
			case DIR:
				if (hasChild) {
					return clazz.getResourceAsStream(pathWith(child).toString());
				}
				throw newExceptionFileWithoutChild();
			case FILE:
				if (!hasChild) {
	...
	public boolean exist(String... child) {
		...
		switch (type()) {
			case DIR:
				if (hasChild) {
					checkedPath = checkedPath.resolve(ERR.NE(child[0]));
				}
				break;
			case FILE:
				if (hasChild) {
	...
	public Path copyToLocation_(Path dstDir, boolean createDstDir, UFS.COPY.CopyOpt copyOpt) throws IOException {
		if (createDstDir) {
			UFS_BASE.MKDIR.mkdirsIfNotExist(dstDir);
		}
		switch (copyOpt) {
	...
	public Path copyToDirIfNotExist(Path dstDir, boolean crateDstDir) throws IOException {
		...
		return copyToDir(dstDir, crateDstDir, UFS_BASE.COPY.CopyOpt.FD_SKIP_IF_EXIST);
	}
	...
	public Path copyToDir(Path dstDir, boolean createDstDir, UFS.COPY.CopyOpt copyOpt) throws IOException {
		try {
			...
			if (isJarResource()) {
				...
				dst = UFS_BASE.COPY.copyToDir(toResourcePath(), dstDir, createDstDir, copyOpt);
			}
			if (L.isDebugEnabled()) {
	...
	private Path copyToDirFromJar(Path dstDir, boolean createDstDir, UFS.COPY.CopyOpt copyOpt) throws IOException {
		Path destFileOrDir = dstDir.resolve(path().getFileName());
		boolean existSave = EFT.existSave(destFileOrDir, isFileOrDir);
		if (copyOpt == UFS_BASE.COPY.CopyOpt.FD_SKIP_IF_EXIST && existSave) {
			return destFileOrDir;
		}
		UFS_BASE.MKDIR.createDirs_(dstDir, createDstDir);
		if (isFileOrDir) {
	...
	public List ls(String... child) throws IOException {
		if (isJarResource()) {
			...
			String path = UFD.createDirWithChild(path(), child).toString();
