...
public class UPackageResource {
	...
	private static void copyFdRr(final File srcFileOrDir, final File destDir, UFS.COPY.CopyOpt copyOpt) throws IOException {
		ERR.isTrue(destDir.isDirectory(), "dest dir must be dir", destDir);
			...
			if (entry.getName().startsWith(srcFileOrDirConnection.getEntryName())) {
				...
				switch (copyOpt) {
					...
					case FD_REPLACE_IF_EXIST:
						break;
					default:
						throw new WhatIsTypeException("Copy opt from jar supported only '%s' or %s", UFS_BASE.COPY.CopyOpt.FD_SKIP_IF_EXIST, UFS_BASE.COPY.CopyOpt.FD_REPLACE_IF_EXIST);
				}
