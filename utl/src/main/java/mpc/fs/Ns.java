...
public class Ns implements Serializable, IPath {
	...
	public static Ns findChild(Path path, Ns... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Child ns not found '%s'", path), defRq);
	}
	...
	public static Ns ofRpa(String namespace, String child) {
		return new Ns(Env.RPA.toString(), namespace, child);
	}
...
//	}
	...
	public List<Path> dLs(EFT fileType, List<Path>... defRq) {
		Path path = toPath();
		return UFS.existDir(path) ? UDIR.ls(path, fileType) : ARG.throwErr(() -> new RequiredRuntimeException("Ns dir '%s' not exist", path), defRq);
	}
