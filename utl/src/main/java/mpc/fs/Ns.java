...
public class Ns implements Serializable, IPath {
	...
	public Ns(String rootDir, String namespace, String child) {
		...
		this.child = child;
		if (child != null) {
			ERR.NE(namespace, "child without ns", child);
		}
	...
	public Path getPathChild() {
		return path(ERR.notEmpty(getNamespaceChildValue()));
	}
	...
	public static Ns ofSafeName(Path rootDir) {
		ERR.isGE(rootDir.getNameCount(), 2, "Except ns length >=2 ");
		return new Ns(rootDir.getParent().toString(), rootDir.getFileName().toString());
	}

	public static Ns ofSafeChild(Path rootDir) {
		ERR.isGE(rootDir.getNameCount(), 3, "Except ns length >=3 ");
	...
	public void createIfNotExist(boolean... mkdirsOrSingleDirOrCheck) {
		UFS_BASE.MKDIR.createDirsOrSingleDirOrCheckExist(path(), mkdirsOrSingleDirOrCheck);
	}

	public List<Path> ls(EFT fileType, List<Path>... defRq) {
		Path path = path();
