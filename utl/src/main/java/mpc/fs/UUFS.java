...
public class UUFS {
	...
	public static Map<String, Path> toMapByFn(Collection<Path> pathes) {
		return pathes.stream().collect(Collectors.toMap(p -> p.getFileName().toString(), p -> p, (v1, v2) -> X.throwIllegalStateException("twins '%s' found", UF.fn(v1), UF.fn(v2)), LinkedHashMap::new));
	}
...
//		}
	...
	public static void rotateToHistory(Path fileToRotateClone, boolean skipChange) {
		...
		Path nextPath = UF.parentClone(fileToRotateClone, ".day" + now.day);
		Path plusDayPath = UF.parentClone(fileToRotateClone, ".day" + plusDay);
