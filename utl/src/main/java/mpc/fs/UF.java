...
public class UF {
	...
	public static String normFileStartEnd(String path) {
		return normDir(normFileStart(path));
	}
	...
	public static String normFile(String dir, String child) {
		return normDir(ERR.NE(dir)) + normFile(ERR.NE(child));
	}
	...
	public static String clearFileNameRemoveSlash(String fileName) {
		return fileName = fileName.replaceAll("[^a-zA-Z0-9\\.\\-]", "_");
	}

	public static String clearFileNameKeepSlash(String fileName) {
		return fileName = fileName.replaceAll("[^a-zA-Z0-9\\.\\-\\\\/]", "_");
	}

	public static String clearFileNameRURemoveSlash(String fileName) {
		return fileName = fileName.replaceAll("[^а-яА-Яa-zA-Z0-9\\.\\-]", "_");
	}
	...
	public static String getFileNameFromUrl_WithoutExtension(String url) {
		...
		return USToken.firstGreedy(file, '.', file);
	}
	...
	public static String clearStringCyrRemoveSlash(String searchPath) {
		return clearFileNameRemoveSlash(Cyr2Lat.cyr2lat(searchPath));
	}

	public static String clearStringCyrKeepSlash(String searchPath) {
		return clearFileNameKeepSlash(Cyr2Lat.cyr2lat(searchPath));
	}
	...
	public static String getNameWoExt(String path, String... defRq) {
		return USToken.firstGreedy(path, '.', defRq);
	}
	...
	public static String getNameWoExtQk(Path p) {
		String fname = p.getFileName().toString();
		return USToken.firstGreedy(fname, '.', fname);
	}

	public static boolean isRootOrParentNameOfDir(String e) {
		return EQ.eq(".", e) || EQ.eq("..", e);
	}

	public static File f(String file) {
		ERR.notEmpty(file, "file is null");
		return new File(file);
	}

	public static Path p(String file, String... childs) {
		return Paths.get(file, childs);
	}

	public static URL u(String url) throws MalformedURLException {
		return new URL(url);
	}

	public static String ufn(String url) throws MalformedURLException {
		return ufn(u(url));
	}

	public static String ufn(URL url) throws MalformedURLException {
		return p(url.getPath()).getFileName().toString();
	}
	...
	public static String replaceHomeCharacter(String s) {
		if (s.startsWith("~")) {
			return normDir(Env.HOME_LOCATION.toAbsolutePath().toString()) + s.substring(1);
		}
		return s;
	}
	...
	public static String toAbsPathPfx(File file) {
		return "file://" + file.getAbsolutePath();
	}

	public static String toAbsPathPfx(Object file) {
		String absPath = file instanceof File ? ((File) file).getAbsolutePath() : file instanceof String ? toAbsPath((String) file) : file instanceof Path ? ((Path) file).toAbsolutePath().toString() : null;
		return "file://" + ERR.NN(absPath);
	}
	...
	public static String fnWithSize(Path path) {
		return fn(path) + "(" + X.sizeOfHu(path, ByteUnit.MB) + "Mb)";
	}

	public static String wrap(String fn) {
		return "file://" + fn;
	}

	public static String unwrap(String fn) {
		ERR.state(fn.startsWith("file://"));
	...
	public static boolean isValidCleanFilename(String filename) {
		return X.notEmpty(filename) && filename.matches(Regexs.WORD_RU);
	}

	public static String fPr(String path) {
		return fn(Paths.get(path));
	}

	public static String fPr(Path path) {
		return path.getParent().toString();
	}


	public static Path pD(String path, Path... defRq) {
		Path path0 = Paths.get(path);
		if (UFS.existDir(path0)) {
			return path0;
		}
		return ARG.toDefThrow(() -> new RequiredRuntimeException("Except exist dir '%s'", path), defRq);
	}

	public static Path pF(String path, Path... defRq) {
		Path path0 = Paths.get(path);
		if (UFS.existFile(path0)) {
			return path0;
		}
		return ARG.toDefThrow(() -> new RequiredRuntimeException("Except exist file '%s'", path), defRq);
	}

	public static String pr(String path) {
		return p(path).getParent().toString() + "/";
	}
