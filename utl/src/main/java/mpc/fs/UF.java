...
public class UF {
	...
	public static final String PFX_FILE_LN = "file://";

	public static final char[] ILLEGAL_CHARACTERS_URL_PATH = {
			' ', '?', '#', '[', ']', '@', '!', '$', '&', '\'', '(', ')', '*', '+',
			',', ';', '=', '%', '<', '>', '{', '}', '|', '\\', '^', '`', '\"'
	};

	public static String clearUrlPathNameCyrLatin(String pathSegment) {
		return Cyr2Lat.cyr2lat(clearUrlPathName(pathSegment));
	}

	public static String clearUrlPathName(String pathSegment) {
		if (pathSegment == null) return null;

		return pathSegment.chars()
				.mapToObj(c -> {
					// Проверяем, является ли символ допустимым в пути URL
					// Оставляем: буквы, цифры, дефис, подчеркивание, точку, тильду, слеш
					if (Character.isLetterOrDigit(c) ||
							c == '-' || c == '_' || c == '.' || c == '~' || c == '/') {
						return String.valueOf((char) c);
					}
					// Проверяем, не входит ли символ в список запрещенных
					else if (StringUtils.containsNone(String.valueOf((char) c), ILLEGAL_CHARACTERS_URL_PATH)) {
						return String.valueOf((char) c);
					}
...
//	}
	...
	public static String fnp(Path path, String... defRq) {
		if (path != null) {
			...
			if (parent != null) {
				...
				return ARG.throwErr(() -> new RequiredRuntimeException("Path [%s] parent filename is null", path), defRq);
			}
			return ARG.throwErr(() -> new RequiredRuntimeException("Path [%s] parent is null", path), defRq);
		}
		return ARG.throwErr(() -> new RequiredRuntimeException("Path [%s] is null", path), defRq);
	}
	...
	public static String fn(Path path, String... defRq) {
		if (path != null) {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Path [%s] filename is null", path), defRq);
		}
		return ARG.throwErr(() -> new RequiredRuntimeException("Path is null"), defRq);
	}
	...
	public static String ln(File file) {
		return PFX_FILE_LN + file.getAbsolutePath();
	}
	...
	public static String ln(Object file) {
		...
		return PFX_FILE_LN + file;
	}

	public static String unln(String fn) {
		IT.state(fn.startsWith(PFX_FILE_LN));
	...
	public static String fn(Path resolve, int count, String... defRq) {
		if (count > resolve.getNameCount()) {
			return ARG.throwMsg(() -> X.f("Not found path item %s from %s", count, resolve), defRq);
		}
		return fnCount(resolve, count).toString();
	}

	public static String name(Path path, int index, String... defRq) {
		if (index > path.getNameCount()) {
			return ARG.throwMsg(() -> X.f("Not found path item %s from %s", index, path), defRq);
		}
		return path.getName(index).toString();
	}

	public static Path fnCount(Path resolve, int count) {
		Path path = null;
	...
	public static String itemAsStr(Path path, int index, Path... defRq) {
		return item(path, index, defRq).toString();
	}

	public static Path parentClone(Path src, String withName) {
		return src.getParent().resolve(withName);
	}
...
//	public static String fn2(Path path) {
//
//	}
}
