...
//				Path newDst = UFS.generateAutoFilename(dst);
	...
	public static class RM {
		...
		@SneakyThrows
		public static boolean deleteDirBool(Path dir) {
			boolean exist = UFS.existDir(dir);
			if (!exist) {
				return false;
			}
			deleteDir(dir);
...
//		}
	...
	public static class COPY {
		...
		@SneakyThrows
		public static Path copyFd(Path srcFd, Path dstFd) {
			boolean isFile = UFS.isFile(srcFd);
			if (L.isDebugEnabled()) {
				String pfxFile = isFile ? "Copy [File]" : "Copy [Dir]";
				L.debug("{} src '{}' to dst '{}'", pfxFile, srcFd, dstFd);
			}
			if (isFile) {
				FileUtils.copyFile(srcFd.toFile(), dstFd.toFile());
			} else {
				copyDirectory(srcFd, dstFd);
			}
			return dstFd;
		}

		@SneakyThrows
		public static Path copy(Path srcFile, Path dstFile, boolean... mkdirs_dir_not) {
			if (L.isDebugEnabled()) {
	...
	public static class TWINS {
		...
		public static void main(String[] args) {
			...
			List<Path> notTwins = TWINS.findNotTwins(Paths.get("/home/dav/Документы/RC8.DEV2/universe-integration"), Paths.get("/home/dav/Документы/RC8.DEV/universe-integration"), true);
//			Sb twinsReport = (Sb) notTwins;
...
//			Sb twinsReport = TWINS.findTwinsReport(Paths.get("/home/dav/pjnsi/insi/rc/rc3/rc3.1_SKP/frontend-ui/"), Paths.get("/home/dav/pjnsi/0HNJ2/u01/tomcat-9/frontend-ui"), false);
			UFS.p(notTwins);
			X.exit(1);
			X.exit(Rt.buildReport(notTwins));
