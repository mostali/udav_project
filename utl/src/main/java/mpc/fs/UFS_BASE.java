...
public class UFS_BASE {
	...
	public static class MKDIR {
		...
		static void mkdirsIfNotExistImpl(Path source, boolean mkdirsOrSingleDir) {
			if (Files.isDirectory(source)) {
				return;
			} else {
				ERR.isFileNotExist(source);
			}
	...
	public static class MKFILE {
		...
		public static boolean createFileOrIfBlankWithEmptyJson_(Path file) throws IOException {
			if (!Files.exists(file)) {
				return createFileIfNotExistWithEmptyJson_(file);
			}
			...
			RW.write(file, UGson.EMPTY_PATTERN);
			return false;
		}

		public static boolean createFileIfNotExistWithEmptyJson_(Path file) {
			return createFileIfNotExistWithContent_(file, UGson.EMPTY_PATTERN);
		}

		public static boolean createFileIfNotExistWithContent_(Path file, String withContent) {
			if (!Files.exists(file)) {
				MKDIR.createDirs(file.getParent());
				RW.write(file, ERR.NN(withContent), StandardOpenOption.CREATE_NEW);
				return true;
			}
			return false;
		}
	...
	public static class MV {
		...
		public static Path moveIn_(Path moved, Path dst, boolean... mkdirs_mkdir_ornot) throws IOException {
			...
			return Files.move(moved, dst.resolve(moved.getFileName()), StandardCopyOption.ATOMIC_MOVE);
...
//				Path newDst = UFS.generateAutoFilename(dst);
		...
		public static Path move(Path moved, Path dst, Boolean mkdirs_mkdir_ornot, CopyOption... copyOptions) {
			MKDIR.createDirsOrSingleDirOrCheckExist(dst.getParent(), mkdirs_mkdir_ornot);
			return Files.move(moved, dst, copyOptions);
		}
	}
	...
	public static class RM {
		...
		public static void removeFileQuickly(List<String> files) {
			for (String file : files) {
				if (X.notEmpty(file)) {
					removeFileQuickly(Paths.get(file));
				}
			}
		}

		@SneakyThrows
		public static void removeFileQuickly(Path file) {
			removeFileQuickly_(file);
		}

		@SneakyThrows
		public static void removeFileQuickly(File file) {
			removeFileQuickly_(file.toPath());
		}

		@SneakyThrows
		public static void removeFileQuickly(String file) {
			if (X.empty(file)) {
				return;
			}
			removeFileQuickly_(Paths.get(file));
		}


		public static void removeFileQuickly_(Path file) throws IOException {
			if (Files.isRegularFile(file)) {
				Files.delete(file);
				if (L.isDebugEnabled()) {
	...
	public static class COPY {
		...
		public static void copyFileFromPackageToDestFile(Class clas, Path srcPackagePathFile, Path destFile) throws IOException {
			try (InputStream stream = clas.getClassLoader().getResourceAsStream(srcPackagePathFile.toString())) {
				ERR.notNull(stream, "InputStream from package", srcPackagePathFile, "is NULL");
