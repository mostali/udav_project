...
public enum EXT {
	...
	public static void testTwo() {
		ERR.state(Arrays.equals(two("0"), new String[]{"0", null}), "0");
		ERR.state(Arrays.equals(two("0."), new String[]{"0", ""}), "0.");
		ERR.state(Arrays.equals(two("0.1"), new String[]{"0", "1"}), "0.1");
		ERR.state(Arrays.equals(two(".1"), new String[]{"", "1"}), ".1");
		ERR.state(Arrays.equals(two("."), new String[]{"", ""}), ".");
		P.exit();
	}

	public static final EXT[] SET_VIDEO = Arr.of(MP4, _3GP);
	...
	public static EXT ofExt(String ext) {
		...
		for (EXT t : values()) {
			...
			if (extName.equalsIgnoreCase(ext)) {
				return t;
			} else if (extName.startsWith("_") && t.ext != null && t.ext.equalsIgnoreCase(ext)) {
				return t;
			}
		}
		return $$$UND_EXT$$$;
	...
	public static Predicate<Path> buildPredicate(boolean ignoreCase, String... exts) {
		return path -> {
			...
			for (String en : exts) {
				if ((en.length() + 1 == ext.length()) && STR.endsWith(ext, en, ignoreCase, true)) {
					return true;
				}
			}
			return false;
	...
	public static Predicate<Path> buildPredicate(boolean ignoreCase, Enum... ext) {
		return path -> {
			...
			for (Enum en : ext) {
				if (EN.eq(extPath, en, ignoreCase)) {
					return true;
				}
			}
			return false;
