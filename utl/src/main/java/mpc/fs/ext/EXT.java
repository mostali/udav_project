...
public enum EXT {
	...
	TXT, PROPS, PROPS$$, PROPERTIES, LOG, YAML, MD, HTML, CSS, JS, JAVA, GROOVY, XML, ZUL, JSON, SH, PY, META, STATE, HEAD, CSV, SQL, MSV,//
	...
	//	public static final EXT[] G_IMGF = {PNG, JPG, JPEG, WEBP, BMP, GIF};
	...
	public static final EXT[] G_EDITABLE = {GROOVY, JAVA, JS, SH, PY, CSS, ZUL, HTML, PROPS, PROPERTIES, XML, JSON, YAML, LOG, TXT, MD, MSV};
	...
	public static final EXT[] G_BI = {MSV};

	public static final String SPEC_PROPS = "props..";
	public static final String __SPEC_PROPS = ".props..";
	...
	public static EXT of(String file, EXT... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Except ext from '%s'", file), defRq);
	}
	...
	public static EXT ofExtRq(String ext, EXT... defRq) {
		...
		switch (e) {
			case $$$EMP_EXT$$$:
				return ARG.throwMsg(() -> "File without extension", defRq);
			case $$$UND_EXT$$$:
				return ARG.throwMsg(() -> X.f("File has undefined extension '%s'", ext), defRq);
			case $$$NO_EXT$$$:
				return ARG.throwMsg(() -> "File with empty extension", defRq);
			default:
				return e;
		}
	}
	...
	private String nameAsExt() {
		return isNumFirstExt() ? ext.toLowerCase() : (this == PROPS$$ ? SPEC_PROPS : name().toLowerCase());
	}
	...
	public List<Path> lsAll(Path pathFc, List<Path>... defRq) {
		...
		return ls_filter(UFS.ls(pathFc, defRq));
	}

	public boolean hasIn(List<Path> paths) {
		return paths.stream().anyMatch(p -> EXT.of(p) == this);
	}
}
