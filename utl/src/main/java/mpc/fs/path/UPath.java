...
public class UPath {
	...
	public static Path item(Path path, int index, Path... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Path by index '%s' not found", index), defRq);
	}
	...
	public static Path last(Path path, Path... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Except path last item from '%s'", path), defRq);
	}
