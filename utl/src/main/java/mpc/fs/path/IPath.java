...
//		}
	...
	default String fLine0(String... defRq) {
		try {
			List<String> strings = RW.readLinesFast(toPath(), 0);
			return X.notEmpty(strings) ? strings.get(0) : ARG.throwMsg(() -> X.f("Lines not found from file '%s'", toPath()), defRq);
		} catch (Exception ex) {
	...
	default Map<GEXT, List<Path>> dMapGExt(Map<GEXT, List<Path>>... defRq) {
		...
		return ARG.throwMsg(() -> "except dir map", defRq);
	}
	...
	default List<Path> dLsGEXT(GEXT... gexts) {
	...
	default boolean fdRmIfExist() {
		return UFS.RM.deleteDirBool(toPath());
	}
