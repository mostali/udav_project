...
public class UPathToken {
	...
	public static void test() {
		ERR.state(first(Paths.get("/1/2/3"), "2", true).equals(Paths.get("/1/2")), "test1");
		ERR.state(first(Paths.get("/1/2/3"), "2", true).equals(Paths.get("/1/2/")), "test1-1");
		ERR.state(first(Paths.get("1/2/3"), "2", true).equals(Paths.get("1/2")), "test2");

		ERR.state(first(Paths.get("/1/2/3"), "2", false).equals(Paths.get("/1")), "test3");
		ERR.state(first(Paths.get("1/2/3"), "2", false).equals(Paths.get("1")), "test4");
		//
		//
		ERR.state(last(Paths.get("/1/2/3"), "2", true).equals(Paths.get("2/3")), "test-L-1");
		ERR.state(last(Paths.get("/1/2/3"), "2", true).equals(Paths.get("2/3/")), "test-L-1-1");
		ERR.state(last(Paths.get("1/2/3"), "2", true).equals(Paths.get("2/3")), "test-L-2");

		ERR.state(last(Paths.get("/1/2/3"), "2", false).equals(Paths.get("3")), "test-L-3");
		ERR.state(last(Paths.get("1/2/3"), "2", false).equals(Paths.get("3")), "test-L-4");
		ERR.state(last(Paths.get("1/2/3/"), "2", false).equals(Paths.get("3")), "test-L-4");
		ERR.state(last(Paths.get("1/2/3/"), "2", false).equals(Paths.get("3/")), "test-L-4");
	}
	...
	public static Path pathItems(Path path, int count_start_end, Path... defRq) {
		ERR.isNotZero(count_start_end);
		boolean first = count_start_end > 0;
		List<Path> many = first ? ArrItem.firstMany(path.iterator(), count_start_end, null) : ArrItem.lastMany(path.iterator(), Math.abs(count_start_end), null);
		if (many != null) {
