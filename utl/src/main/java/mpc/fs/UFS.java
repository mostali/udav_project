...
//		X.exit(c1.equals(c2));
	...
	public static boolean existDir(String path, boolean... withContent) {
		return existDir(Paths.get(path), withContent);
	}
	...
	public static boolean isFile0(Path destination) {
		return destination != null && !Files.isDirectory(destination);
	}
	...
	public static List<Path> ls(Path path, List... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Except dir '%s'", path), defRq);
	}

	public static List<Path> lsFiles(Path path, List... defRq) {
		return ls(path, Files::isRegularFile, defRq);
	}

	@SneakyThrows
	public static List<Path> lsDirs(Path path, List... defRq) {
		return ls(path, Files::isDirectory, defRq);
	}
	...
	public static void writeTmpFile(String data) {
		...
		RW.write(file, data);
		L.info("Created tmp file " + UF.PFX_FILE_LN + file);
	}
	...
	public static Path getFileExisted(String path, Path... defRq) {
		...
		return Files.isRegularFile(path0) ? path0 : ARG.throwMsg(() -> X.f("Except existed file '%s'", path), defRq);
	}

	public static Path getDirExisted(String path, Path... defRq) {
		Path path0 = Paths.get(path);
		return Files.isDirectory(path0) ? path0 : ARG.throwMsg(() -> X.f("Except existed folder '%s'", path), defRq);
	}
	...
	public static void p(List<Path> paths, Logger... logger) {
		Sb sb = new Sb();
		paths.forEach(p -> sb.NL(QDate.of(p.toFile()) + " - " + p));

		Logger loggerUse = ARG.toDefOr(L, logger);
		ARG.applyVoidIfDef(l -> l.info(sb.toString()), loggerUse);
	}
}
