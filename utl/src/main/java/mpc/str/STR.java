...
//		Sys.exit(substr("hello", 1, 2));
		...
		P.exit(ArrItem.lastMany(list1, 0));
		Sys.exit(ArrItem.lastMany("1234", 3));
...
//		boolean obj = list.retainAll(AR.as(1, 2, 54));
		...
		Sys.exit(UST.BD("01"));
		Sys.exit(ArrItem.item(Arr.as(1, 2), 12, null));
		Sys.exit(UST.INT("7d"));
...
//		U.exit("2:" + two[2]);
	...
	public static String removeStartPfxWithCheckRq(String exp, String pfx, boolean... isPfxRq) {
		boolean hasPfx = exp.startsWith(pfx);
		if (!hasPfx) {
			if (ARG.isDefEqTrue(isPfxRq)) {
				throw new FIllegalStateException("Prefix '%s' is required for expression '%s'", pfx, exp);
			}
		} else {
			exp = exp.substring(pfx.length());
		}
		return exp;
	}
	...
	public static boolean contains(Collection<String> seq, String needle, boolean ignoreCase, boolean eqSafe) {
		return seq.stream().filter(s -> EQ.equalsString(s, needle, ignoreCase, eqSafe)).findAny().isPresent();
	}

	public static boolean containsIC(CharSequence seq, String needle) {
		return StringUtils.containsIgnoreCase(seq, needle);
	}

	public static boolean contains(CharSequence seq, String needle, boolean ignoreCase) {
		return ignoreCase ? StringUtils.containsIgnoreCase(seq, needle) : StringUtils.contains(seq, needle);
	}
	...
	public static boolean containsAll(String val, boolean ignoreCase, String... any) {
		ERR.notEmpty(any);
		for (String needle : any) {
			if (!contains(val, needle, ignoreCase)) {
				return false;
	 ...
	 * *************************************************************
	 */
	public static boolean startsWithAny(String val, boolean ignoreCase, String... any) {
		return startsWithAny(val, ignoreCase, true, any);
	}

	public static boolean startsWithAny(String val, boolean ignoreCase, boolean safeNullEq, String... any) {
		ERR.notEmpty(any);
		for (String needle : any) {
			if (startsWith(val, needle, ignoreCase, safeNullEq)) {
				return true;
			}
		}
		return false;
	...
	public static boolean startsWithNoOne(String val, boolean ignoreCase, boolean safeNullEq, String... any) {
		ERR.notEmpty(any);
		for (String needle : any) {
			if (startsWith(val, needle, ignoreCase, safeNullEq)) {
				return false;
...
//	@Deprecated
//	public static boolean startsWith_RMM(CharSequence seq, String needle, boolean safeNullEq, boolean ignoreCase) {
//		if (seq == null && safeNullEq) {
//			return false;
//		}
//		return ignoreCase ? StringUtils.startsWithIgnoreCase(seq, needle) : StringUtils.startsWith(seq, needle);
//	}

	public static boolean startsWithIC(CharSequence seq, String needle) {
		return startsWith(seq, needle, true, true);
	}

	public static boolean startsWith(CharSequence seq, String needle, boolean ignoreCase, boolean safeNullEq) {
		if ((seq == null || needle == null) && safeNullEq) {
			return false;
		}
		return ignoreCase ? StringUtils.startsWithIgnoreCase(seq, needle) : StringUtils.startsWith(seq, needle);
	}
	 ...
	 * *************************************************************
	 */
	public static boolean endsWithIC(CharSequence seq, String needle) {
		return endsWith(seq, needle, true, true);
	}

	public static boolean endsWithAny(String val, boolean ignoreCase, String... any) {
		return endsWithAny(val, ignoreCase, true, any);
	}

	public static boolean endsWith(CharSequence seq, String needle, boolean ignoreCase, boolean safeNullEq) {
		if (seq == null && safeNullEq) {
			return false;
		}
		return ignoreCase ? StringUtils.endsWithIgnoreCase(seq, needle) : StringUtils.endsWith(seq, needle);
	}

	public static boolean endsWithAny(String val, boolean ignoreCase, boolean safeNullEq, String... any) {
		ERR.notEmpty(any);
		for (String needle : any) {
			if (endsWith(val, needle, ignoreCase, safeNullEq)) {
				return true;
			}
		}
		return false;
	...
	public static boolean endsWithNoOne(String val, boolean ignoreCase, boolean safeNullEq, String... any) {
		ERR.notEmpty(any);
		for (String needle : any) {
			if (endsWith(val, needle, ignoreCase, safeNullEq)) {
				return false;
	 ...
	 * *************************************************************
	 */
	public static boolean isContainsAllChar(String value, char _char) {
...
//	}
	...
	public static String[] getBBA(String str, String start, String end, boolean greedyLast, String[]... defRq) {
		try {
			...
			ERR.isNumber(indStart, -1, ERR.EQ.NE, "Start string not found", start, str);
			int indEnd = greedyLast ? str.lastIndexOf(end) : str.indexOf(end);
			ERR.isNumber(indEnd, -1, ERR.EQ.NE, "End string not found", start, str);
			ERR.isLT(indStart, indEnd);
	...
	public static String[] getBA(String data, String start, boolean greedyLast, String[]... defRq) {
		try {
			...
			ERR.isNumber(indStart, -1, ERR.EQ.NE, "Del index not found", start, data);
	...
	public static String insertBetween(String data, String start, String end, boolean greedyLast, String insertData, boolean keepStartEndString, String... defRq) {
		try {
			...
			ERR.isPosOrZero(indStart, "Start string not found", start, data);
			int indEnd = greedyLast ? data.lastIndexOf(end) : data.indexOf(end);
			ERR.isPosOrZero(indEnd, "End string not found", start, data);
	...
	public static boolean startsWithQk(String str, List<String> startPrefix1, List<String> startPrefix2) {
		for (String _startPrefix1 : startPrefix1) {
			for (String _startPrefix2 : startPrefix2) {
				if (startsWithQk(str, _startPrefix1 + _startPrefix2, false)) {
					return true;
				}
			}
		}
	...
	public static boolean startsWithQk(String str, String startPrefix, boolean isPareIsNull) {
		return startPrefix == null ? (str == null ? isPareIsNull : false) : (str.isEmpty() ? startPrefix.isEmpty() : str.startsWith(startPrefix));
	}

	public static boolean endWithQk(String str, String endPrefix, boolean isPareIsNull) {
		return endPrefix == null ? (str == null ? isPareIsNull : false) : (str.isEmpty() ? endPrefix.isEmpty() : str.endsWith(endPrefix));
	}

	public static boolean containsIgnoreCaseAny(String str, String... needle) {
		return containsIgnoreCaseAny(str, Arrays.asList(needle));
	}

	public static boolean containsIgnoreCaseAny(String str, Collection<String> needle) {
		if (X.empty(needle)) {
			return false;
		}
		for (String needl : needle) {
			if (StringUtils.containsIgnoreCase(str, needl)) {
				return true;
			}
		}
		return false;
	...
	public static boolean containsIgnoreCaseAll(String str, String... needle) {
		if (X.empty(needle)) {
			return false;
		}
		for (String needl : needle) {
...
//		}
		return ERR.notNull(str).chars().mapToObj(e -> (char) e).map(String::valueOf).collect(Collectors.toList());
	}
...
//			result.append(String.format(format, row.toArray(new String[0]))).append("\n");
	...
	public static String substrIfStartWith(String str, String startWith, boolean ignoreCase) {
		return startsWith(str, startWith, ignoreCase, true) ? str.substring(startWith.length()) : str;
	}

	public static String substrIfEndsWith(String str, String startWith, boolean ignoreCase) {
		return endsWith(str, startWith, ignoreCase, true) ? str.substring(startWith.length()) : str;
	}
//	public static String substrIfEndsWith(String startWith, boolean ignoreCase, String... args) {
//		UC.NN(args);
//		for (String arg : args) {
//			if (US.endsWith(arg, startWith, true, ignoreCase)) {
//				return arg.substring(startWith.length());
//			}
//		}
//		return null;
//	}

	public static String choiceFirstNotEmpty(String... args) {
		for (String arg : args) {
			if (X.notEmpty(arg)) {
				return arg;
			}
	...
	public static String wrapTag(Object data, String tag, Object... args) {
		if (args.length == 0) {
			return wrapTag(data, tag);
		}
		return "<" + tag + " " + JOIN.SPACE(args) + ">" + data + "</" + tag + ">";
	}
...
//	}
	...
	public static String substrIf(String str, int... maxLength) {
		return ARGi.isDef(maxLength) ? STR.substr(str, 0, ARGi.toDef(maxLength), str) : str;
	}
	...
	public static String substr(String str, int startIndex, int endIndex, String... defRq) {
		try {
			ERR.isPosOrZero(startIndex);
			if (startIndex == endIndex) {
				return "";
			}
			ERR.isGT(endIndex, startIndex);
	...
	public static boolean isUpperCase(String str, int... index) {
		return str != null && !str.isEmpty() && Character.isUpperCase(str.charAt(ARGi.toDefOr(0, index)));
	}
