...
public class ObjTo {
	...
	public static <T> T anyTo(Object objectOrCharSequence, Class<T> clazz) {
		ERR.notNull(objectOrCharSequence);
	...
	public static <T> T objToRq(Object obj, Class<T> clazz) {
		...
		if (clazz.isAssignableFrom(obj.getClass())) {
			...
			return obj instanceof Number ? (T) toPrimitiveNumber((Number) obj, clazz) : (T) toPrimitiveNoNumber((Number) obj, clazz);
	...
	private static Object toPrimitiveNoNumber(Object obj, Class primitiveClass) {
		if (obj == null) {
			return getInitValueForPrimitive(primitiveClass);
		}
	...
	public static Object getInitValueForPrimitive(Class primitiveClass) {
		ERR.state(primitiveClass.isPrimitive(), "except primitive class [%s]", primitiveClass);
		if (primitiveClass == Integer.TYPE) {
			return (int) 0;
		} else if (primitiveClass == Long.TYPE) {
			return (long) 0;
