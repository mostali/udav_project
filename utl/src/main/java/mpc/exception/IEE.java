...
public interface IEE<E> {
	...
	default boolean is(Throwable err, boolean... ignoreCase) {
		return err instanceof EException && EN.eq(((EException) err).type(), type(), ignoreCase);
	}

	public default String getMessageAny() {
		Exception err = eerr();
		String msg = UErr.getCauseMessageOr(err.getCause(), null, null);
		return msg == null ? err.getMessage() : msg;
	}

	static <E extends Enum> boolean is(Throwable err, Enum type, boolean... ignoreCase_or_typeEq) {
		return err instanceof EException && EN.eq(((EException) err).type(), type, ignoreCase_or_typeEq);
	}

	static String getCleanMessage(IEE error, boolean... any) {
		ICleanMessage clnMsg = UErr.getCleanMessageType((Throwable) error);
		if (clnMsg != null) {
		...
		String msg = UErr.getCauseMessageOrMessage((Throwable) error);
		return msg != null ? msg : error.type().name();
	}

	static String getMessage(IEE iee, String parentMessage) {
	...
	static String toString(IEE iee, String parentMessage) {
		...
		if (ee.getCause() != null && X.notEmpty(ee.getCause().getMessage()) && !msgt.equals(ee.getCause().getMessage())) {
			List<String> errors = UErr.getAllMessages(ee.getCause());
