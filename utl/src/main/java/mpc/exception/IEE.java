...
public interface IEE<E> {
	...
	static <T> T convertTo(Throwable t, Class<T> type, T... defRq) {
		return type.isAssignableFrom(t.getClass()) ? type.cast(t) : ARG.throwMsg(new RequiredRuntimeException("Throwable '%s' is not compatible type '%s'", t.getClass().getSimpleName(), type.getSimpleName()), defRq);
	}
