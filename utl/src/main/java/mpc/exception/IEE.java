...
public interface IEE<E> {
	...
	static <T> T convertTo(Throwable t, Class<T> type, T... defRq) {
		return type.isAssignableFrom(t.getClass()) ? type.cast(t) : ARG.throwErr(new RequiredRuntimeException("Throwable '%s' is not compatible type '%s'", t.getClass().getSimpleName(), type.getSimpleName()), defRq);
	}
	...
	static String getMessageV1(IEE iee, String messageFromSuper) {
	...
	static String getMessageV0(IEE iee, String parentMessage) {
		String clnMsg = getCleanMessage(null);
		if (clnMsg != null) {
			return clnMsg;
		}
		Throwable ee = (Throwable) iee;
		String del = STR.DELR;
		String mmsg = iee.type().name().equals(parentMessage) ? parentMessage : iee.type() + del + parentMessage;
		return ee.getCause() == null ? mmsg : X.empty(ee.getCause().getMessage()) ? mmsg : iee.type() + del + ee.getCause().getMessage();
	}

	static String getMessageV2(IEE iee, String parentMessage) {
		Throwable ee = (Throwable) iee;
