...
public class ERR {
	...
	public static <T> T isEq(T checkable, Object condition, boolean safeEquals, Object... message) {
		if (mpc.core.EQ.equals(checkable, condition, safeEquals)) {
			return checkable;
		}
	...
	public static <T> Collection<T> isLength(Collection<T> checkable, long length, Object... message) {
		...
		throw new CheckException(ARG.isNotDef(message) ? X.f("Checked collection length must be equals Length(%s)==Collection(%s)", length, checkable.size()) : STR.formatAll(message));
	}
	...
	public static <T> T[] isLength(T[] checkable, long length, Object... message) {
		...
		throw new CheckException(ARG.isNotDef(message) ? X.f("Checked array length must be equals Length(%s)==Array(%s)", length, checkable.length) : STR.formatAll(message));
	}
	...
	public static <A> A[] isIndex(int index, A[] array, Object... message) {
	...
	public static <N extends Number & Comparable> N isMinMax(N num, N[] minMax) {
		if (minMax.length > 1) {
			return ERR.isBetweenEQ(num, minMax[0], minMax[1]);
		} else {
			return ERR.isGE(num, minMax[0]);
		}
	}
	...
	public static <T> T NNS(T type, Object... message) {
		if (type == null || "null".equalsIgnoreCase(type.toString())) {
	...
	public static <T> Collection<T> NE(Collection<T> type, Object... message) {
		return notEmpty(type, message);
	}
	...
	public static <T> Collection<T> notEmpty(Collection<T> type, Object... message) {
	...
	public static Object notNull(Map map, String key, Object... message) {
		return ERR.NN(ERR.NN(map, MAP_IS_EMPTY).get(ERR.NN(key, KEY_IS_EMPTY)), message);
	}
	...
	public static String isPathname(String path, Object... message) {
		if (path == null || !StringUtils.containsNone(path, ILLEGAL_CHARACTERS_DIRNAME)) {
	...
	public static Path isFileWithContent(Path file, Object... message) {
		if (!UFS.isExistFileWithContent(file)) {
	...
	public static Path isFileOrNotExist(Path file, Object... message) {
		return file != null || Files.notExists(file) || Files.isRegularFile(file) ? file : isFileExist(file, message);
	}
	...
	public static String isDirname(String dirname, Object... message) {
		if (StringUtils.containsNone(dirname, ILLEGAL_CHARACTERS_DIRNAME)) {
			return dirname;
		}
	...
	public static Path isDirWithContent(Path dir, Object... message) {
		...
		if (UFS.isDirWithContent(dir, false)) {
			return dir;
		}
	...
	public static void isDateAfter(Date first, Date last) {
		ERR.state(first.compareTo(last) < 0, "date '%s' must be after date '%s'", first, last);
	}

	public static void isDateBefore(Date first, Date last) {
		ERR.state(first.compareTo(last) > 0, "date '%s' must be before date '%s'", first, last);
	}
	...
	public static void isClassOf(Class var, Class type, Object... message) {
	...
	public static <T> T isTypeGson0(InputStream inputStream, Class<T> type, Object... message) {
		if (inputStream != null) {
	...
	public static <T> T isTypeGson0(Reader reader, Class<T> type, Object... message) {
		if (reader != null) {
	...
	public static <T> T isTypeGson0(CharSequence json, Class<T> type, boolean showJsonContent, Object... message) {
		if (json != null) {
		...
		throw new CheckException(ARG.isNotDef(message) ? X.f("Object [%s] is not gson-type [%s]", toStringVar(json, showJsonContent), type) : STR.formatAll(message));
	}
	...
	public static class NumberComparator<L extends Number & Comparable, R extends Number & Comparable> implements Comparator<L>, IEqualable<L, R> {
		public static final IEqualable<Number, Number> EQ = new NumberComparator(ERR.EQ.EQ) {
		};
		public static final IEqualable<Number, Number> NE = new NumberComparator(ERR.EQ.NE) {
		};
		public static final IEqualable<Number, Number> LT = new NumberComparator(ERR.EQ.LT) {
		};
		public static final IEqualable<Number, Number> LE = new NumberComparator(ERR.EQ.LE) {
		};
		public static final IEqualable<Number, Number> GE = new NumberComparator(ERR.EQ.GE) {
		};
		public static final IEqualable<Number, Number> GT = new NumberComparator(ERR.EQ.GT) {
		};
		...
		public NumberComparator(ERR.EQ ceq) {
			this.ceq = ceq;
		}
		...
		public NumberComparator of(ERR.EQ eq) {
			switch (eq) {
				case EQ:
