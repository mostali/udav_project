...
public class RuProps implements Serializable, IMap<String, String>, IPath {
	...
	public void writeMap_(Path... path) throws IOException {
		Path _path = ERR.notNull(ARG.toDefOr(this.path, path), "set path");
	...
	public String toString() {
		String pfile = pathStr == null ? null : STR.pfile(pathStr);
	...
	public Path path() {
		ERR.notEmpty(pathStr, "RuProps is not file");
	...
	public Map<String, String> readMap_(boolean createFileIfNotExist, boolean fresh, Path... path) throws IOException {
		...
		if (_path != null) {
			...
			if (createFileIfNotExist) {
				UFS_BASE.MKFILE.createFileIfNotExist_(_path);
			}
		}
	...
	public String getString(String key, String... defRq) {
		return UMap.get(readMap_(), key, defRq);
	}
	...
	public <T> T getAs(String key, Class<T> type, T... defRq) {
		return UMap.getAs(readMap_(), key, type, defRq);
	}


	@SneakyThrows
	public void delete() {
		UFS_BASE.RM.deleteDir(path());
	}
	...
	public boolean existsFile() {
		return pathStr == null ? false : UFS.existFile(path());
	}
	...
	public void mkfileIfNotExist(Boolean mkDir_mkDirs_Not) {
		if (!existsFile()) {
			UFS_BASE.MKFILE.createFileIfNotExist(path(), mkDir_mkDirs_Not);
		}
	}
