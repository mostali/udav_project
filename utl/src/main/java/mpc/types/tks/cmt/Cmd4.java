...
public class Cmd4<K, V, E, O1> extends Cmd3<K, V, E> {
	...
	public static <K, V, E, O1> Cmd4<K, V, E, O1> of4(String cmd, String[] cmd4, OBJ<K> key, OBJ<V> val, OBJ<E> ext, OBJ<O1> opt1) {
		key = toEqOrAny(ArrItem.item(cmd4, 0, null), key);
		val = toEqOrAny(ArrItem.item(cmd4, 1, null), val);
		ext = toEqOrAny(ArrItem.item(cmd4, 2, null), ext);
		opt1 = toEqOrAny(ArrItem.item(cmd4, 3, null), opt1);
