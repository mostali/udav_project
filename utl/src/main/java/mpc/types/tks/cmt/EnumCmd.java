...
public class EnumCmd<ECMD extends Enum> extends Cmd {
	...
	public static void main(String[] args) {
		EnumCmd enCmd = new EnumCmd("a ---b -1_2", DEF_3DASH_SEP);
		Map map1 = enCmd.getMapEnumed(ECmd.b);
		Object o = map1.get(ECmd.b);
		String b = enCmd.getCmdBody(ECmd.b, true, true, true);
		X.exit(b);
	}

	public static final String DEF_3DASH_SEP = "\\s+---";

	private Map<ECMD, String> map;
	private List<String> skipPartIfNotExist = new LinkedList<>();
...
//		List<E> vls = EN.getValues(type);
	...
	public EnumCmd setSkipPartIfNotExist(List<String> skipPartIfNotExist) {
		this.skipPartIfNotExist = skipPartIfNotExist;
		return this;
	}

	public EnumCmd(String cmd, String splitterRx) {
		super(cmd);
		this.splitterRx = IT.NE(splitterRx);
	}

	public Map<ECMD, String> getMapEnumed(ECMD... ecmd) {
	...
	public String getMapValue(ECMD eCmd) {
		return getMapEnumed(eCmd).get(eCmd);
	}
	...
	public boolean hasKey(String key, boolean... ignoreCase) {
		try {
			getCmdBody(key, ARG.isDefEqTrue(ignoreCase), false, false);
			return true;
		} catch (Exception e) {
			return false;
		}
	...
	public String getCmdBody(ECMD key, boolean ignoreCase, boolean next, boolean trim, String... defRq) {
		return getCmdBody(key.name(), ignoreCase, next, trim, defRq);
	}
	...
	public String getCmdBody(String key, boolean ignoreCase, boolean next, boolean trim, String... defRq) {
		Map.Entry<ECMD, String> e = null;
		try {
			e = getCmdEntry(key, ignoreCase);
		} catch (Exception ex) {
	...
	public Map.Entry<ECMD, String> getCmdEntry(String key, boolean ignoreCase, Map.Entry<ECMD, String>... defRq) {
		for (Map.Entry<ECMD, String> e : getMapEnumed().entrySet()) {
	...
	public static <ECMD extends Enum> String toStringFromMap(EnumCmd<ECMD> typedCmd, String splitter) {
		ECMD[] vls = (ECMD[]) typedCmd.getMapEnumed().keySet().toArray(new Enum[0]);
