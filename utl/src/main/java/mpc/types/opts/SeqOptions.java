...
//		SeqOptions seq = SeqOptions.ofStrictKeys("a", "b", "-c", "d", "e", "f", "-g", "--h", "-h", "-1000");
	...
	public String getSingle(String[] anyKey, String... defRq) {
		...
		return ARG.throwMsg(() -> X.f(MSGF1_SINGLE_KEY, ARR.of(anyKey)), defRq);
	}
	...
	public String getSingleOrFile(String key, String... defRq) {
		try {
			...
			if (!STR.startsWith(key, UF.PFX_FILE_LN)) {
				return single;
			}
			String file = key.substring(UF.PFX_FILE_LN.length());
			IT.isFileExist(file, "File '%s' not found", file);
			return RW.readString(Paths.get(file));
		} catch (Exception ex) {
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "getSingleOrFile throw error with key '%s'", key), defRq);
		}
	}
	...
	public String getSingle(String key, String... defRq) {
		...
		return ARG.throwMsg(() -> X.f(MSGF1_SINGLE_KEY, key), defRq);
	}
	...
	public List<String> getSingleAll(String key, List<String>... defRq) {
		...
		return X.notEmpty(all) ? all : ARG.throwMsg(() -> X.f("Arg's by key '%s' is required", key), defRq);
	}
...
//	@Deprecated
	...
	public Boolean hasDouble(String key, Boolean... defRq) {
		...
		return ARG.throwErr(new RequiredRuntimeException("Double Arg [%s] is required", key), defRq);
	}
	...
	public Boolean hasDoubleIgnoreCase(String key, Boolean... defRq) {
		...
		return ARG.throwErr(new RequiredRuntimeException("Double Arg [%s] is required, ignoreCase", key), defRq);
	}
