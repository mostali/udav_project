...
public abstract class AbsRunOptions {
	...
	public static void test(AbsRunOptions opts) {
		...
		//test 1
		{
			...
			ERR.isNull(r, "need null");
			Sys.p("Test1 is ok.");

			try {
				opts.getSingle("sd");
				ERR.notNull(null, "test 2 need throw error");
			} catch (Exception ex) {
				Sys.p("Test1.1 is ok:" + ex.getMessage());
			}

			String name = opts.getSingle("name");
			ERR.isEq(name, "S1", "test1.3 eq names");
		...
		//test 3
		{
			...
			ERR.isFalse((Boolean) r, message, "sd");
			Sys.p(message);

			message = "test 2.1 hasDoubleQk:true";
			ERR.isTrue(opts.hasDouble("skip.md5",false), message, "skip.md5");
			Sys.p(message);

			message = "test 2.2 hasDoubleRq throw error";
			try {
				opts.hasDouble("sd");
				ERR.notNull(null, message, "sd");
			} catch (Exception ex) {
