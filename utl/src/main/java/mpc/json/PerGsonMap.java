...
public class PerGsonMap<V> extends GsonMap<V> implements IPath {
	...
	public static PerGsonMap ofOrCreate(Path path) {
		UFS_BASE.MKFILE.createEmptyFileIfNotExist(path, true);
		return of(path);
	}
	...
	public static PerGsonMap of(Class rsrsClass, String rsrcPath) {
		...
		return new PerGsonMap(UGson.toMapFromString(json), rsrc.path());
	}
	...
	public PerGsonMap getAsGsonMap(String key, GsonMap... defRq) {
		...
		return childJson != null ? newEmpty(true) : (PerGsonMap) ARG.toDefThrow(() -> new RequiredRuntimeException("PerGsonMap Child '%s' not found, from map '%s'", key, fPath()), defRq);
	}
	...
	public String toString() {
		return ToString.toStringRfl(this);
	}

	public PerGsonMap clone() {
		return PerGsonMap.of(fPath());
	}
	...
	private static class ChildPerGsonMap extends PerGsonMap {
		...
		public ChildPerGsonMap(PerGsonMap parent, String key, Map childJson) {
			super(key, childJson, parent.fPath());
			this.parent = parent;
		}
