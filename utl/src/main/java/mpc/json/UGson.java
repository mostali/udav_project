...
//	}
	...
	public static Map toMapFromString(CharSequence json, Map... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException(finalErr, "Illegal json '%s'", json), defRq);
	}
	...
	public static JsonArray JA(String response, JsonArray... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Error parse JsonArray from json '%s' (%s)", STR.substrKeepStartEndAndInsertBetween(response, 10, 10, "...", response)), defRq);
		}
	}
	...
	public static <T> T toObjectFromMap(Map map, Class<T> asType, T... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Error parse type '%s' from map '%s'", asType, map), defRq);
		}
	}
	...
	public static Map<Integer, JsonObject> toMapJson_fromListAnyObject(Collection<Object> items) {
		...
		return S_.mapToSet(items, UGson::toJsonObjectFromAnyObject).stream().collect(Collectors.toMap(js -> ctr.getAndIncrement(), js -> js));
	}
	...
	public static <T> T toObject(JsonPrimitive obj, Class<T> clazz, T... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Wrong Value [" + obj + "] for type [" + clazz + "]"), defRq);
	}
...
//			System.out.println(serialized);
	...
	public static <T> T getValueSimpleAs(JsonElement je, String key, Class<T> asType, T... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "JP '%s' as type '%s' not found", key, asType), defRq);
		}
	}
	...
	public static String getValueSimpleOr(JsonElement je, String[] keys, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("JP '%s' not found by keys", ARR.as(keys)), defRq);
	}
	...
	public static String getValueSimple(JsonElement je, String key, String... defRq) {
		...
		if (true) {
			return ARG.throwErr(() -> new RequiredRuntimeException("JP '%s' not found by key", key), defRq);
		}
		JsonElement jeByKey = null;
		if (je != null && je.isJsonObject()) {
			jeByKey = je.getAsJsonObject().get(key);
...
//		}
	...
	public static <T> T objTo(JsonPrimitive json, Class<T> castTo, T... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Type '%s' not found for casting '%s'", castTo, json), defRq);
	}
