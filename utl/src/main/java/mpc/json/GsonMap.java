...
public class GsonMap<V> implements Map<String, V>, Serializable, IRfl, IGetterAs {
	...
	public static void main(String[] args) {
		GsonMap gsonMap = GsonMap.of("\\{\"key\":\"val\"\\}"); // from json string
		GsonMap gsonMap1 = GsonMap.of(Map.of("k1", "v1", "k2", Map.of("k3", Arrays.asList(1, 2, 3)))); // from Map
		GsonMap gsonMap2 = GsonMap.of(Paths.get("file.json"), true); // from new file
		GsonMap gsonMap3 = GsonMap.ofObj(new Pare("key", "val")); // from pojo

		String prettyJson = gsonMap1.toStringPrettyJson();
	}
...
//	}
	...
	public static GsonMap of(Path fileJson, GsonMap... defRq) {
		try {
			...
			return ARG.throwErr(() -> ex instanceof RequiredRuntimeException ? (RequiredRuntimeException) ex : new RequiredRuntimeException(ex, "Error parse file with json: %s", fileJson), defRq);
		}
	}
	...
	public static GsonMap of(String json, GsonMap... defRq) {
		try {
			...
			return ARG.throwErr(() -> ex instanceof RequiredRuntimeException ? (RequiredRuntimeException) ex : new RequiredRuntimeException(ex, "Error parse json string: %s", json), defRq);
		}
	}
	...
	public V get(Object key, V... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("GsonMap Value by key '%s' not found", key), defRq);
	}
	...
	public <T> T getAs(String key, Class<T> asType, T... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("GsonMap Value typeof '%s' by key '%s' not found", asType, key), defRq);
		}
	}
...
//	}
	...
	public List<GsonMap> getAsArrayGsonMap(String key, List<GsonMap>... defRq) {
		...
		return child != null ? child : ARG.throwErr(() -> new RequiredRuntimeException("getAsArrayGsonMap by key '%s' not found", key), defRq);
	}
	...
	public GsonMap getAsGsonMap(String key, GsonMap... defRq) {
		...
		return child != null ? newEmpty(true) : ARG.throwMsg(() -> X.f("GsonMap not found value typeof '%s' by key '%s' not found", GsonMap.class, key), defRq);
	}
...
//		}
	...
	public GsonMap child(String key, GsonMap... defRq) {
		...
		return childJson != null ? childJson : ARG.throwErr(() -> new RequiredRuntimeException("ChildGson '%s' not found", key), defRq);
	}
