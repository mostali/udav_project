...
public class GsonMap<V> implements Map<String, V>, Serializable, IRfl {
	...
	public static GsonMap read(Path file, boolean... createIfNotExist) {
		if (ARG.isDefEqTrue(createIfNotExist)) {
			UFS_BASE.MKFILE.createFileIfNotExistWithContentMkdirs(file, UGson.EMPTY);
		}
	...
	public CharSequence toStringPrettyJson() {
		return UGson.toStringPretty(toStringJson().toString());
	}
...
//	}
	...
	public static GsonMap ofKV(Object... keyValues) {
		return new GsonMap(UMap.of(keyValues));
	}
	...
	public Long getAsLong(Object key, Long... defRq) {
		return getAs(key, Long.class, defRq);
	}

	public Integer getAsInt(Object key, Integer... defRq) {
	...
	public List getAsArray(String key, List<GsonMap>... defRq) {
		List child = (List) get(key, null);
		if (child != null) {
			return child;
		}
