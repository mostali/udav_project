...
public class S_ {
	...
	public static <T, O> List<O> itToList(Iterable<T> items, Function<T, O> function) {
		return ARR.toList(items).stream().map(function).collect(Collectors.toList());
	}

	public static <T, C extends Collection<T>, O> Set<O> mapToLinkedSet(C items, Function<T, O> function) {
		return items.stream().map(function).collect(Collectors.toCollection(LinkedHashSet::new));
	}
