...
public class Arr {
	...
	public static <T> T[] merge(T a, T[] b) {
		...
		T[] c = (T[]) Array.newInstance(type, aLen + bLen);
	...
	public static <T> List<T> ar(T t, Collection<T> all) {
	...
	public static <T> List<T> ar(T... objs) {
	...
	public static <T> Set<T> asHSet(T... collection) {
		return new HashSet() {
			{
				for (T t : collection) {
					add(t);
	...
	public static <T> Set<T> asLSet(T... collection) {
	...
	public static ArrayList ar(Collection lines) {
	...
	public static <T> T cutFirstElement(List<T> slice, int... indexCutableElement) {
		ERR.NE(slice);
		int index = ARGi.isDef(indexCutableElement) ? indexCutableElement[0] : 0;
		ERR.isBetweenLEQ(index, 0, slice.size());
	...
	public static <T> boolean contains(Collection<T> arr, T state) {
		if (X.notEmpty(arr)) {
			for (T t : arr) {
				if (t == state || EQ.equalsUnsafe(t, state)) {
					return true;
				}
			}
		}
	...
	public static <T> boolean contains(T[] arr, T state) {
		if (X.notEmpty(arr)) {
			for (T t : arr) {
				if (t == state || EQ.equalsUnsafe(t, state)) {
					return true;
	...
	public static <T> List<List<T>> partition(List<T> objs, int part, Integer... requiredEven) {
		if (ARG.isDefNNF(requiredEven)) {
			ERR.isEven(objs.size(), ARG.toDef(requiredEven));
		}
	...
	public static <T> List<List<T>> partition(T[] objs, int part, Integer... requiredEven) {
		if (ARG.isDefNNF(requiredEven)) {
			ERR.isEven(objs.length, ARG.toDef(requiredEven));
		}
	...
	public static void trimListSizeByHead(List<List<Object>> rows) {
		int headRowSize = -1;
		List head = ERR.notEmpty(rows).get(0);
		if (head.isEmpty()) {
			return;
		} else {
