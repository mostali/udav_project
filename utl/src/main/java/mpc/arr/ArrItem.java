...
public class ArrItem {
	...
	public static <T> T firstAs(Collection collection, Class<T> as, T... defRq) {
		...
		return ARG.toDefThrow("Item is null", defRq);
	}
	...
	public static <K, V> Map.Entry<K, V> first(Map<K, V> collection, Map.Entry<K, V>... defRq) {
		return first(collection.entrySet(), defRq);
	}
	...
	public static <T> List<T> firstMany(Iterator<T> iterator, int count, List<T>... defRq) {
		...
		throw new RequiredRuntimeException("Item's size '%s' not equals required count '%s'", items.size(), count);
	}
	...
	public static <T> T item(Iterable<T> list, int index, T... defRq) {
		...
		throw new RequiredRuntimeException("Item by index [%s] not found", index);
	}
	...
	public static <T> T item(T[] args, int index, T... defRq) {
		...
		throw new RequiredRuntimeException("Try get item by index '%s' from array with size '%s'", index, args.length);
	}
