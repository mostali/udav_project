...
public class EnvTlp implements ITlp {
	...
	public static EnvTlp ofHlpRs(String usrName, EnvTlp... defRq) {
		...
		return UFS.existFile(srcFileHLP) ? new EnvTlp(srcFileHLP) : ARG.throwErr(() -> new RequiredRuntimeException("Env hlp no found:" + usrName), defRq);
	}
	...
	public static EnvTlp ofHlpOrg(String orgName, String usrName, EnvTlp... defRq) {
		...
		return UFS.existFile(srcFileHLP) ? new EnvTlp(srcFileHLP) : ARG.throwErr(() -> new RequiredRuntimeException("Env org hlp no found:" + orgName), defRq);
	}
	...
	public static EnvTlp ofHlpVkUsrTk(String name, EnvTlp... defRq) {
		...
		return UFS.existFile(srcFileHLP) ? new EnvTlp(srcFileHLP) : ARG.throwErr(() -> new RequiredRuntimeException("Env usrtk hlp no found:" + name), defRq);
	}
	...
	public static EnvTlp ofHlp(String parent, String name, EnvTlp... defRq) {
		...
		return UFS.existFile(srcFileHLP) ? new EnvTlp(srcFileHLP) : ARG.throwErr(() -> new RequiredRuntimeException("Env %s hlp no found:%s", parent, name), defRq);
	}
	...
	public static EnvTlp ofSysAcc(String sys, String acc, EnvTlp... defRq) {
		...
		return UFS.existFile(srcFileHLP) ? new EnvTlp(srcFileHLP) : ARG.throwErr(() -> new RequiredRuntimeException("Env no contain sys '%s' with entity '%s' not found", sys, acc), defRq);
	}
	...
	public static Path usrPath(String usr, String fileOrDir, Path... defRq) {
		...
		return UFS.exist(fileOrDirPath) ? fileOrDirPath : ARG.throwErr(() -> new RequiredRuntimeException("Except existed usrPath '%s'", fileOrDirPath), defRq);
	}
