...
public class APP {
	...
	//blacklist learn mode
	public static final String APK_BBL_MODE = "app.bbl.mode";
	...
	public static final String NS_TASKS = "data/tasks";
	public static final String RPA_NS_ENV = "env";
...
//	public static final AppPropDef0<Integer> APP_TOP_MAXSLEEPSEC = new AppPropDef0("app.top.maxSleepSec", (int) TimeUnit.DAYS.toSeconds(1)).desc("maxSleepSec for updating fetch posts");
	...
	@AutoInitValue(prop = APK_BBL_MODE)
	public static BBL BBL_MODE = BBL.LEARN;
...
//	public static String RPA;
	...
	public static GsonTree TREE_GNCJ() {
		return GsonTree.ofGsonTree(RPA_NS_ENV, "gncj", true);
	}
...
//		return Env.RPA.resolve("gnc.sqlite");
		return AppCore0.of().path(RPA_NS_ENV, "gnc.sqlite");
	}    //global name context

	public static Path TREE_USER_STATE() {
		return AppCore0.of().path(RPA_NS_ENV, "users-state.sqlite");
	}

	public static Path TREE_RADIO() {
		return AppCore0.of().path(RPA_NS_ENV, "radio.sqlite");
	}

	public static Path TREE_BBL() {
		return AppCore0.of().path(RPA_NS_ENV, "bblist.sqlite");
	}

	public static Path TREE_BBLIP() {
		return AppCore0.of().path(RPA_NS_ENV, "bbl-ip-index.sqlite");
	}

	public static Path TREE_VERSIONED() {
		return AppCore0.of().path(RPA_NS_ENV, "versioned.sqlite");
	}

	//global user log
	public static Path TREE_GUL() {
		return AppCore0.of().path(RPA_NS_ENV, "gul.sqlite");
	}
	...
	public static Path TREE_GND_TASKS_V1() {
		return AppCore0.of().path(NS_TASKS, "gnd.sqlite");
	}

	@Deprecated //global daemons
	public static Path TREE_GNDD_V1() {
		return AppCore0.of().path(NS_TASKS, "gndd.sqlite");
	}
	...
	public static String getNetName() {
		return getAppNet(NT.DEF).name();
	}

	public static NT getAppNet(NT... defRq) {
...
//		return appName != null ? appName : AP.get(null, APK_APP_NAME, defRq);
		return APP_NAME != null ? APP_NAME : loadAppName_fromSEA(defRq);
	}

	public static String loadAppName_fromSEA(String... defRq) {
		String appName = getPropFrom_Sys_Env_AP(APK_APP_NAME, null);
		return appName != null ? appName : ARG.throwMsg(() -> X.f("Except app.name (from sys|env|application.properties)"), defRq);
	}
	...
	public static AppCore0 getAppCore0() {
		return AppCore0.of(APP.getAppNet());
	}
...
//			String portStr = dPortStr != null ? dPortStr : AP.get("server.port", null);
			if (portStr == null) {
				return ARG.throwMsg(() -> X.f("Property 'server.port' not found"), defRq);
			}
...
//			return appHost != null ? appHost : AP.get(null, APK_APP_HOST, defRq);
		...
		public static String getAppUrlWithPlaneAndPath(String plane, String page, String item, Pare... args) {
			String url2host = getAppUrlWithPlaneAndPath(plane, page + "/" + item);
			return QueryArg.joinToUrl(url2host, args);

		}

		public static String getAppUrlWithPlaneAndPath(String plane, String path, Pare... args) {
			String url2host = getUsedHttpProtocol() + (X.notEmpty(plane) ? plane + "." : "") + UUrl.normUrl(getAppHost0(), path);
			return QueryArg.joinToUrl(url2host, args);
		}
	...
	public static class LOCATION {
		...
		public static Path getAppDataDir(String dirNameFromData) {
			return Env.RPA_ROOT().resolve(dirNameFromData);
		}
...
//		return IT.isFileExist(Env.HOME_LOCATION.resolve(".data/gsv/data/db$projects.sqlite"));
	...
	public static Integer getVkBotId(Integer... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("App Prop '%s' required", APK_TG_BT_ID), defRq);
	}
	...
	public static String relativizeAppFile(Path path, String... removePfxPath) {
		...
		return ARG.isDef(removePfxPath) ? STR.removeStart(relPath, removePfxPath[0]) : relPath;
	}
