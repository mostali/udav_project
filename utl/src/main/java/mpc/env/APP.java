...
//	}
	...
	public static String getAppDomain(String... defRq) {
	...
	public static Path getAppDataDir(boolean requiredOrDefAppname) {
		if (requiredOrDefAppname) {
			return Env.getAppDataDir(Env.getAppName());
		} else {
			return Env.getAppDataDir(Env.getAppNameOrDef());
		}
	}
	...
	public static Path getPathAP(int... mode) {
		...
		switch (mode[0]) {
			case 0:
				Path resolve = getAppDataDirOrDef().resolve(Env.FILE_APPLICATION_PROPERTIES);
				if (UFS.existFile(resolve)) {
					return getAppDataDirOrDef();
				}
				return null;
			case 1:
