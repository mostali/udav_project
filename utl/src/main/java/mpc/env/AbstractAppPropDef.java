...
//		}
	...
	private T findValueFrom_STOREs(boolean checkTree, boolean checkEnv, boolean checkSys, boolean checkAP, T... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Value '%s' from STORE's not found, checkTree:%s,  checkSys:%s,  checkAP:%s ", getPropName(), checkTree, checkSys, checkAP), defRq);
	}
	...
	public T getDefaultValueFromHolder_or_Supplier(T... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Value '%s' from DEFAULT not found, checkSupplier:%s ", getPropName(), hasDefaultSupplier), defRq);
	}


	public <T> T getAs(Class<T> asType, T... defRq) {
		return ARG.throwErr(() -> new RequiredRuntimeException("Set tree or not call this method, key=%s", getPropName()), defRq);
	}
