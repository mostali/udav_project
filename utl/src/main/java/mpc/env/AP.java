...
public class AP {
	...
	public static boolean isDebugEnable() {
		return "true".equals(get(APP_DEBUG, null));
	}
	...
	public static String getAppName(String... defRq) {
		return get(null, APP_NAME, defRq);
	}

	public static void deleteMe() {
		UFS_BASE.RM.removeFileQuickly(Env.FILE_APPLICATION_PROPERTIES);
	}
	...
	public static Map<String, String> getMap(AppProfile appProfile, Map... defRq) {
		...
		Map<String, String> map = getApplicationProperties_RL(appProfile, null);
		if (map != null) {
	...
	public static Map<String, String> getApplicationProperties_RL(AppProfile appProfile, Map... defRq) {
		FILE of = FILE.of(getApFilename(appProfile));
		if (of.exist()) {
	...
	public static Map<String, String> getApplicationProperties_RSRC(AppProfile appProfile, Map... defRq) {
		String cnt = RES.of(Env.class, "/" + getApFilename(appProfile)).cat(null);
		if (cnt != null) {
	...
	private static String getApFilename(AppProfile appProfile) {
	...
	private static String getPropertyFromAppData(String key) {
		...
		if (X.empty(appName)) {
			appName = NT.DEF.name0();
		}
		FILE of = FILE.of(Env.getAppDataDir(appName).resolve(Env.FILE_APPLICATION_PROPERTIES));
		if (!of.exist()) {
			return null;

		}
		Map ruPropertiesClassic = URuProps.getRuPropertiesClassic(of.readLines());
