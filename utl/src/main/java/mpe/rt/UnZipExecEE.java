...
//		UFS_BASE.MV.moveWithReplaceExisting_()
	...
	public static String unzip(String file, Boolean overwrite_or_skip, String encoding) throws ExecRq {
		Path fileZip = ERR.isFileExist(Paths.get(file));
		Path workDir = fileZip.getParent();
		if (overwrite_or_skip != null) {
			String[] opts = {overwrite_or_skip ? "-o" : "-n"};
			if (X.notEmpty(encoding)) {
				opts = Arr.addArrayElements(opts, "-O", encoding);
			}
...
//		String command = "unzip " + X.toString(opts, "") + " -d " + ERR.isDirExist(outDir) + " " + ERR.isFileExist(file);
		String[] cmds = {"unzip"};
		if (X.notEmpty(opts)) {
			cmds = Arr.addArrayElements(cmds, opts);
		}
		cmds = Arr.addArrayElements(cmds, "-d", ERR.isDirExist(outDir), ERR.isFileExist(file));
		return ExecRq.exec(false, new File(ERR.isDirExist(workDir)), cmds).getMessage();
	}
