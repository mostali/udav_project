...
public class ZipExecEE extends EException {
		...
		public boolean check(Path dstZip) throws ZipExecEE {
			...
			switch (this) {
				...
				case REPLACE:
					...
					UFS_BASE.RM.removeQuicklyFileOrDir(dstZip);
					ERR.isDirOrFileNotExist(dstZip);
					return true;
				default:
	...
	public static Path zipAny(Path runProcessLocation_orNull, Path dstZip, Path any_files_and_dir, CreateZipOption IF_DST_EXIST_OPTION) {
		return zipAny(runProcessLocation_orNull, dstZip, Arr.as(any_files_and_dir), IF_DST_EXIST_OPTION);
	}

	public static Path zipAny(Path runProcessLocation_orNull, Path dstZip, List<Path> any_files_and_dir, CreateZipOption IF_DST_EXIST_OPTION) {
		ExecRq exe = null;
		Exception err = null;
		try {
			File runFrom = runProcessLocation_orNull == null ? null : runProcessLocation_orNull.toFile();
