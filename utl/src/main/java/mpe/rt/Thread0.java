...
public class Thread0<R> extends Thread {
	...
	public static Thread byName(String threadName, Thread... defRq) {
		List<Thread> threads = Sys.getThreads(threadName, ARR.EMPTY_LIST);
		return ARRi.first(threads, defRq);
	}

	public static void observe(Thread0 thread, int sec, FunctionV finish) {
		new Thread(() -> {
			while (thread.isAlive()) {
				SLEEP.sec(sec, "observe thread:" + thread.getNameWithId());
			}
			finish.apply();
		}).start();
	}
...
//	}
	...
	public static String getNameId() {
		return getNameId(Thread.currentThread());
	}
