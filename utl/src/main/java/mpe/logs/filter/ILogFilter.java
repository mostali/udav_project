...
public interface ILogFilter {
	...
	static String toStringFnPart(List<String> mergeFiles) {
		...
		fnames = fnames.stream().map(fn -> USToken.trim(fn, ((Predicate<Character>) ch -> Character.isAlphabetic(ch)).negate())).collect(Collectors.toList());
		fnames = fnames.stream().map(fn -> fn.length() <= 6 ? fn : STR.substrStartEndInsert(fn, 3, 3, "-", fn)).collect(Collectors.toList());
