...
public class LogProcArgs {
	...
	public LogProcArgs(LogGetterDate getterDate, Map<String, String> keys) {
		this(getterDate, UMap.getNE(keys, BY_KEY, null), UMap.getNE(keys, BY_MINUTES, null), UMap.getNE(keys, BY_DATE, null), UMap.getNE(keys, BY_TIME_START, null), UMap.getNE(keys, BY_TIME_END, null), null, null);
	}
	...
	public LogProcArgs(LogGetterDate getterDate, String _byKey, String _byMinutes, String _byDate, String _byTimeStart, String _byTimeEnd, String _byKeyStart, String _byKeyEnd) {
		...
		this.getterDate = getterDate == null ? getterDate = new LogGetterDate(FDate.APP_STANDART_LOG_DATE_FORMAT, QDate.now().year) : getterDate;

		this._byKey = _byKey;
		this._byMinutes = _byMinutes;
		this._byDate = _byDate == null ? null : ERR.isDate(_byDate, getterDate.format);


		if (X.notEmptyAnyString(_byTimeStart, _byTimeEnd)) {
			if (_byTimeStart != null) {
				ERR.NN(_byTimeEnd, "also set end date");
			} else if (_byTimeEnd != null) {
				ERR.NN(_byTimeEnd, "also set start date");
			}
			ERR.isFalse(_byTimeStart.equals(_byTimeEnd), "start/end date is equals");
			ERR.isDateAfter(UST.DATE(_byTimeStart, getterDate.format), UST.DATE(_byTimeEnd, getterDate.format));
	...
	public boolean hasAnyFilter() {
		return X.notEmptyAnyString(_byKey, _byMinutes, _byDate, _byTimeStart, _byTimeEnd);
	}
	...
	public LogProc buildLogLinesProcessor() {
		...
		if (X.notEmpty(_byMinutes)) {
			logLinesProcessor.addLineCondition_ByLastMinutesAgo(ERR.isInt0(_byMinutes));
		}
