...
//		ArrayList<String> strings = logLinesProcessor.processFile("/home/dav/pjbf_tasks/72/content/server.log.2023-12-01-544", logGetterDate, false);
		P.exit(strings);
		logLinesProcessor.addFilter(new LineLevelCondition(UMap.of(Lev.INFO, true)));
...
//		Date end = FDate.parse("20231109120015", FDate.YYYYMMDDmmhhss);
		...
		List<Path> twins = UFS.findTwins(Paths.get("/home/dav/pjbf_tasks/67/server.log"), true);
		P.exit(twins);
...
//		P.exit("ok");

		{
			...
			P.exit(UFS.findTwinsReport(dirWS, dirFR, true));
		}
		...
		LogLineBlock[] logLines = LogFile.parseLogLinesBlock("/home/dav/pjbf_tasks/62/2/test/dlc.log", LogGetterDate.buildDefault());
		LogLineBlock[] logLines2 = LogFile.parseLogLinesBlock("/home/dav/pjbf_tasks/62/2/test/server.log", LogGetterDate.buildDefault());
