...
public class LogStat {
	...
	private static Logger L = LoggerFactory.getLogger(LogStat.class);
...
//		Map<String, ThreadInfo> collectStats = collectStatsByGroup("/home/dav/pjbf_tasks/62/4/mr.server.LL-EFIWDT_S20231109115630-E20231109115715_sl-1700210574IC.log", LogGetterDate.buildDefault());
		Map<String, ThreadInfo> collectStats = collectStatsByThread("/home/dav/pjbf_tasks/62/4/mr.server.LL-EFIWDT_S20231109115630-E20231109115715_sl-1700210574IC.log", LogGetterDate.buildDefault());
	...
	public static Map<String, ThreadInfo> collectStatsByThread(String file, LogGetterDate logGetterDate) {
		...
		for (LogLineBlock logLineBlock : logLineBlocks) {
			LogLine logLine = LogLine.of(logLineBlock.lines[0]);
	...
	public static class ThreadInfoByThread extends ThreadInfo {
		...
		public String toString() {
			String sources = history.stream().map(p -> p.getVal()).collect(Collectors.toSet()).stream().collect(Collectors.joining(" " + SYMJ.GALAXY + " "));
			return super.toString() + X.f(" >>> >>> %s %s", SYMJ.GALAXY, sources);
		}
	...
	public static class ThreadInfoByGroup extends ThreadInfo {
		...
		@Override
		public String toString() {
			String sources = history.stream().map(p -> p.getVal()).collect(Collectors.toSet()).stream().collect(Collectors.joining(" " + SYMJ.ARROW_MOVABLE + " "));
			return super.toString() + X.f(" >>> >>> %s %s", SYMJ.ARROW_MOVABLE, sources);
		}
	}
