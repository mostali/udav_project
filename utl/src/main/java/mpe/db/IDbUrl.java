...
import mpe.cmsg.std.SqlCallMsg;
import mpu.IT;
import mpu.X;
import mpu.core.ARG;
import mpu.core.ARR;
...
//	}
	...
	static IDbUrl ofULP(List<String> url_login_pass) {
		IT.hasLength(url_login_pass, 3, "except 3 elements [url,login,pass]");
		return new IDbUrl() {
			@Override
			public String[] toULP() {
			...
			public String[] toLoginPass(String[]... defRq) {
				try {
					...
					return ARG.throwErr(() -> new RequiredRuntimeException(ex, "toLoginPass"), defRq);
				}
			}
	...
	static IDbUrl of(Path path) {
		return new IDbUrl() {
			@Override
			public String getDbUrlString() {
				return SqlCallMsg.LINE0 + "sqlite:" + path;
			}
	...
	default Path toPath(Path... defRq) {
		if (getSqlPlatform() != SQLPlatform.SQLITE) {
			return ARG.throwMsg(() -> X.f("Only sqlite database support path"));
		}
	...
	static SQLPlatform getPlatform(String jdbcTypeDb, SQLPlatform... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Unknown sql db", jdbcTypeDb), defRq);
	}
	...
	static SQLPlatform getSqlPlatformFromJdbcUrl(String jdbcUrl, SQLPlatform... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("Illegal jdbcUrl '%s' (without SqlPlatform)", jdbcUrl), defRq);
	}

	default String[] toLoginPass(String[]... defRq) {
		return ARG.throwErr(() -> new RequiredRuntimeException("except impl login pass"), defRq);
	}
