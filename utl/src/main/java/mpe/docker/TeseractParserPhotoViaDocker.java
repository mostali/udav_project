...
public class TeseractParserPhotoViaDocker {
	...
	//	public static void main(String[] args) throws ExecRq, IOException, InterruptedException {
	//		P.exit(TeseractParserPhotoViaDocker.parsePhotoRus("/tmp/img0.png"));
	//	}
	...
	public static String parsePhotoRus(String file_photo) throws ExecRq {
		return parsePhotoRus(Paths.get(file_photo));
	}

	public static String parsePhotoRusByUrl(String photo_url) throws ExecRq {
		String fileTmp = "tmp/" + UUID.randomUUID() + ".png";
		try {
			DLD.url2file0(photo_url, fileTmp, true);
			return parsePhotoWithCheckImage(Paths.get(fileTmp), "rus");
		} catch (IOException e) {
			throw ExecRq.FAIL(new RuntimeException(e));
		} finally {
			new File(fileTmp).delete();
		}
	}

	public static String parsePhotoRus(Path file_photo) throws ExecRq {
		return parsePhotoWithCheckImage(file_photo, "rus");
	}

	public static String parsePhotoEng(Path file_photo) throws ExecRq {
		return parsePhotoWithCheckImage(file_photo, "eng");
	}

	public static String parsePhotoWithCheckImage(Path file_photo, String lang) throws ExecRq {
		try {
			...
			if (!STR.containsAll(msg, false, errMsg)) {
				throw execRq;
			}
			if (L.isInfoEnabled()) {
	...
	public static String parsePhoto(Path file_photo, String lang) throws ExecRq {
		if (L.isInfoEnabled()) {
			L.info("TESERACT({})/Start parse file_photo {}", UF.toAbsPathPfx(file_photo));
		}
