...
//	public static final String SHORT_NAME_RU = "ПУБЛИКАЦИЯ";
	...
	public static Integer getMinDelayBetweenPost(CallMsg callMsg, Integer... defRq) {
		return callMsg.getHeaderValueByKey("wall.post.delay.min", Integer.class, defRq);
	...
	public static List<String> getDstTypes(CallMsg callMsg) {
		return SPLIT.allBySpace(callMsg.getHeaderValueByKey("dst.types", ""));
	}
...
//    }
	...
	public Boolean getIsRandom(Boolean... defRq) {
		return getHeaderValueByKey("cl.random", Boolean.class, defRq);
	}
	...
	public static PublCallMsg of(Path file) {
		PublCallMsg callMsg = of(RW.readString(file));
	...
	public static class SingleSrcLine extends CallCmdLine implements Comparable<SingleSrcLine> {
		...
		@Override
		public String toString() {
			return line0 + " " + STR.DELR + (isValid() ? "ok" : getMultiOrSingleErrorOrNullStr());
		}
