...
public class SqlCallMsg extends CallMsg {
	...
	public static final String LINE0 = "jdbc:";
	...
	public String toString() {
		return "SqlCallMsg{" + "msg='" + msg + '\'' + ", line='" + line0 + '\'' + ", state=" + state + ", errs=" + X.sizeOf0(getErrors()) + '}';
	}
	...
	public IDbUrl getJdbcUrl(IDbUrl... defRq) {
		try {
			...
			String[] login = TKN.two(trimCommentPfx(getLinesMsg().get(0)), ":");
			IT.isEq(login[0], "login", "set line with login , e.g. --login:login");

			String[] pass = TKN.two(trimCommentPfx(getLinesMsg().get(1)), ":");
			IT.isEq(pass[0], "password", "set line with password, e.g. --password:password");

			return IDbUrl.ofULP(ARR.as(line0, login[1], pass[1]));
		} catch (Exception ex) {
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Illegal JdbcUrl from sql '%s'", getLinesMsg()), defRq);
		}
	}
	...
	public String getSql(String... defRq) {
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException("Illegal sql body", ex), defRq);
		}
	}

	public List<String> getSqls(List<String>... defRq) {
		String sql = getSql(null);
		return sql != null ? SPLIT.allBy(sql, "\n--\n") : ARG.throwErr(() -> new RequiredRuntimeException("Illegal sql body"), defRq);
	}

	public static SqlCallMsg of(Path file) {
		SqlCallMsg callMsg = of(RW.readString(file));
	...
	public static SqlCallMsg of(INode node, TrackMap.TrackId... trackId) {
		...
		SqlCallMsg sqlCallMsg = (SqlCallMsg) ofQk(inject.nodeDataStrOrNull()).throwIsErr();
	...
	public static boolean isValidKey(String data) {
		return STR.startsWith(data, true, LINE0);
	}
