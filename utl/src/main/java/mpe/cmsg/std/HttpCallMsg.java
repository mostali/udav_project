...
public class HttpCallMsg extends CallMsg {
    ...
    public static final Class<? extends Enum> SUB0 = CON.Method.class;

    public static final String TID = "tid";

    private final String url;

    private final @Getter CON.Method http_method;

    @Override
    public CON.Method subtype(Object... defRq) {
        return http_method;
    }
    ...
    public String getJsonPath(String... defRq) {
        Optional<String> first = headers_i_body_com()[0].stream().filter(s -> s.startsWith("#$.")).findFirst();
        if (first.isPresent()) {
            first = Optional.of(first.get().substring(1));
        }
        return ARG.throwErrOpt(() -> new RequiredRuntimeException("Except json path in header"), first, defRq);
    }
...
//	}
    ...
    public static HttpCallMsg of(Path file) {
        HttpCallMsg callMsg = of(RW.readString(file));
