...
public class JarCallMsg extends CallMsg implements ILongCall {
	...
	public static final String LINE0 = "jartask:";
	...
	public JarCallMsg(String fullMsg) {
		this(fullMsg, false);
	}
	...
	public JarCallMsg(String fullMsg, boolean... lazyValid) {
		...
		String classWithMethodStr = TKN.lastGreedy(line0, LINE0, null);
		if (classWithMethodStr == null) {
			addError("Set KEY & class with method use pattern [%s:package.classname#methhod] in line '%s'", LINE0, line0);
			return;
		}
...
//			String[] args = msg.getHeaders_SEQARGS()
			rsltInvoke = RFL.invokeJarSt0(jcm.getJarPath(), jcm.getClassName(), classMethodName, new Class[]{String[].class}, new Object[]{args});
		} else {
			rsltInvoke = RFL.invokeJarSt0(jcm.getJarPath(), jcm.getClassName(), classMethodName, new Class[]{Object.class, Map.class}, new Object[]{null, jcm.getContext().asMap()});
		}
		return rsltInvoke;
	}
	...
	public String getClassName(String... defRq) {
		return X.notEmpty(className) ? className : ARG.throwMsg(() -> X.f("Set className"), defRq);
	}

	public String getClassMethodName(String... defRq) {
		return X.notEmpty(classMethodName) ? classMethodName : ARG.throwMsg(() -> X.f("Set methodName"), defRq);
	}
	...
	public Path getJarPath(Path parent, Path... defRq) {
		...
		if (dir0 == null) {
			return ARG.throwMsg(() -> X.f("set header param [dir] with optional param [jar.filename]"), defRq);
		}
		Path dir;
...
//			dir = UF.normParentCharacter(dir0, parent != null ? parent.getParent() : Env.RUN_LOCATION.toAbsolutePath());
		...
		if (jarFilename != null) {
			...
			return UFS.existFile(jarFile) ? jarFile : ARG.throwMsg(() -> X.f("File Jar '%s' not found from dir '%s'. Check param [dir] or optional param [jar.filename]", jarFile, dir), defRq);
		}
		List<Path> paths = IPath.of(dir).dLsEXT(EXT.JAR);
		if (X.empty(paths)) {
			return ARG.throwMsg(() -> X.f("File Jar not found from dir '%s'. May be need set optional param [jar.filename]?", dir), defRq);
		} else if (paths.size() > 1) {
//			return ARG.toDefThrowMsg(() -> X.f("Except only one jar file from dir '%s', but found '%s'", dir, STREAM.mapToList(paths, UF::fn)), defRq);
			return X.throwIllegalStateException(X.f("Except only one jar file from dir '%s'. May be need set optional param [jar.filename]?", dir));
		}
		return paths.get(0);
	}
...
//				"msg='" + fullMsg + '\'' +
	...
	private static JarCallMsg ofQk(String msg, boolean... lazyValid) {
