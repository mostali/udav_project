...
public class JqlCallMsg extends CallMsg {
	...
	public static String wrap(String lineValue) {
		return lineValue.startsWith(LINE0) ? lineValue : LINE0 + lineValue;
	}

	public Long getCacheTtlSec(Long... defRq) {
		return getHeaderValueByKey("cache.ttl.sec", Long.class, defRq);
	}

	public static class RunnerLocalJql {
		@SneakyThrows
		public static void main(String[] args) {
//            JqlCallMsg sendCallMsg = JqlCallMsg.of(Paths.get("/opt/appVol/bea/.planes/.index/jql/.forms/jql-one/AppNotes.props"));
			JqlCallMsg sendCallMsg = JqlCallMsg.of(Paths.get("/opt/appVol/bea/.planes/.index/jql/.forms/jql-many/AppNotes.props"));
			Object call = sendCallMsg.call(true);
			X.exit(call);
		}
	}

	public static JqlCallMsg ofAny(Object callMsg, JqlCallMsg... defRq) {
		JqlCallMsg callMsg0;
		if (callMsg instanceof JqlCallMsg) {
			callMsg0 = (JqlCallMsg) callMsg;
		} else if (callMsg instanceof CharSequence) {
			callMsg0 = of(callMsg.toString());
		} else if (callMsg instanceof INode) {
			callMsg0 = of(((INode<?>) callMsg).readNodeDataStr());
		} else if (callMsg instanceof Path) {
			JqlCallMsg msvCallMsg1 = of((Path) callMsg);
			callMsg0 = msvCallMsg1;
		} else {
			return ARG.throwMsg(() -> X.f("Except legal any object [%s]", callMsg), defRq);
		}
		return callMsg0;
	}
...
//        updateMsgWithBodyIfEmpty();

		Object rslt = ZBin.JIRA.newJarCall().invokeMsg(getMsg());
...
//	}
	...
	public static JqlCallMsg of(Path file) {
		JqlCallMsg callMsg = of(RW.readString(file));
	...
	public String getIssueKey(String... defRq) {
		String keyAsTaskUrl = getKeyAsTaskUrl(null);
		if (keyAsTaskUrl != null) {
			String last = TKN.last(keyAsTaskUrl, "/", null);
			if (last != null) {
				return last;
			}
		}
		return ARG.throwMsg(() -> X.f("JqlCallMsg '%s' except key as IssueKey line0 : %s", toObjMsgId(), line0), defRq);
	}
