...
public class NodeID implements IErrorsCollector, INodeID {
	...
	public static final String COMS_DIR = ".coms";
	public static final String FORMS_DIR = ".forms";
	public static final String PAGES_DIR = ".pages";
	public static final String PLANES_DIR = ".planes";
...
//		NodeID.of("/page")
	...
	public static void testJUnit() {
		...
		testSingle("/nodeName", State.ITEM_I_PAGE);
		...
		testSingle("pageName/nodeName", State.ITEM_I_PAGE);
...
//		return path == null || path.length == 0 || path[0] == null ? "" : (path[0].equals(PAGE_INDEX_ALIAS) ? "" : path[0]);
	...
	public static Pare<String, String> wrapSdn(Pare<String, String> sdn) {
		return Pare.of(wrapPlane(sdn.key()), wrapPage(sdn.val()));
	}

	public static String toCanonicId(INodeID iNodeID) {
		return wrapPlane(iNodeID.spaceName()) + DEL + iNodeID.pageName() + DEL + iNodeID.nodeName();
	}

	public static String toCanonicId(String nodeID) {
		return NodeID.of(nodeID).canonicId();
	}
	...
	public enum State {
		EMPTY, SINGLE, ITEM_I_PAGE, FULL, BIG
	}
...
//			sdn = null;
	...
	@SneakyThrows
	public static NodeID of(String nodeID, NodeID... defRq) {
		NodeID node = ofQk(nodeID);
		if (!node.hasErrors()) {
			return node;
		}
		return ARG.throwErr(() -> node.getMultiOrSingleErrorOrNull(), defRq);
	}
	...
	public static NodeID of(Path file, NodeID... defRq) {
		...
		if (nameCount > 3) {
			...
			if (FORMS_DIR.equals(formsDir)) {
				NodeID nodeID = new NodeID(Pare.of(UF.name(file, 0), UF.name(file, 1)), UF.name(file, 3));
				if (nodeID.isValid()) {
					return nodeID;
				}
				return ARG.throwMsg(() -> nodeID.getMultiOrSingleErrorOrNullStr(), defRq);
			}
			return ARG.throwMsg(() -> X.f("Illegal path [%s] item for defined NodeID", formsDir), defRq);
		}
		return ARG.throwErr(() -> new RequiredRuntimeException("Except path count > 3 [%s]", file), defRq);
