...
public class NodeData<N extends INode> implements INode<N> {
	...
	public String readNodeDataStr(String... defRq) {
		return nodeDataStr != null ? nodeDataStr : ARG.throwMsg(() -> X.f("Except nodedata '%s'", nodeName()), defRq);
	}

	@Override
	public String line0(String... defRq) {
		String s = readNodeDataStr(null);
		return s != null ? ARRi.firstLine(s) : ARG.throwMsg("E", defRq);
	}
	...
	public String nodeDataStrOrNull() {
		return nodeDataStr;
	}
	...
	public <T extends CallMsg> T getCallMsg(T... defRq) {
		return callMsg != null ? (T) callMsg : ARG.throwMsg(() -> X.f("Except callMsg impl"), defRq);
	}
