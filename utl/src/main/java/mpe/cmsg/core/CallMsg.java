...
public class CallMsg extends ErrorCollector implements ICallMsg, Serializable {

	private static <T extends CallMsg> T __OF_STRING(Class<T> callMsgType, CharSequence data, T... defRq) {
		Object o = RFL.invokeSt(callMsgType, "of", ARR.of(String.class), ARR.of(data.toString()), data, defRq);
		return (T) o;
	}

	public static <T extends CallMsg> T ofAny(Object msg, boolean checkLazyInjectType, Class<T> callMsgType, T... defRq) {

		T callMsg;

		Function<String, T> ofStringCaller = (data) -> __OF_STRING(callMsgType, data, defRq);

		INodeInjector iNodeInjector = INodeInjector.get();

		if (callMsgType.isAssignableFrom(msg.getClass())) {
			callMsg = callMsgType.cast(msg);
			if (checkLazyInjectType) {
//				Object fromSrc = callMsg.getFromSrc();
//				if (fromSrc == null) {
//					return callMsg;
//				}
				NodeData inject = iNodeInjector.doInjectAny(callMsg, null);
				callMsg = __OF_STRING(callMsgType, inject.readNodeDataStr());
			}
		} else if (msg instanceof INode) {
			INode<?> src = (INode<?>) msg;
			callMsg = ofStringCaller.apply(src.readNodeDataStr());
			callMsg.setFromSrc(src);
			if (checkLazyInjectType) {
//				Object fromSrc = callMsg.getFromSrc();
				NodeData inject = iNodeInjector.doInject((INode) msg, null);
				callMsg = __OF_STRING(callMsgType, inject.readNodeDataStr());
			}
		} else if (msg instanceof CharSequence) {
			String string = msg.toString();
			if (checkLazyInjectType) {
				NodeData inject = iNodeInjector.doInject(new StringNode(string), null);
				string = inject.readNodeDataStr();
			}
			callMsg = ofStringCaller.apply(string);
		} else if (msg instanceof Path) {
			Path src = (Path) msg;
			if (checkLazyInjectType) {
				NodeData inject = iNodeInjector.doInject(new FileNode(src), null);
				callMsg = ofStringCaller.apply(inject.readNodeDataStr());
			} else {
				callMsg = ofStringCaller.apply(RW.readString(src));
			}
			callMsg.setFromSrc(src);
		} else {
			return ARG.throwMsg(() -> X.f("Except legal CallMsgType [%s] for any object [%s]", callMsgType, msg), defRq);
		}
		return callMsg;
	}
	...
	public boolean isEnableHeaderFlag(String key, Boolean... defRq) {
		return getHeaderValueByKey(key, Boolean.class, defRq);
	}
...
//	}
	...
	public CallMsg setFromSrc(Object src) {
		IT.isNull(this.fromSrc, "except null src");
		this.fromSrc = src;
		return this;
	}
