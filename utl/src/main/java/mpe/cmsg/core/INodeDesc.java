...
public interface INodeDesc {
	...
	default String stdTypeLC() {
		return stdTypeUC().toLowerCase();
	}
	...
	default ICallMsg newInstanceCallMsg(Path data) {
		ICallMsg callMsg = newInstanceCallMsg(RW.readContent(data));
		((CallMsg) callMsg).setFromSrc(data);
		return callMsg;
	}
