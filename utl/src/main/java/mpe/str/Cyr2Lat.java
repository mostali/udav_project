...
//	}
	public static String lat2cyr(String s) {
		...
		int i = 0;
		while (i < s.length()) {// Идем по строке слева направо. В принципе,
								// подходит для обработки потока
			char ch = s.charAt(i);
			...
			if (ch == 'J') { // Префиксная нотация вначале
				...
				case 'O':
					...
					break;
				case 'H':
					if (i + 1 < s.length() && toUpperCase(s.charAt(i + 1)) == 'H') { // проверка
																						// на
																						// постфикс
																						// (вариант
																						// JHH)
			...
			} else if (i + 1 < s.length() && toUpperCase(s.charAt(i + 1)) == 'H') {// Постфиксная
																					// нотация,
																					// требует
																					// информации
																					// о
				...
				case 'C':
					...
					break;
				case 'S':
					if (i + 2 < s.length() && toUpperCase(s.charAt(i + 2)) == 'H') { // проверка
																						// на
																						// двойной
																						// постфикс
						sb.append(ch('Щ', lc));
