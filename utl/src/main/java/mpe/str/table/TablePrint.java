...
//		TablePrint.toStringFromListList()
		X.exit(toStringFromListList(ARR.as(ARR.as(1, 2, "asd")), "asd"));
...
//		TableList tableList = new TableList(2, "Key", "Value");
	...
	public static String toStringFromListListWithCols(List cols, List<List> rows, String returnIfEmpty) {
		return X.empty(rows) ? returnIfEmpty : toStringFromListList(cols, rows, returnIfEmpty);
	}

	public static String toStringFromListList(List<List> rows, String returnIfEmpty) {
		if (X.empty(rows) || X.empty(rows.get(0))) {
			return returnIfEmpty;
		}
		List<String> cols = ARR.asListWithRange(rows.get(0).size() - 1);
		return X.empty(rows) ? returnIfEmpty : toStringFromListList(cols, rows, returnIfEmpty);
	}

	public static String toStringFromListList(List<List> rows, String returnIfEmpty, int limit) {
		String tbl = TablePrint.toStringFromListList(rows, returnIfEmpty);
		return rows.size() > limit ? "*** ROWS LIMIT(" + limit + ") HAPPENS ***\n" + tbl : tbl;
	}

	public static String toStringFromListList(List<String> cols, List<List> rows, String returnIfEmpty) {
		if (X.empty(rows)) {
			return returnIfEmpty;
		}
		List<String> colsAsStrings = (List) cols.stream().map(String::valueOf).collect(Collectors.toList());
		TablePrint printTable = TablePrint.of(colsAsStrings.toArray(new String[colsAsStrings.size()]));
		for (List<List> row : rows) {
			printTable.addRowAs(row.toArray());
		}
		return printTable.toStringBuilder(true).toString();
	}
