...
public class HttpCallMsg extends CallMsg {
	...
	public static boolean isValidKeyFirstLine(String data) {
		return USToken.first(data, " ", CON.Method.class, null) != null;
	}
	...
	public String sendHttpCall100_or_custom400_404(boolean... trimRight) throws IOException {
		try {
			String rsp = sendHttpCall_200_204(trimRight);
			return rsp;
	...
	public String sendHttpCall_200_204(boolean... trimRight) throws IOException {
		if (rsp != null) {
			return null;
		}
		rsp = HttpCallMsg.sendHttpCall0(this, String.class, 200, 204);
	...
	public <T> T sendHttpCallAs(Class<T> rspType, boolean... trimRight) throws IOException {
		return UST.strTo(sendHttpCall_200_204(trimRight), rspType);
	}

	public String sendHttpCallAsJson(boolean... trimRight) throws IOException {
		String rsp = sendHttpCall_200_204();
	...
	public static <T> T sendHttpCall0(HttpCallMsg httpCallMsg, Class<T> rspAs, Integer... successHttpCode) throws IOException {
		...
		switch (httpCallMsg.http_method) {
			case GET: {
				rsp = JHttp.GET_BODY(httpCallMsg.url(), httpCallMsg.getHeadersAsHeadersArgs(), httpCallMsg.getBodyAsString(), rspAs, successHttpCode);
				break;
			}
			case POST: {
				rsp = JHttp.POST_BODY(httpCallMsg.url(), httpCallMsg.getHeadersAsHeadersArgs(), httpCallMsg.getBodyAsString(), rspAs, successHttpCode);
				break;
			}
			case DELETE: {
				rsp = JHttp.DELETE_BODY(httpCallMsg.url(), httpCallMsg.getHeadersAsHeadersArgs(), httpCallMsg.getBodyAsString(), rspAs, successHttpCode);
				break;
			}
			case PUT: {
