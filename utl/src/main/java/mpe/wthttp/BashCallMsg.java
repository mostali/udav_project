...
public class BashCallMsg extends CallMsg {
	...
	public Object call(boolean throwIfHasError) {
		return invokeShFile(throwIfHasError);
	}
	...
	public Pare<Integer, List<String>> invokeShFile(boolean throwIfHasError) {
		...
		if (fileSh != null) {
			...
			if (workDir == null) {
				codePare = Sys.exec_filetmp("/bin/bash", super.fileData, null, throwIfHasError);
			} else {
				L.info("WorkDir*:" + UFS.ls(workDir));
				codePare = Sys.exec_filetmp("/bin/bash", super.fileData, workDir, throwIfHasError);
			}
			return codePare;
		}
