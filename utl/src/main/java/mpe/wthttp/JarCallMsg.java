...
public class JarCallMsg extends CallMsg {
	...
	public static Object invokeJarMethod(JarCallMsg msg) {
		msg.doLazyValid();
		return RFL.invokeJarSt_(msg.getJarPath(), msg.getClassName(), msg.getClassMethodName(), new Class[]{Object.class, Map.class}, new Object[]{null, msg.getContext().asMap()});
	}
	...
	public Path getJarPath(Path... defRq) {
		...
		Path dir = UF.normHomeCharacter(dir0);
		...
		List<Path> paths = IPath.of(dir).fLsEXT(EXT.JAR);
		if (X.empty(paths)) {
