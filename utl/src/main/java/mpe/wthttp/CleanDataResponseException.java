...
public class CleanDataResponseException extends RuntimeException implements ICleanMessage {
	...
	public static CleanDataResponseException ofJson(int status, Map json) {
		...
		if (APP.IS_DEBUG_ENABLE) {
			rsltJson = gsonMap.toStringPrettyJson(true);
		} else {
	...
	public static CleanDataResponseException ofNotEmptyRsp_or400(String rsp, Supplier<String> msg400) {
		if (X.empty(rsp)) {
//			throw RestStatusException.C400(UGson.toStringJson(MAP.of("error", msg400.get())));
			throw RestStatusException.C400(msg400.get());
		}
