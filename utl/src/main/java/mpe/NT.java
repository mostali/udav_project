...
public enum NT {
	...
	public static NT ofNET(String net, NT... defRq) {
		...
		return ARG.throwMsg(() -> X.f("Undefined net [%s]", net), defRq);
...
//				return ARG.toDefThrow(() -> new WhatIsTypeException(this), defRq);
	...
	public Path rpa(String... path) {
		Path resolve = Env.RPA.getParent().resolve(nameLC());
		return ARG.isDef(path) ? resolve.resolve(ARG.toDef(path)) : resolve;
	}

	public void set() {
		Env.setAppName(this, true);
	}
}
