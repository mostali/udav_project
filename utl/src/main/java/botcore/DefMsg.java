...
public enum DefMsg {
	...
	public static String buildAppsMsg(DefMsg type, String nt, Enum[] apps, String... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("DefMsg '%s', nt(%s), apps(%s) is empty", type, nt, ARR.as(apps)), defRq);
	}
	...
	public static String buildOwnerStartMessage() {
		return Env.getAppName().toUpperCase() + " Bot started " + QDate.now().mono6_h2m2s2();
	}
	...
	public static String readFirstDefMsg(DefMsg defMsg, String nt, String app, String... defRq) {
		...
		String finalNt = nt;
		String finalApp = app;
		return ARG.throwErr(() -> new RequiredRuntimeException("Rsrc '%s', nt(%s), app(%s) not found", finalNt, finalApp, defMsg), defRq);
	}
	...
	public static DefMsg ofMsg(String msg, DefMsg... defRq) {
		switch (msg.toLowerCase()) {
			...
			case "/help":
				return HELP;
			default:
				return ARG.throwErr(() -> new WhatIsTypeException(msg), defRq);
		}
	}
