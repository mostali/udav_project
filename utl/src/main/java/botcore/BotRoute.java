...
public class BotRoute extends BotBaseRoute {
	...
	public BotRoute() {
		...
		if (ano == null) {
			...
			if (isDefaultRoute()) {
				...
				ERR.NE(name, "illegal route class name (keyRoute is empty)", routeClass);
			}
		...
		ERR.isFalse(keyAliasRoute != null && isRx, "Illegal route configuration (key & keqEq)", cn(), keyAliasRoute, keyEq);
	...
	protected void afterUpdate(Object response) {
		if (response == null || response instanceof Number) {
			...
			sendMsg(Arr.asHSet((IClb) response));
		} else if (response instanceof Throwable) {
			sendError((Throwable) response);
		} else if (response instanceof ICleanMessage) {
			sendMsg(((ICleanMessage) response).getCleanMessage());
			...
			sendMsg((INetRsp) response);
		} else if (response instanceof Collection) {
			Collection collection = ERR.NE((Collection) response);
	...
	private Serializable sendMsg(INetRsp response) {
	...
	public Serializable sendMsgError(String msg, Object... args) {
		return sendMsg(SYMJ.FAIL_RED_THINK + X.f(msg, args));
	}

	public Serializable sendMsgWarn(String msg, Object... args) {
		return sendMsg(SYMJ.WARN + X.f(msg, args));
	}

	public Serializable sendMsgFail(String msg) {
		return sendMsg(SYMJ.WARN3 + msg);
	}
	...
	public Object sendError(Throwable ex) {
		if (ex instanceof CleanMessageRuntimeException) {
			...
			return sendMsg(SYMJ.WARN + ((CleanMessageRuntimeException) ex).getCleanMessage());
		} else if (ex instanceof NotifyMessageRtException) {
			NotifyMessageRtException nmsg = (NotifyMessageRtException) ex;
			return sendMsg(nmsg.getCleanMessageOrMessage());
		} else if (ex instanceof INetRsp.NetResponseException) {
			return sendMsg(((INetRsp.NetResponseException) ex).rsp);
		...
		if (App.IS_APP_DEBUG) {
			return sendMsg(err + "\n" + ex.getClass().getSimpleName() + ":" + UErr.getAllMessages(ex) + ":");
		}
		...
		return sendMsgFail(FSA.ERROR_SURPRISE_SERVER_ERROR);
	}
	...
	public Serializable sendMsg(String message) {
	...
	public Serializable sendMsg(Long toChatId, IBotMsg message) {
		throw new UnsupportedOperationException(cn() + ":sendMessage:" + toChatId + ":" + message);
	}

	public Object sendMsg(Collection<IClb> clb) {
		throw new UnsupportedOperationException(cn() + ":sendMessage:" + clb);
	}


	public Object sendMsg(String message, List<List<?>> keys) {
	...
	public static Serializable getEmsgId(BotRoute botRoute) {
		return botRoute.sendMsg(SYMJ.TIME_DONE + " Loading..");
	}
