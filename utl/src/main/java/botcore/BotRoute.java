...
public class BotRoute extends BotBaseRoute {
	...
	protected void afterUpdate(Object response) {
		if (response == null) {
			return;//|| response instanceof Number
...
//		else if (response.getClass().getSimpleName().contains("SendAudio")) {
////			Collection collection = IT.NE((Collection) response);
////			collection.forEach(i -> {
////				if (i instanceof CharSequence) {
...
//		}
	...
	public Serializable sendMsg_AUDIO(File file) {
...
//		}
		...
		return sendMsg_MD(STR.wrapPreAsMd(ERR.toStringRoleMode(ex)));
	}
