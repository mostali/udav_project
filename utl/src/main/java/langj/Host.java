...
public class Host<T> extends AbsType<T> {
	...
	public Host(T value) {
		super(ToString.uuid(), value, value == null ? null : (Class<T>) value.getClass());
	}

	public Host(Host<T> value) {
		this(ToString.uuid(), value);
	}
	...
	public static <T> Host<T> of(T host) {
		return host == null ? NULL : new Host(host, host.getClass());
	}

	public static <T> Host<T> of(T value, Class<T> type) {
	...
	public static Host eval(String code, Host host, Ctx ctx, int level) {
		...
		ERR.state(objPt.ecode() != ECode.ACTION);
	...
	public Host evalFieldAny_(String field) throws NoSuchFieldException, IllegalAccessException {
		checkNullType();
		AbsFieldType aft = AbsFieldType.any_(type(), val(), ERR.notBlank(field));
		return Host.of(aft.value_(), aft.type());
	}
