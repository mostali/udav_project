...
//		}
		switch (this) {
			...
			case CHAR: {
				Integer firstLastSE = STR.findFirstSE(code, CH.SQ.ch, CH.SQ.ch, true);
				return new ObjPt(code, this).ch(code.substring(firstLastSE));
			}

			case STRING: {
				Integer firstLastSE = STR.findFirstSE(code, CH.DQ.ch, CH.DQ.ch, true);
			...
			case NEW: {
				ERR.isTrue(code.startsWith("new "));
