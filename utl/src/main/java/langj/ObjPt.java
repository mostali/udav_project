		...
		switch (eCode) {
			case NUM:
				...
				if (numStr.code.contains(".")) {
					_type = Host.of(Double.parseDouble(numStr.code), Double.class);
				} else {
					_type = Host.of(Long.parseLong(numStr.code), Long.class);
				}
				break;
			case STRING:
				_type = Host.of(string, String.class);
				break;
			case CHAR:
				_type = Host.of(ch.charAt(0), Character.class);
				break;
			case BODY_RB: {
...
//				}
			...
			case CLASS:
				_type = Host.of(ERR.NN(clazz), clazz);
				break;
			case LINK:
				_type = Host.getLink(ctx, ERR.NN(link));
				break;
			case FIELD:
			...
			case METHOD:
				...
				try {
					...
					_type = Host.of(result, returnType);
			...
			case NEW:
				...
				if (X.blank(body)) {
					_type = new Host(RFL.inst(clazz), clazz);
					break;
				}
				String args = body;
...
//	}
	...
	public boolean isLastIndex() {
		return code.length() - 1 == ERR.isPosOrZero(index_last);
	}

	public boolean hasNextSymbol(int... plusLast) {
		return index_last + ARGi.toDefOr(0, plusLast) < code.length() - 1;
	}
