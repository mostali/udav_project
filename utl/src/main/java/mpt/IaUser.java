...
public interface IaUser extends IUser {
	...
	default FID firstFID(FID... defRq) {
		...
		return ARG.throwErr(() -> new RequiredRuntimeException("User '%s' not found from any nets*%s", getUserSid(), X.sizeOf(userNets)), defRq);
	}
	...
	default INetUser getNetOf(String NET, INetUser... defRq) {
		...
		return ARG.throwErrOpt(() -> new RequiredRuntimeException("User '%s' Net '%s' not found", getUserSid(), NET), first, defRq);
	}
