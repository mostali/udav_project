	...
	public static String toFull_Simple(TrmRsp rsp) {
		switch (rsp.status()) {
			...
			case OK:
				...
				if (X.isNullAll(msgWithCode, result)) {
				...
				return msgWithCode;
			case FAIL:
				return UErr.getStackTrace(rsp.getCause());
			default:
	...
	public static StringBuilder toFull_Report(TrmRsp rsp, int tabLevel, boolean withCause) {
		...
		if (rsp.isFail()) {
			...
			if (withCause) {
				...
				UErr.getAllMessages(src).forEach(pw::println);
			}
		} else if (rsp.getCause() != null) {
			pw.print(withCause ? NL : ": ");
			String msg = withCause ? rsp.getCause().toString() : UErr.getAllMessages(rsp.getCause()).toString();
			pw.print(TAB + "E:" + msg);
		}
		return rpr;
	}
