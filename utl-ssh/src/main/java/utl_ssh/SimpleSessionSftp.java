...
public class SimpleSessionSftp extends SessionSftp {
	...
	public void downloadFile2Dir(String srcFile, String destParentDir, boolean... mkdirsOrSingleDirOrCheck) throws SftpException {
		...
		try {
			...
			UFS_BASE.MKDIR.createDirsOrSingleDirOrCheckExist(Paths.get(destParentDir), mkdirsOrSingleDirOrCheck);
			if (L.isInfoEnabled()) {
	...
	public void uploadFile2Dir(String srcFile, String destParentDir, boolean... mkdiriFnotExist_recursiveOrNotRecursive) throws SftpException {
		if (L.isInfoEnabled()) {
			L.info("SSH:uploadFile2Dir [{}] to [{}], [mkdiriFnotExist_recursiveOrNotRecursive]={}", srcFile, destParentDir, ARG.toDefBoolean(mkdiriFnotExist_recursiveOrNotRecursive));
		}
		...
		if (X.empty(destParentDir)) {
			...
			// ses.printPwd();
			Boolean res = ARG.toDefBoolean(mkdiriFnotExist_recursiveOrNotRecursive);
			if (res == null) {
