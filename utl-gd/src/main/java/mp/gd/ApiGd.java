...
public class ApiGd {
	...
	public static Spreadsheet loadSpreadsheet(Path fileKeyPath, String sheetId, List<String> ranges, boolean... isIncludeData) throws IOException {
		...
		if (L.isInfoEnabled()) {
			if (L.isDebugEnabled()) {
				L.debug(SYMJ.JET + "loadValues_With_FileKey from spreadsheet [{}] [{}/{}], gd-key [{}]", _isIncludeData ? "META" : "SIMPLE", sheetId, ranges, fileKeyPath);
			} else {
				L.info(SYMJ.JET + "loadValues_With_FileKey from spreadsheet [{}] [{}/{}]", _isIncludeData ? "META" : "SIMPLE", sheetId, ranges);
			}
		}
		Sheets service3 = getSheetsService(fileKeyPath);
...
//			ValueRange response3 = get.execute();
	...
	public static void uploadValues(Path fileKeyPath, String sheetId, String commonRange, List<List> values) throws IOException {
		if (L.isInfoEnabled()) {
			L.info(SYMJ.JET + "GD: UPLOAD: Writing sheet:{} values:{} range:{}", sheetId, values.size(), commonRange);
		}
		ValueRange body = new ValueRange().setValues((List)values);
