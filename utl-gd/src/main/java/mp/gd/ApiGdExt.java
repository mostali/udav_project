...
public class ApiGdExt extends ApiGd {
	...
	public static class GoogleSheetData {
		...
		public static GoogleSheetData of(SheetInfoWithDataRange sheetInfo) {
			ERR.notEmpty(sheetInfo.sheetId, "SheetId argument is empty", sheetInfo);
			ERR.notEmpty(sheetInfo.listWithRange, "ListWithRange argument is empty", sheetInfo);
			ERR.notNull(sheetInfo.getCredentialsPathLib(), "CredentialsPathLib argument is empty", sheetInfo);
		...
		public static GoogleSheetData of(String userAliasName, String sheetId, String listWithRange) {
			ERR.notEmpty(sheetId, "SheetId argument is empty");
			ERR.notEmpty(listWithRange, "ListWithRange argument is empty");
			ERR.notEmpty(userAliasName, "UserAliasName argument is empty");
		...
		public List<GridData> get_GridData(boolean includeData, Integer... sheetIndex) {
			List<Sheet> sheets = loadSpreadsheet(includeData).getSheets();
			int _sheetIndex = ARG.isDef(sheetIndex) ? sheetIndex[0] : 0;
		...
		public static class ListRange {
			public static String toPattern(String list, String x, String y) {
				return ERR.notEmpty(list) + "!" + ERR.notEmpty(x) + ":" + ERR.notEmpty(y);
			}
			...
			public static void check(String listWithRange) {
				...
				ERR.notEmpty(listWithRange__[0], "SheetName is empty");
			}
		}
...
//		List<List<Object>> values;
		...
		if (userAlias_orPathKey instanceof String) {
			...
			ApiGdExt.writeValues_AuthPathKey((Path) userAlias_orPathKey, sheetId, range, rows);
		} else if (userAlias_orPathKey instanceof Integer) {
			Integer mode = (Integer) userAlias_orPathKey;
			switch (mode) {
				case 0:
	...
	public static void writeValues_AuthAuto(String sheetId, String listWithRange, List<List<Object>> rows) throws IOException {
		...
		if (keyPath != null) {
			...
			writeValues_AuthPathKey(Paths.get(keyPath), sheetId, listWithRange, rows);
			return;
		}
		String userAlias = AP.get("gs.key.alias", null);
		if (userAlias != null) {
	...
	public static void writeValues_AuthUserAlias(String userAlias, String sheedId, String commonRange, List<List<Object>> values) throws IOException {
		writeValues_AuthPathKey(findFileKeyPath(userAlias), sheedId, commonRange, values);
	}

	public static void writeValues_AuthPathKey(Path fileKeyPath, String sheedId, String commonRange, List<List<Object>> values) throws IOException {
	...
	public static Sheets getSheetsServiceByLib(String[] credentialsLib) throws IOException {
		...
		if (M2Repo.EnvRepo.contains(ERR.isLength(ERR.notNull(credentialsLib), 4))) {
			fileKeyAsLib = M2Repo.EnvRepo.repo.getLibFullPath(credentialsLib);
		}
		return getSheetsService(ERR.isFileWithContent(ERR.NN(fileKeyAsLib)));
	}
