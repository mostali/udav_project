...
//			   ", T=" + (typeAsString == null ? "" : typeAsString) +
	...
	class LinkPatternBuilder {
		...
		boolean findPatternValue(String pattern) {
			...
			ERR.isEq(patternType[0], PageFactory.TYPE_MARKER_LINK_PATTERN, "This pattern is not link-pattern", pattern);
		...
		private String[] toPatternType(String patternName) {
			return USToken.twoByChars(patternName, PageFactory.TYPE_MARKER_LINK_PATTERN, USToken.SplitByChars.ALLOWED);
		}
	...
	private Map<String, String> createMapValues(List<List<List<Object>>> slices, int indName, int[] indValue, String delimetrCol, String delimetrRow) {
		...
		for (List<List<Object>> slice : slices) {
			...
			String name = (String) ArrItem.item(slice.get(0), indName);
			if (X.empty(name)) {
			...
			switch (slice.size()) {
				case 1:
					List vals = ArrItem.itemsMany(slice.get(0), indValue);
	...
	private void initExtData() {
		...
		Object ext1 = ArrItem.item(getCurrentRow(), ++level);
		if (ext1 == null || USlice.isEmptyObj(ext1)) {
			return;
		}
		Object ext2 = ArrItem.item(getCurrentRow(), ++level);
		if (ext2 == null) {
