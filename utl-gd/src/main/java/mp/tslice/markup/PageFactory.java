...
public class PageFactory {
	...
	private final static Map<String, ApiGdExt.GoogleSheetData> _SHEETS = UL.cache_map_FILO(30);
	private final static Map<String[], List> SHEETS_SLICES = UL.cache_map_FILO(30);

	private final static Map<String, UNode.PageBuilderHtml> PAGE_BUILDERS = UL.cache_map_FILO(30);
	...
	public static UNode.PageBuilderHtml getPageBuilderOrCreate(String[] sheetInfo, String pageName) {
		...
		ERR.notNull(sheetDataLoader, "Sheet Data Loader is null", sheetInfo);
		ERR.notEmpty(sheetDataLoader.get_VALUES(), "Sheet Data is empty", sheetInfo);

		List<List<Object>> slice0 = findPageSlice(sheetInfo, pageName);

		ERR.notEmpty(slice0, "Page slice is empty", sheetInfo, pageName);
	...
	private static List<List<List<Object>>> findSheetSlice(String[] sheetContext) {
		...
		return ERR.notNull(slices, "List with slices is null, after init", sheetContext);
	}
