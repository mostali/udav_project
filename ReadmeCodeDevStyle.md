...
Все написанное ниже - мое личное мнение, как разработчика на Java, сложившееся после многолетней практики написания ПО на Java
...
- боль с придумыванием имени перемнной в общем случае решается тремя вопросами - Что это? Что делает? Для чего(внешние связи) ?
...
- названия переменных, методов, классов - как правило, должны называться позитивно, т.е. избегать использование частиц 'не' или негативных подходов (через исключения). Поскольку, отрицание усложняет чтение кода, и более того использование двух 'не' порождает двоякое толкование (т.е. 'НЕ НЕ делать' - т.е. ДА делать | 'НЕ НЕ делать' - т.е. не выполнять совсем )
- для описания какой либо функциональности программы - использовать позитивные ветки. Что позволит избежать сложностей и ошибок при расширении функционала, т.е. при добавлении новых сценариев
- в идеале,наименования максимально коротки, и максимально подходящи семантически (по смыслу)
