...
import mp.utl_odb.mdl.AXUModel;
...
public class NetActivityModel<M extends AXUModel> extends AXUModel<M> {
	...
	public static List<NetActivityModel> loadActivitys(TypeDb db, Integer limit, QDate dateLast) {
		...
		qps.add(QP.paramAscOrDescOrRand(CN.DT, false));

		return db.getModels(qps.toArray(new QP[0]));
	}

	public static List<NetActivityModel> loadActivitys(TypeDb db, QP... qps) {
		return db.getModels(QP.merge(qps, QP.paramAscOrDescOrRand(CN.DT, false)));
	}
