...
public class NetGrpId<M extends NetGrpModel> extends NetSrcDbId<M, NetGrpSrvEE> {
	...
	public Integer getGroupNIDint(boolean... checkNegative) {
		return ARG.isDefEqTrue(checkNegative) ? ERR.isNegNotZero(getGroupNIDint()) : getGroupNIDint();
	}

	@SneakyThrows
	public Long getGroupNIDlong(boolean... checkNegative) {
		return ARG.isDefEqTrue(checkNegative) ? ERR.isNegNotZero(getGroupNIDlong_()) : getGroupNIDlong_();
	}
