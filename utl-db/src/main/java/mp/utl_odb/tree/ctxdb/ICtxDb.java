...
public interface ICtxDb<M extends ICtxDb.CtxModel> {
	...
	static String getTablename(Path path, String... defRq) {
		...
		return ARG.throwMsg(() -> X.f("CtxDb tablename not found from {}", UF.ln(path)), defRq);
	}
	...
	static ICtxDb of(Path path, ICtxDb... defRq) {
		...
		return tablename != null ? of(path, tablename) : ARG.throwMsg(() -> X.f("CtxDb not found from %s", UF.ln(path)), defRq);
	}
	...
	static <M extends CtxModel> void beforeUpdate(M model) {
		Long ctr = (Long) model.getFieldValueObjType(CN.COUNTER, Long.class, 0L);
		try {
			model.setObjectField(CN.COUNTER, ctr + 1);
		} catch (RequiredRuntimeException ex) {
			L.warn("Unsupported ctx-model " + model + "\n-- " + ex.getMessage());
		}
	}
...
//		return put(Key.of(key), Key.Val.of(X.toString(value, null)), Key.Ext.of(X.toString(ext, null)), time);
	...
	default boolean isExistModelByVal(String val) {
		return getModelBy(CKey.Val.of(val)) != null;
	}

	default boolean isExistModelBy(QP... by) {
...
//		return getModels().isEmpty();
	...
	default <M> M getModelFirstOrLast(Boolean firstOrLastOrRandom, M... defRq) {
		try {
			M model = (M) DBU.getModelFirstOrLastOrRandom_NoCast(getDbUrl(), getModelClass(), firstOrLastOrRandom);
			return model != null ? model : ARG.throwMsg(() -> X.f("getModelFirstOrLastRq:%s", firstOrLastOrRandom), defRq);
		} catch (Exception ex) {
			return ARG.throwErr(ex, defRq);
		}
	}

	default <M> M getModelFirstOrLastBy(Boolean firstOrLastOrRandom, QP[] qps, M... defRq) {
		try {
			M model = (M) DBU.getModelFirstOrLastOrRandom_NoCast(getDbUrl(), getModelClass(), firstOrLastOrRandom, qps);
			return model != null ? model : ARG.throwMsg(() -> X.f("getModelFirstOrLastRq:%s", firstOrLastOrRandom), defRq);
		} catch (Exception ex) {
			return ARG.throwErr(ex, defRq);
		}
	...
	default M getModelFirstOrLastBy_noCast(Boolean firstOrLastOrRandom, String col, QP... qps) {
		return DBU.getModelFirstOrLastOrRandom_NoCast(getDbUrl(), getModelClass(), firstOrLastOrRandom, col, qps);
	}

	default M getModelFirstOrLastBy_Cast2Int(Boolean firstOrLastOrRandom, String col, QP... qps) {
		return DBU.getModelFirstOrLastOrRandom_Cast2Int(getDbUrl(), getModelClass(), firstOrLastOrRandom, col, qps);
	}

	default M getModelByKeyOrCreate(String byKey) {
		M modelBy = getModelByKey(byKey);
		return modelBy != null ? modelBy : put(byKey);
	}
	...
	default List<M> getModelsLike(CKey key, Integer... limit) {
		QP qp = key.pLIKE();
		return ARG.isNotDef(limit) ? getModels(qp) : getModels(qp, QP.limit(ARG.toDef(limit)));
	}
	...
	default <T> T getFieldValueAs(String nkey, Class<T> asType, T... defRq) {
		try {
			T t = UST.strTo(getValue(nkey), asType);
			return t;
		} catch (Exception ex) {
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Error UTree#getAs '%s'", asType), defRq);
		}
	}
	...
	default String getValue(String nkey, String... defRq) {
		...
		return m != null ? m.getValue() : ARG.throwErr(new RequiredRuntimeException("Key '%s' value is required", nkey), defRq);
	}
...
//		return getModels().stream().map(m-> PareEntry.of(m.getKey(),m.getValue())).collect(Collectors.toMap(k->k, v->v,(v1, v2)->v2));
		return getModels().stream().map(m -> new String[]{m.getKey(), m.getValue()}).collect(Collectors.toMap(k -> k[0], v -> Objects.toString(v[1], ""), (v1, v2) -> v2));
	}
	...
	class CtxModel<M extends AModel> extends AModel<M> implements IGetterAs {
...
//		@Setter
//		@DatabaseField
//		private @Getter Long ctr;

		public <T> T getJsonAs(OCols col, String key, Class<T> as, T... defRq) {
			GsonMap map = getAs(col, GsonMap.class, GsonMap.EMPTYMAP);
			return (T) map.getAs(key, as, defRq);

		}

		public void setJson(OCols col, String key, Object val) {
			GsonMap map = getAs(col, GsonMap.class, null);
			if (map == null) {
				map = new GsonMap();
			}
			map.put(key, val);
			setObjectField(col.name(), map.toStringPrettyJson());
		}

		public void updateJson(OCols col, GsonMap map) {
		}

		@Override
		public <T> T getAs(String key, Class<T> asType, T... defRq) {
			return getAs(OCols.valueOf(key), asType, defRq);
		}

		public <T> T getAs(OCols col, Class<T> asType, T... defRq) {
			T fieldValueObjType = getFieldValueObjType(col.name(), asType, defRq);
			return fieldValueObjType;
		}
		...
		public static List<AbsType> toAbsRow(CtxModel model, String... tablename) {
			...
			AbsType<QDate> time1 = AbsType.of(CN.TIME, QDate.of(model.getTime(), null), QDate.class);
			AbsType<String> ext1 = AbsType.of(CN.EXT, model.getExt(), String.class);
			AbsType<String> val = AbsType.of(CN.VAL, model.getValue(), String.class);
			AbsType<String> key1 = AbsType.of(CN.KEY, model.getKey(), String.class);

			List<AbsType> baseCols = ARR.asAL(AbsType.of(CN.ID, model.getId(), Long.class), key1, val, ext1, time1);
			if (tablename0 == null) {
				return baseCols;
			}
			switch (tablename0) {
...
//					baseCols5.add(AbsType.of("ctr", (Integer)model.getFieldValueObj("ctr"), String.class));
					return baseCols;
				case D5:
					ARR.as("o1", "o2", "o3", "o4", "o5").stream().map(cn -> AbsType.of(cn, (String) model.getFieldValueObjRq(cn), String.class)).forEach(baseCols::add);
					return baseCols;
				case D10:
					ARR.as("o1", "o2", "o3", "o4", "o5", "o6", "o7", "o8", "o9", "o10").stream().map(cn -> AbsType.of(cn, (String) model.getFieldValueObjRq(cn), String.class)).forEach(baseCols::add);
					return baseCols;
				default:
