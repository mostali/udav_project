...
//	}
	...
	public static <M extends ICtxDb.CtxModel> void afterPut(ICtxDb<M> ctxDb, M m) {
		...
		if (L.isDebugEnabled()) {
			...
			L.debug(SYMJ.FILE_DB + "Put ctx-model by key '{}' in tree {}\n{}", shorKey, UF.ln(ctxDb.getDbFilePath()), m);
		} else if (L.isInfoEnabled()) {
			String shorKey = STR.toStringSE(m.getKey(), 20, m.getKey());
			L.info(SYMJ.FILE_DB + "Put ctx-model by key '{}' in tree {}", shorKey, UF.ln(ctxDb.getDbFilePath()));
		}
	...
	public static <M extends ICtxDb.CtxModel> M getCtxModelBy(ICtxDb<M> iCtxDb, String colName, Object value, Boolean firstLastRandom) {
		return false == iCtxDb.isExistDb() ? null : DBU.getModelFirstOrLastOrRandom_NoCast(iCtxDb.getDbUrl(), iCtxDb.getModelClass(), firstLastRandom, QP.pEQ(colName, value));
	}
