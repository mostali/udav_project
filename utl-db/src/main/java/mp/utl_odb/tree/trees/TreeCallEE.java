...
//		P.exit(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "put", "k1", "v1").call());
		ERR.state(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "put", "k1", "v1").call().equals(Pare3.of("k1", "v1", null)), "<--" + TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "put", "k1", "v1").call());
		ERR.state(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "get", "k1", null).call().equals(Pare3.of("k1", "v1", null)), "<--" + TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "get", "k1", null).call());
		ERR.state(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "has", "k1", null).call().equals(true), "<--" + TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "has", "k1", null).call());
		ERR.state(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "ls", "k1", null).call().equals(Arr.as("k1")), "<--" + TreeCall.byKeyValue(Pare.of("testp", "testk"), "ls", "k1", null).call());
		ERR.state(TreeCall.byKeyValue(Pare.of("java-tests", "testk"), "rm", "k1", "v1").call().equals(Pare3.of("k1", "v1", null)), "already removed");
...
//			}
			...
			TreeCallType treeCallType = EN.valueOf(action, TreeCallType.class, true, null);
			...
			switch (treeCallType) {
				...
				case GET: {
					checkExistDb();
					CtxtDb.CtxTimeModel ctxTimeModelByKey = tree.getCtxTimeModelByKey(key);
					return (T) X.throwIfReturnNull(() -> EE.KEY_NOT_FOUND.I("Key '%s' not found", key), ctxTimeModelByKey).toPare3();
				}
				case RM: {
					checkExistDb();
					CtxtDb.CtxTimeModel ctxTimeModelByKey = tree.getCtxTimeModelByKey(key);
				...
				case LS: {
					...
					List<CtxtDb.CtxTimeModel> models = tree.getModels();
				...
				case HAS:
					...
					return (T) (Boolean) tree.contains(key);
		...
		private UTree getTree() {
			ERR.notEmptyAll(treeParent, treeKey);
