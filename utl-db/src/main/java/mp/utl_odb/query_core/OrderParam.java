...
public class OrderParam {
	...
	private boolean withCastToInt;

	public void withCastToInt(boolean... withCastToInt) {
		this.withCastToInt = ARG.isDefNotEqFalse(withCastToInt);
	}
	...
	public QueryBuilder apply(QueryBuilder<?, String> queryBuilder) {
		switch (EOrderParam.valueOf(orderRaw)) {
			case ASC: {
				if (withCastToInt) {
					return queryBuilder.orderByRaw("CAST(" + columnName + " AS INTEGER) ASC");
				} else {
			...
			case DESC: {
				if (withCastToInt) {
					return queryBuilder.orderByRaw("CAST(" + columnName + " AS INTEGER) DESC");
				} else {
