...
public class ApiCom extends Api0 {
	...
	public Issue0 getIssue0(String issueKey) {
		Issue claim = jiraClient.getRestClient().getIssueClient().getIssue(issueKey).claim();
		return new Issue0(claim){
			@Override
			public JiraCli jc() {
				return jiraClient;
			}
...
//		newIssue.getFields().set
	...
	public String createIssue(String projectCodeName, long taskTypeId, String taskTitle, String taskDescription) {
		BasicIssue task = jiraClient._Com().createIssue(projectCodeName, taskTypeId, taskTitle);
		String issueKey = task.getKey();

		jiraClient._Edit().updateIssueDescription(issueKey, taskDescription);
