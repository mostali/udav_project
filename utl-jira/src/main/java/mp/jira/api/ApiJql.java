...
public class ApiJql extends Api0 {
	...
	public <T> Iterable<T> getAllTasksByJql(String jql, Class<T> asType) {
		Iterable<Issue> tasks = loadAllTasksByJql(jql);
		if (asType == Issue.class) {
			return (Iterable<T>) tasks;
		} else if (asType == mp.jira.api.Issue0.class) {
			return (Iterable<T>) STREAM.itToList(tasks, t->new Issue0(t){
				@Override
				public JiraCli jc() {
					return jiraClient;
				}
	...
	private Iterable<Issue> loadAllTasksByJql(String jql) {
		Promise<SearchResult> filters = jiraClient.getRestClient().getSearchClient().searchJql(jql);
