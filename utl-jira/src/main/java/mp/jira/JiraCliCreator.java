...
//	}
	...
	public JiraCli buildByMsg() {
		...
		if (authUsr.hasNN(0)) {
			...
			Tuple tuple = Tuple.ofMap(headersMap, JIRA_HLP);
			IT.state(X.notEmptyAllObj_Str_Cll_Num(tuple.objs), "set all cred [ %s , %s , %s ] args %s", "jira.login", "jira.pass", "jira.host", tuple);
			JiraUser byHlp = createByHlp(tuple.objs);
