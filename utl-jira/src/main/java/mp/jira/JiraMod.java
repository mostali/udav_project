...
public class JiraMod {
	...
	public static final String DEF_EB_PROJECTS = "EXP,SUP,TSE,BU,ARP,ESD,ACT,INC,EB,NSI";
...
//	}
	...
	public static <T> T get_all_task_default_eb() {
		return invokeLines(ARR.of("-task", "*", SOID.toKey(MK_PROJECTS), DEF_EB_PROJECTS));
	}
...
//	@ZType.ZMethodAno
//	@SneakyThrows
//	public static <T> T invokeLinesEnv() {
//		String prop = Sys.getEnvOrSysProp("task", null);
//		if ("*".equals(prop)) {
//			return invokeLines(new String[]{"-task", "*", "-projects", DEF_EB_PROJECTS});
//		}
...
//				throw new FUnsupportedOperationException("What is task %s?", task);
	...
	public static <T> T invokeContext0(Object auth, Map context) {
		...
		}
		{
			...
			if (all != null) {
				JiraCli jiraCli = JiraCli.ofAuth(auth);
				String projects = (String) MAP.get(context, MK_PROJECTS, DEF_EB_PROJECTS);
