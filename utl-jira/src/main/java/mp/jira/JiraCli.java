...
public class JiraCli {
	...
	public static void main(String[] args) throws IOException, ExecutionException, InterruptedException, SubtaskControlEE {
		...
		String orgUrl = AP.getAppOrg("org");
...
//			String urlWithComment = orgUrl+"/browse/ISUUUEEEE______ID?focusedCommentId=12137758&page=com.atlassian.jira.plugin.system.issuetabpanels:comment-tabpanel";
		...
		JiraCli myJiraClient = getOrCreateByUsr(dav1);
//		Iterable<Issue> allTasks1 = myJiraClient.getAllTasks();
		Iterable<Issue> allTasks = getAllTasks(dav1);
		P.exit(allTasks);
...
//		U.exit(subtasks);
	...
	public static JiraUser createUsr_DefaultORG(String usrName) {
		String org = AP.getAppOrg("org");
		EnvTlp envTlp = EnvTlp.ofHlpOrg(org, usrName);
		return JiraUser.create(usrName, new String[]{IT.NN(envTlp.readLogin(), "login is null"), envTlp.readPass()});
	}
	...
	public static JiraCli initMode0_AP() {
		...
		String user = AP.get("user", null);
		if (user != null) {
			return JiraCli.getOrCreateByUsr(user);
		}

		String jiraurl = AP.get("jiraurl", null);
		String login = AP.get("login", null);
		String pass = AP.get("pass", null);
		if (X.emptyAny(jiraurl, login)) {
			throw new FIllegalStateException("set all keys to app.props 'jiraurl' + 'login' + 'pass'");
		}
...
//		static final MyJiraClient INSTANCE = new MyJiraClient();
//	}
//	public static MyJiraClient get() {
//		return restClient;
//	}
	...
	private static Map<String, JiraCli> store = (Map<String, JiraCli>) UL.cache_map_FILO(10);
...
//		}
	...
	private static List<String> downloadAttachs(JiraUser user, Issue issue, long commentId, Path toDir, boolean skipExisted) {
		...
		Comment comment = ST.find(ARR.toList(issue.getComments()), (Comment c) -> c.getId().equals(commentId));

		Path toDirComment = toDir.resolve(commentId + "");

		UFS_BASE.MKDIR.createDirsOrSingleDirOrCheckExist(toDirComment, true);

		List<Attachment> attachments = ARR.toList(issue.getAttachments());

		List<String> markAttahs = pickAttach(comment.getBody());

		List<Attachment> commentAttachs = ST.filter(attachments, (Attachment i) -> markAttahs.contains(i.getFilename()));
	...
	private static List<String> pickAttach(String commentContent) {
		List<String> all = ST.map(URx.findAllGroup(commentContent, "(\\[\\^.+?\\])"), (String s) -> STR.substrStartEnd(s, 2, 1));
		return all;
	}
...
//	}
	...
	public Iterable<Issue> getAllTasks() throws ExecutionException, InterruptedException {
...
//	}
	public static Iterable<Issue> getAllTasks(JiraUser user) throws ExecutionException, InterruptedException {
		String project = "project in (EXP, SUP, TSE, BU, ARP, ESD, ACT, INC, EB)";
		String status = "status in (Open, \"In Progress\", Resolved, Design, \"In Development\", \"In Build\", Tested, Analysis, Testing, Formed, \"Information Required\", Realization, Correction, \"Запрос информации\", Delay, \"В ожидании\", Пауза, CodeReview, \"Ready in build\", Blocked)";
		String issuetype = "issuetype in (Improvement, Task, \"Задача ДТА\", \"Request management\", \"Change request\", Request, \"Component Improvement\", Bug, Patch)";
		return getAllTasks(user, "assignee=currentuser() AND " + status + " AND " + project + " AND " + issuetype);
	}
	...
	public static Iterable<Issue> getAllWorkSubTasks(JiraUser user) throws ExecutionException, InterruptedException {
		return getAllTasks(user, "assignee=currentuser() AND issuetype = Sub-task AND Status=3");
	}

	public static Iterable<Issue> getAllTasks(JiraUser user, String jql) throws ExecutionException, InterruptedException {
		JiraCli myJiraClient = getOrCreateByUsr(user);
