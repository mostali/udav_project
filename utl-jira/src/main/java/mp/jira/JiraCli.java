...
public class JiraCli {
	...
	public static void main(String[] args) throws IOException, ExecutionException, InterruptedException {
		...
		try {
			...
			L.info("bussines message '{}' {} {}", "arg1", ex, "arg3");
		}
		X.exit(1);
		;
//		getAllTasksByJql
...
//		SubtaskControlEE.runStartTransition(cli, getIssue(cli.getJiraRestClient(), "ISUUUE____ID"));
	...
	public JiraRestClient getRestClient() {
		...
		if (L.isInfoEnabled()) {
			L.info("JiraRestClient [INIT] to {} with user {}", user.getUrl2jira(), user.getLogin());
		}
	...
	public Issue0 issue(String issueKey) {
		return _Com().getIssue0(issueKey);
	}

	public Issue issueNative(String issueKey) {
		return _Com().getIssue(issueKey);
	}
}
