...
//		U.exit(subtasks);
	...
	public static JiraCli ofAuth(Object auth) {
		...
		if (auth instanceof String) {
			...
			rsp = JiraUser.createByArgs((String[]) auth).createClient();
		} else {
...
//		}
	...
	public static class JiraUser {
		...
		public static JiraUser createByArgs(String[] hlp) {
			switch (hlp.length) {
				case 0:
				case 1:
					String usr = hlp.length == 0 ? Env.getUserName() : hlp[1];
					EnvTlp enTlp = EnvTlp.ofSysAcc("jira", usr);
					return JiraUser.create(enTlp.readHostWithPort(), ARR.of(enTlp.readLogin(), enTlp.readPass()));
				case 2:
					return JiraUser.create(AP.get(APK_JIRA_URL), ARR.of(hlp[0], hlp[1]));
				case 3:
...
//		newIssue.
		L.info("Run project issue create {} ({}) - {}", newIssue, issueType, issueSummary);
