...
public class UJsoup {
	...
	public static Element select(String html, String selector, int index) {
		...
		return ArrItem.item(els, index, null);
	}
	...
	public static Document html2doc_(String html) {
		return ERR.notNull(Jsoup.parse(html));
	}

	public static Document html2doc_(File file, String... charset) throws IOException {
		return Jsoup.parse(ERR.isFileExist(file), ARG.toDefOr(Charset.defaultCharset().toString(), charset));
	}
	...
	public static void addAttrContent(Element element, String attrContent) {
		...
		html.append("<div ").append(ERR.notEmpty(attrContent)).append("></div>");
		Element first = Jsoup.parse(html.toString());
		ERR.notNull(element).attributes().addAll(first.attributes());
	}

	public static boolean isAttrWithContent(String attrContent) {
		String[] two = USToken.twoByChars(attrContent, SYM.WORD_EN_NUM_DASH + SYM.COLON, USToken.SplitByChars.ALLOWED);
