...
public class GdbBin {
	...
	public static final String JARNAME_GDBMOD_V3 = "gdb2-mod.jar";
...
//		Path pathKey = Paths.get("/home/dav/.data/tsm/__GD/gd.key.json");
		Path pathKey = Paths.get("/opt/appVol/tsm/__GD/gd.key.json");

		Object o = ZBin.XD.invokeArgs(pathKey, "1E2KPeDIJsryqKe3mFOsYLcKNI5FfynZBPYWoc8nqTj0", "anypage!A1:Z", "loader");
		X.exit(o);
//		Object rows = GdbBin.invokeContext(UMap.of("sheet", "1R8jTpQuXMEqXlUqt1fC2nLGzVOP8wisJXftl6TZ-1ew", "range", "tasks!A1:Z", "return", ReturnAsType.rows));
...
//			List columnMetadata = rspMetaData.getAsArray("columnMetadata");
	...
	public static class FileCacheManager {
		...
		public static final int MIN_CACHE_IF_NULL = 10;
		...
		private void clearCache() {
			List<Path> sheetCache = getCacheFiles().stream().filter(p -> UF.fn(p).contains(gdReader.sheetId)).collect(Collectors.toList());
			sheetCache.forEach(UFS::delete);
			L.info("Cache clean all : " + UFS.toFileNames(sheetCache));
		}
		...
		private Pare<QDate, Path> findActualLastCacheFile(boolean withMeta, QDate maxDate) {
			...
			for (Path file : getCacheFiles()) {
				...
				String sheetWithRange = qDatePare.val()[1];
				if (!gdReader.sheetId.equals(qDatePare.val()[0]) || !UF.clearFilename(gdReader.range).equals(sheetWithRange)) {
					continue;
				}
			...
			return Pare.ofEntry(ARRi.last(map));
		}

		private List<Path> getCacheFiles() {
			return UFS.lsFiles(Paths.get(CACHE_DIR), ARR.EMPTY_LIST);
		}

		private static void checkAndCleanMap(LinkedHashMap<QDate, Path> map) {
			if (map.size() > MIN_CACHE_IF_NULL) {
				Iterator<Path> iterator = map.values().iterator();
				int elementsToRemove = map.size() - MIN_CACHE_IF_NULL;
		...
		private Path cacheFile(boolean withMeta, QDate maxDate) {
			...
			return Paths.get(CACHE_DIR).resolve(UF.clearFilename(filename));
		}
		...
		public Pare<QDate, GsonMap> loadCachedDataOrLoad(boolean withMeta, QDate maxDate) {
			...
			if (checkCache) {
				...
				if (data != null) {
					L.info("Cache is hot [FOUND]. Sheet [{}/{}] was loaded [{}]", gdReader.sheetId, gdReader.range, maxDate);
				}
			} else {
				clearCache();
			}
			if (data == null) {
				L.info("Cache expired or not exist [LOADING] Sheet [{}/{}] was loaded [{}]", gdReader.sheetId, gdReader.range, maxDate);
				Object rspJsonStr = loadSheetRows(gdReader.pathMod, gdReader.pathGdKey, gdReader.sheetId, gdReader.range, withMeta);
				GsonMap rspJson;
				if (rspJsonStr instanceof GsonMap) {
					rspJson = (GsonMap) rspJsonStr;
				} else if (rspJsonStr instanceof String) {
					rspJson = GsonMap.of((String) rspJsonStr);
				} else {
					throw new WhatIsTypeException(rspJsonStr.getClass());
				}
				data = Pare.of(QDate.now(), rspJson);

//				boolean updateCache = maxDate != null;
				boolean updateCache = true;
				if (maxDate == null) {
					maxDate = QDate.now().addMinutes(MIN_CACHE_IF_NULL);
				}
				if (updateCache) {
		...
		public static <T> T loadSheetRows(Path modJar, Path gdAuth, String sidBacklog, String range, boolean... withMeta) {
			...
			return Matrix.rspToValues(rsp, isWithMeta, false);
		}
	}
