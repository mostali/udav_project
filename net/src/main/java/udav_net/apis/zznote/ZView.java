...
public class ZView {
	...
	public static void main(String[] args) {
		...
		Collection<Path> paths = UFS.searchFiles(Env.ROOT_PJM, p -> {
			return EXT.JAVA.isPath(p);
		}, false);
		for (Path pathJavaClass : paths) {

			String classContent = RW.readContent(pathJavaClass);
...
//		Class[] mpfs = scan("mp");
//		for (Class mpf : mpfs) {
//			Path srcClass = getSourcePathOfClass("utl", mpf);
//
//			String data = RW.readContent(srcClass);
//
//			P.e(data);
//		}
//		P.exit(mpfs.length);
