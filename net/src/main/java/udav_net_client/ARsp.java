...
public class ARsp extends AbsNetRsp<HttpEntity, String> {
	...
	public <T> T toType(Class<T> type, T... defRq) {
		...
		try {
			...
			Object finalAny = any;
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Illegal response '%s' to convert type '%s'", finalAny, type), defRq);
		}
	}
	...
	public byte[] errBytes(byte[]... defRq) {
		...
		try {
			...
			return ARG.throwErr(() -> new RequiredRuntimeException(ex, "Init apCli error-body bytes is null"), defRq);
		}
	}
